...
from training.fault_detector import FaultDetector, global_fault_detector
from training.task_migrator import TaskMigrator, initialize_task_migrator
...
class DistributedOptimizer:
    ...
    async def register_node(self, node_id: str, node_info: Dict[str, Any]):
        ...
        try:
            ...
            # 注册到故障检测器
            self.fault_detector.register_node(node_id, {'assigned_tasks': []})
            ...
            if best_node in self.nodes:
                ...
                if best_node in self.fault_detector.nodes_status:
                    ...
                    if task_id and task_id not in node_status.assigned_tasks:
                        node_status.assigned_tasks.append(task_id)
            
            return result
        except Exception as e:
            self.error_handler.handle_error(e, context)
            logger.error(f"分发训练任务失败: {e}")
    ...
    async def _assign_task_to_node(self, node_id: str, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            self.error_handler.handle_error(e, context)
            logger.error(f"分配任务给节点失败: {node_id} - {e}")
            return {'status': 'failed', 'error': str(e)}
            ...
            if node_id in self.nodes:
                ...
                if node_id in self.fault_detector.nodes_status:
                    ...
                    if task_id in node_status.assigned_tasks:
                        node_status.assigned_tasks.remove(task_id)
            
            return True
        except Exception as e:
            self.error_handler.handle_error(e, context)
            logger.error(f"收集任务结果失败: {node_id} - {task_id} - {e}")
            return False
