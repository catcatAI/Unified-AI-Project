...
def test_data_manager() -> None,
    ...
    logger.info(f"   扫描到 {len(catalog)} 个文件")
    ...
    logger.info(f"   为概念模型准备了 {len(concept_data)} 个训练文件")
...
def test_resource_manager() -> None,
    ...
    logger.info(f"   CPU使用率, {resources['cpu']['usage_percent'].1f}%")
    logger.info(f"   内存使用率, {resources['memory']['usage_percent'].1f}%")
    logger.info(f"   GPU数量, {len(resources['gpu'])}")

    # 测试模型资源需求
    requirements = resource_manager.get_model_resource_requirements("concept_models")
    logger.info(f"   概念模型资源需求, {requirements}")
...
def test_collaborative_training_manager() -> None,
    ...
    logger.info(f"   为 {len(model_data)} 个模型准备了训练数据")

    # 分配资源
    model_resources = manager.allocate_resources_for_models()
    logger.info(f"   为 {len(model_resources)} 个模型分配了资源")

    # 获取训练状态
    status = manager.get_training_status()
    logger.info(f"   训练状态, {status['is_training']}")
