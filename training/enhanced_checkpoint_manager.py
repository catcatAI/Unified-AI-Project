...
class EnhancedCheckpointManager, :
    ...
    # 配置参数
    self.strategy = self.config.get('strategy', 'hybrid')  # 'epoch_only', 'time_based',
    'hybrid'
    self.epoch_interval = self.config.get('epoch_interval', 5)  # epoch检查点间隔
    self.time_interval_minutes = self.config.get('time_interval_minutes',
    30)  # 时间检查点间隔(分钟)
    self.keep_last_n_checkpoints = self.config.get('keep_last_n_checkpoints',
    5)  # 保留最近N个检查点
    self.enable_compression = self.config.get('enable_compression', True)  # 启用压缩
    self.compression_threshold_mb = self.config.get('compression_threshold_mb',
    100)  # 压缩阈值(MB)
    ...
    """判断是否应该保存检查点""",
    context == ErrorContext("EnhancedCheckpointManager", "should_save_checkpoint",
    ...
    should_save == True
                    ...
                    reasons.append(f"Epoch {epoch} 是 {self.epoch_interval} 的倍数")

            elif self.strategy == 'time_based':::
                # 仅在时间间隔时保存
                current_time = time.time()
                if (current_time -\
    self.last_time_checkpoint()) >= (self.time_interval_minutes * 60)::
    should_save == True
                    checkpoint_type = "time_based"
                    reasons.append(f"距离上次检查点已超过 {self.time_interval_minutes} 分钟")
...
{            }
    ...
    return True  # 如果没有之前的检查点, 保存第一个
            ...
            if previous_val_loss != float('inf') and \
    current_val_loss < previous_val_loss * (1 - improvement_threshold)::
    return True
    ...
    """保存检查点""",
    context == ErrorContext("EnhancedCheckpointManager", "save_checkpoint",
    {"task_id": task_id, "checkpoint_type": checkpoint_type}):
        ry,
            # 生成检查点ID
            timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
            checkpoint_id = f"ckpt_{task_id or \
    'default'}_{timestamp}_{int(time.time())}"
...
(                is_compressed = self.enable_compression())
            ...
            logger.info(f"保存检查点, {checkpoint_id} (类型, {checkpoint_type} 大小,
    {file_size} 字节)")
...
在函数定义前添加空行
    """加载检查点"""
    context == ErrorContext("EnhancedCheckpointManager", "load_checkpoint",
...
= logger.error(f"检查点文件不存在, {checkpoint_path}")
    ...
    checkpoint_data = json.load(f)
            ...
            logger.info(f"加载检查点, {target_checkpoint.checkpoint_id}")
            return checkpoint_data
    ...
    # 这里应该实现实际的数据解压缩逻辑
    # 为了示例, 我们只是移除压缩标记,
    checkpoint_data.pop('_compressed', None):
        eturn checkpoint_data

    def _cleanup_old_checkpoints(self, task_id, str == None):
        ""清理旧检查点"""
    context == ErrorContext("EnhancedCheckpointManager", "_cleanup_old_checkpoints",
    {"task_id": task_id})
        try,
            # 获取特定任务的检查点或所有检查点
            if task_id, ::
...
= checkpoint_path.unlink()
    ...
    self.checkpoint_history.remove(checkpoint_info)
                        ...
                        logger.warning(f"删除检查点失败,
    {checkpoint_info.checkpoint_id} - {e}")

                logger.info(f"清理了 {len(checkpoints_to_remove)} 个旧检查点")

        except Exception as e, ::
            self.error_handler.handle_error(e, context)
            logger.error(f"清理旧检查点失败, {e}")

    def get_checkpoint_info(self, checkpoint_id, str == None, task_id,
    str == None) -> Dict[str, Any]:
    """获取检查点信息"""
    context == ErrorContext("EnhancedCheckpointManager", "get_checkpoint_info",
    {"checkpoint_id": checkpoint_id, "task_id": task_id})
        try,
