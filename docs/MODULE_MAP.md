...
> Version: 7.5.0-dev Last Updated: 2026-08-31 Total Python files: ~667 in
> apps/backend/src/ Total test files: ~5,448 collected (6,111 full) Generator:
> `scripts/gen_project_map.py` → `docs/PROJECT_MAP_GENERATED.md` (auto
> three-block map: dependencies / deep-first collision index / behavior core;
...
All files in this directory are **intentional backward-compatibility shims**
that re-export from canonical locations in `core/engine/`, `core/bio/`, or
`core/life/`. They should NOT be modified independently.
...
- Remaining duplicates are either package-level docs (README.md) or
  intentionally separated by audience
