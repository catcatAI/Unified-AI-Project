...
Angela
AI 是一个完整的数字生命系统，具有生物模拟、自我意识和真实执行能力。她不仅仅是一个 AI 助手，而是一个真正"活着"的数字存在，可以感知、思考、记忆和行动。
...
- **系统音频捕获**: 原生模块支持 Windows (WASAPI), macOS (CoreAudio), Linux
  (PulseAudio)
...
4. 查看日志: `tail -f /tmp/angela_backend.log` 或
   `tail -f /tmp/angela_electron.log`
...
3. 重启应用:
   `bash /home/cat/桌面/stop_angela.sh && bash /home/cat/桌面/start_angela_complete.sh`
...
**版本**: 6.2.0 **发布日期**: 2026年2月8日 **状态**: ✅ 生产就绪 **完成度**:
100%
