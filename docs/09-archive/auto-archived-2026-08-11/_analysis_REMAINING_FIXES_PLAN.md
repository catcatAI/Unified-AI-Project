...
- `"次方"` not recognized in `_parse_chinese_word_problem` — no power/exponent
  detection

**Fix** (`math_verifier.py`):

1. Add pow detection before add/subtract checks: `("次方", "次幂", "幂")` →
   `f"{a}**{b}"`
...
1. `_detect_learning_intent` only matches
   `["記住","學習","記錄","調整","理解","教我","記住這個"]` — `"我叫小明"` has
   none → falls to general handler
2. No structured extraction: regex `關於`-prefix only; stores raw blob
   `"Learning: 記住我叫小明"`
...
1. Add intent triggers: `self-intro` keywords
   `("我是", "我叫", "我的", "最喜歡")`
2. Add `_handle_user_info_intent()` with regex extraction: `name=小明`,
   `favorite=獵戶座星雲`
3. Store structured items with `data_type="user_profile"`, indexed by
   `user_name`
4. On each turn, query for `user_profile` items of current user and inject into
   context
...
- HTTP server loads config from `multi_llm_config.json` which doesn't set
  `llm_mode`

**Fix**: Set `"llm_mode": "standard"` in `multi_llm_config.json` or add
auto-detection that prefers available cloud LLM over NeuroBlender when Gemini
key is present and API responds.
