...
完整盤查 `GOOGLE_API_KEY` 與 `GEMINI_API_KEY`
在整個 codebase 中的定義、讀取、使用情況，以及 Angela 的 Google
Drive 整合與 LLM 對話管線之間的資料流。
...
**關鍵斷點：`_handle_drive_intent()` 回傳 `str`，直接中斷 `generate_response()`
流程，不經過 `_handle_general_intent()`，Angela 無法「理解」檔案內容。**
...
**缺少：`create_file()`、`upload_file()`、`create_folder()`。** 雖然 scope 有
`drive.file`，`MediaFileUpload` 也已 import，但完全沒有實作。
...
但 `_handle_general_intent()` 傳給 LLM 的 `relevant_memories`
是從 HAM 隨機檢索的，**沒有針對 Drive 檔案的專門檢索路徑**。檔案內容雖然存了，但 LLM 不一定會看到。
...
2. `chat_service.py`：`_handle_drive_intent()`
   回傳結構化 dict（含內容），而非 raw str
3. `angela_llm_service.py`：`_construct_angela_prompt()` 加入
   `context["drive_files"]` 渲染
4. `chat_service.py`：`_handle_general_intent()` 的 context dict 新增
   `drive_files` 鍵
...
`GOOGLE_API_KEY` 是無用的死配置，應安全移除。真正的問題不在 key 本身，而在
**Drive 與 LLM 之間的管線中斷**—Angela 目前無法「理解」她讀到的檔案，也無法產出檔案。Phase
B 和 C 才是修復的核心。
