...
| Multimodal           | Auto-enabled in ED3NEngine.**init**() ✅                                                                 |
...
1. **End-to-end test from chat input → response** hitting Model Bus + all
   backends (stub external deps)
2. **Context carries bio_state, emotion, user profile** through the pipeline
   (for GARDEN)
3. **Coverage >50%** on core + ed3n + garden + router + chat_service + providers
4. **Telemetry dashboard** — latency p50/p95/p99 per tier, cache hit rates,
   pruning stats
5. **Knowledge base ingestion** — GARDEN learns from real docs instead of
   synthetic Q&A
