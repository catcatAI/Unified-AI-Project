...
> **⚠️ SUPERSEDED — 2026-06-26** This audit has been absorbed into
> COMPREHENSIVE_AUDIT_2026-06-25.md. See docs/COMPREHENSIVE_AUDIT_2026-06-25.md
...
| M7  | ****init**.py 未匯出 CL**             | `__init__.py`                    | `ContinuousLearningPipeline` 未在 `__all__` 中        |
...
- [x] P0.1: `ed3n_engine.py:141` — 多模態改為 lazy init（已確認非問題 —
      ImageEncoder/AudioEncoder 導入已是 lazy）
...
- [x] P0.6: `dictionary.py:79` — `hash(bigram)` →
      `zlib.adler32(bigram.encode())`
- [x] P0.7: `garden.py:36` — `GARDENEngine(compatibility_mode=True)`
- [x] P0.8: `query_classifier.py:52-57` — 從 MATH 移除「多少」，加入 KNOWLEDGE
- [x] P0.9: `router.py:522-523` — cloud backend adapter (_CloudAdapter wraps
      generate()→process())
- [ ] ~~P0.10: `dictionary.py:36` — `_lazy_torch()`
      加 try/except~~（torch 為硬依賴，ImportError 是正確行為）
...
- ~~「多少」包含在 MATH pattern → 事實查詢被誤導至 ED3N~~
  ✅ 已修復：從 MATH 移除「多少」且加入 KNOWLEDGE
