...
版本：v0.1 日期：2025-08-06 負責：Backend/Architecture 團隊
...
- 熱端點群（/api/v1/hot/...）與對應服務（HotReloadService、ToolDispatcher、PersonalityManager、HSP/MCP
  connectors）
...
- 緩衝回朔（Buffered backtracking）與時待區（Staging
  area）：在輸出前允許回溯/修正，記錄為新的 policy 事件
