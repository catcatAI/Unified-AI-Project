...
This addendum records recent and planned API changes to keep the planning set
...
- GET `/api/v1/models/available`
  - Returns a list of ModelProfile (provider, model_name, context_window,
    capabilities, cost/latency estimates), discovered from adapters and
    environment keys.
...
- The v1 Atlassian endpoints have been extended:
  - POST `/api/v1/atlassian/jira/issue` accepts `priority` and `labels` (labels
    supports comma-separated string or array).
