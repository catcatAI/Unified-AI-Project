...
在项目中发现并修复了 [apps/backend/pytest.ini](../apps/backend/pytest.ini)
文件中的 Git 合并冲突标记。这解决了后端测试无法运行的问题。
...
6. **后端测试失败**
   - 检查 [apps/backend/pytest.ini](../apps/backend/pytest.ini)
     文件是否包含合并冲突标记
...
_本指南基于 Unified-AI-Project 项目批处理脚本的实际功能整理_
