...
| `python scripts/run_luanti_agent.py`                  | **Luanti 遊戲自主代理**（獨立進程；勿與統一啟動器混淆） | Luanti＋`apps/luanti-mods/agent_poller` 已裝（bridge URL 用 minetest.conf `agent_poller_bridge_url`，預設 localhost:30003） | agent log `Angela shutdown complete`／遊戲內 `AngelaBot` 動作 | 2026-09-20 |
