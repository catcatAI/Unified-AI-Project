...
> [!IMPORTANT] **架構轉型**: 本專案已從 Ray 分佈式架構轉向 **Local Async
> (本地異步)**
> 架構，以提升 Windows 環境穩定性與降低硬體門檻。這是一個有意識的設計決策，旨在實現「低資源 AGI」的核心目標。

### 為何放棄 Ray？

**原因**:

1. **Windows 兼容性問題**: Ray worker 進程無法正確繼承
   `PYTHONPATH`，導致模組導入失敗。
...
RUN pip install -e "apps/backend[standard]"
