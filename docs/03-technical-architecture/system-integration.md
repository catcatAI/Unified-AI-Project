...
Unified AI 的後端集成架構由兩個核心組件組成，分別負責 **生命週期管理** 與
**認知執行**：

1.  **`UnifiedAISystem`
    (`enhanced_system_integration.py`)**: 負責系統啟動、自我維護、健康監控與 API 入口路由。
2.  **`UnifiedControlCenter` (`unified_control_center.py`)**: (Phase
    14 新增) 負責 AI 任務的編排、並發執行與代理調度。
...
5.  該處理方法呼叫對應的管理器或服務 (例如:
    `self.dialogue_manager.process_message(...)`) 來執行實際的邏輯。
6.  執行結果被包裝成標準格式並返回。
