...
| mypy 型別債 | 收斂中（棘輪門鎖定） | `pyproject.toml`<br>`scripts/mypy_budget_gate.py`<br>`scripts/mypy_budget.txt` | **verified** | `python scripts/mypy_budget_gate.py`；門機制可驗證；基線 518（R80 鎖定；歷史 559→R77 MSBA 清零→540→R80 遊戲規劃棧清零→518）；新增債 CI 直接紅燈 | 2026-09-22 |
