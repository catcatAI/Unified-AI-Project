...
>
> **分數**：6.0/10（含 LLM）。純原生能力為**多維度**、無單一總分：確定性引擎強（數理化 9.5 / 知識 10 / 符號推理 10 / 自主 9.0）；**神經網路淨貢獻 ≈35%**（DET-CARRY 差分 = HYBRID−DET-ONLY，ED3N 35.3% / GARDEN 35.6%，非 SNN-ONLY 失真分，見 INTELLIGENCE_ASSESSMENT.md §1.1 & §4.1.1）。所有開放域對話智慧來自 LLM API。
...
- 每個宣稱必須可由實際程式碼路徑 + git commit 驗證
  - 分數必須反映真實能力（含 LLM：6.0/10；純原生為多維度、無單一總分：確定性引擎強，神經淨貢獻 ≈35% DET-CARRY 差分；見 assessment §1.1 & §4.1.1）
