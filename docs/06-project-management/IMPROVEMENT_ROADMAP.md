...
| **§X #201b: MD sync + orphan cleanup + usage docs** | (1) MD test count sync across 5 files (5,016/5,017/5,019→4,439). (2) CHANGELOG.md: added §X #197-201 entries. (3) Deleted orphan dirs tests/ai/compression/ + tests/ai/dialogue/. (4) Deleted 4 dead test files importing deleted Phase 11 modules. (5) Repaired test_phase1_core_activation.py (-1 deleted import, +5 tests). (6) Moved 23 utility scripts from tests/utils/→scripts/utils/ (2 earlier + 21 now — all standalone, zero importers). (7) Deleted zero-assertion test_verify_fixes.py. (8) Fixed test_base.py: __test__=False + Any import bug. (9) Fixed tests/run_enterprise_tests.bat path (was pointing to wrong directory). (10) Created docs/usage/QUICK_START.md + docs/usage/SCENARIOS.md. (11) Updated README.md (EN + ZH indexes + Quick Start cross-refs). (12) Updated ACTIVE_SCRIPTS.md (28 scripts/utils/ entries). (13) Updated CAUSAL_CHAIN_COMPLETENESS.md (§X #201b row + summary). Baseline: 4,439 tests — 0 errors (35.93s). | 23 files + 5 MD + 2 new docs | ✅ Done (§X #201b) |
