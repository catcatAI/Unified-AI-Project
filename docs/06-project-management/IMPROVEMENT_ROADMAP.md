...
| **Test quality: fix test_audit_comprehensive + orphan module tracking (§X #144)** | test_audit_comprehensive.py: restructured diagnostic print-based script — removed unconditional pytest.skip(), wrapped all ~250 lines of diagnostic code inside if-main guard, fixed broken import line. test_cli_imports.py: added 3 orphan ai.code_inspection.* module paths to _DELETED_MODULES. Net: +3 tests (5,033→5,036). | test_audit_comprehensive.py, test_cli_imports.py | ✅ Done (§X #144) |
