<!--
  ...
  STATUS: active
  LANGUAGE: zh-tw
    LAST_MODIFIED: 2026-08-28 (updated test count to 5,448 (6,111 full), open-domain 1.0)
...
>
> **分數**：6.0/10（含 LLM）。純原生能力為**多維度**、無單一總分：確定性引擎強（數理化 9.5 / 知識 10 / 符號推理 10 / 自主 9.0）；**神經 SNN 本職 = 學關聯**（關聯能力 1.0，ED3N & GARDEN，見 INTELLIGENCE_ASSESSMENT.md §4.1.2），不背知識。所有開放域對話智慧來自 LLM API。
...
| **測試數量** | pytest 收集 | **5,448 tests** (tests/; 6,111 full, re-verified 2026-09-01) | ✅ 0 failures |
...
```bash
# 執行所有測試（基線：5,448 tests/ + 6,111 full, 2026-09-01）
...
- 每個宣稱必須可由實際程式碼路徑 + git commit 驗證
  - 分數必須反映真實能力（含 LLM：6.0/10；純原生為多維度、無單一總分：確定性引擎強，神經 SNN 本職學關聯、關聯能力 1.0；見 assessment §1.1 & §4.1.2）
