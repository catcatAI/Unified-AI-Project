...
> **Note**: This table tracks 29 key items (12 DONE, 17 PENDING) but is NOT exhaustive. Full codebase audit found **~190+ AI-related classes** across `ai/`, `core/`, `services/` (20+ subsystems). **⚠️ "存在" ≠ "正常運作"** — see industry comparison below. Most engines are architectural skeletons: VisualDecoder projection weights are now trained on CIFAR-10 (42× loss reduction) but CNN texture branch remains random. AudioWaveformDecoder and SequenceGenerator weights are still random. Logic is 1990s-level (VisualEncoder = histogram+edges, CausalReasoning = Pearson correlation), or output is placeholder/stub (CerebellumEngine, TaskGenerator). Only ~14 systems have REAL computation quality. The LLM API wrappers provide the only production intelligence.
