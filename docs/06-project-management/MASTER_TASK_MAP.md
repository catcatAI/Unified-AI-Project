...
| 6 | P4 long function refactor (28 files >100 lines) | 5/28 done (+construct_angela_prompt: 144→24L, extracted 4 helpers: _build_core_prompt, _attach_cognition_blocks, _attach_action_result, _attach_continuation_guard, Jun 28). 26 functions >100 lines remain in apps/backend/src | 26 functions >100 lines; 5 refactored | Effort (large) |
...
| 24 | **FullTrainingPipeline + ContinuousMultimodalLearning (CML) wiring** | **FIXED** (Jun 28). CML was creating an isolated `FullTrainingPipeline` with its own independent `SharedLatentSpace`, encoders, and decoders — micro-training had zero effect on production encode/decode. Now `MultimodalService._get_cml()` passes the production pipeline (shared latent_space, visual_encoder, audio_encoder, visual_decoder, audio_decoder) via the new `pipeline` constructor parameter. `ContinuousMultimodalLearning._get_pipeline()` returns the provided pipeline if set (backward compatible: falls back to creating an isolated one if not provided). CML micro-training now directly improves production components. Remaining: initial training trigger at startup (auto-run pipeline if weights absent). | `continuous_multimodal_learning.py:59-60` accepts pipeline param; `multimodal_service.py:160` passes `self._get_pipeline()` | Startup training trigger |
