...
> **Test count baseline**: `pytest` (full testpaths) = **~5,468 collected / 0 errors** on 2026-07-03 (verified after §X #130; tests/ only: 4,948).
...
| 35 | **CNS: GlobalStateStore 升級為事件總線** | ✅ **DONE** — `emit_event()`, `subscribe_event(priority)`, `unsubscribe_event()` 已實作。10 新 test 驗證 sync/async/priority/error isolation。已接入 4/8 閉環: **EmotionSystem** (emotion.updated + emotion.behavioral_adjustment), **AutonomousLifeCycle** (lifecycle.decision_executed + lifecycle.behavioral_adjustment), **CausalReasoningEngine** (causal.learned + causal.prediction), **MetaController** (meta.confidence_recorded + meta.closed_loop_adjusted + meta.weighted_adjustment)。剩餘 4 閉環 (IntentModel, ExecutionGate, Heartbeat, Router) 待 wire。 | `global_store.py` (+10 tests), `emotion_system.py` (2 events), `autonomous_life_cycle.py` (5 events), `causal_reasoning_engine.py` (2 events), `meta_controller.py` (3 events) | 8 獨立閉環 → 4/8 wired, 12 CNS event types defined |
| 36 | **衝突協商器: MetaController weighted aggregation → PriorityNegotiator** | router.py `_prepare_generation_context()` 目前用硬編碼 Priority 1→3.5 鏈（lifecycle→emotion→intent→causal），每個優先級完全覆蓋上一個。解法：將 MetaController 的 reliability-weighted aggregation 提取為通用 `PriorityNegotiator`，讓 lifecycle/emotion/intent/causal 註冊為投票者。需要：① `PriorityNegotiator` 類（weighted fusion + confidence tracking）② 註冊 API（register_voter）③ router.py 改用 negotiator 取代硬編碼鏈。 | `ai/meta/meta_controller.py` (weighted aggregation pattern), `services/llm/router.py` (merge target), `services/llm/router.py:_prepare_generation_context()` (current hardcoded chain) | 當 Emotion 說「探索」、LifeCycle 說「保守」時，Priority 硬壓沒有真正協商 |
