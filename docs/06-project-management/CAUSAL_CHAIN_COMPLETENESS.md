...
| #201b | MD sync + orphan cleanup + integration test rewrite + usage docs. (1) MD test count sync (5,016→4,439, 5 files). (2) Deleted orphan dirs + 4 dead test files. (3) Repaired test_phase1_core_activation.py. (4) Moved 23 utility scripts tests/utils/→scripts/utils/ (2+21). (5) Fixed test_base.py: __test__=False + Any import. (6) Rewrote 6 mock-only integration tests — replaced `assert AsyncMock(return_value=True) is True` (tested Python mock mechanics, not project code) with real import + instantiation tests using production classes (KnowledgeGraphAgent, EvolutionEngine, SystemManager, DictionaryLayer, HSPConnector). 10 passed, 2 skipped (pre-existing MessageBridge bug). (7) Created docs/usage/ (QUICK_START.md, SCENARIOS.md). (8) Updated all MDs. 4,439 tests — 0 errors. | 維護+品質+文檔 | 無 C³ 影響 |
