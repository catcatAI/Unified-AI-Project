...
> **評分原則（2026-07-15 重建）**：所有分數由「可重複量測值 + 既定公式」算出，**禁止手填**。分兩類避免混淆：
> - **能力準備度 (Capability Readiness)**：基礎設施 / 數據 / 訓練的就緒程度（架構與數據已就位 → 高）。
> - **實際泛化 (Actual Generalization)**：在未見過任務上、原生引擎**自己**做到的（訓練集過擬合 + 知識/推理 0% → 低）。
> 兩類不可壓成單一「智能分」；總覽用多維表格，不合成 1 個數。
...
| **實際泛化** | `unseen_benchmark_pass_rate (排除 math 委派) × 10` | `benchmark_ed3n_garden.py` (K+R 領域) | 實際 |

> 所有 `×10` 為 0–10 映射。子指標原始值見 §1.1。重測命令見 §4.1。

### 1.1 分數總表（2026-07-15 重新測量）

| 維度 | 量測原始值 | 分數 | 說明 |
|------|-----------|------|------|
| **架構分** | 19/20 子系統接通 | **9.5/10** | 606 .py, 88 routes, 0 STUB marker |
| **知識+推理分** | 數據集 0.92 + 重現 0.807 | **8.6/10** | 460,281 條目; ED3N 0.914 / GARDEN 0.700（訓練集, 過擬合風險） |
...
| **實際泛化 (Actual)** | unseen K+R = 0/10 | **1.0/10** | 數學委派 Python `ast`; 知識/推理 0% (ED3N) |

> ⚠️ **讀法**：上面 5 個準備度分數高（基礎建設就緒），但**實際泛化只有 1.0/10**——這才是「原生引擎真實智能」。準備度高 ≠ 能力強，歷史分數膨脹正是把準備度當能力。
...
> **Angela 原生引擎實際泛化 = 1.0/10**（落在 0-2 區間，接近規則式腳本 + 字典檢索）。準備度分數（架構/知識/查詢/多模態/自主）反映「基礎建設就緒度」，不是「已具備的智能」。
...
> **ED3N 於 2026-07-15 重新測量**：math 5/5 (100%), knowledge 0/5 (0%), reasoning 0/5 (0%) → **33.3% (5/15)**。
> **GARDEN 於同環境未能完成**（>200s timeout：sentence_transformers / ChromaDB 不可用 → 回退卡住）。先前紀錄的 GARDEN 40% **不可重現**，本表不再引用為事實。

| 領域 | ED3N (re-measured) | 測試數 | 說明 |
|------|-------------------|--------|------|
| **數學** | 100% (5/5) | 5 | 經 `_try_math_eval` → Python `ast` 安全求值（**非 ED3N 學會**）|
| **知識** | 0% (0/5) | 5 | 字典無英文知識映射 |
| **推理** | 0% (0/5) | 5 | 字典無邏輯推理映射 |
| **總計 (含 math 委派)** | **33.3% (5/15)** | 15 | 通過率幾乎全是數學 |
| **總計 (排除 math 委派)** | **0% (0/10)** | 10 | 原生引擎在未見過任務上的真實貢獻 |

> **關鍵**：總通過率 33.3% 幾乎全是數學，而數學是委派給 Python `ast`，不是原生引擎學會的。扣除數學後，原生引擎在知識/推理上 **0/10**。這就是 §1.1「實際泛化 1.0/10」的來源（1.0 為慷慨值，含已驗證的 CN→EN 字典檢索 8/8）。
