...
| 2026-07-16 | — | (unreleased) | **訓練管線實跑 + forward 效能優化**：1) `scripts/train_pipeline.py` 實際跑完 8 個步驟（48 分鐘），產出並落盤 `data/checkpoints/` 下 `ed3n_full.json` / `network.json` / `garden_checkpoint/` / `joint_trainer_*` / `reflex_patterns.json` / `training_report.json`，全部經推論端 `ED3NEngine.load` / `GARDENEngine.load` 驗證可載入（S/L 斷裂已閉合）。報告：ED3N 訓練 42363 樣本（acc 0.9145）、GARDEN 1186 樣本（acc 0.7000）、ED3N 字典 250 條、GARDEN 字典 2199 條；終端實測 `1+1=2`、GARDEN 問候可答。2) **`CoreNetwork.forward()` 效能優化**：原 final 掃描迭代「全部神經元」O(N)，改為只掃描「輸入鍵 + 其直連鄰居 + 傳播結果」候選集 O(活化鄰域)，行為不變；在 40 萬連線網路上單次 forward 由 >100ms 降至 ~8ms，使 2-epoch ED3N 訓練可在合理時間內完成。3) **誠實確認**：本次事實型資料集（數學/邏輯/知識）訓練後 `network conns ≈ 0`——因架構規定知識存字典/SNN 只學關聯，事實型樣本不進 SNN 權重；SNN 關聯能力（directional/transitive/ranking/perturbation）由 `scripts/validate_association.py` 另行驗證（兩引擎 1.0），非本訓練管線產出。 |
