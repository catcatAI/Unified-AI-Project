...
2. **DET-CARRY 極高（35%+）**：系統絕大部分正確回答來自確定性引擎。SNN 單獨幾乎不貢獻（ED3N 7.8%、GARDEN 0.0%）。
...
**結論**：用戶提的差分公式（總分 − 確定性 = 開放域神經）在當前 OR-短路架構下**不直接等於 SNN 能力**，因為確定性引擎會短路、SNN 根本沒機會上場；正確做法是**強制 SNN 上場**（SNN-ONLY 欄）。測出來的低分不是「SNN 該被扣分」，而是標示「下一步必須把知識訓練進神經權重（路線 2）」——目前系統是確定性主導的混合系統，神經成分為輔助/未承載知識。這才是誠實的能力邊界。
