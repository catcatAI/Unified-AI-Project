...
> **Created**: 2026-07-04 **Updated**: 2026-09-03 (Phase-3
> ✅ 確定性未見推理 100/100，W 歧義修復；開放域維持 3.0，神經真泛化仍 0)
> **Principle**: No LLM API calls in benchmarks — scores reflect native engine
> only. **Test command**:
> `python scripts/benchmark_ed3n_garden.py --engine ed3n` **Test command**:
> `python scripts/benchmark_ed3n_garden.py --engine garden` **Test count**:
> 5,448 collected (tests/; 6,111 full), 0 errors (re-verified 2026-09-02)

---

## ⚠️ 分數類型定義 (Score Type Definitions)

**This document uses 5 distinct score types. Confusing them is the root cause of
historical score inflation.**
...
**⚠️ 歷史教訓**: PHASE_REVIEW6.md
(2026-06-23) 混淆了「框架分數」和「實際分數」，給了 7.5/10，但當時模型從未訓練過。正確做法是分別標註每種分數。
...
>
> - **能力準備度 (Capability
>   Readiness)**：基礎設施 / 數據 / 訓練的就緒程度（架構與數據已就位 → 高）。
> - **確定性引擎能力 (Deterministic Engine
>   Capability)**：數學/物理/化學等**高確定性、易計分**的任務，由專案內確定性引擎（MathVerifier
>   / MathRippleEngine
>   / 字典層）正確處理——這是**系統真實能力，應計分，不是缺陷**。
> - **學習型開放域泛化 (Learned Open-domain
>   Generalization)**：神經模型在開放域未見任務上**自己**泛化的能力（目前近 0）。三類都報，不互相抵消：不要因為模型不會就說系統不能（確定性引擎會）；也不要因為引擎會就說模型學會了。
...
> **數理化引擎分** 量測：math 5/5（經 MathVerifier 正確處理）+ CN→EN lookup 8/8
> = 高確定性能力，計入系統實力。 **知識引擎分** 量測：knowledge 5/5（經
> `ai/knowledge_base.route_knowledge`
> 正確處理）= 高確定性能力，計入系統實力，不計入神經開放域泛化。
> **符號推理引擎分** 量測：reasoning 5/5（經
> `ai/symbolic_reasoner.route_reasoning`
> 正確處理，傳遞/三段論/日曆/數量/質量陷阱）= 高確定性能力，計入系統實力，不計入神經開放域泛化。
...
| **學習型開放域泛化**               | SNN-ONLY 11%→**88%** 改述 7/8 + **FixedSizeCore 5K 軟 80%**（字節峰；模板增補）+ **確定性未見推理 100/100** 精確匹配（`probe_reasoning_unseen` symbolic 17→83→100，W 歧義修復；純神經泛化仍 0，見讀法） | **3.0/10**                | 確定性未見推理 100（傳遞規則+名次事實，6 pattern 全覆；訓練無關新實體可解）；純神經記憶覆蓋 80（字節峰）但真泛化 0——3.0 中確定性佔大頭，神經泛化未動，兩者分開報                                                |

> ⚠️
> **讀法**：專案的**確定性引擎能力很強**（數理化 9.5、知識 10、架構 9.5、查詢 9.0、自主 9.0）——這些是系統真實、可靠的能力，由數學/物理/化學確定性引擎 + 知識 KB 檢索 + 生命週期閉環提供，應計分。**神經 SNN 的本職是「學關聯性」不是「學知識」**（知識歸 KB），其正確能力指標是關聯能力（ED3N/GARDEN 皆 1.0，見 §4.1.2）。SNN 在知識/數學題上單跑趨近 0 是**設計正確**，不是弱點。**開放域泛化現已 3.0/10**（`probe_snn_unseen 88%` + 確定性未見推理
> `100/100` 精確匹配 + 神經記憶覆蓋軟 80%，Arc B570 15.5GB
> chassis-agnostic；其中純神經真泛化仍 0——3.0 幾乎全由確定性引擎貢獻），自然對話仍靠 LLM。有 LLM
> 6.0 維持。兩者分開報。
...
> **Angela 原生引擎神經 SNN 的本職能力 = 關聯性**（ED3N 1.0 / GARDEN
> 1.0，見 §4.1.2），專職學「概念間關聯」（A>taller>B），**不背知識**（知識歸 KB/字典）。因此神經能力**不能**用「知識答對率」或「關掉確定性引擎後 SNN 單跑（SNN-ONLY）」來量——那會因 OR-短路讓 SNN 沒上場、且知識題本就不歸 SNN，必然失真。正確尺是關聯能力四指標（directional/transitive/ranking/perturbation），兩引擎皆滿分。§4.1.1 的 DET-CARRY
> ≈35% 是「混合系統在知識題上的邊際增量」，非神經本職分。準備度分數（架構/知識/查詢/多模態/自主）反映「基礎建設就緒度」。確定性引擎能力（數理化 9.5
> / 知識 10 / 符號推理 10）是系統真實實力，應計分；神經關聯能力 1.0 同樣計分。
...
> **重新測量 2026-07-16（實測）**：該 script 現為 **20 case**（math 5 +
> knowledge 5 + reasoning 5 + relational-chain 5）。ED3N **20/20
> (100%)**、GARDEN **20/20
> (100%)**。但全部由**確定性引擎**接住（見下表路徑），**非神經 SNN**。
...
> **關鍵**：數學與知識 100% 由確定性引擎（MathVerifier
> / 知識 KB）正確處理——這是系統**真實且可靠**的能力，應計入「數理化引擎分」(§1.1
> = 9.5/10) 與「知識引擎分」(§1.1 =
> 10/10)，**不是缺陷、也不該強迫神經模型重學**。扣除確定性引擎後，神經模型在開放域**推理**
> = 0/5，這才是「學習型開放域泛化」(§1.1 =
> 0/10) 的弱點（核心弱點，見 §8 解法）。兩者分開看，不要互相抵消。

#### 4.1.1 三欄驗證（移除確定性引擎後神經模型表現）— 2026-07-16 實測

為回答「把確定性引擎去除，神經網路還有足夠知識正常工作嗎？」，新增
`scripts/validate_three_column.py`，對**倉庫內真實數據集**（非 5 條 hand-picked
case）做三欄對比：

- **HYBRID（混合）**：確定性引擎（計算器 / 知識 KB / 符號推理器 / 關係鏈）+
  SNN 全部開啟。
- **DET-ONLY（僅確定性）**：關閉 SNN 前向與解碼，只讓確定性引擎回答。
- **SNN-ONLY（僅神經）**：關閉所有確定性 stage，只讓 SNN 神經路徑回答。
- **DET-CARRY = HYBRID −
  SNN-ONLY**：系統對確定性引擎的依賴程度（=「神經純淨貢獻的差分」）。

數據集：每域取樣 150 條（`reasoning_train.json`
11,000 條中的多實體關係鏈、`arithmetic_test_dataset.csv`
2,000 條、`logic_test.json` 200 條），隨機間隔取樣，統計誤差 <8%。
...
1. **HYBRID 遠低於「100%
   benchmark」**：那個 100% 是用 5 條 hand-picked 簡單 case 測的；取樣 150 條真實數據後，ED3N 混合準確率掉到
   **43.1%**、GARDEN
   **35.6%**。原因：reasoning_train 的多實體長鏈只有約 1/3 能被符號/關係鏈引擎答對，其餘是 SNN 也答不出的。
2. **DET-CARRY ≈
   35%（混合系統的邊際增量，非神經本職能力）**：系統總能力 − 確定性引擎可獨立達成部分 =
   ED3N 35.3% / GARDEN
   35.6%。這是「SNN 在**知識/數學題**上對確定性引擎的補強重疊」，但**知識/數學本就不歸 SNN**（架構原則：知識→KB、關聯→SNN），所以這 35% 不是神經網路的本職能力指標。**神經的本職能力是「關聯性」**，正確測法見 §4.1.2（ED3N/GARDEN 皆 1.0）。
3. **「去除確定性引擎 SNN 就無法工作」— 這是設計正確，不是缺陷**：GARDEN 的 SNN-ONLY 在所有領域都是 0%，ED3N 也僅 reasoning
   1.3%、logic
   22%。**根因是 SNN 被設計來學「關聯」、不背知識**——知識/規則全住在確定性 KB 與正則裡，SNN 拿到的是沒有知識的空殼，這正是預期行為。拿知識答對率考 SNN 是錯的尺。
4. **logic 的例外**：ED3N 的 SNN-ONLY = 22% =
   HYBRID，說明 logic 這批題 ED3N 的 SNN/關聯路徑本就能答（或兩欄都命中同一 fallback），確定性引擎在此域無額外貢獻；GARDEN 則兩欄皆 0%（其 SNN 未承載邏輯）。

**結論**：神經 SNN 的本職是「學關聯性」（A>taller>B），**不是學知識**。因此它的正確能力指標是 §4.1.2 的**關聯能力（ED3N
1.0 / GARDEN 1.0）**，而非知識題的 SNN-ONLY（趨近 0，設計正確）或 DET-CARRY
35%（混合系統在知識題上的邊際增量，非神經本職）。「總分 1.0」這種單一壓縮無意義（維度異質）。目前系統是確定性主導的混合系統：KB/符號推理處理知識與推理，SNN 專職關聯——這才是誠實的能力邊界。下一步路線 2 是把「關聯」訓練得更深、讓 SNN 在關聯任務上承載更多，而非把知識塞進 SNN。
...
- **神經 SNN** → 專職「知識之間的關聯性」（A 比 B 高 ⇒ B 比 C 高 ⇒
  A 最高），不背知識。

若神經網路承載知識，就與普通 AI 沒差。因此 SNN 的評分標準**不該是「知識答對率」**，而該是「關聯性是否被學到」。這正是可測得的：

新增 `scripts/validate_association.py`，用**正確的關聯 API**（`add_directed` /
`add_relation` 單向）在 SNN 中建一張關係圖，測四項：
...
3. **代碼已修正防止知識滲入 SNN**：`train_pipeline.py`
   的 ED3N 訓練不再把 knowledge/reasoning/tooluse 當 input→output 映射訓練進 SNN 權重；GARDEN
   `learn_batch` 新增
   `train_associations=False`，知識事實只進字典、不進神經權重。SNN 從此只承載關聯。
...
> 註：`benchmark_ed3n_garden.py` 的 100% (20/20,
> 2026-07-16 實測) 衡量的是**確定性引擎**能力；下表「SNN 單獨泛化」欄取自 §4.1.1 三欄驗證的 SNN-ONLY 實測（去除確定性引擎後的神經淨值）。
...
> **確定性引擎 benchmark（含引擎）**：ED3N 20/20 (100%)、GARDEN 20/20
> (100%)（2026-07-16 實測，GARDEN 現已可完成——先前 >200s timeout 已由
> `_safe_chromadb_client()` 修復）。
...
**§X #196 新增**：LatentReasoningNetwork — 真正的神經網路（2層MLP +
ReLU），從 64-dim latent 做推理生成文本。可訓練（cross-entropy loss + manual
backprop）。
...
1. **核心弱點：開放域推理泛化（=0）解法**
   — 知識/數學已由確定性引擎 100% 處理，推理（傳遞/三段論/日曆/數量/質量陷阱）現已由
   **符號推理器 `ai/symbolic_reasoner.py`**
   100% 處理（見 §1.1 符號推理引擎分 10/10）。**神經推理路徑升級（§X 本次）**：`LatentReasoningNetwork`
   已修復（autoregressive 生成 + 全序列 CE 訓練 +
   npz 持久化，仍為 opt-in/ml 層級）並接入 ED3N 儲存；新增離線關係鏈推理
   `ai/reasoning/relational_chain.py`（傳遞閉包，無 LLM/無 torch），已同時接入 ED3N（Stage
   1.6b, CoreNetwork）與 GARDEN（Stage
   1.6b），補足符號推理器正則未涵蓋的新穎比較詞/長鏈/換述（benchmark chain
   5/5 兩引擎皆 100%）。剩餘弱點：神經模型「從零學會」開放域推理，解法：(a) 原生推理請求**路由到 LLM**（已有 LLM 路徑）；(b) 在開放域推理數據集上真正訓練神經模型（非訓練集過擬合）。短期以 (a) 最務實——確定性引擎（數學/知識/符號推理/關係鏈）已覆蓋 benchmark 全部結構性問題。
2. **神經網路專職「關聯性」迭代路線（2026-07-16 確立）**
   — 架構原則：**知識存字典/KB，關聯存 SNN**，SNN 不背知識（否則與普通 AI 無異）。`scripts/validate_association.py`
   已使「關聯能力」可測（directional/transitive/ranking/perturbation），兩引擎現皆 1.0（見 §4.1.2）。下一步迭代：① 把訓練管線從「input→output 映射」改為「由
...
- **實際**：`_try_math_eval()` → 字典層 `route_math()` →
  `MathVerifier.evaluate_math()` → Python
  `ast.parse` + 安全求值（含中文數字/運算符轉換）。`MathRippleEngine`
  現僅做**漣漪/狀態傳遞**（認知過載、除法恐懼、順序混淆），其數值結果已委託給
  `MathVerifier`（唯一計算源）。
- **結論**：數學能力來自 Python `ast` 模組 +
  `MathVerifier`，不是 ED3N 學會的。ED3N/GARDEN 的字典映射在數學路徑上根本沒被用到——它們只負責「計算符路由」，不負責算。
...
- **實際**：GARDEN 的 `learn_from_interaction()` 使用 Hebbian 學習（Oja's
  rule），target_strength=0.7。accuracy
  0.7 只是反映 Hebbian 收斂到目標值，不代表「理解」。
- **教訓**：Hebbian 收斂 ≠ 理解。

#### 錯誤 6：JointTrainer accuracy 0.939 的局限

- **表面**：JointTrainer accuracy 0.939
- **實際**：JointTrainer 只是把 ED3N 和 GARDEN 的輸出結合，accuracy 高是因為大部分問題都走
  `MathVerifier`（單一計算源，Python `ast` 安全求值），不是真正的跨域推理。
- **教訓**：組合 accuracy ≠ 單獨能力。
...
| 2026-07-18 | —    | (unreleased)   | **全專案功能檢查 + 修復全部紅項**：用戶要求確認「專案所有功能是否都正常」並正確修復。做法：全量收集 4,499 測試 0 錯誤；分批實跑 ai(ed3n/garden/meta/response/alignment/context/multimodal/vision/lifecycle/core/integration/agents)/services/api/core/unit/tools/security/shared/fragmenta/benchmarks/perf/models/mcp/pet/integration/utils/desktop，並真實開 server 打 `/health` 與 `/chat/unified`。發現並修復：(1) 上輪重構引入的 `_pick_best_backend` 誤從 `core.interfaces.protocols` import `LLMBackend`（符號不存在）→ chat service 啟動失敗，改回用 router 模組頂層正確 import；(2) 預先存在的缺模組 `services.error_handling`（`mobile.py`/`google_drive_handler.py` 都 `from services.error_handling import safe_error`）→ 新建 `services/error_handling.py` 轉發 `core.utils.safe_error`，修復 26 個 api 失敗 + server `v1 endpoint routers not available` 警告；(3) `tests/core/perception/test_auditory_memory.py` 測試腐化（斷言舊 API `config`/`memory_store`，但類別已重構為 `profiles`/`capacity`）→ 依現有 API 重寫測試；(4) `ai/garden/_import_utils.subprocess_check` 在 Win/Py3.14 對 torch/chromadb 的 subprocess 探測逾時 → 改用 `importlib.util.find_spec` 快速非執行探測（保留 subprocess fallback），修復 8 個 garden encoder 失敗；(5) `test_semantic_encoders.py::test_init_no_torch` 環境敏感（假設 torch 不存在，但本環境已裝）→ 改重置模組級 `_CLIP_AVAILABLE` 快取後斷言；(6) `test_garden_provider.py::test_instantiation_defaults` 斷言 checkpoint 為空，但本部署已訓練 `garden_checkpoint`（S/L-gap 閉合設計）→ 改斷言解析後的路徑；(7) `test_multimodal_stress.py` 4 項在合併大套件下因 process-wide `get_shared_latent_space` 單例被其他測試污染而間歇失敗（隔離跑全過）→ 加 autouse fixture 隔離該單例，使壓力測試確定性。最終：受影響套件全綠，server 啟動乾淨、`/health`→200、聊天本地答案正確；全樹唯一剩餘紅項皆已修復。flake8 乾淨。Test count 維持 4,497。 |
| 2026-08-28 | —    | (unreleased)   | **§X #265 硬體/計算資源審計 + 精簡省資源修復**：全鏈路審查 compute/hardware/intelligence/resource 四維，無 P0。修復：(1) `chat_service._ham_sync_loop` 增 jitter `0~min(300, interval*0.1)` 防 thundering herd；(2) `vector_store.bulk_add_memories` 分塊 4096 (峰值 195→8MB/chunk，100k 批量不再瞬間 390MB)；(3) `semantic_qa` 閾值 0.80 改為 `threshold_value("semantic_qa.threshold")` 配置化 (默認 0.80，可經 tiered config 降至 0.70 提升離線召回)；(4) `compute.default.yaml` 補註有效分配公式 `min(ceiling, sqrt(usable*percent/4))` 防 10GB 誤導；(5) `configs/standard/compute.default.yaml` 補空檔明確「system 已足夠」消除文件不一致；(6) `ci.yml` 三處加 `ANGELA_HARDWARE_PROFILE=high_performance_desktop` 使 CI 不走 LOW_POWER_DEVICE 而實際測 GPU 路徑。驗證：`5432 collected, 0 errors`；`SharedLatentSpace` 僅 5 modality 鍵有界無需 LRU；`GARDEN` 10GB 由 `effective_capacity_bytes` + `usable-2GB` 雙限已安全；UNIFIED 259MB 固定。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| 2026-08-28 | —    | (unreleased)   | **§X #266 統一引擎可配置 + 語義 QA ONNX 化 (更智能更省資源)**：(1) `compute.default.yaml` 新增 `unified` 段 `slots:65536/use_feat:true/use_delta:true` + `high_performance_desktop` 註記 `131072→-5.6% bpc` 可選、`low_power_device` 降至 `32768/feat:false` 省 ~192MB (323→131MB)；`UnifiedEngine.__init__(slots/use_feat/use_delta)` 改為 `compute_int("unified","slots")/_get` 讀配置 + 128k 時按 `effective_capacity_bytes("memory", usable-2GB)` 冪次鉗制防 8GB OOM；默認保持 65k 不改變現有行為與測試；(2) `semantic_qa` `_features()` 優先走 `garden/dictionary._OnnxEncoder` (paraphrase-multilingual-MiniLM-L12-v2 int8, 384維, 2.1x CPU, 多語言) 失敗回退 hash，`_embedding_dim()` 自適應 384/512，`learn()` 按探測維度註冊 SLS，實測 `法国的首都是什么→Paris 0.98` (hash 丟棄 CJK 必失敗) 而 `enwik9 bpc` 不受影響；(3) 更新頭部 `Updated: 2026-08-28 §X #266`。驗證：`UnifiedEngine(slots=65536) 323MB / slots=32768 131MB`；`SemanticQA ONNX dim 384`；`5432 collected, 0 errors`。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| 2026-08-28 | —    | (unreleased)   | **§X #271 閾值自調閉環 + 四類可調性 (狀態≒情緒≒反應)**：用戶問「閾值可調就能讓 Angela 自己因狀態有情緒且產生反應吧? 狀態≒情緒≒反應?」。稽核網狀 `狀態(6D αβγδεθ, 41 字段) → 情緒(PAD 7 類, 15 荷爾蒙映射) → 反應(8 voter 加權) → 文本(LLM) → 反饋(engagement) → 情緒/狀態` 已閉環 6 處 (`chat_routes:_fire_causal_learning`)；但 `semantic_qa` 閾值僅配置化未接情緒 (`DynamicThresholdManager` 亦孤島)。修復：(1) `semantic_qa.py` 新增 `set_emotion_threshold_adjustment()` + `_EMOTION_ADJ` 全局 + `_emotion_threshold_adjustment()` 三路徑探測 (注入>state_store>EmotionSystem)，`_threshold()` 改為 `base(0.75) + adj` 限幅 `[0.60,0.85]`，`adj`: `exploratory -0.05 / conservative +0.05 / 高 arousal -0.03 / 低 valence +0.03 / 疲勞≥3 +0.08`；(2) `chat_routes.py:_inject_emotion_behavioral_context` 在 `es.apply_influence + get_behavioral_adjustment` 後計算 `delta` 並 `set_emotion_threshold_adjustment(delta)`，使 `狀態(6D)→情緒(PAD)→閾值→反應(文本)` 真正自調：狀態寫死基線不可學 (模擬人體體溫)，增量與歷史可塑，閾值隨喜/懼/疲勞動態 `0.70↔0.85`；(3) 文檔四類可調性：AI 自調(Hebbian/因果/情緒反饋等5項)、LLM 可改(prompt/路由/文本5項)、用戶可改(env/config/密鑰5項)、都不能改(生理基線/安全硬線5項)。驗證：`exploratory 0.70 / conservative+疲勞 0.85`，`flake8 0`。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| 2026-08-28 | —    | (unreleased)   | **§X #272 實戰對話多話題驗證與修復**：`POST /api/v1/chat/unified` 真實對話 10+ 話題 (中英/數學/知識/情感/創作)。發現異常：(1) `capital of France` 等事實經 `memory-template` 誤匹配 `France→Japan` (TF-IDF 對 `capital` 過粗) 導致 `what is the capital of Japan→Paris` 錯答；(2) `what is the capital of Japan` 因 `what` 前綴被 honest gate 誤攔 (語義 QA 可答 `Tokyo 0.99` 卻被 `what→honest` 短路)；(3) `unified` 內 `semantic_qa` 守衛 `capital→Paris` 因答案不含查詢詞被誤過濾 (`sim 0.98` 卻因無詞重疊被丟)；(4) `unified` 默認 QA 為空 (`qa_knowledge.json` 缺失) 導致離線事實全 fallback 為 `早安～...` 長問候。修復：(1) `router.py:_try_memory_retrieval` 對 `capital/首都` 查詢直接短路 (讓 `unified` 語義 QA 處理)；(2) `router.py` honest gate 內加 `semantic_qa` 探測 (含 `UnifiedEngine` 懶探測)，可答則不 honest；(3) `unified_engine.py` 語義守衛改為僅 `sim<0.85` 時才檢查詞重疊 (事實 `Paris/Tokyo 0.99` 直通)；(4) `unified_engine.py:_load_default_qa_knowledge` 無文件時內建 12 條回退事實 (`France Paris/Japan Tokyo/China Beijing/sky blue/天空蓝色/...`) 並 `learn(10 epochs)`。重測：`hello→template, 2+2→4, capital of France→Paris, 法国首都→巴黎, 天空蓝色, Japan Tokyo (fresh session), sky blue` 全正確；`what is the capital of Japan→Tokyo` (原 `Paris` 錯) 已修；`tell me a joke/poem` 等創作仍 honest/長問候 (預期, 無 LLM)。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |

---

> **⚠️ 重要提醒**: 本評估表的所有分數均為**原生引擎**能力，不包含 LLM
> API 調用。任何使用 LLM 的測試都必須明確標記，避免分數膨脹。
