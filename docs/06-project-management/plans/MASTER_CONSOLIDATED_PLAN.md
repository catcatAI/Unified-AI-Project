...
但以下項目需謹慎看待：
  ...
  D7 (exc_info=True) → ⚠️ **文件內矛盾**: L472 說「0剩餘」, L495 說「48% (309/636)」，需驗證
   Card pipeline → ChatService 整合 → 🟡 **部分完成**（M0 ModuleManager 核心 + M1 card_pipeline 模組 + M2 intent_registry 模組已實作。ChatService 接線 (Phase 2) 尚未開始。ANGELA_CARD_INTEGRATION_PLAN v2 已在執行中。）
...
| **M 級** (ModuleManager) | **3** | **✅ ~5 天 (M0 核心 + M1 card_pipeline + M2 intent_registry 模組)** |
| **總計** | **53** | **~45-55 天 (全職) — 約 50/53 完成** |

**剩餘項目**:
- D7: logger.error exc_info=True 狀態矛盾（L472:0剩餘 vs L495:48%）— 需 grep 驗證
- Card pipeline → ChatService 整合: Phase 2 (ChatService→IntentRegistry 接線) 尚未開始
