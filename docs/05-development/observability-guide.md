...
This guide aligns the /api/v1/hot/status endpoint fields with a minimal
dashboard schema for quick visibility.
...
- `hsp: object` — communication status from
  HSPConnector.get_communication_status()
- `mcp: object` — communication status from
  MCPConnector.get_communication_status()
...
- The `learning.tools` block is reserved for the Action–Tool Policy aggregation
  to be implemented.
- Values are best-effort and may be null; clients should handle missing fields
  gracefully.
  ...
  "panels": [
    {
      ...
      "fields": [
        ...
        { "label": "Pending ACKs", "path": "metrics.hsp.pending_acks_count" },
        {
          ...
          "path": "metrics.hsp.retry_counts_active",
        },
      ],
    },
    {
      ...
      "fields": [
        ...
        {
          "label": "Fallback Init",
          "path": "metrics.mcp.fallback_initialized",
        },
      ],
    },
    {
      ...
      "fields": [
        ...
        {
          "label": "Tool Success %",
          "path": "metrics.learning.tools.success_rate",
        },
      ],
    },
    {
      ...
      "fields": [
        {
          ...
          "path": "metrics.lis.incidents_recent",
        },
        {
          "label": "Antibodies (recent)",
          "path": "metrics.lis.antibodies_recent",
        },
      ],
    },
  ],
...
- Surface last update timestamp and, optionally, a link to raw JSON for
  debugging.

## 5. Next Steps

- Implement Action–Tool Policy logging in ToolDispatcher and aggregate into
