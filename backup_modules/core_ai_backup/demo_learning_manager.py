...
class DemoLearningManager:
    ...
    def __init__(self, config_path: str = "configs/demo_credentials.yaml") -> None:
        ...
        # 添加測試所需的屬性
        self.training_configs: Dict[str, Any] = 
    ...
    async def start_learning(self, model_id: str, config: Dict[str, Any]) -> Dict[str, Any]:
        """開始學習"""
    ...
    async def activate_demo_mode(self, credentials: Dict[str, Any]) -> None:
        """激活演示模式
        ...
        for action in actions:
            ...
            try:
                _ = await self._execute_action(action_name)
            except Exception as e:
    ...
    async def _execute_action(self, action_name: str) -> None:
        ...
        if action_name == "enable_demo_mode":
            _ = await self._enable_demo_mode
        elif action_name == "initialize_learning":
            _ = await self._initialize_learning
        elif action_name == "setup_mock_services":
            _ = await self._setup_mock_services
        elif action_name == "configure_auto_cleanup":
            _ = await self._configure_auto_cleanup
        else:
    ...
    async def _initialize_learning(self) -> None:
        ...
        # 保存初始學習數據
        _ = await self._save_learning_data
    ...
    async def _configure_auto_cleanup(self) -> None:
        """配置自動清除"""
        logger.info("配置自動清除")
    ...
    async def _learning_monitor_loop(self) -> None:
        """學習監控循環"""
        while self.demo_mode:
            try:
                _ = await asyncio.sleep(60)  # 每分鐘檢查一次
                _ = await self._collect_learning_data
            except Exception as e:
    ...
    async def _cleanup_monitor_loop(self) -> None:
        ...
        while self.demo_mode:
            try:
                _ = await asyncio.sleep(3600)  # 每小時檢查一次
                _ = await self._perform_cleanup(cleanup_config)
            except Exception as e:
