...
class LightweightCodeModel:
    ...
    def _extract_method_parameters(self, method_node: Union[ast.FunctionDef, ast.AsyncFunctionDef]) -> List[Dict[str, Any]]:
        """
        ...
        for i, arg_node in enumerate(all_args):
            ...
            if i >= pos_defaults_start_idx and i < num_pos_args: # Positional/regular arg with default
                ...
                # 确保 default_val_node 不是 None 再调用 ast.unparse
                if default_val_node is not None:
        ...
        if args.vararg:
            ...
            if args.vararg.annotation:
                # 确保 annotation 不是 None 再调用 ast.unparse
            ...
            if isinstance(node, ast.ClassDef):
                ...
                for item in node.body:
                    if isinstance(item, (ast.FunctionDef, ast.AsyncFunctionDef)):
                        ...
                        if item.returns:
                            # 确保 item.returns 不是 None 再调用 ast.unparse
                ...
                if node.returns:
                    # 确保 node.returns 不是 None 再调用 ast.unparse
