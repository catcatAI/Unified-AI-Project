...
from typing import List, Dict, Any, Optional, Tuple, TypedDict, TYPE_CHECKING  # Added TypedDict
...
class ContentAnalyzerModule:
    ...
    def __init__(self, spacy_model_name: str = "en_core_web_sm") -> None:
        ...
        if ContentAnalyzerModule._nlp_model is None:
            try:
                ...
                ContentAnalyzerModule._nlp_model = nlp_model
                ...
                try:
                    ...
                    # 重新导入spacy确保在当前作用域中可用
                    import spacy as spacy_module
                    ContentAnalyzerModule._nlp_model = spacy_module.load(spacy_model_name)
                    print(f"Successfully downloaded and loaded spaCy model: {spacy_model_name}")
                except Exception as e:
                    print(f"Failed to download/load {spacy_model_name}: {e}")
    ...
    def _add_custom_matcher_patterns(self):
        ...
        person_title_org_pattern = [
            ...
            _ = {"POS": {"IN": ["NOUN", "PROPN"]}},  # TITLE (e.g., "CEO", "founder")
            {"LOWER": "of"},
    ...
    def analyze_content(self, text: str) -> Tuple[KnowledgeGraph, Any]:
        """
                    ...
                    for entity_id, entity_data in entities.items:
                        ...
                        if not org_entity_id:
                            ...
                            if of_pos != -1 and of_pos + 1 < len(doc):
                                ...
                                # 创建一个模拟token用于实体创建
                                class MockToken:
                                    def __init__(self, text, idx, ent_type, pos) -> None:
                                        self.text = text
                                        self.idx = idx
    ...
    def process_hsp_fact_content(
        self, hsp_fact_payload, source_ai_id: str
    ):
        """
        ...
        if statement_type == "natural_language":
            ...
            # Extract components
            subject_uri = structured_data.get("subject_uri") if isinstance(structured_data, dict) else None
            predicate_uri = structured_data.get("predicate_uri") if isinstance(structured_data, dict) else None
            object_uri = structured_data.get("object_uri") if isinstance(structured_data, dict) else None
            object_literal = structured_data.get("object_literal") if isinstance(structured_data, dict) else None
            object_datatype = structured_data.get("object_datatype") if isinstance(structured_data, dict) else None
            ...
            if object_is_uri and object_uri is not None:
                ...
                # 确保 mapped_object_id 不是 None 再调用 startswith
                if mapped_object_id is not None and isinstance(mapped_object_id, str):
                    prefixes = self.internal_uri_prefixes.get("entity_type", ["cai_type:"])
                    if isinstance(prefixes, str):
                        prefixes = [prefixes]
                    # 确保 mapped_object_id 是字符串再调用 startswith
                    if isinstance(mapped_object_id, str) and any(mapped_object_id.startswith(prefix) for prefix in prefixes):
                        object_type = mapped_object_id
            else:
                # For literals, create a node with a generated ID
                literal_str = str(object_literal) if object_literal is not None else ""
            ...
            if not self.graph.has_node(mapped_subject_id):
                subject_label = subject_uri.split("/")[-1].split("#")[-1] if subject_uri else "unknown_subject"
                self.graph.add_node(
                    mapped_subject_id,
                    label=subject_label,
                    type="HSP_URI_Entity",
            ...
            processed_triple_info: ProcessedTripleInfo = {
                "subject_id": mapped_subject_id if isinstance(mapped_subject_id, str) else "",
                "predicate_type": mapped_predicate_type if isinstance(mapped_predicate_type, str) else "",
                "object_id": mapped_object_id if isinstance(mapped_object_id, str) else "",
                "original_subject_uri": subject_uri if isinstance(subject_uri, str) else "",
                "original_predicate_uri": predicate_uri if isinstance(predicate_uri, str) else "",
                "original_object_uri_or_literal": object_uri if isinstance(object_uri, str) else object_literal,
