...
class ProjectCoordinator:
    def __init__(self,
                 ...
                 hsp_connector: Optional[HSPConnector],  # 修改为可选参数
                 agent_manager: Optional[AgentManager],  # 修改为可选参数
        ...
        self.ai_id = self.hsp_connector.ai_id if self.hsp_connector else "project_coordinator"  # 处理 None 情况
        self._load_prompts
    ...
    async def handle_project(self, project_query: str, session_id: Optional[str], user_id: Optional[str]) -> str:
        ...
        # 修改检查条件，正确处理 hsp_connector 为 None 的情况
    ...
    async def _send_hsp_request(self, capability: HSPCapabilityAdvertisementPayload, parameters: Dict[str, Any], description: str) -> Tuple[Optional[str], Optional[str]]:
        ...
        # Check if hsp_connector is available
        ...
        # Safely call send_task_request method
        correlation_id = await self.hsp_connector.send_task_request(hsp_task_payload, mqtt_topic) if self.hsp_connector else None
        if correlation_id:
        ...
        # Return the correct format: (correlation_id, user_message)
