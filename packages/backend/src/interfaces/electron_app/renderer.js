document.addEventListener("DOMContentLoaded", () => {
  ...
  const hspTaskResponseDisplay = document.getElementById(
    "hspTaskResponseDisplay",
  );
  ...
  function showView(viewId) {
    ...
    if (
      viewId === "hspServicesView" &&
      hspServiceListDiv.children.length === 0
    ) {
      loadHspServices();
    }
  }
  ...
  async function sendMessage() {
    ...
    if (!currentSessionId) {
      appendMessage(
        "Session not started. Please wait or try restarting.",
        "system-error",
      );
  ...
  async function loadHspServices() {
    hspServiceListDiv.innerHTML =
      '<p class="loading-message"><i>Loading HSP services...</i></p>';
    try {
      if (window.electronAPI && window.electronAPI.invoke) {
  ...
  function updateTaskStatusDisplay(correlationId, statusData) {
    ...
    if (
      ...
      if (activePolls[correlationId]) {
        ...
        console.log(
          `Polling stopped for task ${correlationId}. Status: ${statusData.status}`,
        );
      }
    }
  }
      ...
      if (window.electronAPI && window.electronAPI.invoke) {
        ...
        if (response.correlation_id) {
          ...
          if (response.correlation_id && !response.error) {
            if (activePolls[response.correlation_id]) {
              // Clear any old poll for this ID
