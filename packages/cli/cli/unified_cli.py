...
def main():
    ...
    parser.add_argument("--url", default=os.environ.get("CLI_BASE_URL", "http://localhost:8000"),
                        help="Backend API URL (or set CLI_BASE_URL)")
    parser.add_argument("--token", default=os.environ.get("CLI_TOKEN"),
                        help="Auth token (or set CLI_TOKEN)")
    parser.add_argument("--timeout", type=int,
                        default=int(os.environ.get("CLI_TIMEOUT", "10")),
                        help="Request timeout in seconds (or set CLI_TIMEOUT)")
    ...
    atlassian_subparsers = atlassian_parser.add_subparsers(dest="atlassian_command",
                                                           help="Atlassian commands")
    atlassian_subparsers.add_parser("status", help="Check Atlassian CLI status")
    ...
    jira_subparsers.add_parser("projects", help="List Jira projects")
    ...
    jira_issues_parser.add_argument("--limit", type=int, default=10,
                                    help="Number of issues to return")
    ...
    confluence_parser = atlassian_subparsers.add_parser("confluence",
                                                        help="Confluence operations")
    confluence_subparsers = confluence_parser.add_subparsers(dest="confluence_command",
                                                              help="Confluence commands")
    confluence_subparsers.add_parser("spaces", help="List Confluence spaces")

    confluence_search_parser = confluence_subparsers.add_parser("search",
                                                                 help="Search Confluence content")
    confluence_search_parser.add_argument("query", help="Search query")
    confluence_search_parser.add_argument("--limit", type=int, default=10,
                ...
                if result and result.get('success'):
                    ...
                    for issue in issues:
                        if isinstance(issue, dict):
                            print(f"  - {issue.get('key', 'N/A')}: "
                                  f"{issue.get('summary', 'N/A')}")
                        else:
                            print(f"  - {issue}")
                else:
                    err = result.get('error', 'Unknown error') if result else 'No response'
                    print(f"Failed to get issues: {err}")
            elif args.jira_command == "create":
                print(f"Creating Jira issue in {args.project}...")
                result = client.create_jira_issue(args.project, args.summary,
                ...
                if result and result.get('success'):
                    ...
                    for space in spaces[:10]:
                        if isinstance(space, dict):
                            print(f"  - {space.get('key', 'N/A')}: "
                                  f"{space.get('name', 'N/A')}")
                        else:
                            print(f"  - {space}")
                else:
                    err = result.get('error', 'Unknown error') if result else 'No response'
                    print(f"Failed to get spaces: {err}")
