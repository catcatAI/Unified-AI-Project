...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function loadAvailableModels() {
    ...
    if (window.electronAPI && window.electronAPI.live2d) {
      window.electronAPI.live2d
        .getModels()
        .then((models) => {
          console.log(
            '[Settings] Found models:',
            models.map((m) => m.name)
          )
  ...
  function applySettingsToUI(settings) {
    ...
    if (windowOpacity) {
      ...
      if (opacityValue)
        opacityValue.textContent = `${Math.round((settings.windowOpacity || 1) * 100)}%`
    }
    ...
    if (continuousRecognition)
      continuousRecognition.checked = settings.continuousRecognition !== false
    ...
    if (clickIntensity) {
      ...
      if (clickValue)
        clickValue.textContent = `${Math.round((settings.clickIntensity || 0.5) * 100)}%`
    }
    ...
    if (touchIntensity) {
      ...
      if (touchValue)
        touchValue.textContent = `${Math.round((settings.touchIntensity || 0.8) * 100)}%`
    }
    ...
    if (emotionIntensity) {
      ...
      if (emotionValue)
        emotionValue.textContent = `${Math.round((settings.emotionIntensity || 0.7) * 100)}%`
    }
  ...
  async function updateMonitorUI() {
    ...
    try {
      ...
      if (monMemTotal)
        monMemTotal.textContent = `${(data.hardware.memory.total / 1024 ** 3).toFixed(1)} GB`
  ...
  function applySettingsToApplication(settings) {
    ...
    if (window.angelaApp && window.angelaApp.live2dManager) {
      if (settings.renderMode) {
        if (
          settings.renderMode === 'live2d' &&
          window.angelaApp.live2dManager.getMode() === 'fallback'
        ) {
          window.angelaApp.live2dManager.switchToLive2D()
        } else if (
          settings.renderMode === 'fallback' &&
          window.angelaApp.live2dManager.getMode() === 'live2d'
        ) {
