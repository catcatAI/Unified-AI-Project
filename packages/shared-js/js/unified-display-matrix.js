...
// 获取Angela角色配置（如果已加载）
const ANGELA_CONFIG =
  typeof window.ANGELA_CHARACTER_CONFIG !== 'undefined' ? window.ANGELA_CHARACTER_CONFIG : null
...
class UnifiedDisplayMatrix {
  ...
  handleTouch(screenX, screenY, touchType = 'pat') {
    ...
    try {
      ...
      try {
        hapticIntensity = this.calculateHapticIntensity(0.8, canvasCoords, {
          width: 20,
          height: 20,
        })
      ...
      result.success =
        ...
        result.errors.every(
          (e) =>
            e.step === 'haptic_feedback' ||
            e.step === 'live2d_expression' ||
            e.step === 'notify_listeners'
        )
  ...
  setUserScale(scale) {
    ...
    if (clampedScale !== this.currentState.userScale) {
      ...
      console.log(
        '[UDM] Scale:',
        `${(oldScale * 100).toFixed(0)}% → ${(clampedScale * 100).toFixed(0)}%`
      )
    }
  }
  ...
  getTransformMatrix(from, to) {
    const matrices = {
      ...
      'canvas→resource': {
        scaleX:
          this.resourceMatrix[this.currentState.resourcePrecision].width /
          this.currentState.baseWidth,
        scaleY:
          this.resourceMatrix[this.currentState.resourcePrecision].height /
          this.currentState.baseHeight,
      },
      'resource→canvas': {
        scaleX:
          this.currentState.baseWidth /
