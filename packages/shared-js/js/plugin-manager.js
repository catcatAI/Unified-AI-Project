class PluginManager {
    ...
    try {
      ...
      if (errCount >= this.config.maxErrorsBeforeDisable) {
        ...
        if (plugin) {
          ...
          this._log(
            'error',
            `Plugin ${pluginName} auto-disabled after ${errCount} consecutive errors`
          )
        }
      }
  ...
  async executeHook(hookName, data = null) {
    ...
    const handlers = this.hooks.get(hookName).filter((h) => {
    ...
    for (const { handler, pluginName } of handlers) {
      const outcome = await this._executeWithTimeout(
        handler,
        data,
        pluginName,
