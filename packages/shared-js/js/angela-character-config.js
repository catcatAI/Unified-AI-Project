...
function validateCharacterConfig() {
    ...
    for (const [zoneName, zoneData] of Object.entries(ANGELA_CHARACTER_CONFIG.touch_zones)) {
      ...
      if (zoneData.rect) {
        if (!Array.isArray(zoneData.rect) || zoneData.rect.length !== 4) {
          result.errors.push(
            `Touch zone "${zoneName}" has invalid rect format: expected [x1, y1, x2, y2]`
          )
          result.valid = false
        }
      }
      ...
      if (zoneData.expression && !validExpressions.includes(zoneData.expression)) {
        result.warnings.push(
          `Touch zone "${zoneName}" has unknown expression: ${zoneData.expression}`
        )
      }
      ...
      if (zoneData.tactile_type && !validTactileTypes.includes(zoneData.tactile_type)) {
        result.warnings.push(
          `Touch zone "${zoneName}" has unknown tactile_type: ${zoneData.tactile_type}`
        )
      }
    }
  }
  ...
  if (ANGELA_CHARACTER_CONFIG.image_path && typeof window !== 'undefined') {
    ...
    const img = new Image()
    img.onload = () =>
      console.log(`[CharacterConfig] Image found: ${ANGELA_CHARACTER_CONFIG.image_path}`)
    img.onerror = () => {
      result.warnings.push(
        `Image not found or cannot be loaded: ${ANGELA_CHARACTER_CONFIG.image_path}`
      )
    }
