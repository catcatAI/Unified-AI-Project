...
class StateMatrix4D {
  ...
  _trimHistoryByMemory() {
    try {
      ...
      if (memoryUsage > this.historyMemoryThreshold) {
        ...
        console.log(
          `[StateMatrix4D] 清理历史记录：移除 ${removeCount} 条记录（内存使用 ${memoryUsage} bytes > ${this.historyMemoryThreshold} bytes）`
        )
        this.history.splice(0, removeCount)
      }
    } catch (error) {
    ...
    this.historyCleanupInterval = setInterval(() => {
      try {
        ...
        if (this.history.length > 0) {
          const memoryUsage = this._estimateHistoryMemory()
          console.log(
            `[StateMatrix4D] 历史记录统计：${this.history.length} 条记录，约 ${memoryUsage} bytes`
          )
        }
      } catch (error) {
  ...
  _applyComplexMappings() {
    ...
    if (this.delta) {
      ...
      this.live2DManager.setParameter(
        'ParamBodyAngleX',
        currentBodyX + (Math.random() - 0.5) * bodyAngleVariation
      )
  ...
  _applyFallbackLayers() {
    ...
    console.log(
      `[StateMatrix] Applied fallback layers: expression=${this.live2DManager.expressionIndex}, pose=${this.live2DManager.poseIndex}`
    )
  }
  ...
  applyInfluence(source, target, amount) {
    ...
    if (target === 'alpha') {
      ...
      if (source === 'alpha') {
        ...
        targetDim.values.happiness = Math.min(
          1,
          targetDim.values.happiness + amount * comfort * 0.1
        )
      ...
      if (source === 'gamma') {
        const happiness = sourceDim.values.happiness || 0.5
        targetDim.values.engagement = Math.min(
          1,
          targetDim.values.engagement + amount * happiness * 0.1
        )
        targetDim.values.presence = Math.min(
          1,
          targetDim.values.presence + amount * happiness * 0.08
        )
      }

      if (source === 'beta') {
        const curiosity = sourceDim.values.curiosity || 0.5
        targetDim.values.attention = Math.min(
          1,
          targetDim.values.attention + amount * curiosity * 0.1
        )
      }
    }
  ...
  _computeOverallMood(state) {
    ...
    // 计算主要情绪维度的平均分
    const gammaAvg =
      (state.dimensions.gamma.emotions.happiness +
        state.dimensions.gamma.emotions.calm +
        state.dimensions.gamma.emotions.love +
        state.dimensions.gamma.emotions.trust) /
      4

    const alphaAvg =
      (state.dimensions.alpha.emotions.energy +
        state.dimensions.alpha.emotions.comfort +
        state.dimensions.alpha.emotions.arousal +
        state.dimensions.alpha.emotions.vitality) /
      4
  ...
  _throttledSendStateUpdate(dimensionName, changes) {
    ...
    // 計算平均更新間隔
    const avgInterval =
      this._updateRateHistory.reduce((a, b) => a + b, 0) / this._updateRateHistory.length
  ...
  handleInteraction(type, data = {}) {
    try {
      ...
      console.error(
        '[StateMatrix4D] Interaction handling failed:',
        error,
        'type:',
        type,
        'data:',
  ...
  handleInteractionSpeech(data) {
    try {
      this.updateDelta({
        attention: 1.0,
        engagement: 0.9,
        bond: Math.min(1, this.delta.values.bond + 0.05),
      })
