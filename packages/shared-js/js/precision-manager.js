class PrecisionManager {
  ...
  setGlobalPrecision(level) {
    ...
    console.log(
      `Global precision set to: ${this.getPrecisionName(this.precisionLevel)} (scale: ${this.scale})`
    )
  ...
  autoAdjustPrecision() {
    ...
    if (recommended !== this.precisionLevel) {
      console.log(
        `Auto-adjusting precision from ${this.getPrecisionName(this.precisionLevel)} to ${this.getPrecisionName(recommended)}`
      )
  ...
  updateMetrics() {
    ...
    this.metrics.avg_precision =
      precisions.length > 0 ? precisions.reduce((a, b) => a + b, 0) / precisions.length : 0

    this.metrics.cells_with_memory = Object.values(this.cells).filter(
      (c) => c.memory_ref !== null
    ).length
  }
  ...
  getPrecisionStats() {
    ...
    for (let i = 0; i <= 4; i++) {
      stats[`DEC${i}`] = Object.values(this.cells).filter((c) => c.precision_level === i).length
    }
