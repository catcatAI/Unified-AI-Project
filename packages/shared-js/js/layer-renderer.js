 ...
 *
 * Manage three-layer overlay rendering system
 * - Layer 1: 主立繪層 (base)
...
class LayerRenderer {
    ...
    try {
      ...
      for (const [imageId, imageData] of Object.entries(images)) {
        ...
        if (layerType && this.layers.hasOwnProperty(layerType)) {
          ...
          console.log(
            `[LayerRenderer] Loaded ${layerType} layer: ${imageId}, transparency: ${img.hasTransparency}`
          )
        }
      }
  ...
  _renderSpriteSheet(img, config, layerType, hasTransparency) {
    ...
    if (overlayPositions && currentItem.name && overlayPositions[currentItem.name]) {
      ...
      console.log(
        `[LayerRenderer] Rendered ${layerType} layer: ${currentItem.name} at (${targetX}, ${targetY})`
      )
    } else {
      // 降級方案：使用網格位置渲染（居中）
      ...
      console.warn(
        `[LayerRenderer] Using fallback grid rendering for ${layerType} layer: ${currentItem.name}`
      )
    }
  }
        ...
        if (!this.layerConfig[layerType]) {
          ...
          if (this.layerConfig[layerType].opacity < 0 || this.layerConfig[layerType].opacity > 1) {
            result.warnings.push(
              `Layer ${layerType} opacity out of range [0,1]: ${this.layerConfig[layerType].opacity}`
            )
          }
          ...
          if (!validBlendModes.includes(this.layerConfig[layerType].blendMode)) {
            result.warnings.push(
              `Layer ${layerType} has unknown blendMode: ${this.layerConfig[layerType].blendMode}`
            )
          }
        }
      }
      ...
      for (const layerType of requiredLayers) {
        ...
        if (!layer || !layer.image) {
          ...
          if (layer.image.width === 0 || layer.image.height === 0) {
            result.errors.push(
              `Layer ${layerType} image has invalid dimensions: ${layer.image.width}x${layer.image.height}`
            )
            result.valid = false
          }
    ...
    return {
      ...
      layers: {
        base: this.layers.base
          ? { loaded: true, imageId: this.layers.base.imageId }
          : { loaded: false },
        expression: this.layers.expression
          ? { loaded: true, imageId: this.layers.expression.imageId }
          : { loaded: false },
        pose: this.layers.pose
          ? { loaded: true, imageId: this.layers.pose.imageId }
          : { loaded: false },
      },
