/**
 * =============================================================================
 * ANGELA-MATRIX: L6[Execution Layer] γ [C] L1+
 * =============================================================================
 *
...
class Live2DManager {
  constructor(canvas, unifiedDisplayMatrix = null, performanceManager = null) {
    console.log(
      '[Live2DManager] Constructor with UDM:',
      !!unifiedDisplayMatrix,
      'PM:',
      !!performanceManager
    )
  ...
  _getModelPath() {
    ...
    if (
      typeof ANGELA_CHARACTER_CONFIG !== 'undefined' &&
  ...
  async _loadCharacterImage() {
    ...
    for (const [imageId, imageConfig] of Object.entries(config)) {
      try {
        const img = new Image()
        const root = window.AngelaPlatform ? window.AngelaPlatform.getImageRoot() : ''
        img.src = `${root}${imageConfig.path}`
  ...
  _renderFallback() {
    ...
    if (config.type === 'single_image') {
      ...
      if (this.lastRenderedImageId !== this.currentCharacterImageId) {
        console.log(
          '[Live2DManager] Rendered single image:',
          config.name,
      ...
      if (params.scaleToHeight) {
        ...
        renderY = params.offsetY
          ? params.offsetY * udmScale
          : (canvas.height - cellRenderHeight) / 2
      ...
      if (
        ...
        console.log(
          '[Live2DManager] Rendered sprite sheet cell:',
          config.name,
  ...
  _detectBodyPart(x, y) {
    ...
    if (
      x >= bbox.x * scale &&
      x < (bbox.x + bbox.width) * scale &&
      y >= bbox.y * scale &&
      y < (bbox.y + bbox.height) * scale
    ) {
  ...
  setExpression(expression) {
    ...
    try {
      if (this.expressions[expression]) {
        ...
        console.warn(
          `[Live2DManager] Unknown expression: ${expression}, available:`,
          Object.keys(this.expressions)
        )
        // 回退到neutral表情
