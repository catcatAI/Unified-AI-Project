class PerformanceManager {
  ...
  detectCapabilities() {
    ...
    // GPU 检测
    this.capabilityMatrix.gpu.available =
      this.capabilityMatrix.webgl2.available || this.capabilityMatrix.webgl.available
  ...
  getCapabilityReport() {
    const report = {
      state: this.capabilityState.complete
        ? '完整版'
        : this.capabilityState.degraded
          ? '降级版'
          : '基础版',
      capabilities: {},
      missing: [],
  ...
  _determineCapabilityState() {
    ...
    // 检查必需能力
    for (const [cap, info] of Object.entries(this.capabilityMatrix)) {
      // Treat 'pending' as not available for completion check
      const isAvailable = info.available === true // Strict true check

      if (info.required && !isAvailable) {
        missingRequired.push(cap)
        if (info.fallback) {
          fallbacks.push({ capability: cap, fallback: info.fallback })
        }
      } else if (!info.required && !isAvailable && info.available !== 'pending') {
        // Optional items can stay pending without marking as missing?
        // Better to be strict:
    ...
    console.log(
      '[PerformanceManager] 能力状态:',
      this.capabilityState.complete
        ? '完整版'
        : this.capabilityState.degraded
          ? '降级版'
          : '基础版',
      this.capabilityState.missing_caps.length > 0
        ? `缺失能力: ${this.capabilityState.missing_caps.join(', ')}`
        : '所有能力正常'
    )
  }
  ...
  setWallpaperMode(mode) {
    ...
    const wallpaperHandler =
      window.wallpaperHandler || (window.angelaApp && window.angelaApp.wallpaperHandler)
    if (wallpaperHandler) {
  ...
  autoAdjustPerformance() {
    ...
    if (this.pendingModeChange) {
      if (now - this.pendingModeChangeTime >= 5000) {
        ...
        } else if (pendingMode === 'upgrade' && newRatio > 1.3) {
          // Higher threshold for upgrade (1.3 instead of 1.15)
    ...
    } else if (ratio > 1.3 && now >= this.modeLockUntil) {
      // Only suggest upgrade if not locked
  ...
  downgradePerformance() {
    ...
    // Downgrade: Move towards lower index (very_low is 0)
    if (currentIndex > 0) {
  ...
  upgradePerformance() {
    ...
    // Upgrade: Move towards higher index (ultra is max)
