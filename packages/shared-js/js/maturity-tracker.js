class MaturityTracker {
  ...
  handleLevelUp(oldLevel, newLevel) {
    ...
    console.log(
      `🎉 Level Up! L${oldLevel} (${this.maturityLevels[oldLevel].en_name}) → L${newLevel} (${levelInfo.en_name})`
    )
    ...
    if (this.stateMatrix) {
      ...
      this.stateMatrix.updateBeta({
        ...
        creativity: Math.min(
          1,
          this.stateMatrix.beta.values.creativity + levelImpact.creativity * 0.2
        ),
      })
  ...
  getStatus() {
    ...
    const progressToNext = nextLevel
      ? ((this.memoryCount - levelInfo.min_memory) /
          (nextLevel.min_memory - levelInfo.min_memory)) *
        100
      : 100
