  ...
  _validateInitializationOrder(component) {
    ...
    for (const dependency of dependencies) {
      ...
      if (dependency.startsWith('window.')) {
        ...
        if (!window[globalVar]) {
          console.error(
            `[AngelaApp] Initialization order error: ${component} requires ${dependency} to be set first`
          )
        ...
        if (!initialized) {
          console.error(
            `[AngelaApp] Initialization order error: ${component} requires ${dependency} to be initialized first`
          )
          throw new Error(`Missing dependency: ${dependency}`)
        }
      }
  ...
  async _initializeUDM() {
    ...
    const setCanvasSize = (canvasEl) => {
      if (canvasEl) {
        ...
        console.log(
          `[AngelaApp] ${canvasEl.id} dimensions set: ${canvasEl.width}x${canvasEl.height} (physical), ${baseWidth}x${baseHeight} (CSS), DPR: ${devicePixelRatio}`
        )
      }
    }
  ...
  _setupUIControls() {
    ...
    if (this.backendWebSocket) {
      this.backendWebSocket.on('angela_response', (data) => {
        console.log(
          '[App] angela_response event received:',
      ...
      if (window.electronAPI) {
        ...
        for (const evt of events) {
          try {
            ...
            console.warn('[AngelaApp] Failed to remove IPC listener:', evt)
          }
        }
