...
const FrontendUtils = {
  ...
  resolveModelPath(modelPath, options = {}) {
    ...
    if (normalizedPath.startsWith('/') || normalizedPath.includes(':')) {
      ...
      if (this.isDir(modelDir)) {
        // 查找模型文件
        const modelFile =
          this.findFileInDir(modelDir, requiredPattern) ||
  ...
  isFile(filePath) {
    try {
      if (typeof window !== 'undefined' && window.electronAPI && window.electronAPI.file) {
        return (
          window.electronAPI.file.existsSync(filePath) &&
          window.electronAPI.file.isFileSync(filePath)
        )
      }
  ...
  isDir(dirPath) {
    try {
      if (typeof window !== 'undefined' && window.electronAPI && window.electronAPI.file) {
        return (
          window.electronAPI.file.existsSync(dirPath) &&
          window.electronAPI.file.isDirectorySync(dirPath)
        )
      }
      if (typeof require !== 'undefined' && typeof require('fs') !== 'undefined') {
        const fs = require('fs')
    ...
    register(canvas, gl) {
      if (!this.contexts.has(canvas)) {
        if (this._createdContexts.length >= this.maxContexts) {
          console.warn(
            `WebGL Context count limit reached (${this.maxContexts}), cleaning up oldest`
          )
          this.cleanupOldest()
        }
        ...
        console.log(
          `WebGL Context registered (${this._createdContexts.length}/${this.maxContexts})`
        )
      }
    },
