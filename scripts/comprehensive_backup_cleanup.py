...
def clean_all_backup_dirs():
    ...
    for item in project_root.iterdir():
        if (item.is_dir() and 
            (item.name.startswith("backup_") or 
             item.name.startswith("auto_fix_") or
             "backup" in item.name.lower())):
            print(f"  发现可能的备份目录: {item.name}")
            try:
                shutil.rmtree(item)
                print(f"    ✓ 已清理 {item.name}")
            except Exception as e:
                print(f"    ✗ 清理 {item.name} 时出错: {e}")
        ...
        if item.is_dir() and item.name.startswith(("auto_fix_", "backup_")):
            try:
                ...
                if mod_time < cutoff_date:
                    print(f"  删除旧备份目录: {item.name} (修改时间: {mod_time})")
                    shutil.rmtree(item)
            except Exception as e:
