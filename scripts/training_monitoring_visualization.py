...
_ = sys.path.insert(0, str(training_path))
...
class TrainingMonitoringVisualizationDemo:
    ...
    async def setup_monitoring(self):
        """设置监控"""
        _ = logger.info("🔧 设置训练监控...")
        
        # 启动监控
        _ = self.monitor.start_monitoring()
    ...
    async def simulate_training_process(self):
        ...
        for scenario in training_scenarios:
            ...
            _ = logger.info(f"🚀 开始训练场景: {scenario_name} ({epochs} epochs)")
            ...
            for epoch in range(1, epochs + 1):
                ...
                metrics = {
                    ...
                    _ = 'val_loss': round(loss * 1.1, 4),  # 验证损失稍高
                    _ = 'val_accuracy': round(accuracy * 0.95, 4),  # 验证准确率稍低
                    _ = 'learning_rate': 0.001 * (0.95 ** epoch)  # 学习率衰减
                }
                ...
                _ = self.monitor.record_epoch_completion(epoch, epoch_duration)
                ...
                if epoch % 5 == 0 or epoch == epochs:
                    ...
                    _ = all_training_data.append(training_data_point)
                ...
                if epoch % 3 == 0:
                    ...
                    _ = logger.debug(f"   🖥️  系统资源 - CPU: {cpu_percent:.1f}% - 内存: {memory_percent:.1f}%")
        
        _ = logger.info("✅ 所有训练场景完成")
        return all_training_data
    
    async def demonstrate_anomaly_detection(self):
        """演示异常检测功能"""
        _ = logger.info("🔍 演示异常检测功能...")
        ...
        for metrics in normal_metrics:
            _ = anomaly_detector.update_baseline(metrics)
    ...
    async def demonstrate_performance_analysis(self):
        ...
        for epoch, duration in enumerate(epoch_times, 1):
            _ = performance_analyzer.record_epoch_time(epoch, duration)
        
        # 分析性能趋势
        analysis = performance_analyzer.analyze_performance_trends()
        
        if analysis['status'] == 'analyzed':
            _ = logger.info(f"   趋势: {analysis['trend']}")
            _ = logger.info(f"   平均时间: {analysis['mean_duration']:.2f}秒")
            _ = logger.info(f"   总epochs: {analysis['total_epochs']}")
            
            if analysis['performance_issues']:
                _ = logger.warning("   发现性能问题:")
                for issue in analysis['performance_issues']:
                    _ = logger.warning(f"     - {issue['message']}")
        else:
            _ = logger.info("   数据不足，无法分析性能趋势")
        
        return analysis
    
    async def generate_visualizations(self, training_data: List[Dict]):
        """生成可视化图表"""
        _ = logger.info("🎨 生成训练可视化图表...")
        
        try:
            # 生成训练进度图
            progress_plot = self.visualizer.create_training_progress_plot(training_data)
            _ = logger.info(f"   生成训练进度图: {progress_plot}")
            
            # 生成系统资源使用图
            resource_plot = self.visualizer.create_system_resources_plot()
            _ = logger.info(f"   生成系统资源图: {resource_plot}")
            
            # 生成异常检测热力图
            anomaly_heatmap = self.visualizer.create_anomaly_detection_heatmap(training_data)
            _ = logger.info(f"   生成异常检测热力图: {anomaly_heatmap}")
            
            # 生成综合报告
            report = self.visualizer.create_training_report(training_data)
            _ = logger.info(f"   生成综合报告: {report}")
            
            _ = logger.info("✅ 所有可视化图表生成完成")
            return True
            
        except Exception as e:
            _ = logger.error(f"❌ 生成可视化图表失败: {e}")
            return False
    
    async def show_system_status(self):
        """显示系统状态"""
        _ = logger.info("🖥️  获取系统状态...")
        ...
        _ = print("📊 训练监控系统状态报告")
        print("="*60)
        _ = print(f"时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        _ = print(f"监控启用: {system_status.get('monitoring_enabled', False)}")
        
        # 显示资源使用情况
        resources = system_status.get('resources', {})
        if resources:
            _ = print(f"\n🔧 系统资源:")
            _ = print(f"   CPU使用率: {resources.get('cpu_percent', 'N/A'):.1f}%")
            _ = print(f"   内存使用率: {resources.get('memory_percent', 'N/A'):.1f}%")
            _ = print(f"   磁盘使用率: {resources.get('disk_percent', 'N/A'):.1f}%")
        ...
        if performance_analysis.get('status') == 'analyzed':
            _ = print(f"\n📈 性能分析:")
            _ = print(f"   平均epoch时间: {performance_analysis.get('mean_duration', 0):.2f}秒")
            _ = print(f"   性能趋势: {performance_analysis.get('trend', 'unknown')}")
            _ = print(f"   总epochs: {performance_analysis.get('total_epochs', 0)}")
    ...
    async def cleanup(self):
        ...
        # 停止监控
        _ = self.monitor.stop_monitoring()
        
        _ = logger.info("✅ 资源清理完成")

async def main() -> None:
    """主函数"""
    _ = print("🚀 Unified-AI-Project 训练监控和可视化演示")
    print("=" * 50)
    ...
    try:
        # 1. 设置监控
        _ = await demo.setup_monitoring()
        
        # 2. 显示初始系统状态
        _ = await demo.show_system_status()
        
        # 3. 模拟训练过程
        training_data = await demo.simulate_training_process()
        
        # 4. 演示异常检测
        _ = await demo.demonstrate_anomaly_detection()
        
        # 5. 演示性能分析
        _ = await demo.demonstrate_performance_analysis()
        
        # 6. 生成可视化图表
        _ = await demo.generate_visualizations(training_data)
        
        # 7. 显示最终系统状态
        _ = await demo.show_system_status()
        
        # 8. 清理资源
        _ = await demo.cleanup()
        
        _ = print("\n🎉 训练监控和可视化演示完成!")
        _ = print(f"📊 可视化图表已保存到: {demo.visualizer.output_dir}")
        
    except Exception as e:
        _ = logger.error(f"演示过程中发生错误: {e}")
        # 确保即使出错也清理资源
        _ = await demo.cleanup()
