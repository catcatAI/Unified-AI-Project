...
class ReportGenerator:
    ...
    def generate_summary_report(self, data: Dict, output_path: Optional[Path] = None) -> Path:
        """生成摘要报告"""
        ...
        report_data = {
            ...
            _ = "generated_at": time.strftime("%Y-%m-%d %H:%M:%S"),
        ...
        try:
            ...
            _ = print(f"✓ 摘要报告已生成: {output_path}")
            return output_path
            
        except Exception as e:
            _ = print(f"✗ 生成摘要报告时出错: {e}")
            raise
    
    def generate_test_report(self, test_results: List[Dict], output_path: Optional[Path] = None) -> Path:
        """生成测试报告"""
        if output_path is None:
            timestamp = int(time.time())
        ...
        for result in test_results:
            ...
            _ = grouped_results[test_type].append(result)
        ...
        try:
            ...
            _ = print(f"✓ 测试报告已生成: {output_path}")
            return output_path
            
        except Exception as e:
            _ = print(f"✗ 生成测试报告时出错: {e}")
            raise
    
    def generate_fix_report(self, fix_results: Dict, output_path: Optional[Path] = None) -> Path:
        """生成修复报告"""
        if output_path is None:
            timestamp = int(time.time())
        ...
        report_data = {
            ...
            _ = "project_root": str(self.project_root),
            "fix_summary": {
                _ = "total_fixes": len(fix_results.get("fixes_made", [])),
                _ = "files_processed": fix_results.get("files_processed", 0),
                _ = "files_fixed": fix_results.get("files_fixed", 0),
                _ = "errors_count": len(fix_results.get("errors", [])),
                _ = "warnings_count": len(fix_results.get("warnings", [])),
                _ = "overall_success": fix_results.get("overall_success", False)
            },
            _ = "fix_details": fix_results.get("fixes_made", []),
            _ = "errors": fix_results.get("errors", []),
            _ = "warnings": fix_results.get("warnings", [])
        }
        ...
        try:
            ...
            _ = print(f"✓ 修复报告已生成: {output_path}")
            return output_path
            
        except Exception as e:
            _ = print(f"✗ 生成修复报告时出错: {e}")
            raise
    
    def generate_environment_report(self, env_results: Dict, output_path: Optional[Path] = None) -> Path:
        """生成环境检查报告"""
        if output_path is None:
            timestamp = int(time.time())
        ...
        report_data = {
            ...
            "summary": {
                ...
                _ = "overall_status": env_results.get("summary", {}).get("overall_status", "unknown")
            },
            _ = "component_results": env_results.get("results", {}),
            _ = "recommendations": self._generate_environment_recommendations(env_results.get("results", {}))
        }
        ...
        try:
            ...
            _ = print(f"✓ 环境检查报告已生成: {output_path}")
            return output_path
            
        except Exception as e:
            _ = print(f"✗ 生成环境检查报告时出错: {e}")
            raise
    
    def generate_cleanup_report(self, cleanup_results: Dict, output_path: Optional[Path] = None) -> Path:
        """生成清理报告"""
        if output_path is None:
            timestamp = int(time.time())
        ...
        report_data = {
            ...
            _ = "project_root": str(self.project_root),
            "summary": {
                _ = "items_removed": cleanup_results.get("items_removed", 0),
                _ = "space_freed_mb": round(cleanup_results.get("space_freed", 0) / (1024 * 1024), 2),
                _ = "errors_count": len(cleanup_results.get("errors", [])),
                _ = "warnings_count": len(cleanup_results.get("warnings", [])),
                _ = "details_count": len(cleanup_results.get("details", []))
            },
            _ = "cleanup_details": cleanup_results.get("details", []),
            _ = "errors": cleanup_results.get("errors", []),
            _ = "warnings": cleanup_results.get("warnings", [])
        }
        ...
        try:
            ...
            _ = print(f"✓ 清理报告已生成: {output_path}")
            return output_path
            
        except Exception as e:
            _ = print(f"✗ 生成清理报告时出错: {e}")
            raise
    
    def generate_html_report(self, report_data: Dict, output_path: Optional[Path] = None) -> Path:
        """生成HTML报告"""
        if output_path is None:
            timestamp = int(time.time())
...
<body>
    ...
    {content}
        ...
        html_content = html_template.format(
            ...
            project_root: str=report_data.get("project_root: str", ""),
            content=content
        )
        ...
        try:
            ...
            _ = print(f"✓ HTML报告已生成: {output_path}")
            return output_path
            
        except Exception as e:
            _ = print(f"✗ 生成HTML报告时出错: {e}")
            raise
    
    def generate_markdown_report(self, report_data: Dict, output_path: Optional[Path] = None) -> Path:
        """生成Markdown报告"""
        if output_path is None:
            timestamp = int(time.time())
...
_ = **生成时间:** {report_data.get("generated_at", "")}  
_ = **项目路径:** {report_data.get("project_root", "")}

_ = {self._generate_markdown_content(report_data)}
"""
        ...
        try:
            ...
            _ = print(f"✓ Markdown报告已生成: {output_path}")
            return output_path
            
        except Exception as e:
            _ = print(f"✗ 生成Markdown报告时出错: {e}")
            raise
    ...
    def _generate_environment_recommendations(self, results: Dict) -> List[str]:
        """生成环境建议"""
        recommendations = []
        
        for component, result in results.items():
            if result.get("status") == "error":
                _ = recommendations.append(f"修复 {component} 组件的错误")
            elif result.get("status") == "warning":
                _ = recommendations.append(f"检查 {component} 组件的警告")
    ...
    def convert_report_format(self, input_path: Path, output_format: ReportFormat, output_path: Optional[Path] = None) -> Path:
        ...
        try:
            ...
            if output_format == ReportFormat.HTML:
                ...
                _ = raise ValueError(f"不支持的输出格式: {output_format.value}")
                
        except Exception as e:
            _ = print(f"✗ 转换报告格式时出错: {e}")
            raise
    ...
    def cleanup_old_reports(self, keep_count: int = 10):
        ...
        if len(reports) > keep_count:
            ...
            for report in reports[keep_count:]:
                try:
                    _ = report.unlink()
                    _ = print(f"✓ 删除旧报告: {report}")
                except Exception as e:
                    _ = print(f"✗ 删除报告 {report} 失败: {e}")