...
def check_directory_structure() -> Dict[str, List[str]]:
    ...
    if (SRC_DIR / "ai").exists():
        for item in (SRC_DIR / "ai").iterdir():
            if item.is_dir():
                structure["ai_modules"].append(item.name)
...
def update_enhanced_auto_fix():
    ...
    # 更新导入映射部分
    mapping_lines = []
    mapping_lines.append("# 新的导入映射 - 适应重构后的目录结构")
...
def main():
    ...
    structure = check_directory_structure()
    print("当前目录结构:")
    print(f"  AI模块: {structure['ai_modules']}")
    print(f"  核心模块: {structure['core_modules']}")
    ...
    # 更新增强版自动修复工具
    print("\n更新增强版自动修复工具...")
    update_enhanced_auto_fix()
    
    # 更新后端增强版自动修复工具
    print("\n更新后端增强版自动修复工具...")
    update_backend_enhanced_auto_fix()
