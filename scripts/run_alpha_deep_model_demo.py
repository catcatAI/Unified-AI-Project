...
def main():
    ...
    """
    print("--- AlphaDeepModel Demonstration Script ---")
    ...
    if os.path.exists(ham_manager.core_storage_filepath):
        ...
        print(f"Removed old demo file: {ham_manager.core_storage_filepath}")
    ...
    if not memory_id:
        print("Error: Failed to store experience in HAM.")
        return
    ...
    if not recalled_result:
        print("Error: Failed to recall gist from HAM.")
        return
    ...
    print("NOTE: `recall_gist` provides a human-readable summary. For this demo, we will construct a DeepParameter object manually based on the recalled data to simulate the next step in the pipeline.")
    ...
    print(f"Compressed Size (binary):    {compressed_size} bytes")
