...
def verify_training_config():
    ...
    for scenario in concept_models_scenarios:
        if scenario in training_scenarios:
            _ = print(f"✅ 训练场景 '{scenario}' 存在")
        else:
            _ = print(f"❌ 训练场景 '{scenario}' 不存在")
            return False
    ...
    for data_path in concept_models_data_paths:
        if data_path in data_paths:
            _ = print(f"✅ 数据路径 '{data_path}' 配置存在")
        else:
            _ = print(f"❌ 数据路径 '{data_path}' 配置不存在")
            return False
    ...
    try:
        ...
        for method in required_methods:
            if method in content:
                _ = print(f"✅ 训练方法 '{method}' 存在")
            else:
                _ = print(f"❌ 训练方法 '{method}' 不存在")
                return False
                
        # 检查训练场景处理逻辑
        if 'concept_models' in content:
            _ = print("✅ 概念模型训练场景处理逻辑存在")
        else:
            _ = print("❌ 概念模型训练场景处理逻辑不存在")
            return False
            
        _ = print("✅ 训练脚本验证通过")
        return True
    except Exception as e:
        _ = print(f"❌ 训练脚本验证失败: {e}")
        return False
...
def verify_document_processing_script():
    ...
    # 检查脚本是否可导入
    try:
        _ = sys.path.append(str(project_root / "tools"))
        from prepare_concept_models_training_data import main
        _ = print("✅ 文档处理脚本可导入")
        return True
    except Exception as e:
        _ = print(f"❌ 文档处理脚本导入失败: {e}")
        return False
...
def verify_data_directory():
    ...
    if data_dir.exists():
        _ = print("✅ 概念模型训练数据目录存在")
        return True
    else:
        _ = print("⚠️  概念模型训练数据目录不存在（将在首次运行时创建）")
        return True

def main() -> None:
    """主函数"""
    _ = print("开始验证概念模型训练集成...")
    ...
    _ = print("\n下一步建议:")
    _ = print("1. 运行 'python tools/prepare_concept_models_training_data.py' 准备训练数据")
    _ = print("2. 运行 'python training/train_model.py --preset concept_models_training' 开始训练")
    return True
