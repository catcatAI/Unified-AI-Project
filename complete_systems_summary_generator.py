...
class SystemSummaryGenerator:
    ...
    def analyze_io_operations(self, content: str) -> Dict[str, Any]:
        ...
        io_stats = {
            ...
            "io_intensity": "low"  # 字符串类型
        }
        
        io_stats["total_io_operations"] = sum(v for k, v in io_stats.items() if isinstance(v, int) and k != "total_io_operations")
    ...
    def create_comprehensive_report(self) -> str:
        ...
        for filename, data in self.systems_data.items():
            ...
            main_functions = function_analysis["main_functions"]
            if main_functions:
                report.append("#### 🎯 核心函数")
        ...
        # I/O模式总结
        total_print = sum(data["io_analysis"]["print_operations"] for data in self.systems_data.values() if data.get("status") == "success")
        total_input = sum(data["io_analysis"]["input_operations"] for data in self.systems_data.values() if data.get("status") == "success")
        total_file_ops = sum(data["io_analysis"]["file_open_operations"] for data in self.systems_data.values() if data.get("status") == "success")
        total_json = sum(data["io_analysis"]["json_operations"] for data in self.systems_data.values() if data.get("status") == "success")
        report.extend([
        ...
        # 算法特征汇总
        total_search = sum(data["algorithm_analysis"]["search_patterns"] for data in self.systems_data.values() if data.get("status") == "success")
        total_ml = sum(data["algorithm_analysis"]["ml_ai_patterns"] for data in self.systems_data.values() if data.get("status") == "success")
        total_optimization = sum(data["algorithm_analysis"]["optimization_patterns"] for data in self.systems_data.values() if data.get("status") == "success")
        total_pattern = sum(data["algorithm_analysis"]["pattern_matching"] for data in self.systems_data.values() if data.get("status") == "success")
        report.extend([
        ...
        # 安全评估汇总
        total_security_score = sum(data["security_analysis"]["security_score"] for data in self.systems_data.values() if data.get("status") == "success")
        total_vulnerabilities = sum(len(data["security_analysis"]["dangerous_functions"]) for data in self.systems_data.values() if data.get("status") == "success")
        total_security_measures = sum(len(data["security_analysis"]["security_measures"]) for data in self.systems_data.values() if data.get("status") == "success")
        average_security_score = total_security_score / max(len([d for d in self.systems_data.values() if d.get("status") == "success"]), 1)
        report.extend([
        ...
        # 性能分析汇总
        total_performance_score = sum(data["performance_analysis"]["performance_score"] for data in self.systems_data.values() if data.get("status") == "success")
        total_long_lines = sum(data["performance_analysis"]["long_lines"] for data in self.systems_data.values() if data.get("status") == "success")
        total_complexity = sum(data["performance_analysis"]["complexity_score"] for data in self.systems_data.values() if data.get("status") == "success")
        average_performance_score = total_performance_score / max(len([d for d in self.systems_data.values() if d.get("status") == "success"]), 1)
        report.extend([
        ...
        # 问题总结
        total_issues = len([d for d in self.systems_data.values() if d.get("status") == "success" and
    ...
    def main(self):
        ...
        try:
            ...
            analyzed_files = len([d for d in self.systems_data.values() if d.get("status") == "success"])
            print(f"\n📊 报告统计:")
