...
def check_project_dependencies():
    ...
    try:
        ...
        if node_modules.exists():
            logger.debug("Node.js 依赖已安装")
            return True
        else:
            logger.debug("Node.js 依赖未安装")
            return False
    except Exception as e:
        logger.error(f"检查项目依赖时出错: {e}")
        return False
...
def check_python_venv():
    ...
    try:
        ...
        if venv_path.exists():
            logger.debug("Python 虚拟环境已创建")
            return True
        else:
            logger.debug("Python 虚拟环境未创建")
            return False
    except Exception as e:
        logger.error(f"检查Python虚拟环境时出错: {e}")
        return False
