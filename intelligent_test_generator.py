...
class CodeAnalyzer:
    ...
    def _extract_parameters(self, func_node: ast.FunctionDef) -> List[Dict[str, Any]]:
        ...
        for i, arg in enumerate(args.args):
            ...
            if i >= num_args - num_defaults:
                ...
                param_info["default"] = self._extract_default_value(defaults[default_idx])
            else:
    ...
    def _extract_default_value(self, default_node: ast.AST) -> Any:
        """提取默认值"""
        if isinstance(default_node, ast.Constant):
            ...
            return [self._extract_default_value(elt) for elt in default_node.elts]
        elif isinstance(default_node, ast.Dict):
            return {
                self._extract_default_value(key): self._extract_default_value(value)
...
class TestPatternAnalyzer:
    ...
    def identify_function_patterns(self, function_info: Dict[str, Any]) -> List[str]:
        """识别函数模式"""
        patterns = []
...
class IntelligentTestGenerator:
    ...
    def generate_tests_for_file(self, file_path: str) -> List[TestCase]:
        ...
        try:
            ...
            for func_name, func_info in analysis_result["functions"].items():
                func_test_cases = self._generate_function_tests(func_info, analysis_result)
                test_cases.extend(func_test_cases)
                
            # 为每个类生成测试用例
            for class_name, class_info in analysis_result["classes"].items():
                class_test_cases = self._generate_class_tests(class_info, analysis_result)
    ...
    def _generate_function_tests(self, func_info: Dict[str, Any], analysis_result: Dict[str, Any]) -> List[TestCase]:
        ...
        for pattern in patterns:
            pattern_tests = self._create_pattern_specific_tests(func_info, pattern)
    ...
    def _create_basic_function_test(self, func_info: Dict[str, Any]) -> Optional[TestCase]:
        """创建基本功能测试"""
        func_name = func_info["name"]
    ...
    def _create_pattern_specific_tests(self, func_info: Dict[str, Any], pattern: str) -> List[TestCase]:
        """创建模式特定测试"""
        test_cases = []
        func_name = func_info["name"]
    ...
    def _create_boundary_tests(self, func_info: Dict[str, Any]) -> List[TestCase]:
        """创建边界条件测试"""
        test_cases = []
        func_name = func_info["name"]
        parameters = func_info.get("parameters", [])
    ...
    def _create_error_handling_tests(self, func_info: Dict[str, Any]) -> List[TestCase]:
        """创建错误处理测试"""
        test_cases = []
        func_name = func_info["name"]
    ...
    def _generate_class_tests(self, class_info: Dict[str, Any], analysis_result: Dict[str, Any]) -> List[TestCase]:
        """为类生成测试用例"""
        test_cases = []
        ...
        for method_name, method_info in class_info["methods"].items():
            method_test_cases = self._generate_function_tests(method_info, analysis_result)
            # 为类方法测试添加类前缀
            for test_case in method_test_cases:
                test_case.name = f"test_{class_name}_{method_name}" + test_case.name[len(f"test_{method_name}"):]
...
def {test_case.name}():
    ...
    # TODO: Implement test logic for {test_case.function_name}
    # Parameters: {', '.join([p.name for p in test_case.parameters])}
...
"""
    ...
    def get_generated_tests(self, limit: int = None) -> List[TestCase]:
        """获取生成的测试用例"""
        if limit:
