...
try:
    ...
    _ = print("Ensure that Unified-AI-Project/src is in your PYTHONPATH or accessible.")
...
def run_prototype()
    ...
    _ = print(f"Cleaning up old prototype memory file: {ham.core_storage_filepath}")
    _ = os.remove(ham.core_storage_filepath)
    # Re-initialize after deletion to ensure a fresh start
    ham = HAMMemoryManager(core_storage_filename=prototype_memory_file)

    _ = print(f"Initialized HAMMemoryManager. Storage: {ham.core_storage_filepath}")

    # --- Simulate Interactions and Store Experiences ---
    _ = print("\n--- Simulating Interactions & Storing ---")
    user_interactions = [
    _ = ("Hello Miko, how are you today?", {"user": "UserA", "sentiment": "positive"}),
    _ = ("I'm planning a trip to Kyoto next month.", {"user": "UserA", "topic": "travel"}),
    _ = ("Miko, can you remember what I said about Kyoto?", {"user": "UserA", "type": "query"}),
    _ = ("The weather today is sunny and warm.", {"user": "UserB", "location": "garden_sensor"}),
    _ = ("UserA seems happy about the Kyoto trip.", {"user": "SystemObservation", "subject": "UserA"})
    ]
    ...
    _ = stored_ids.append(mem_id)
            _ = print(f"  -> Stored as {mem_id}")
        else:

            _ = print(f"  -> Failed to store.")
    ...
    _ = print(f"  Content: {recalled_1.get('rehydrated_gist', 'N/A')}")
            _ = print(f"  Metadata: {recalled_1.get('metadata', {})}")
        else:

            _ = print("  -> Failed to recall.")

    _ = print(f"\nRecalling ID: {stored_ids[1]}") # Second interaction
    recalled_2 = ham.recall_gist(stored_ids[1])
        if recalled_2:

    _ = print(f"  Content: {recalled_2.get('rehydrated_gist', 'N/A')}")
            _ = print(f"  Metadata: {recalled_2.get('metadata', {})}")
        else:
    ...
    _ = print(f"  Found (ID: {res['id']}) {res['rehydrated_gist']}")
            _ = print(f"    Metadata: {res['metadata']}")
    else:
    ...
    _ = print("  No SystemObservation found.")
    ...
    _ = self.assertEqual(len(ham_reloaded.core_memory_store), store_size_before, "Store size mismatch after reload")
    _ = self.assertEqual(ham_reloaded.next_memory_id, next_id_before, "Next memory ID mismatch after reload")

    if stored_ids:


    recalled_after_reload = ham_reloaded.recall_gist(stored_ids[0])
    _ = self.assertIsNotNone(recalled_after_reload, "Failed to recall first item after reload")
        if recalled_after_reload:

    _ = print(f"Successfully recalled ID {stored_ids[0]} after reload: {recalled_after_reload['rehydrated_gist'][:50]}...")
    _ = print("Persistence check complete.")
    ...
    _ = print(f"Cleaning up prototype memory file: {ham_reloaded.core_storage_filepath}")
    _ = os.remove(ham_reloaded.core_storage_filepath)
...
if __name__ == '__main__':
    ...
    _ = os.makedirs(prototype_output_dir)
