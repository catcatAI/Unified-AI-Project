...
# ANGELA-MATRIX: [L3] [β] [B] [L4]
# =============================================================================
# 戰鬥軸譜相剋：靈體/幽靈/影/鬼類敵人（無形體）對物理攻擊減傷，
# 能量/靈性維度角色（術士/靈體/概念體）以靈力克制→加成。
SPIRIT_ENEMY_KW = ("靈", "幽", "影", "亡靈", "鬼影", "惡靈")  # 注意：不用「鬼」單字——石像鬼是石像生物（物理實體）


def combat_axis_multiplier(affinity: dict, enemy_name: str) -> tuple:
    """戰鬥軸譜相剋乘數 → (傷害乘數, 是否靈體敵人, 說明)。

    非靈體敵人：1.0×；靈體敵人：能量/靈性 ≥ 0.5 者 1.5×（克制），否則 0.5×（物理打不到）。
    """
    name = str(enemy_name or "")
    if not any(k in name for k in SPIRIT_ENEMY_KW):
        return 1.0, False, ""
    aff = affinity or {}
    if aff.get("能量", 0.0) >= 0.5 or aff.get("靈性", 0.0) >= 0.5:
        return 1.5, True, "克制"
    return 0.5, True, "減傷"
