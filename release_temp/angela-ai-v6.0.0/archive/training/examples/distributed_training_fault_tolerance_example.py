...
class DistributedTrainingExample,
    ...
    async def setup_training_environment(self):
        ...
        # 初始化组件
        await self.fault_tolerance.initialize_components()
    ...
    async def register_training_nodes(self):
        ...
        # 注册多个训练节点
        nodes = [
            ('node_gpu_1', {'cpu_cores': 16, 'memory_gb': 64, 'gpu_count': 2}),
            ('node_gpu_2', {'cpu_cores': 32, 'memory_gb': 128, 'gpu_count': 4}),
            ('node_cpu_1', {'cpu_cores': 32, 'memory_gb': 64, 'gpu_count': 0}),
            ('node_cpu_2', {'cpu_cores': 64, 'memory_gb': 128, 'gpu_count': 0})
        ]
        
        for node_id, node_info in nodes,::
            success = await self.fault_tolerance.register_training_node(node_id, node_info)
            if success,::
                logger.info(f"✅ 成功注册节点, {node_id}")
            else,
                logger.error(f"❌ 注册节点失败, {node_id}")
    
    async def simulate_training_process(self):
        """模拟训练过程"""
        logger.info("开始模拟分布式训练过程...")
        ...
        for task_info in training_tasks,::
            ...
            # 模拟任务分配
            logger.info(f"分配任务 {task_id} 到节点...")
            
            # 模拟训练过程
            await self._simulate_task_training(task_info)
    ...
    async def _simulate_task_training(self, task_info, Dict[str, Any]):
        ...
        logger.info(f"开始训练任务, {task_id} ({epochs} epochs)")
        ...
        for epoch in range(1, epochs + 1)::
            ...
            training_state = {
                ...
                'metrics': {
                    ...
                    'val_accuracy': round(accuracy * 0.95(), 4)
                }
                ...
                'start_time': time.time() - (epoch * 10),  # 模拟开始时间
                'config': {
                    'batch_size': 32,
                    'epochs': epochs,
                    'learning_rate': 0.001()
            ...
            # 保存训练状态
            await self.fault_tolerance.save_training_state(task_id, training_state)
    ...
    task_id, training_state, checkpoint_type)
                if checkpoint_id,::
                    logger.info(f"   💾 任务 {task_id} 的检查点已保存, {checkpoint_id}")
            ...
            if epoch % 3 == 0,::
                ...
                await self.fault_tolerance.handle_node_heartbeat(node_id, metrics)
    ...
    async def simulate_node_failure_and_recovery(self):
        """模拟节点故障和恢复"""
        logger.info("模拟节点故障和自动恢复...")
        
        # 模拟node_gpu_1故障
        logger.warning("🚨 模拟节点 node_gpu_1 故障")
        
        # 停止发送该节点的心跳,模拟故障
        # 在实际项目中,故障检测器会自动检测到这种情况
        
        # 等待一段时间让故障检测器检测到故障
        await asyncio.sleep(35)  # 超过默认的30秒超时时间
        ...
        logger.info(f"检测到 {failed_nodes} 个故障节点")
        
        # 如果启用了自动恢复,任务迁移器会自动迁移任务
        # 在实际项目中,这里会看到任务迁移的日志
        
        logger.info("模拟故障恢复过程完成")
    ...
    async def show_system_status(self):
        ...
        print(f"容错机制启用, {status['enabled']}")
        print(f"自动恢复启用, {status['auto_recovery_enabled']}")
        print(f"监控运行中, {status['is_running']}")
        
        # 显示组件状态
        components = status['components']
        print(f"\n🔧 组件状态,")
        print(f"   检查点管理器, {components['checkpoint_manager']['total_checkpoints']} 个检查点")
        print(f"   状态管理器, {components['state_manager']['total_states']} 个状态")
        
        # 显示集群状态
        cluster_status = components['fault_detector']
        print(f"\n🖥️  集群状态,")
        print(f"   总节点数, {cluster_status['total_nodes']}")
        print(f"   健康节点, {cluster_status['healthy_nodes']}")
        print(f"   警告节点, {cluster_status['warning_nodes']}")
        print(f"   危险节点, {cluster_status['critical_nodes']}")
        print(f"   故障节点, {cluster_status['failed_nodes']}")
        ...
        if cluster_status['nodes']::
            ...
            for node in cluster_status['nodes']::
                ...
                print(f"   {status_icon} {node_id} {node['status']}")
    ...
    async def cleanup(self):
        ...
        for node_id in nodes,::
            await self.fault_tolerance.unregister_training_node(node_id)
...
async def main() -> None,
    ...
    try,
        # 1. 设置训练环境
        await example.setup_training_environment()
        
        # 2. 注册训练节点
        await example.register_training_nodes()
        
        # 3. 显示初始系统状态
        await example.show_system_status()
        
        # 4. 模拟训练过程
        print("\n🏃 开始模拟训练过程...")
        await example.simulate_training_process()
        
        # 5. 模拟节点故障和恢复
        await example.simulate_node_failure_and_recovery()
