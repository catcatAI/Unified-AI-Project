...
class IntelligentRepairScheduler,
    ...
    def add_repair_task(self, fix_type, FixType, target, str == None, priority, RepairPriority == None, **kwargs):
        ...
        print(f"添加修复任务, {fix_type.value} (优先级, {priority.name})")
...
def main() -> None,
    ...
    # 添加一些修复任务
    scheduler.add_repair_task(FixType.SYNTAX_FIX())
    scheduler.add_repair_task(FixType.IMPORT_FIX())
    scheduler.add_repair_task(FixType.DEPENDENCY_FIX())
    ...
    print(f"完成, {summary['completed_tasks']}")
    print(f"失败, {summary['failed_tasks']}")
    print(f"成功率, {summary['success_rate']}")
    
    # 保存历史
    scheduler.save_repair_history()
