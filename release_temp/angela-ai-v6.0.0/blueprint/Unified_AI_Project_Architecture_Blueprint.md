...
│   ├── cli/               # 命令行工具
│   └── ui/                # 共享 UI 組件
├── scripts/                # 自動化腳本 (測試, 維護, 開發)
├── configs/                # 項目配置檔案
├── test_data/              # 測試數據
├── docs/                   # 項目文檔 (已整理為數字前綴結構)
...
- **狀態**: ⚠️ 臨時方案 (關鍵字搜尋)
- **備註**: 由於環境問題，ChromaDB 向量資料庫暫時無法使用，目前採用關鍵字搜尋的臨時方案。詳情請參閱 `docs/05-development/technical_debt/chromadb_issue.md`。
...
- **備註 (由 AI 助理於 2025年11月14日 更新)**: 經底層程式碼審查，原藍圖狀態已過時。此系統中的所有組件 (`EnvironmentSimulator`, `CausalReasoningEngine`, `UnifiedSymbolicSpace`, `AdaptiveLearningController`) 均已具備 V1 級別的功能性實現。`AlphaDeepModel` 的 `compress` 方法原為佔位符，現已由助理使用 PCA 演算法完成真實的功能實現。
- **關鍵組件**:
  - **EnvironmentSimulator**: 環境模擬器。(`environment_simulator.py`)
  - **CausalReasoningEngine**: 因果推理引擎。(`causal_reasoning_engine.py`)
  - **AdaptiveLearningController**: 自適應學習控制器。(`adaptive_learning_controller.py`)
  - **AlphaDeepModel**: 數據壓縮和學習的深度模型。(`alpha_deep_model.py`)
  - **UnifiedSymbolicSpace**: 統一符號空間。(`unified_symbolic_space.py`)

### 3.5 語言免疫系統 (Linguistic Immune System - LIS) (來自代碼庫)
- **描述**: 一個用於檢測、分析和修復語言模型輸出偏差或惡意內容的防禦性系統。
- **狀態**: ✅ 基礎模擬實現
- **位置**: `apps/backend/src/ai/lis/`
- **備註 (由 AI 助理於 2025年11月16日 更新)**: 已增強組態外部化、學習模式持久化、上下文分析佔位符和智慧編輯佔位符，使其更強健和靈活。

### 3.6 公式引擎 (Formula Engine) (來自代碼庫)
- **描述**: 一個用於解析、執行和管理數學或邏輯公式的專用引擎。
- **狀態**: ✅ 基礎模擬實現
- **位置**: `apps/backend/src/ai/formula_engine/`
- **備註 (由 AI 助理於 2025年11月16日 更新)**: 已增強自訂函數註冊、公式管理（載入/儲存/新增/檢索）和組態外部化，使其更強健和靈活。
...
- **備註 (由 AI 助理於 2025年11月16日 更新)**: M1 效率核心已增強組態外部化和真實監控佔位符整合，使其更強健和靈活。
...
### 6.1 開發優先級
- **P0 (立即優先級)**:
  1. **BaseAgent 核心修復**: 確保代理系統基礎功能。 (`✅ 已完成`)
  2. **工具系統驗證**: 驗證所有工具的真實可用性。 (`✅ 已完成`)
  3. **依賴配置統一**: 建立統一的依賴和配置管理。 (`✅ 已完成`)
- **P1 (高優先級)**:
  1. **多代理協調**: 實現代理間的通信和協調。 (`✅ 已完成`)
  2. **模型服務集成**: 確保 MultiLLM 服務正常運行。 (`✅ 已完成`)
  3. **性能基準建立**: 建立真實的性能測試基準。 (`✅ 已完成`)
...
### 7.1 完整啟動序列
1. **後端啟動**: `python apps/backend/main.py`
2. **前端啟動**: `cd apps/frontend-dashboard && npm run dev`
3. **CLI驗證**: `python -m packages.cli health`
...
## 8. 專案完成路線圖 (Project Completion Roadmap)

本文件詳細規劃了專案從當前狀態到 100% 完成度的路徑，包括將模擬功能轉化為真實實作的步驟、解決技術債務的計畫，以及持續維護專案結構和文件的策略。

詳情請參閱：[專案完成路線圖](docs/06-project-management/PROJECT_COMPLETION_ROADMAP.md)
