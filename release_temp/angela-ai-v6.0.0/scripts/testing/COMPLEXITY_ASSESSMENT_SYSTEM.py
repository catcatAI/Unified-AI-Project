...
class ProjectComplexityAssessment:
    ...
    def assess_project_complexity(self) -> Tuple[ComplexityMetrics, str]:
        ...
        if metrics.python_files > 0:
            metrics.avg_file_lines = metrics.total_lines / metrics.python_files        # 分析架构复杂度
    ...
    def _analyze_git_history(self) -> Dict:
        ...
        try:
            # 获取提交数量
            result = os.popen(f'git -C "{self.project_root}" rev-list --count HEAD 2>/dev/null').read().strip()
            commits = int(result) if result and result.isdigit() else 0
            # 获取分支数量
            result = os.popen(f'git -C "{self.project_root}" branch -a | wc -l 2>/dev/null').read().strip()
    ...
    def _determine_complexity_level(self, metrics: ComplexityMetrics) -> str:
        ...
        if total_score < 0.2:
            ...
            return "mega"            
    def evaluate_repair_approach(self, target_path: Optional[str] = None) -> Dict:
        """评估修复方法的适用性"""
        print("🔍 开始修复方法适用性评估...")
