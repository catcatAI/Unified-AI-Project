...
echo [DEBUG] PROJECT_ROOT defined as: !PROJECT_ROOT!

:: Handle command-line argument for non-interactive execution
set "_ARG_CHOICE="
if not "%~1"=="" (
    set "_ARG_CHOICE=%~1"
)
...
:get_user_choice
set "choice="
if defined _ARG_CHOICE (
    set "choice=!_ARG_CHOICE!"
    echo [INFO] Auto-selecting option: !choice!
) else (
...
echo.
echo [DEBUG] Current directory before pushd: !CD!
pushd "!PROJECT_ROOT!"
echo [DEBUG] Current directory after pushd: !CD!
echo [DEBUG] Attempting to execute: tools\health-check.bat > health_check_run_output.log 2>&1
cmd /c "tools\health-check.bat" > health_check_run_output.log 2>&1
set "HEALTH_CHECK_EXIT_CODE=!errorlevel!"
echo [DEBUG] Health check script exit code: !HEALTH_CHECK_EXIT_CODE!
type health_check_run_output.log
del health_check_run_output.log
if !HEALTH_CHECK_EXIT_CODE! gtr 0 (
    echo [ERROR] Health check script reported errors.
    echo [%date% %time%] Health check script reported errors, exit code: !HEALTH_CHECK_EXIT_CODE! >> "%LOG_FILE%" 2>nul
) else (
    echo [INFO] Health check script completed successfully.
    echo [%date% %time%] Health check script completed successfully >> "%LOG_FILE%" 2>nul
)
popd
echo [DEBUG] Current directory after popd: !CD!
if defined _ARG_CHOICE (
    goto end_script
)
echo.
...
set "UNIFIED_FIX_SCRIPT=%~dp0tools\tools\scripts\unified_auto_fix.bat"
