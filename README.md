...
**Quick facts**: 612 Python files in backend src (~96K lines). Electron + Live2D desktop companion (50 JS files across shared-js/desktop/web). Pixel art engine (PyQt6 renderer). **~4,439 tests (tests/) — 0 errors.**  
...
> **STATUS (2026-07-07)**: §X #201-201b — Test collection hang fixed via lazy imports (4 `__init__.py`), 3 production bugs fixed, 72 pre-existing test failures resolved, orphan test directories/files cleaned, 2 utility scripts moved from tests/→scripts/, MD sync across all files. **4,439 tests collected in 39.79s — 0 errors.**
...
| **Tests** | ✅ PASSING | ~4,439 tests collected (tests/), 0 collection errors, 1,589+ verified passing |
...
| **測試** | ✅ 通過 | ~4,439 (tests/) — 0 collection errors |
