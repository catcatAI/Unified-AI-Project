...
**Quick facts**: 612 Python files in backend src (~96K lines). Electron + Live2D desktop companion (50 JS files across shared-js/desktop/web). Pixel art engine (PyQt6 renderer). **~5,017 tests (tests/) — 0 errors.**  
...
> **STATUS (2026-07-04)**: §X #155-157 — Frontend IPC gaps fixed (2 missing validChannels: reload-model, always-on-top-changed), 8 bare catch→logging in production JS, 17 dead test files deleted, weak test strengthened. **5,017 tests — 0 errors.**
...
| **Tests** | ✅ PASSING | ~5,017 tests collected (tests/), 0 errors, 0 collection errors |
...
| **測試** | ✅ 通過 | ~5,017 (tests/) — 0 collection errors |
