...
>
> ⚠️ **Honest capability note**: Version is `7.5.0-dev` (Alpha). The system needs an **external LLM API key** (OpenAI / Gemini / Ollama) for real conversational ability. Its *native* **deterministic engines (math/physics/chemistry) work correctly and score high (9.5/10 — real, high-certainty capability, not a defect)**; **neural open-domain generalization ≈ 0/10**. Capability-readiness per dimension (high — infra is ready): architecture **9.5**, knowledge+reasoning **8.6**, query+learning **9.0**, multimodal **5.1**, autonomy **9.0** — see `docs/06-project-management/INTELLIGENCE_ASSESSMENT.md` §1. "Production-ready" means the **runtime/codebase is shippable**, not that the built-in AI is a finished product.
