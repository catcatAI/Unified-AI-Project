...
**Quick facts**: 612 Python files in backend src (~96K lines). Electron + Live2D desktop companion (50 JS files across shared-js/desktop/web). Pixel art engine (PyQt6 renderer). **~4,488 tests (tests/) — 0 errors. Security: 0 Dependabot + 0 CodeQL + 0 Secret Scanning = fully clean.**  
...
>
> ⚠️ **Honest capability note**: Version is `7.5.0-dev` (Alpha). The system needs an **external LLM API key** (OpenAI / Gemini / Ollama) for real conversational ability. Its *native* **deterministic engines work correctly and score high — math/physics/chemistry 9.5/10, factual knowledge 10/10, and symbolic reasoning 10/10 (real, high-certainty capability, not a defect)**; **neural open-domain reasoning generalization ≈ 0/10** (genuinely novel reasoning needs the LLM or a symbolic reasoner). Capability-readiness per dimension (high — infra is ready): architecture **9.5**, knowledge+reasoning **8.6**, query+learning **9.0**, multimodal **5.1**, autonomy **9.0**. Native benchmarks (no LLM): **ED3N re-measured 33.3% (5/15)** and **GARDEN 35.6%** on the full `benchmark_ed3n_garden.py` suite — the earlier "100% (15/15)" figure came from 5 hand-picked cases (math 5/5 + knowledge 5/5 + reasoning 5/5, all handled by the *deterministic* engines MathVerifier / knowledge_base / symbolic_reasoner, NOT the neural SNN). See `docs/06-project-management/INTELLIGENCE_ASSESSMENT.md` §1, §4.1 & §4.2 for the honest breakdown. "Production-ready" means the **runtime/codebase is shippable**, not that the built-in AI is a finished product.
...
| **Tests** | ✅ PASSING | ~4,488 tests collected (tests/), 0 collection errors, 2,212+ verified passing |
...
| **測試** | ✅ 通過 | ~4,488 (tests/) — 0 collection errors |
| **智能分數** | ✅ 已評分 (多維) | 6.0/10 (有 LLM) / 0/10 (純原生 神經開放域推理泛化); 確定性引擎: 數理化9.5/知識10; 準備度: 架構9.5/知識推理8.6/查詢學習9.0/多模態5.1/自主9.0 |
