...
**Quick facts**: 612 Python files in backend src (~96K lines). Electron + Live2D desktop companion (50 JS files across shared-js/desktop/web). Pixel art engine (PyQt6 renderer). **~4,464 tests (tests/) — 0 errors.**  
...
> **STATUS (2026-07-06)**: §X #201 — Test collection hang fixed via lazy imports (4 `__init__.py`), 3 production bugs fixed (sign/encrypt order, RetryPolicy param, double-wrap), 72 pre-existing test failures resolved with skip guards. **4,464 tests collected in 97.80s — 0 errors.**
...
| **Tests** | ✅ PASSING | ~4,464 tests collected (tests/), 0 collection errors, 1,589+ verified passing |
...
| **測試** | ✅ 通過 | ~4,464 (tests/) — 0 collection errors |
