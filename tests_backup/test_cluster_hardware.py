...
async def test_hardware_and_cluster():
    ...
    print(f"✅ 硬體概覽: {profile.cpu.brand} | {len(profile.gpu)} GPU(s) | {profile.memory.total // 1024} GB RAM")
    ...
    print(f"✅ 模型配置: Size={config.model_config.size.value}, Precision={config.model_config.precision}")
    ...
    print(f"✅ Vision 精度圖譜: {vision_config.default_precision.value}, Shape: {vision_config.default_shape.dimensions}")
