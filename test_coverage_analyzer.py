...
class TestCoverageAnalyzer(unittest.TestCase):
    ...
    def test_init(self):
        ...
        self.assertIn("line_coverage", self.analyzer.thresholds)
        self.assertIn("branch_coverage", self.analyzer.thresholds)
        self.assertIn("function_coverage", self.analyzer.thresholds)
    ...
    def test_parse_coverage_data(self):
        ...
        # 验证解析结果
        self.assertIsInstance(metrics, CoverageMetrics)
        self.assertEqual(metrics.line_coverage, 85.5)
        self.assertEqual(metrics.total_lines, 1000)
        self.assertEqual(metrics.covered_lines, 855)
        self.assertEqual(metrics.total_branches, 200)
        self.assertEqual(metrics.covered_branches, 197)  # 200 - 3
        self.assertEqual(metrics.branch_coverage, 98.5)  # (197/200)*100
        self.assertAlmostEqual(metrics.function_coverage, 66.67, places=2)  # (2/3)*100，四舍五入到小数点后两位
        self.assertIsNotNone(metrics.timestamp)
        
    def test_estimate_function_coverage(self):
        """测试估算函数覆盖率"""
        # 模拟文件覆盖率数据
        coverage_data = {
            "files": {
        ...
        self.assertEqual(function_coverage, 60.0)
        
    def test_estimate_function_coverage_empty_data(self):
        """测试估算空数据的函数覆盖率"""
        # 空数据
        coverage_data = {}
        ...
        # 验证估算结果：没有函数，覆盖率应该是0%
        self.assertEqual(function_coverage, 0.0)
        
    def test_check_coverage_thresholds(self):
        """测试检查覆盖率阈值"""
        # 创建测试指标
    ...
    def test_check_coverage_thresholds_failures(self):
        """测试检查未通过的覆盖率阈值"""
        # 创建测试指标（低于阈值）
        metrics = CoverageMetrics()
        ...
        # 验证检查结果
        self.assertFalse(results["line_coverage_ok"])
        self.assertFalse(results["branch_coverage_ok"])
        self.assertFalse(results["function_coverage_ok"])
    ...
    def test_generate_coverage_report(self):
        ...
        # 验证报告内容
        self.assertIn("覆盖率分析报告", report)
        self.assertIn(f"行覆盖率: {metrics.line_coverage:.2f}%", report)
        self.assertIn(f"分支覆盖率: {metrics.branch_coverage:.2f}%", report)
...
class TestCoverageAnalyzerIntegration(unittest.TestCase):
    ...
    def test_run_coverage_analysis_success(self, mock_subprocess_run):
        """测试成功运行覆盖率分析"""
        ...
        with patch('builtins.open', mock_open(read_data=json.dumps(mock_coverage_data))) as mock_file:
            with patch('os.path.exists', return_value=True):
                with patch('os.remove'):
                    ...
                    self.assertEqual(metrics.line_coverage, 90.0)
                    
                    # 验证调用了正确的命令
                    self.assertEqual(mock_subprocess_run.call_count, 2)  # run和json两个命令
                    
    @patch('subprocess.run')
    def test_run_coverage_analysis_failure(self, mock_subprocess_run):
        """测试运行覆盖率分析失败"""
    ...
    @patch('subprocess.run')
    def test_generate_coverage_report_failure(self, mock_subprocess_run):
        """测试生成覆盖率报告失败"""
        # 模拟第一个命令成功，第二个命令失败
