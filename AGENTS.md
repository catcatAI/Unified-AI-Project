...
>
> ✅ **NOTE (Updated 2026-07-02, §X #117)**: **§X #117**: CausalReasoningEngine C³ 4.5→**6.0/10** — closed-loop via `_get_causal_routing_adjustment()` in chat_routes.py. Reads causal predictions (`user_input`/`query_complexity`/`conversation_momentum`/`interaction_value` avg strength) → computes `temperature_bias`/`max_tokens_bias`. Injected into context when confidence ≥ 0.25. Applied in router.py `_prepare_generation_context()` (Priority 3.5): temperature adjusted by bias (clamped 0.1-1.5), max_tokens by bias (clamped 128-1024). 8 new tests. **4,794 tests collected (tests/) — 0 errors.**
