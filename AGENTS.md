...
> ✅ **NOTE (Updated 2026-07-04, §X #154)**: **§X #154**: Frontend critical IPC fixes — preload.js: fixed 2 channel name mismatches (setIgnoreMouseEvents, setClickThroughRegions), fixed 2 send→invoke method mismatches, added 5 missing validChannels (websocket-disconnected/error/send-result, backend-ip-changed, render-mode). main.js: added missing window-restore handler, fixed 2 bare catch blocks → proper logging. Test quality: strengthened 7 weak tests in test_phase1_2.py (5 print-only → real assertions) + test_anchor_vectors.py (2 print-only → real assertions). **5,028 tests collected (tests/) — 0 errors.** - **Phase A1-A4: External dictionary download + convert + import pipeline
