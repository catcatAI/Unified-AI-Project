...
| 檔案 | 行號 | 類型 | 描述 |
|------|------|------|------|
| `apps/backend/src/core/autonomous/live2d_avatar_generator.py` | 791 | TODO | 等待真實的分層微服務上線後可開啟此段 |
| `apps/backend/src/core/managers/dependency_manager.py` | 20 | TODO | Replace with actual yaml when available |
...
| 項目 | 優先級 | 狀態 |
|------|--------|------|
| API-client.js語法錯誤 | P0 | 待修復 |
| settings.html HTML錯誤 | P0 | 待修復 |
| 測試文件語法錯誤（31個文件） | P0 | 待修復 |
| 重複函數定義 settings.js | P1 | 待修復 |
| 硬編碼值配置化 | P2 | 待修復 |
...
| 優先級 | 項目 | 數量 | 備註 |
|--------|------|------|------|
| P1 | `except Exception` 需加解釋注釋 | ~901 | 後端 |
| P2 | JS 模組缺失 | 0 | **已全部解決** |
| P2 | 硬編碼值 | 50+ | 後端 |
| P2 | TODO/FIXME 標記 | 2 | backend/src |
| P3 | 配置文件驗證 | - | 建議增強 |
