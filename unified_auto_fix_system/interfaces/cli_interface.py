...
#             "project": FixScope.PROJECT,
        ...
        priority = priority_map.get(getattr(args, 'priority', 'normal'), FixPriority.NORMAL)
...
# 
    ...
    def _output_fix_report(self, report, output_file: Optional[str] = None):
        ...
        print(f"总计问题: {report.get_total_issues_found()}")
        print(f"修复问题: {report.get_total_issues_fixed()}")

        print(f"成功率: {report.get_success_rate():.1%}")
        ...
        # 输出到文件
        if output_file:
            self._output_json(asdict(report), output_file)
