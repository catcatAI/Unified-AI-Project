...
class DependencyAnalyzer(ast.NodeVisitor):
    ...
    def visit_Call(self, node: ast.Call):
        ...
        if isinstance(node.func, ast.Name):
            ...
            # 创建调用依赖边
            source_id = f"{self.file_path}function,{node.lineno}"  # 简化处理
            target_id = f"{func_name}function"
            dep = LogicEdge(
                source_id=source_id,
                target_id=target_id,
                edge_type="calls"
    ...
    def visit_Name(self, node: ast.Name):
        ...
        if isinstance(node.ctx, ast.Load):
            # 分析变量引用依赖
            name = node.id
            source_id = f"{self.file_path}name,{node.lineno}"
            target_id = f"{name}variable"
            dep = LogicEdge(
                source_id=source_id,
                target_id=target_id,
                edge_type="uses"
