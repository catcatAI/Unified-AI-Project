...
class DependencyFixer(BaseFixer):
                ...
                if package_info:
                    ...
                    if package_name:
                        ...
                        if not self._is_package_installed(package_name):
                            ...
                            if required_version and installed_version and not self._version_satisfies(installed_version, required_version):
                                issues.append(DependencyIssue(
                                    package_name=package_name,
    ...
    def _version_satisfies(self, installed_version: str, required_version: str) -> bool:
        ...
        try:
            # 移除版本操作符
            required_version_clean = re.sub(r'^[<>=!~]+', '', required_version)
            # 简单的版本比较
            return installed_version == required_version_clean
        except Exception:
            return False
    ...
    def _check_installed_packages(self) -> List[DependencyIssue]:
        ...
        try:
            ...
            self.logger.warning(f"检查已安装包时出错: {e}")
    ...
    def _get_fixed_by_type(self, issues_by_type: Dict[str, List[DependencyIssue]], total_fixed: int) -> Dict[str, int]:
        """获取按类型修复的数量"""
        # 简化处理：按比例分配修复数量
        fixed_by_type = {}
