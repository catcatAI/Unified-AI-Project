...
class DriverDetector {
  ...
  async _detectAudioDriver() {
    ...
    try {
      // Try ALSA info
      const audioInfo = await this._runCommand(
        'aplay -l 2>/dev/null || cat /proc/asound/cards 2>/dev/null'
      )
  ...
  async _detectNetworkDriver() {
    ...
    try {
      const networkInfo = await this._runCommand(
        'lsmod 2>/dev/null | grep -E "e1000|rtl|ath|iwl" | head -5'
      )
