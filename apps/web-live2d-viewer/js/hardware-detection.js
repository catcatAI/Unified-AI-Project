...
class HardwareDetector {
  ...
  _assessWallpaperMode(profile) {
    ...
    if (
      ram >= 16 &&
      (gpuName.includes('RTX') || gpuName.includes('Radeon') || gpuName.includes('M'))
    ) {
      return '3D'
    }
...
class StateMatrixIntegration {
  ...
  _applyBehavioralParameters() {
    ...
    this.live2dManager.setParameter(
      'ParamBodyAngleX',
      this.live2dManager.getParameter('ParamBodyAngleX') +
        (Math.random() - 0.5) * bodyAngleVariation
    )
  }
...
class DynamicPerformanceManager {
  ...
  _optimizeForHardware() {
    ...
    // 智能性能調整
    const isLowEnd =
      !profile.gpu_info.available ||
      profile.cpu_cores <= 2 ||
      (profile.ram_gb && profile.ram_gb <= 4)

    const isHighEnd =
      profile.gpu_info.available && profile.cpu_cores >= 6 && profile.ram_gb && profile.ram_gb >= 16
  ...
  _downgradePerformance() {
    ...
    const reducedEffects = currentEffects.filter(
      (e) => e !== 'depth-of-field' && e !== 'screen-space-reflections'
    )
