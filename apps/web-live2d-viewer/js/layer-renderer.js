...
class LayerRenderer {
    ...
    _renderSingleImage(img, params) {
        ...
        // 存儲基礎層偏移量以供遮罩使用
        if (img === this.layers.base?.image) {
            this.baseOffset = { x, y, width: targetWidth, height: targetHeight };
        }
    ...
    _applyMaskToBaseLayer() {
        ...
        // 獲取顯示縮放比例
        const displayScale = this.udm ? this.udm.getUserScale() : 1.0;
        const basePos = this.baseOffset || { x: 0, y: 0 };
        ...
        if (poseConfig.headMaskRect) {
            ...
            // 繪製頭部遮罩（完全透明）
            // 修正：遮罩位置應相對於基礎層
            const maskX = basePos.x + Math.floor(mask.x * displayScale);
            const maskY = basePos.y + Math.floor(mask.y * displayScale);
            const maskW = Math.ceil(mask.width * displayScale);
            const maskH = Math.ceil(mask.height * displayScale);
        ...
        if (poseConfig.handMaskRects && poseConfig.handMaskRects.length > 0) {
            for (const maskRect of poseConfig.handMaskRects) {
                const maskX = basePos.x + Math.floor(maskRect.x * displayScale);
                const maskY = basePos.y + Math.floor(maskRect.y * displayScale);
                const maskW = Math.ceil(maskRect.width * displayScale);
                const maskH = Math.ceil(maskRect.height * displayScale);
