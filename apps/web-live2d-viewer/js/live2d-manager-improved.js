...
class Live2DModelValidator {
    ...
    static async validate(modelPath) {
        ...
        try {
            // 检查 model3.json
            const modelJsonResponse = await fetch(`${window.electronAPI?.platform === 'web' ? '' : (window.electronAPI?.platform === 'web' ? '' : 'local://')}${modelPath}`);
            ...
            if (!mocFileName) {
                ...
                const mocPath = `${window.electronAPI?.platform === 'web' ? '' : (window.electronAPI?.platform === 'web' ? '' : 'local://')}${baseDir}${mocFileName}`;
                try {
            ...
            if (textures.length === 0) {
                ...
                for (let i = 0; i < textures.length; i++) {
                    const texPath = `${window.electronAPI?.platform === 'web' ? '' : (window.electronAPI?.platform === 'web' ? '' : 'local://')}${baseDir}${textures[i]}`;
                    try {
