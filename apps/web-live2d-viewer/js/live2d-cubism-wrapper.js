...
class Live2DCubismWrapper {
    ...
    async loadCubismScript() {
        ...
        try {
            // In web environment, we will primarily load from our local libs
            await this._loadScript('libs/live2dcubismcore.min.js');
            await this.waitForCubismSDK();
            ...
            console.error('Failed to load Live2D Cubism SDK:', error);
            // Fallback to local live2d-fallback.js if main SDK fails
            try {
                console.log('Main SDK load failed, attempting to load local fallback...');
                await this._loadScript('libs/live2d-fallback.js');
                if (window.loadLocalLive2DSDK) {
                    await window.loadLocalLive2DSDK();
                    console.log('Successfully loaded local fallback SDK');
                }
            } catch (fallbackError) {
