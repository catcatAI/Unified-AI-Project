  ...
  _validateSize(value) {
    ...
    try {
      ...
      if (serialized.length > this.validationConfig.maxSizePerItem) {
        console.warn(
          '[StorageManager] Data size exceeds limit:',
          serialized.length,
          '>',
          this.validationConfig.maxSizePerItem
        )
        return false
      }
      ...
      if (typeof value === 'string' && value.length > this.validationConfig.maxSizePerString) {
        console.warn(
          '[StorageManager] String length exceeds limit:',
          value.length,
          '>',
          this.validationConfig.maxSizePerString
        )
        return false
      }
