...
class _Collection:
    def __init__(self, name: str, embedding_function: Optional[Any] = None, metadata: Optional[Dict[str, Any]] = None):
        self.name = name
...
class PersistentClient(_BaseClient):
    def __init__(self, path: Optional[str] = None, settings: Optional[Any] = None):
        super().__init__()
        self._path = path
...
class HttpClient(_BaseClient):
    def __init__(self, host: str = "localhost", port: int = 8000, settings: Optional[Any] = None):
        super().__init__()
        self._host = host
        self._port = port
        self._settings = settings
