...
class _Collection:
    def __init__(self, name: str, embedding_function: Optional[Any] = None, metadata: Optional[Dict[str, Any]] = None) -> None:
self.name = name
...
_ = result["ids"].append(_id)
                if 'documents' in include:
_ = result["documents"].append(self._docs[_id])
                if 'metadatas' in include:
_ = result["metadatas"].append(self._metas.get(_id, {}))
                if 'embeddings' in include:
_ = result["embeddings"].append(self._embeds.get(_id, []))
...
result.pop('embeddings', None)
    ...
    def query(self, query_texts: List[str], n_results: int = 10, include: Optional[List[str]] = None) -> Dict[str, Any]:
include = include or []
...
self._collections: Dict[str, _Collection] = {}
    ...
    def get_or_create_collection(self, name: str, embedding_function: Optional[Any] = None, metadata: Optional[Dict[str, Any]] = None) -> _Collection:
...
class HttpClient(_BaseClient):
    def __init__(self, host: str = "localhost", port: int = 8000, settings: Optional[Any] = None) -> None:
super().__init__()
self._host = host
self._port = port
self._settings = settings
