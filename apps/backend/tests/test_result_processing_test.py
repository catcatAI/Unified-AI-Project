...
from ..scripts.test_result_analyzer import TestResultAnalyzer, FailurePattern, PerformanceRegression
from ..scripts.test_result_feedback import TestResultFeedbackSystem, DevelopmentTask

# 修复TestResultAnalyzer类，添加缺失的方法
class ExtendedTestResultAnalyzer(TestResultAnalyzer):
    ...
    def generate_analysis_report(self, test_results, failure_patterns, report_file="analysis_report.json"):
        """生成分析报告"""
        analysis_report = {
...
class ExtendedTestResultFeedbackSystem(TestResultFeedbackSystem):
...
class TestResultVisualizerTest(unittest.TestCase):
    ...
    def test_visualize_test_distribution(self):
        """测试生成测试分布图"""
...
class TestResultFeedbackSystemTest(unittest.TestCase):
    ...
    def test_integrate_with_development_workflow(self):
        """测试与开发流程集成"""
        suggestions = self.feedback_system.generate_improvement_suggestions(
            self.analysis_report)
