...
async def base_agent(mock_hsp_connector):
    ...
    # Patch core_services functions in the BaseAgent's module scope
    with patch('src.agents.base_agent.initialize_services', new_callable=AsyncMock):
        with patch('src.agents.base_agent.get_services') as mock_get_services:
            with patch('src.agents.base_agent.shutdown_services', new_callable=AsyncMock):
                # Configure mock_get_services to return our mock_hsp_connector
                mock_get_services.return_value = {
                    'hsp_connector': mock_hsp_connector,
                    # Add other services if BaseAgent needs to access them during _ainit
                ...
                await agent._ainit() # This will use the mocked get_services
    return agent
