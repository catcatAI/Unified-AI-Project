...
async def test_base_agent_start(base_agent) -> None:
    ...
    with patch('apps.backend.src.core_services.initialize_services') as mock_init_services, \
        ...
        _ = mock_init_services.return_value.set_result(None)
        ...
        _ = mock_init_services.assert_called_once()
        if base_agent.hsp_connector:
            _ = base_agent.hsp_connector.register_on_task_request_callback.assert_called_once_with(base_agent.handle_task_request)
...
async def test_base_agent_stop(base_agent) -> None:
    ...
    with patch('apps.backend.src.core_services.initialize_services') as mock_init_services, \
        ...
        assert base_agent.is_running is False
        _ = mock_shutdown_services.assert_called_once()
...
async def test_base_agent_handle_task_request(base_agent) -> None:
    ...
    _ = base_agent.hsp_connector.send_task_result.assert_called_once()
    ...
    _ = assert "not implemented" in result_payload["error_details"]["error_message"].lower()
