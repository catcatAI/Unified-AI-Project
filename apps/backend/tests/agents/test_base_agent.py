...
async def test_base_agent_init(base_agent):
    with patch('src.core_services.initialize_services', new_callable=AsyncMock):
        with patch('src.core_services.get_services') as mock_get_services:
            mock_get_services.return_value = {} # Init does not need hsp_connector
...
async def test_base_agent_start_stop(base_agent, mock_hsp_connector):
    ...
    with patch('src.core_services.initialize_services', new_callable=AsyncMock) as mock_init_services:
        with patch('src.core_services.get_services') as mock_get_services:
            with patch('src.core_services.shutdown_services', new_callable=AsyncMock) as mock_shutdown_services:
...
async def test_base_agent_handle_task_request(base_agent, mock_hsp_connector):
    ...
    with patch('src.core_services.initialize_services', new_callable=AsyncMock) as mock_init_services:
        with patch('src.core_services.get_services') as mock_get_services:
            mock_get_services.return_value = {'hsp_connector': mock_hsp_connector}
            await base_agent._ainit() # Need to call _ainit here to set up the agent with mocked services
            task_payload = {
