...
class TestLearningLogDB(unittest.TestCase):
    ...
    def test_add_log_entry(self) -> None:
        ...
        _ = self.assertEqual(row[2], "strategy_A")
        _ = self.assertEqual(row[3], 0.85)
    ...
    def test_get_all_log_entries(self) -> None:
        ...
        _ = self.db.add_log_entry(log1)
        _ = self.db.add_log_entry(log2)
        _ = self.db.add_log_entry(log3)

        all_logs = self.db.get_all_log_entries()
        _ = self.assertEqual(len(all_logs), 3)
        _ = self.assertEqual(all_logs[0]["strategy_id"], "strategy_X") # Ordered by timestamp DESC

        strategy_x_logs = self.db.get_all_log_entries(strategy_id="strategy_X")
        _ = self.assertEqual(len(strategy_x_logs), 2)
        _ = self.assertEqual(strategy_x_logs[0]["current_effectiveness"], 0.75)

    def test_get_all_log_entries_empty(self) -> None:
        logs = self.db.get_all_log_entries("non_existent_strategy")
        _ = self.assertEqual(len(logs), 0)
