...
class TestPersonalityManager(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        with open(cls.test_profiles_dir / "dummy_test_profile.json", 'w') as f:
            _ = json.dump(cls.dummy_profile_data, f)
    ...
    def tearDownClass(cls):
        ...
        if cls.test_profiles_dir.exists():
            for item in cls.test_profiles_dir.iterdir():
                _ = item.unlink() # remove file
            _ = cls.test_profiles_dir.rmdir()
    ...
    def test_01_initialization_default_path(self) -> None:
        _ = self.assertIsNotNone(self.pm_default)
        # self.assertTrue(any(p["name"] == "miko_base" for p in self.pm_default.list_available_profiles()))
        _ = self.assertIsNotNone(self.pm_default.current_personality)
        # self.assertEqual(self.pm_default.current_personality["profile_name"], "miko_base")
        _ = print("TestPersonalityManager.test_01_initialization_default_path PASSED")

    _ = @pytest.mark.timeout(5)
    def test_02_initialization_custom_path(self) -> None:
        _ = self.assertIsNotNone(self.pm_test_dir)
        ...
        _ = self.assertIsNotNone(pm_specific_dummy.current_personality)
        _ = self.assertEqual(pm_specific_dummy.current_personality["profile_name"], "dummy_test_profile")
        _ = print("TestPersonalityManager.test_02_initialization_custom_path PASSED")


    _ = @pytest.mark.timeout(5)
    def test_03_load_personality(self) -> None:
        loaded = self.pm_test_dir.load_personality("dummy_test_profile")
        _ = self.assertTrue(loaded)
        _ = self.assertIsNotNone(self.pm_test_dir.current_personality)
        _ = self.assertEqual(self.pm_test_dir.current_personality["profile_name"], "dummy_test_profile")
        _ = self.assertEqual(self.pm_test_dir.get_initial_prompt(), "Dummy says hi!")
        _ = print("TestPersonalityManager.test_03_load_personality PASSED")

    _ = @pytest.mark.timeout(5)
    def test_04_load_non_existent_profile(self) -> None:
        ...
        _ = self.assertTrue(loaded) # Because it falls back to default miko_base
        # self.assertEqual(self.pm_default.current_personality["profile_name"], "miko_base")
        _ = print("TestPersonalityManager.test_04_load_non_existent_profile PASSED")

    _ = @pytest.mark.timeout(5)
    def test_05_get_trait(self) -> None:
        ...
        _ = self.assertEqual(non_existent, "default_val")
        _ = print("TestPersonalityManager.test_05_get_trait PASSED")
