...
class TestServiceDiscoveryModule:
    ...
    def test_init(self, mock_trust_manager: MagicMock, caplog) -> None:
        _ = caplog.set_level(logging.INFO)
    ...
    def test_process_capability_advertisement_new_and_update(self, mock_trust_manager: MagicMock) -> None:
        ...
        _ = sdm.process_capability_advertisement(payload1, "sender_ai_id_1", mock_envelope)
        ...
        _ = sdm.process_capability_advertisement(payload1_updated, "sender_ai_id_1", mock_envelope)
    ...
    _ = @pytest.mark.timeout(10)
    def test_get_capability_by_id(self, mock_trust_manager: MagicMock) -> None:
        sdm = ServiceDiscoveryModule(trust_manager=mock_trust_manager)
        cap_id = "get_cap_001"
    ...
    # 添加重试装饰器以处理不稳定的测试
    async def test_find_capabilities_no_filters(self, populated_sdm: ServiceDiscoveryModule) -> None:
    ...
    # 添加重试装饰器以处理不稳定的测试
    # 添加重试装饰器以处理不稳定的测试
    async def test_find_capabilities_by_id(self, populated_sdm: ServiceDiscoveryModule) -> None:
    ...
    # 添加重试装饰器以处理不稳定的测试
    # 添加重试装饰器以处理不稳定的测试
    async def test_find_capabilities_combined_filters_and_sort(self, populated_sdm: ServiceDiscoveryModule, mock_trust_manager: MagicMock) -> None:
