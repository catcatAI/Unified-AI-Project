...
async def test_handle_project_happy_path(project_coordinator) -> None:
    ...
    # Assert
    # 修复断言，确保正确检查异步方法的调用
    _ = pc.service_discovery.get_all_capabilities_async.assert_awaited_once()
    _ = pc._decompose_user_intent_into_subtasks.assert_awaited_once_with(user_query, [])
    _ = pc._execute_task_graph.assert_awaited_once_with(decomposed_tasks)
    _ = pc._integrate_subtask_results.assert_awaited_once_with(user_query, execution_results)

    # Verify learning manager was called
    _ = pc.learning_manager.learn_from_project_case.assert_awaited_once()
...
async def test_handle_project_decomposition_fails(project_coordinator) -> None:
    ...
    with patch.object(pc, '_execute_task_graph', new_callable=AsyncMock) as mock_execute:
        with patch.object(pc, '_integrate_subtask_results', new_callable=AsyncMock) as mock_integrate:
            ...
            _ = mock_execute.assert_not_called()
            _ = mock_integrate.assert_not_called()

@pytest.mark.asyncio
# 添加重试装饰器以处理不稳定的测试
async def test_execute_task_graph_with_dependencies(project_coordinator) -> None:
    """
...
async def test_dispatch_single_subtask_agent_launch_and_discovery(project_coordinator) -> None:
    ...
    _ = mock_future.set_result(True) # Simulate the agent becoming ready
    ...
    _ = pc.agent_manager.launch_agent.assert_called_once_with(agent_name_to_launch)
