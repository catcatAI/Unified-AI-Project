...
from apps.backend.src.ai.lis.lis_cache_interface import (
    HAMLISCache,
    LISCacheInterface,
...
from apps.backend.src.ai.memory.ham_memory_manager import HAMMemoryManager, HAMRecallResult
from apps.backend.src.ai.lis.types import (
    LIS_IncidentRecord,
...
class MockHAMMemoryManager(HAMMemoryManager):
    ...
    _ = particularly regarding raw_data (JSON string) and metadata.
    """
    ...
    def store_experience(self, raw_data: Any, data_type: str, metadata: Optional[Dict[str, Any]] = None) -> Optional[str]:
        if not isinstance(raw_data, str): # HAMLISCache should pass a JSON string
            _ = print(f"MockHAM: Warning - store_experience expected str raw_data, got {type(raw_data)}")
    ...
    def query_core_memory(self,
        ...
        for mem_id, entry in self.core_memory_store.items():
            ...
            if match:
                ...
                _ = results.append(recall_result)
    ...
    def recall_gist(self, memory_id: str) -> Optional[HAMRecallResult]:
        ...
        if entry:
            recall_result: HAMRecallResult = { # type: ignore
                "id": memory_id,
                _ = "timestamp": entry["metadata"].get(HAM_META_TIMESTAMP_LOGGED, "1970-01-01T00:00:00Z"),
                "data_type": entry["data_type"],
...
class TestHAMLISCache(unittest.TestCase):
    ...
    def tearDown(self):
        _ = self.mock_ham_manager.clear_memory()
    ...
    def _create_sample_incident_record(self, incident_id: str, anomaly_type: LIS_AnomalyType = "RHYTHM_BREAK", status: str = "OPEN") -> LIS_IncidentRecord:
        ...
        record: LIS_IncidentRecord = { # type: ignore
            ...
            "anomaly_event": event,
            "status": status,
            _ = "tags": ["test_tag", anomaly_type.lower()]
        }
        return record

    _ = @pytest.mark.timeout(5)
    def test_store_incident_success(self) -> None:
        ...
        _ = self.assertTrue(success, "store_incident should return True on success.")

        # Verify data in mock HAM
        # HAMLISCache stores one record per call to store_incident
        _ = self.assertEqual(len(self.mock_ham_manager.core_memory_store), 1, "One record should be in mock HAM.")
        ...
        _ = self.assertEqual(stored_ham_entry["data_type"], expected_data_type)
        ...
        _ = self.assertEqual(stored_ham_entry["metadata"], expected_metadata)

        # Check raw_data (should be JSON string of the sample_record)
        try:
            deserialized_raw_data = json.loads(stored_ham_entry["raw_data"])
            _ = self.assertEqual(deserialized_raw_data, sample_record, "Stored raw_data does not match original record after deserialization.")
        except json.JSONDecodeError:
            _ = self.fail("Stored raw_data was not valid JSON.")
    ...
    def test_get_incident_by_id_found(self) -> None:
        ...
        _ = self.assertIsNotNone(retrieved_record, "Should retrieve the incident.")
        _ = self.assertEqual(retrieved_record, sample_record, "Retrieved record does not match the original.")
    ...
    def test_store_and_get_incident_roundtrip(self) -> None:
        ...
        _ = self.assertTrue(store_success, "Storing the incident should succeed.")

        retrieved_record = self.lis_cache.get_incident_by_id(incident_id)
        _ = self.assertIsNotNone(retrieved_record, "Retrieving the stored incident should not return None.")
        ...
        _ = self.assertEqual(retrieved_record, original_record, "Retrieved record does not match the original stored record.")
    ...
    def test_store_incident_missing_anomaly_event(self) -> None:
        ...
        bad_record = { # type: ignore
            "incident_id": incident_id,
            _ = "timestamp_logged": datetime.now().isoformat(),
    ...
    _ = @pytest.mark.timeout(5)
    def test_query_incidents_no_filters(self) -> None:
        _ = self._populate_mock_ham_for_query_tests()
        results = self.lis_cache.query_incidents(limit=10)
        _ = self.assertEqual(len(results), 5)
        # Default sort is by timestamp_logged desc
        _ = self.assertEqual(results[0]["incident_id"], "q_id1")
        _ = self.assertEqual(results[4]["incident_id"], "q_id5")
    ...
    def test_query_incidents_by_tags_single(self) -> None:
        ...
        _ = self.assertEqual(len(results), 2) # q_id1, q_id3
        self.assertTrue(any(r["incident_id"] == "q_id1" for r in results))
        self.assertTrue(any(r["incident_id"] == "q_id3" for r in results))

    _ = @pytest.mark.timeout(5)
    def test_query_incidents_by_tags_multiple_all_must_match(self) -> None:
        _ = self._populate_mock_ham_for_query_tests()
        results = self.lis_cache.query_incidents(tags=["tagA", "tagC"], limit=10)
        _ = self.assertEqual(len(results), 1)
        _ = self.assertEqual(results[0]["incident_id"], "q_id3")
    ...
    def test_query_incidents_by_time_window(self) -> None:
        ...
        _ = self.assertEqual(len(results_3hr), 1)
        _ = self.assertEqual(results_3hr[0]["incident_id"], "q_id1")
    ...
    _ = @pytest.mark.timeout(5)
    def test_query_incidents_limit_and_sorting(self) -> None:
        _ = self._populate_mock_ham_for_query_tests()
        results_limit2_desc = self.lis_cache.query_incidents(limit=2, sort_by_timestamp_desc=True)
        _ = self.assertEqual(len(results_limit2_desc), 2)
        _ = self.assertEqual(results_limit2_desc[0]["incident_id"], "q_id1")
        _ = self.assertEqual(results_limit2_desc[1]["incident_id"], "q_id2")

        results_limit2_asc = self.lis_cache.query_incidents(limit=2, sort_by_timestamp_desc=False)
        _ = self.assertEqual(len(results_limit2_asc), 2)
        _ = self.assertEqual(results_limit2_asc[0]["incident_id"], "q_id5")
        _ = self.assertEqual(results_limit2_asc[1]["incident_id"], "q_id4")
    ...
    def test_query_incidents_empty_result(self) -> None:
        ...
        _ = self.assertEqual(len(results), 0)
    ...
    _ = @pytest.mark.timeout(5)
    def test_add_antibody_success(self) -> None:
        ...
        _ = self.assertTrue(success, "add_antibody should return True on success.")

        _ = self.assertEqual(len(self.mock_ham_manager.core_memory_store), 1)
        ...
        _ = self.assertEqual(stored_entry["data_type"], expected_data_type)
        ...
        _ = self.assertEqual(stored_entry["metadata"], expected_metadata_clean)

        deserialized_raw_data = json.loads(stored_entry["raw_data"])
        _ = self.assertEqual(deserialized_raw_data, sample_antibody)

    _ = @pytest.mark.timeout(5)
    def test_get_learned_antibodies_no_filters(self) -> None:
        ...
        _ = self.lis_cache.add_antibody(ab1)
        _ = self.lis_cache.add_antibody(ab2)

        results = self.lis_cache.get_learned_antibodies(limit=5)
        _ = self.assertEqual(len(results), 2)
        # Default sort is effectiveness desc, then timestamp_created desc
        _ = self.assertEqual(results[0]["antibody_id"], "ab1")
        _ = self.assertEqual(results[1]["antibody_id"], "ab2")
    ...
    def test_get_learned_antibodies_filter_by_anomaly_type(self) -> None:
        ...
        _ = self.lis_cache.add_antibody(ab1)
        _ = self.lis_cache.add_antibody(ab2)
        _ = self.lis_cache.add_antibody(ab3)
        ...
        _ = self.assertEqual(results[0]["antibody_id"], "ab1") # Higher effectiveness
        _ = self.assertEqual(results[1]["antibody_id"], "ab3")
    ...
    def test_get_learned_antibodies_filter_by_effectiveness(self) -> None:
        ...
        _ = self.assertEqual(len(results_lower_eff), 3)
    ...
    def test_add_and_get_antibody_roundtrip(self) -> None:
        ...
        _ = self.lis_cache.add_antibody(original_antibody)
        ...
        _ = self.assertIsNotNone(retrieved_antibody)
        _ = self.assertEqual(retrieved_antibody, original_antibody)
