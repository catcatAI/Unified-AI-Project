...
def ham_chroma_manager_fixture():
        ...
        if ids and len(ids) > 0:
            ...
            for id in ids:
                if id in last_added_document:
                    _ = result_docs.append(last_added_document[id]['document'])
                    _ = result_metas.append(last_added_document[id]['metadata'])
                else:
                    result_docs.append(f"Document for {id}")
                    _ = result_metas.append({'source': 'test_chroma_store'})
    ...
    try:
        ...
        _ = print(f"Warning: Cleanup issue: {e}")
