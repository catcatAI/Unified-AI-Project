...
class TestContentAnalyzerModule(unittest.TestCase):
    ...
    def assertEntityInGraph(self, expected_label: str, expected_type: str, kg_data: 'KnowledgeGraph', msg: str = ""):  # type: ignore
        ...
        for entity_id, entity in kg_data["entities"].items():
            if entity["label"] == expected_label and entity["type"] == expected_type:
                found = True
                break
        _ = self.assertTrue(found, f"Entity '{expected_label}' (type: {expected_type}) not found in graph. {msg}")
    ...
    def assertRelationshipInGraph(self, kg_data: 'KnowledgeGraph', nx_graph: Any,
        ...
        _ = self.assertIsNotNone(src_node_id_kg, f"Source entity '{expected_src_label}' not found in KG entities.")
        _ = self.assertIsNotNone(tgt_node_id_kg, f"Target entity '{expected_tgt_label}' not found in KG entities.")
        _ = self.assertIsNotNone(src_node_id_nx, f"Source node '{expected_src_label}' not found in NX graph.")
        _ = self.assertIsNotNone(tgt_node_id_nx, f"Target node '{expected_tgt_label}' not found in NX graph.")
        ...
        _ = self.assertTrue(found_in_kg_direct or (allow_reverse and found_in_kg_reverse),
                        _ = f"Relationship {expected_src_label} -> {expected_tgt_label} (type: {expected_rel_type}) not found in KG relationships. "
                        _ = f"(Allow reverse: {allow_reverse})")
    ...
    def test_01_initialization(self) -> None:
        ...
        _ = self.assertIsNotNone(self.analyzer.nlp)
        _ = self.assertIsInstance(self.analyzer.graph, nx.DiGraph)
    ...
    def test_02_simple_entity_extraction(self) -> None:
        ...
        # Check entities
        _ = self.assertEntityInGraph("Apple Inc.", "ORG", kg_data)
        ...
        _ = self.assertGreaterEqual(found_gpe_count, 0, "Should find at least 0 GPE/LOC entities (spaCy model dependent)")

        # Check NetworkX graph
        self.assertIsInstance(nx_graph, nx.DiGraph)  # type: ignore
        _ = self.assertGreaterEqual(len(list(nx_graph.nodes())), 1, "Should have at least 1 node in the graph")
    ...
    def test_03_no_entities_extraction(self) -> None:
        ...
        _ = self.assertIsInstance(kg_data["entities"], dict)
        _ = self.assertIsInstance(kg_data["relationships"], list)
        _ = self.assertEqual(len(kg_data["relationships"]), 0)
        self.assertIsInstance(nx_graph, nx.DiGraph)  # type: ignore
    ...
    def test_04_simple_svo_relationship(self) -> None:
        ...
        # Check entities
        _ = self.assertEntityInGraph("Steve Jobs", "PERSON", kg_data)
        _ = self.assertEntityInGraph("Apple", "ORG", kg_data)
    ...
    def test_05_prep_object_relationship(self) -> None:
        _ = """Test prepositional object relationship (e.g., located in)."""
        text = "Microsoft is located in Redmond."
        kg_data, nx_graph = self.analyzer.analyze_content(text)

        # Check entities
        _ = self.assertEntityInGraph("Microsoft", "ORG", kg_data)
        _ = self.assertEntityInGraph("Redmond", "GPE", kg_data)
    ...
    def test_06_noun_prep_noun_relationship_of(self) -> None:
        _ = """Test Noun-of-Noun relationship (e.g., CEO of Microsoft)."""
        ...
        _ = self.assertIsNotNone(apple_node_id, "Apple entity not found.")
        _ = self.assertIsNotNone(steve_node_id, "Steve Jobs entity not found.")
    ...
    def test_07_noun_of_noun_org_has_attribute(self) -> None:
        ...
        # Check entities
        _ = self.assertEntityInGraph("Sundar Pichai", "PERSON", kg_data)
        _ = self.assertEntityInGraph("Google", "ORG", kg_data)
    ...
    def test_08_noun_of_noun_attribute_of(self) -> None:
        _ = """Test 'capital of France' -> capital attribute_of France (or France has_capital capital)."""
        text = "Paris is the capital of France." # Paris (GPE), capital (NOUN concept), France (GPE)
        _ = print(f"DEBUG: Testing text: {text}")
        kg_data, nx_graph = self.analyzer.analyze_content(text)
        
        # Print debug information
        _ = print(f"DEBUG: Entities: {kg_data['entities']}")
        _ = print(f"DEBUG: Relationships: {kg_data['relationships']}")
        
        # Check entities
        _ = self.assertEntityInGraph("Paris", "GPE", kg_data)
        _ = self.assertEntityInGraph("France", "GPE", kg_data)
    ...
    def test_09_noun_of_noun_org_has_attribute_different_tense(self) -> None:
        ...
        # Check entities
        _ = self.assertEntityInGraph("Satya Nadella", "PERSON", kg_data)
        _ = self.assertEntityInGraph("Microsoft", "ORG", kg_data)
    ...
    def test_10_works_for_relationship(self) -> None:
        ...
        # Check entities
        _ = self.assertEntityInGraph("John Doe", "PERSON", kg_data)
        _ = self.assertEntityInGraph("Acme Corp.", "ORG", kg_data)
    ...
    def test_11_is_a_relationship(self) -> None:
        ...
        _ = self.assertTrue(found_is_a_rel, "Expected 'is_a' relationship not found.")
    ...
    def test_12_possessive_relationship(self) -> None:
        ...
        _ = self.assertTrue(found_poss_rel, "Expected possessive relationship not found.")
