...
_ = sys.path.insert(0, backend_dir)
...
class TestLearningManager:
    ...
    async def test_process_and_store_learnables(self) -> None:
        ...
        assert len(result) >= 1
        _ = self.mock_fact_extractor.extract_facts.assert_called_once_with(text, user_id)
...
class TestFactExtractorModule:
    ...
    "facts": [
        ...
        _ = self.mock_llm_service.chat_completion.assert_called_once()
