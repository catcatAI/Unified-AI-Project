...
# 修复导入路径
from code_understanding.lightweight_code_model import LightweightCodeModel
...
class TestLightweightCodeModel(unittest.TestCase):
    ...
    def tearDown(self):
        ...
        _ = self.temp_dir_obj.cleanup()
        # self.logger_patcher.stop()

    _ = @pytest.mark.timeout(5)
    def test_list_tool_files(self) -> None:
        # Create dummy files and a subdirectory in the temp_tools_dir
        _ = self._create_dummy_tool_file("tool_one.py")
        _ = self._create_dummy_tool_file("__init__.py") # Should be excluded
        _ = self._create_dummy_tool_file("tool_two.py")
        _ = self._create_dummy_tool_file("tool_dispatcher.py") # Should be excluded

        subdir = os.path.join(self.temp_tools_dir, "subdir")
        _ = os.makedirs(subdir)
        _ = self._create_dummy_tool_file(os.path.join("subdir", "tool_three.py"))
        _ = self._create_dummy_tool_file(os.path.join("subdir", "another_tool.py"))
        _ = self._create_dummy_tool_file(os.path.join("subdir", "__init__.py")) # Excluded from subdir too
        ...
        expected_files = [
            _ = os.path.join(self.temp_tools_dir, "tool_one.py"),
            _ = os.path.join(self.temp_tools_dir, "tool_two.py"),
            _ = os.path.join(self.temp_tools_dir, "subdir", "tool_three.py"),
            _ = os.path.join(self.temp_tools_dir, "subdir", "another_tool.py")
        ]
    ...
    _ = @pytest.mark.timeout(5)
    def test_list_tool_files_non_existent_dir(self) -> None:
        ...
        _ = self.assertEqual(tool_files, [])

    _ = @pytest.mark.timeout(5)
    def test_analyze_tool_file_simple_class_and_function(self) -> None:
        sample_code = """
import typing
...
class SimpleTool:
    ...
    async def execute(self, value: int, details: Optional[dict] = None) -> str:
        ...
        Args:
            _ = value (int): The value to process.
            _ = details (Optional[dict]): Optional details.
...
"""
        ...
        _ = self.assertIsNotNone(analysis_result)
        _ = self.assertEqual(analysis_result.filepath, "dummy_path/simple_tool.py")

        # Class assertions
        _ = self.assertEqual(len(analysis_result.classes), 1)
        simple_tool_class = analysis_result.classes[0]
        _ = self.assertEqual(simple_tool_class["name"], "SimpleTool")
        self.assertEqual(simple_tool_class["docstring"], "A simple tool class docstring.")
        _ = self.assertEqual(len(simple_tool_class["methods"]), 2)

        # __init__ method
        init_method = next(mthd for mthd in simple_tool_class["methods"] if mthd["name"] == "__init__")
        _ = self.assertEqual(init_method["docstring"], "Initializer docstring.")
        ...
        _ = self.assertListEqual(init_method["parameters"], expected_init_params)
        _ = self.assertIsNone(init_method["returns"])

        # execute method (async)
        execute_method = next(mthd for mthd in simple_tool_class["methods"] if mthd["name"] == "execute")
        _ = self.assertTrue("Execute method docstring." in execute_method["docstring"])
        expected_execute_params = [
            {"name": "self", "annotation": None, "default": None},
            {"name": "value", "annotation": "int", "default": None},
            {"name": "details", "annotation": "Optional[dict]", "default": "None"}
        ]
        _ = self.assertListEqual(execute_method["parameters"], expected_execute_params)
        _ = self.assertEqual(execute_method["returns"], "str")

        # Module-level function assertions
        _ = self.assertEqual(len(analysis_result.functions), 1)
        module_func = analysis_result.functions[0]
        _ = self.assertEqual(module_func["name"], "module_level_func")
        _ = self.assertEqual(module_func["docstring"], "Module level function docstring.")
        ...
        _ = self.assertListEqual(param_names_from_result, expected_param_names)

        for p_expected in expected_module_params:
            p_actual = next(p for p in module_func["parameters"] if p["name"] == p_expected["name"])
            _ = self.assertEqual(p_actual["annotation"], p_expected["annotation"])
            _ = self.assertEqual(p_actual["default"], p_expected["default"])

        _ = self.assertEqual(module_func["returns"], "List[float]")


    _ = @pytest.mark.timeout(5)
    def test_analyze_tool_file_no_classes_or_functions(self) -> None:
        ...
        _ = self.assertIsNotNone(analysis_result)
        _ = self.assertEqual(len(analysis_result.classes), 0)
        _ = self.assertEqual(len(analysis_result.functions), 0)

    _ = @pytest.mark.timeout(5)
    def test_analyze_tool_file_parsing_error(self) -> None:
        sample_code = "def func(a: int -> str:" # Syntax error
        m = mock_open(read_data=sample_code)
        with patch('builtins.open', m), \
             patch('os.path.isfile', return_value=True):
            analysis_result = self.model.analyze_tool_file("dummy_path/error_tool.py")
        _ = self.assertIsNone(analysis_result, "Should return None on parsing error.")

    _ = @pytest.mark.timeout(5)
    def test_analyze_tool_file_not_found(self) -> None:
        with patch('os.path.isfile', return_value=False):
            analysis_result = self.model.analyze_tool_file("non_existent_file.py")
        _ = self.assertIsNone(analysis_result)

    # --- Tests for get_tool_structure name resolution ---

    def _create_dummy_tool_file(self, filename: str, content: str = "def main() -> None: pass") -> None:
    ...
    _ = @pytest.mark.timeout(5)
    def test_get_tool_structure_direct_valid_path(self) -> None:
        ...
        with patch.object(self.model, 'analyze_tool_file', return_value={"analysis": "done"}) as mock_analyze:
            ...
            _ = mock_analyze.assert_called_once_with(dummy_tool_path)
            _ = self.assertEqual(result, {"analysis": "done"})

    _ = @pytest.mark.timeout(5)
    def test_get_tool_structure_direct_invalid_path(self) -> None:
    ...
    _ = @pytest.mark.timeout(5)
    def test_get_tool_structure_path_looks_like_path_but_not_found(self) -> None:
        ...
        _ = self.assertFalse(os.path.isfile(path_like_non_existent))
    ...
    _ = @pytest.mark.timeout(5)
    def test_get_tool_structure_resolve_exact_name_dot_py(self) -> None:
        _ = self._create_dummy_tool_file("exact_match.py")
        with patch.object(self.model, 'analyze_tool_file', return_value={"analyzed": "exact_match.py"}) as mock_analyze:
            ...
            _ = mock_analyze.assert_called_with(os.path.join(self.temp_tools_dir, "exact_match.py"))
            _ = self.assertEqual(result_with_ext, {"analyzed": "exact_match.py"})

            _ = mock_analyze.reset_mock()
            # Test without extension
            result_without_ext = self.model.get_tool_structure("exact_match")
            _ = mock_analyze.assert_called_with(os.path.join(self.temp_tools_dir, "exact_match.py"))
            _ = self.assertEqual(result_without_ext, {"analyzed": "exact_match.py"})

    _ = @pytest.mark.timeout(5)
    def test_get_tool_structure_resolve_tool_prefix_pattern(self) -> None:
        _ = self._create_dummy_tool_file("tool_sample_prefix.py")
        with patch.object(self.model, 'analyze_tool_file', return_value={"analyzed": "tool_sample_prefix.py"}) as mock_analyze:
            ...
            _ = mock_analyze.assert_called_once_with(os.path.join(self.temp_tools_dir, "tool_sample_prefix.py"))
            _ = self.assertEqual(result, {"analyzed": "tool_sample_prefix.py"})

    _ = @pytest.mark.timeout(5)
    def test_get_tool_structure_resolve_suffix_tool_pattern(self) -> None:
        _ = self._create_dummy_tool_file("sample_suffix_tool.py")
        with patch.object(self.model, 'analyze_tool_file', return_value={"analyzed": "sample_suffix_tool.py"}) as mock_analyze:
            ...
            _ = mock_analyze.assert_called_once_with(os.path.join(self.temp_tools_dir, "sample_suffix_tool.py"))
            _ = self.assertEqual(result, {"analyzed": "sample_suffix_tool.py"})

    _ = @pytest.mark.timeout(5)
    def test_get_tool_structure_prefers_exact_over_pattern(self) -> None:
        ...
        with patch.object(self.model, 'analyze_tool_file', side_effect=side_effect_analyzer) as mock_analyze:
            ...
            _ = mock_analyze.assert_called_once_with(os.path.join(self.temp_tools_dir, "pref_exact.py"))
            _ = self.assertEqual(result, {"path": os.path.join(self.temp_tools_dir, "pref_exact.py")})

    _ = @pytest.mark.timeout(5)
    def test_get_tool_structure_ambiguous_patterns(self) -> None:
        _ = self._create_dummy_tool_file("tool_ambiguous_pattern.py")
        _ = self._create_dummy_tool_file("ambiguous_pattern_tool.py")
        with self.assertLogs(logger='apps.backend.src.core_ai.code_understanding.lightweight_code_model', level='WARNING') as cm:
    ...
    _ = @pytest.mark.timeout(5)
    def test_get_tool_structure_name_not_found(self) -> None:
        with self.assertLogs(logger='apps.backend.src.core_ai.code_understanding.lightweight_code_model', level='WARNING') as cm:
    ...
    _ = @pytest.mark.timeout(5)
    def test_get_tool_structure_invalid_tools_directory(self) -> None:
