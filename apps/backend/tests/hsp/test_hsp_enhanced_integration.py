...
class TestHSPEnhancedIntegration:
    ...
    async def test_hsp_connector_message_queue_overflow(self, hsp_connector_fixture, mock_broker) -> None:
        ...
        def message_callback(payload, sender, envelope):
            _ = received_messages.append((payload, sender, envelope))
            
        _ = connector.register_on_fact_callback(message_callback)
        ...
        _ = assert len(received_messages) > 0
        _ = print(f"Test passed: HSP connector handled message queue overflow, received {len(received_messages)} messages")
    ...
    async def test_hsp_connector_network_disconnect_recovery(self, hsp_connector_fixture, mock_broker) -> None:
        ...
        def connect_callback(envelope=None):
            _ = connection_events.append(("connected", time.time()))
            
        def disconnect_callback(envelope=None):
            _ = connection_events.append(("disconnected", time.time()))
            
        _ = connector.register_on_connect_callback(connect_callback)
        _ = connector.register_on_disconnect_callback(disconnect_callback)
        ...
        _ = print("Test passed: HSP connector recovered from network disconnection")
    ...
    async def test_hsp_connector_large_message_handling(self, hsp_connector_fixture, mock_broker) -> None:
        ...
        payload = {
            ...
            _ = "metadata": {"test": "large_message", "size": len(large_content)}
        }
        ...
        _ = print("Test passed: HSP connector handled large message")
    ...
    async def test_hsp_connector_concurrent_subscriptions(self, hsp_connector_fixture, mock_broker) -> None:
        ...
        def opinion_callback(payload, sender, envelope):
            _ = received_opinions.append((payload, sender, envelope))
            
        _ = connector.register_on_fact_callback(fact_callback)
        _ = connector.register_on_opinion_callback(opinion_callback)
        ...
        _ = print("Test passed: HSP connector handled concurrent subscriptions")
    ...
    async def test_hsp_connector_message_ordering(self, hsp_connector_fixture, mock_broker) -> None:
        ...
        for i in range(10):
            ...
            _ = message_ids.append(f"ordered_fact_{i}")
            await mock_broker.publish(
                "hsp/knowledge/facts/test",
                _ = json.dumps(envelope).encode('utf-8')
            )
        ...
        _ = print("Test passed: HSP connector preserved message ordering")
    ...
    async def test_hsp_connector_error_handling(self, hsp_connector_fixture, mock_broker) -> None:
        ...
        _ = print("Test passed: HSP connector handled error gracefully")