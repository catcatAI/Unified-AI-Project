...
class MockMqttBroker:
    ...
    async def start(self):
        ...
        if self.on_connect_callback:
            if asyncio.iscoroutinefunction(self.on_connect_callback):
                _ = await self.on_connect_callback()
            else:
                _ = self.on_connect_callback()
    ...
    async def shutdown(self):
        ...
        if self.on_disconnect_callback:
            if asyncio.iscoroutinefunction(self.on_disconnect_callback):
                _ = await self.on_disconnect_callback()
            else:
                _ = self.on_disconnect_callback()
    ...
    async def publish(self, topic, payload, qos=1, retain=False, **kwargs):
        ...
        _ = self.published_messages.append(message)
        ...
        try:
            ...
            if envelope.get("message_type") == "HSP::TaskRequest_v0.1":
                ...
                _ = await self._send_automatic_ack(envelope)
            ...
            if self._topic_matches(sub_topic, topic):
                for callback in callbacks:
                    ...
                    if asyncio.iscoroutinefunction(callback):
                        _ = await callback(None, topic, payload, qos, None)
                    else:
                        _ = callback(None, topic, payload, qos, None)
        
        # Also call the on_message_callback if set (for HSP connector compatibility)
        if self.on_message_callback:
            if asyncio.iscoroutinefunction(self.on_message_callback):
                _ = await self.on_message_callback(topic, payload)  # 调用时只传递topic和payload
            else:
                _ = self.on_message_callback(topic, payload)  # 调用时只传递topic和payload
                        
        # Return a mock future to simulate the real MQTT client behavior
        future = asyncio.Future()
        _ = future.set_result(None)
        return future
    ...
    async def _send_automatic_ack(self, request_envelope):
        ...
        ack_envelope = {
            ...
            _ = "sender_ai_id": request_envelope.get("recipient_ai_id", "test_recipient"),
            _ = "recipient_ai_id": request_envelope.get("sender_ai_id", "test_sender"),
        ...
        # Publish the ACK
        _ = await self.publish(ack_topic, json.dumps(ack_envelope).encode('utf-8'))
    ...
    def subscribe(self, topic, qos=1, callback=None):
        ...
        # Also add the on_message_callback if it exists and is not already in the list
            ...
            _ = self.subscriptions[topic].append(self.on_message_callback)
            ...
            if self._topic_matches(sub_topic, topic):
                for callback in callbacks:
                    if asyncio.iscoroutinefunction(callback):
                        ...
                        _ = asyncio.create_task(callback(client, topic, payload, qos, properties))
                    else:
                        _ = callback(client, topic, payload, qos, properties)
    ...
    def subscribe_client(self, client_id, topic):
        ...
        if handler not in self.subscriptions[topic]:
            _ = self.subscriptions[topic].append(handler)
