...
from ...src.hsp.types import HSPFactPayload, HSPOpinionPayload
...
class TestHSPConnector:
    ...
    def test_init(self, hsp_connector) -> None:
        """测试初始化"""
    ...
    @pytest.mark.asyncio
    async def test_connect(self, hsp_connector) -> None:
        """测试连接"""
    ...
    @pytest.mark.asyncio
    async def test_disconnect(self, hsp_connector) -> None:
        """测试断开连接"""
    ...
    @pytest.mark.asyncio
    async def test_publish_fact(self, hsp_connector) -> None:
        """测试发布事实"""
        # 创建测试事实载荷
    ...
    @pytest.mark.asyncio
    async def test_publish_opinion(self, hsp_connector) -> None:
        """测试发布观点"""
        # 创建测试观点载荷
    ...
    @pytest.mark.asyncio
    async def test_subscribe_to_facts(self, hsp_connector) -> None:
        """测试订阅事实"""
        callback = AsyncMock()
    ...
    @pytest.mark.asyncio
    async def test_subscribe_to_opinions(self, hsp_connector) -> None:
        """测试订阅观点"""
        callback = AsyncMock()
    ...
    @pytest.mark.asyncio
    async def test_get_connector_status(self, hsp_connector) -> None:
        """测试获取连接器状态"""
        status = hsp_connector.get_connector_status()
        assert "hsp_available" in status
    
    @pytest.mark.asyncio
    async def test_handle_fact_message(self, hsp_connector) -> None:
        """测试处理事实消息"""
        # 创建测试事实消息
        fact_message = {
            "payload": {
        ...
        _ = await hsp_connector._handle_fact_message(fact_message)
        # 这个测试主要是确保方法可以被调用而不会出错
    
    @pytest.mark.asyncio
    async def test_handle_opinion_message(self, hsp_connector) -> None:
        """测试处理观点消息"""
        # 创建测试观点消息
        opinion_message = {
            "payload": {
        ...
        # 不需要mock内部总线，因为这是实现细节
        _ = await hsp_connector._handle_opinion_message(opinion_message)
        # 这个测试主要是确保方法可以被调用而不会出错