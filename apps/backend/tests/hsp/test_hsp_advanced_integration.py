...
class TestHSPAdvancedIntegration:
    ...
    async def test_hsp_connector_concurrent_task_processing(self, hsp_connector_fixture, mock_broker):
        ...
        def task_request_callback(payload, sender, envelope):
            task_requests.append((payload, sender, envelope))
            
        def task_result_callback(payload, sender, envelope):
            task_results.append((payload, sender, envelope))
            
        connector.register_on_task_request_callback(task_request_callback)
        connector.register_on_task_result_callback(task_result_callback)
    ...
    @pytest.mark.asyncio
    @pytest.mark.timeout(30)
    async def test_hsp_connector_fact_propagation_with_filtering(self, hsp_connector_fixture, mock_broker):
    ...
    @pytest.mark.asyncio
    @pytest.mark.timeout(30)
    async def test_hsp_connector_capability_advertisement_and_discovery(self, hsp_connector_fixture, mock_broker):
        ...
        def capability_callback(payload, sender, envelope):
            capabilities_advertised.append((payload, sender, envelope))
            
        connector.register_on_capability_advertisement_callback(capability_callback)
    ...
    @pytest.mark.asyncio
    @pytest.mark.timeout(30)
    async def test_hsp_connector_error_recovery_and_retry(self, hsp_connector_fixture, mock_broker):
        ...
        def success_callback(payload, sender, envelope):
            successful_ops.append((payload, sender, envelope))
            
        def error_callback(payload, sender, envelope):
            failed_ops.append((payload, sender, envelope))
            
        connector.register_on_fact_callback(success_callback)
        ...
        print("Test passed: HSP connector handled error recovery and retry")

    @pytest.mark.asyncio
    @pytest.mark.timeout(30)
    async def test_hsp_connector_memory_integration(self, hsp_connector_fixture, mock_broker):
        ...
        def memory_callback(payload, sender, envelope):
            memory_operations.append((payload, sender, envelope))
            
        connector.register_on_fact_callback(memory_callback)
    ...
    @pytest.mark.asyncio
    @pytest.mark.timeout(30)
    async def test_hsp_connector_security_and_authentication(self, hsp_connector_fixture, mock_broker):
        ...
        def auth_callback(payload, sender, envelope):
            authenticated_messages.append((payload, sender, envelope))
            
        connector.register_on_fact_callback(auth_callback)
        ...
        print("Test passed: HSP connector handled security and authentication")