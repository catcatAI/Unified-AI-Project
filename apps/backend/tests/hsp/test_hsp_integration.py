...
class TestHSPFactPublishing:
    ...
    @pytest.mark.asyncio
    async def test_learning_manager_publishes_fact_via_hsp(self, main_ai_hsp_connector: HSPConnector, broker: Mock):
        """测试学习管理器通过HSP发布事实"""
        # 准备测试数据
        test_fact = {
            "id": "fact_001",
...
class TestHSPFactConsumption:
    ...
    @pytest.mark.asyncio
    async def test_main_ai_consumes_nl_fact_and_updates_kg_check_trust_influence(self, main_ai_hsp_connector: HSPConnector):
...
class TestHSPIntegration:
    ...
    @pytest.mark.asyncio
    async def test_hsp_connector_concurrent_task_processing(self, main_ai_hsp_connector: HSPConnector):
        """测试HSP连接器并发任务处理"""
        # 准备测试数据
        tasks = []
        results = []
