...
class TestAIAgentIntegration(SystemIntegrationTest):
    ...
    @pytest.mark.system_integration
    @pytest.mark.asyncio
    async def test_agent_hsp_communication_integration(self):
        """测试代理与HSP通信集成"""
        # 创建测试数据
    ...
    @pytest.mark.system_integration
    @pytest.mark.asyncio
    async def test_agent_learning_integration(self):
        """测试代理与学习系统集成"""
        # 创建测试数据
        agent_config = self.data_factory.create_agent_config(
        ...
        learning_manager.return_value.start_learning.assert_called_once()
        llm_service.return_value.generate_response.assert_called_once_with(
            training_data["input"]
        )
        learning_manager.return_value.process_feedback.assert_called_once()
