...
class TestMultiServiceCoordinationIntegration(SystemIntegrationTest):
    ...
    @pytest.mark.system_integration
    @pytest.mark.asyncio
    async def test_multi_service_coordination_integration(self):
        """测试多服务协调集成"""
        # 创建测试数据
        complex_request = {
            "type": "complex_task",
...
class TestServiceFailureRecoveryIntegration(SystemIntegrationTest):
    ...
    @pytest.mark.system_integration
    @pytest.mark.asyncio
    async def test_service_failure_recovery_integration(self):
        """测试服务故障恢复集成"""
        # 获取mock服务
        agent_manager = self.get_mock_service("agent_manager")
        memory_manager = self.get_mock_service("memory_manager")
        ...
        assert len(recovered_agents) > 0
        assert memory_recovery_result is True
