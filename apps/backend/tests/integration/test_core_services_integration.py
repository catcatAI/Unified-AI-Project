...
class TestCoreServicesIntegration(SystemIntegrationTest):
    ...
    async def test_service_initialization_integration(self) -> None:
        ...
        for service_mock in services.values():
            _ = service_mock.return_value.initialize.assert_called_once()
    ...
    async def test_service_lifecycle_integration(self) -> None:
        ...
        # 验证mock调用
        for service_mock in services.values():
            _ = service_mock.return_value.start.assert_called_once()
            _ = service_mock.return_value.get_status.assert_called_once()
            _ = service_mock.return_value.stop.assert_called_once()
...
class TestMultiServiceCoordinationIntegration(SystemIntegrationTest):
    ...
    async def test_multi_service_coordination_integration(self) -> None:
        ...
        for agent_id in request_result["assigned_agents"]:
            ...
            _ = task_assignments.append(assignment_result)
        
        # 6. 生成最终响应
        final_response = await llm_service.return_value.generate_response(
            _ = f"Generate comprehensive response based on {len(created_agents)} agents' work"
        )
        ...
        _ = llm_service.return_value.generate_response.assert_called_once()
        _ = memory_manager.return_value.store_memory.assert_called_once_with(result_memory)
...
class TestServiceFailureRecoveryIntegration(SystemIntegrationTest):
    ...
    async def test_service_failure_recovery_integration(self) -> None:
        ...
        # 模拟服务故障
        service_failures = {
            _ = "agent_manager": Exception("Agent manager service failed"),
            _ = "memory_manager": Exception("Memory manager service failed"),
            _ = "llm_service": Exception("LLM service failed")
        }
        ...
        _ = agent_manager.return_value.recover_failed_agents.assert_called_once()
        memory_manager.return_value.handle_service_failure.assert_called_once_with(
            service_failures["memory_manager"]
        )
        _ = memory_manager.return_value.restore_from_backup.assert_called_once()
        llm_service.return_value.handle_service_failure.assert_called_once_with(
            service_failures["llm_service"]
        )
        _ = llm_service.return_value.switch_to_backup_model.assert_called_once()
