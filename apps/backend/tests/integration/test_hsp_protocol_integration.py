...
class TestHSPProtocolIntegration(SystemIntegrationTest):
    ...
    async def test_hsp_connection_integration(self) -> None:
        ...
        _ = hsp_connector.return_value.disconnect.assert_called_once()
...
class TestHSPMessageRoutingIntegration(SystemIntegrationTest):
    ...
    async def test_hsp_message_routing_integration(self) -> None:
        ...
        for topic in topics:
            ...
            _ = subscribe_results.append(result)
        ...
        for i, message in enumerate(routing_messages):
            ...
            _ = publish_results.append(result)
        ...
        # 验证mock调用
        for topic in topics:
            _ = hsp_connector.return_value.subscribe.assert_any_call(topic)
        
        for i, message in enumerate(routing_messages):
            topic = topics[i % len(topics)]
            _ = hsp_connector.return_value.publish.assert_any_call(message, topic)
...
class TestHSPQualityOfServiceIntegration(SystemIntegrationTest):
    ...
    async def test_hsp_qos_integration(self) -> None:
        ...
        # 验证mock调用
        _ = hsp_connector.return_value.set_qos_level.assert_called_once_with(2)
