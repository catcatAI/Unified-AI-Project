...
class TestSystemLevelIntegration(SystemIntegrationTest):
    ...
    def setup_test_data(self, setup_test, mock_external_services):
        """设置测试数据"""
    ...
    @pytest.mark.slow
    @pytest.mark.asyncio
    async def test_end_to_end_user_interaction_integration(self):
        """测试端到端用户交互集成"""
        # 创建测试数据
        ...
        for agent_config in test_data["agents"]:
            ...
            # 启动代理
            await agent_manager.return_value.start_agent(agent_config["agent_id"])
    ...
    @pytest.mark.slow
    @pytest.mark.asyncio
    async def test_multi_agent_collaborative_task_integration(self):
        """测试多代理协作任务集成"""
        # 创建测试数据
        task_request = {
