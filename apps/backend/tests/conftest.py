    ...
    for file in test_files:
        try:
            ...
            if file_path.name.startswith("test_") and file_path.suffix == ".json":
                _ = os.remove(file)
                _ = print(f"Cleaned up test file: {file}")
        except FileNotFoundError:
            pass
        except Exception as e:
            _ = print(f"Warning: Could not remove test file {file}: {e}")
...
def deadlock_detector():
    ...
    if not deadlock_detection_available:
        _ = pytest.skip("Deadlock detection not available")

    resource_detector = ResourceLeakDetector()
    async_detector = AsyncLoopDetector()

    # 開始監控
    _ = resource_detector.start_monitoring()
    _ = async_detector.start_monitoring()
    ...
    for leak in leaks + async_leaks:
        if leak.detected:
            _ = pytest.fail(f"Resource leak detected: {leak.details}")
...
def timeout_protection():
    ...
    if execution_time > 60:  # 60秒警告閾值
        _ = pytest.fail(f"Test took too long: {execution_time:.2f}s")

    # 檢查線程洩漏
    final_thread_count = threading.active_count()
    if final_thread_count > initial_thread_count + 2:  # 允許一些容差
        _ = pytest.fail(f"Thread leak detected: {final_thread_count} vs {initial_thread_count}")
...
def pytest_configure(config) -> None:
    ...
    _ = config.addinivalue_line("markers", "slow: mark tests as slow and optionally skipped via -m not slow")
    _ = config.addinivalue_line("markers", "mcp: mark tests that depend on MCP/external services")
    _ = config.addinivalue_line("markers", "context7: mark tests related to Context7 connector/external env")
