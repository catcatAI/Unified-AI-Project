...
def test_hsp_create_and_pending_then_complete(client: TestClient, hsp_fakes):
    # Create task
...
def test_hsp_create_capability_not_found(client: TestClient, hsp_fakes):
...
def test_hsp_status_unknown(client: TestClient):
