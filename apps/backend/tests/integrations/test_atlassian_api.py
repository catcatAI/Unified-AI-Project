...
from apps.backend.src.services.atlassian_api import AtlassianConfig, atlassian_router
...
class TestAtlassianAPI:
    ...
    def test_atlassian_cli_bridge_initialization(self, mock_run):
    ...
    @patch('apps.backend.src.integrations.atlassian_cli_bridge.subprocess.run')
    def test_get_jira_projects(self, mock_run):
        """测试获取 Jira 项目列表"""
        # 模拟命令行执行结果
        mock_result = MagicMock()
        mock_result.returncode = 0
    ...
    def test_enhanced_atlassian_bridge_initialization(self):
