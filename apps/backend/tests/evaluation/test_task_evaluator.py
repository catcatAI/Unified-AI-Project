...
class TestMetricsCalculator(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_calculate_objective_metrics_success(self) -> None:
        ...
        _ = self.assertEqual(metrics["completion_time"], 10.0)
        _ = self.assertEqual(metrics["success_rate"], 1.0)
        # cpu = 0.1 + (10.0 * 0.05) = 0.6
        # mem = 50 + (10.0 * 2) = 70
        _ = self.assertEqual(metrics["resource_usage"], {"cpu_usage": 0.6, "memory_mb": 70.0})
    ...
    async def test_calculate_objective_metrics_failure(self) -> None:
        ...
        metrics = await self.calculator.calculate_objective_metrics(task, execution_result)
        _ = self.assertEqual(metrics["completion_time"], 5.0)
        _ = self.assertEqual(metrics["success_rate"], 0.0)
        # cpu = 0.1 + (5.0 * 0.05) + (1 * 0.02) = 0.1 + 0.25 + 0.02 = 0.37
        # mem = 50 + (5.0 * 2) + (1 * 10) = 50 + 10 + 10 = 70
        _ = self.assertEqual(metrics["resource_usage"], {"cpu_usage": 0.37, "memory_mb": 70.0})
...
class TestFeedbackAnalyzer(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_analyze_positive_feedback(self) -> None:
        ...
        _ = self.assertEqual(analysis["sentiment"], "positive")
        _ = self.assertEqual(analysis["sentiment_score"], 1)
        _ = self.assertIn("performance", analysis["categories"])
        _ = self.assertIn("feature_request", analysis["categories"])
    ...
    async def test_analyze_negative_feedback(self) -> None:
        ...
        analysis = await analyzer.analyze(user_feedback)
        _ = self.assertEqual(analysis["sentiment"], "negative")
        _ = self.assertEqual(analysis["sentiment_score"], -1)
        _ = self.assertIn("accuracy", analysis["categories"])
        _ = self.assertIn("bug", analysis["categories"])
    ...
    async def test_analyze_neutral_feedback(self) -> None:
        ...
        analysis = await analyzer.analyze(user_feedback)
        _ = self.assertEqual(analysis["sentiment"], "neutral")
        _ = self.assertEqual(analysis["sentiment_score"], 0)
        _ = self.assertEqual(analysis["categories"], [])
    ...
    async def test_analyze_mixed_feedback(self) -> None:
        ...
        analysis = await analyzer.analyze(user_feedback)
        _ = self.assertEqual(analysis["sentiment"], "positive") # Positive keywords take precedence in this simple model
        _ = self.assertEqual(analysis["sentiment_score"], 1)
        _ = self.assertIn("performance", analysis["categories"])
        _ = self.assertIn("bug", analysis["categories"])
    ...
    async def test_analyze_usability_feedback(self) -> None:
        ...
        _ = self.assertEqual(analysis["sentiment"], "neutral")
        _ = self.assertEqual(analysis["sentiment_score"], 0)
        _ = self.assertIn("usability", analysis["categories"])
        _ = self.assertEqual(len(analysis["categories"]), 1)
...
class TestTaskExecutionEvaluator(unittest.IsolatedAsyncioTestCase):
    ...
    def tearDown(self):
        ...
        if os.path.exists(self.evaluator.db.db_path):
            _ = os.remove(self.evaluator.db.db_path)
        # Clean up test storage directory
        if os.path.exists(self.evaluator.storage_path):
            _ = shutil.rmtree(self.evaluator.storage_path)
    ...
    async def test_evaluate_task_execution_failure(self) -> None:
        ...
        _ = self.assertEqual(evaluation["task_id"], "task_failure")
        _ = self.assertEqual(evaluation["metrics"]["success_rate"], 0.0)
        _ = self.assertEqual(evaluation["metrics"]["quality_score"], 0.5)
        _ = self.assertEqual(evaluation["feedback"]["sentiment"], "negative")
        _ = self.assertGreater(len(evaluation["improvement_suggestions"]), 0) # Should have error/performance/quality suggestions
        _ = self.evaluator._store_evaluation.assert_called_once()
    ...
    async def test_generate_improvements_error(self) -> None:
        ...
        _ = self.assertGreater(len(suggestions), 0)
        _ = self.assertEqual(suggestions[0]["type"], "error_analysis")
    ...
    async def test_generate_improvements_performance(self) -> None:
        ...
        suggestions = await self.evaluator._generate_improvements(task, result, metrics)
        _ = self.assertGreater(len(suggestions), 0)
        _ = self.assertEqual(suggestions[0]["type"], "performance")
    ...
    async def test_generate_improvements_quality(self) -> None:
        ...
        suggestions = await self.evaluator._generate_improvements(task, result, metrics)
        _ = self.assertGreater(len(suggestions), 0)
        _ = self.assertEqual(suggestions[0]["type"], "quality")
    ...
    async def test_generate_improvements_general(self) -> None:
        ...
        suggestions = await self.evaluator._generate_improvements(task, result, metrics)
        _ = self.assertEqual(len(suggestions), 1)
        _ = self.assertEqual(suggestions[0]["type"], "general")
    ...
    async def test_evaluate_task_execution_no_expected_output(self) -> None:
        ...
        _ = self.assertEqual(evaluation["task_id"], "task_no_expected_output")
        _ = self.assertEqual(evaluation["metrics"]["success_rate"], 1.0)
        _ = self.assertEqual(evaluation["metrics"]["quality_score"], 0.95) # Should use fallback score
        _ = self.evaluator._store_evaluation.assert_called_once()
