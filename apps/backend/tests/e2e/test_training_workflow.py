...
from apps.backend.src.core_ai.demo_learning_manager import DemoLearningManager
...
class TestTrainingWorkflowE2E:
    ...
    @pytest.mark.asyncio
    async def test_complete_training_workflow(self, demo_learning_manager, execution_manager):
        """测试完整的训练工作流程"""
        # Mock依赖组件
        ...
        execution_manager._execute_training_task.assert_called_once_with(training_task)
    ...
    async def test_memory_integration_in_training_workflow(self, demo_learning_manager):
        ...
        with patch('apps.backend.src.ai.memory.ham_memory_manager.HAMMemoryManager') as mock_memory_manager:
            ...
            mock_memory_instance.store_memory.assert_called_once_with(training_memory)
    ...
    @pytest.mark.asyncio
    async def test_agent_collaboration_in_training_workflow(self):
        """测试训练工作流程中与代理系统的协作"""
        with patch('apps.backend.src.core_ai.agent_manager.AgentManager') as mock_agent_manager:
            ...
            mock_agent.process_training_request.assert_called_once_with(training_request)
            
            # 2. 验证代理协调资源分配
            assert "gpu_1" in agent_response["assigned_resources"]
    
    @pytest.mark.asyncio
    async def test_hsp_communication_in_training_workflow(self):
        """测试训练工作流程中与HSP协议的通信"""
        with patch('apps.backend.src.hsp.connector.HSPConnector') as mock_hsp_connector:
            ...
            mock_hsp_instance.publish_fact.assert_called_once_with(training_progress_fact, "hsp/knowledge/facts/training")
...
class TestMultiSystemIntegrationE2E:
    ...
    @pytest.mark.asyncio
    async def test_complete_ai_system_workflow(self):
        """测试完整的AI系统工作流程"""
        ...
        with patch('apps.backend.src.core_ai.agent_manager.AgentManager') as mock_agent_manager:
            ...
            for i in range(4):
                ...
                mock_specialized_agents.append(mock_agent)
            ...
            with patch('apps.backend.src.hsp.connector.HSPConnector') as mock_hsp_connector:
                ...
                mock_hsp.publish_fact.assert_called_once_with(result_fact, "hsp/knowledge/facts/analysis")