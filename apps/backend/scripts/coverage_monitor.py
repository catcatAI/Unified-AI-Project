...
class CoverageMonitor:
    ...
    def __init__(self, project_root: str = None, db_path: str = None) -> None:
        ...
        self.db_path = Path(db_path) if db_path else self.project_root / "coverage_history.db"
        self._init_database()
    ...
    def record_coverage_data(self, coverage_data: Dict[str, Any], 
                           commit_hash: str = None, build_number: int = None, 
    ...
    def get_coverage_trend(self, days: int = 30, environment: str = None) -> List[Dict[str, Any]]:
        """
        获取覆盖率趋势数据
    ...
    def get_module_coverage_comparison(self, module_name: str, days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            branch_rates = [point["branch_rate"] for point in comparison_data["data_points"]]
    ...
    def generate_trend_report(self, days: int = 30, output_file: str = None) -> str:
        """
        生成趋势报告
        ...
        if output_file is None:
            output_file = self.project_root / f"coverage_trend_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        else:
        ...
        if trend_data:
            ...
            branch_rates = [point["branch_rate"] for point in trend_data]
            
            report_data["summary"]["line_rate"] = {
                "current": line_rates[-1] if line_rates else 0,
                "average": sum(line_rates) / len(line_rates) if line_rates else 0,
                "min": min(line_rates) if line_rates else 0,
                "max": max(line_rates) if line_rates else 0,
