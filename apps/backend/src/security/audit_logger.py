...
class AuditLogger:
    ...
    def log_event(self, event: AuditEvent) -> None:
        ...
        try:
            ...
            Exception
        ) as e:  # broad exception acceptable: ensure logging errors don't crash the caller
    ...
    def log_permission_check(
        ...
        event = AuditEvent(
            ...
            action=action,
            success=granted,
            details={"permission_type": permission_type, "granted": granted, **(details or {})},
            session_id=session_id,
            ip_address=ip_address,
            user_agent=user_agent,
        )
    ...
    def _flush_buffer(self) -> None:
        ...
        try:
            ...
            Exception
        ) as e:  # broad exception acceptable: ensure flush errors don't crash the buffer management
        ...
        try:
            if os.path.exists(self.log_file_path):
                ...
                if file_size > self.max_log_size:
                    rotated_path = (
                        f"{self.log_file_path}.{datetime.now().strftime('%Y%m%d_%H%M%S')}"
                    )
                    os.rename(self.log_file_path, rotated_path)
                    logger.info(f"Rotated audit log file to {rotated_path}")
        except (
            Exception
        ) as e:  # broad exception acceptable: ensure rotation errors don't crash the flush operation
                ...
                with open(self.log_file_path, "r", encoding="utf-8") as f:
                    ...
                    for line in reversed(lines[-(limit - len(events)) :]):
                        try:
                            ...
                            Exception
                        ) as e:  # broad exception acceptable: ensure corrupted log lines don't crash retrieval
                            logger.error(f"Error parsing audit log line: {e}", exc_info=True)
            return events[-limit:]
        except (
            Exception
        ) as e:  # broad exception acceptable: ensure get errors return empty list, not crash
                ...
                with open(self.log_file_path, "r", encoding="utf-8") as f:
                    for line in f:
                        try:
                            ...
                            Exception
                        ) as e:  # broad exception acceptable: ensure corrupted log lines don't crash search
                            logger.error(f"Error parsing audit log line: {e}", exc_info=True)
            return events
        except (
            Exception
        ) as e:  # broad exception acceptable: ensure search errors return empty list, not crash
