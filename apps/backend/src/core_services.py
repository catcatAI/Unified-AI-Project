...
lm_config: Optional[Dict[str, Any]] = None # Added llm_config
):
    """
...
hroma_client = chromadb.HttpClient(
                    host="localhost",
                    port=8001
                )
...
sp_enabled = config.get("hsp_service", {}).get("enabled", True)
        if hsp_enabled:
...
rint(f"Core Services: WARNING - HSPConnector for {ai_id} failed to connect to {hsp_broker_address}:{hsp_broker_port}")
...
f hsp_connector_instance and service_discovery_module_instance:
    ...
    if not fact_extractor_instance:
        # 确保llm_interface_instance不为None再创建FactExtractorModule
        if llm_interface_instance is not None:
            fact_extractor_instance = FactExtractorModule(llm_service=llm_interface_instance)
        else:
            # 如果llm_interface_instance为None，记录错误但不创建FactExtractorModule
            print("Warning: llm_interface_instance is None, cannot create FactExtractorModule")
    ...
    if not learning_manager_instance and ham_manager_instance:
        # 只有当fact_extractor_instance不为None时才创建LearningManager
...
asattr(learning_manager_instance, 'process_and_store_hsp_fact') and
...
allable(getattr(current_learning_manager, 'process_and_store_hsp_fact', None)):
                            # 使用统一的方法创建异步任务
                            asyncio.create_task(
            ...
            print("Warning: fact_extractor_instance is None, cannot create LearningManager")
...
earning_manager=learning_manager_instance,  # type ignore
            content_analyzer=content_analyzer_instance,
...
rint("Core Services: All services initialized (or attempted).")
