...
class RetryPolicy, :
    ...
    1} / {self.max_attempts} Network error during {func.__name__}. Retrying in {"delay":.2f}s... Error, {e}")
...
class CircuitBreaker, :
    ...
    def _fail(self):
        ...
        if self.failures >= self.failure_threshold, ::
            self.state = "OPEN"
            self.logger.warning(f"Circuit Breaker,
    Failure threshold reached ({self.failures} failures). State changed to OPEN.")
