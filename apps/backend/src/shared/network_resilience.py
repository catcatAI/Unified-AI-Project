...
class RetryPolicy:
    ...
    def __init__(
        self, max_attempts: int = 3, backoff_factor: float = 2.0, max_delay: float = 30.0
    ) -> None:
        ...
        async def wrapper(*args, **kwargs):
            if self.state == "OPEN":
                if (time.time() - self.last_failure_time) > self.recovery_timeout:
                    ...
                    self.logger.info(
                        f"Circuit Breaker for {func.__name__} transitioned to HALF_OPEN. Probing..."
                    )
                else:
                    raise CircuitBreakerOpenError(
                        f"Circuit breaker is OPEN. {func.__name__} is unavailable."
                    )
