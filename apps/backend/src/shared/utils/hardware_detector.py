...
class SystemHardwareProbe:
    ...
    def detect(self) -> HardwareProfile:
        ...
        logger.info(
            f"Hardware detected: {profile.accelerator_type.value} | Tier: {profile.performance_tier}"
        )
        return profile
    ...
    def get_ollama_recommendations(self, profile: HardwareProfile) -> Dict[str, Any]:
        ...
        return {
            "gpu_layers": (
                -1
                if profile.accelerator_type in [AcceleratorType.NVIDIA, AcceleratorType.AMD]
                else 0
            ),
...
class ModeRecommender:
    ...
    def recommend_mode(
        self, preferred_mode: Optional[str] = None
        ...
        if preferred_mode and preferred_mode in self.config.get("angela_modes", {}):
            ...
            meets_req, reason = self._meets_requirements(
                profile, mode_config.get("hardware_requirements", {})
            )
            if meets_req:
                return (
                    preferred_mode,
                    f"User selected {preferred_mode}, hardware compatible",
                    mode_config,
                )
            else:
        ...
        for mode_name in modes_to_check:
            ...
            if meets_req:
                return (
                    mode_name,
                    f"Auto-selected {mode_name} based on hardware: {profile.ram_total_gb:.1f}GB RAM, {profile.accelerator_name}",
                    mode_config,
                )
    ...
    def _meets_requirements(
        ...
        if profile.ram_total_gb < requirements.get("min_ram_gb", 0):
            return (
                False,
                f"RAM: {profile.ram_total_gb:.1f}GB < {requirements['min_ram_gb']}GB required",
            )

        if (
            requirements.get("gpu_required", False)
            and profile.accelerator_type == AcceleratorType.NONE
        ):
