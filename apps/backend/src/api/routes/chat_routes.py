...
async def chat_with_audio(
    ...
    """Chat with optional audio context (P41).

    Analyzes uploaded audio using AudioCaptionService for semantic understanding
    and injects the description into chat context.
    """
    if not session_id:
    ...
    if file and file.content_type and file.content_type.startswith("audio/"):
        try:
            ...
            # Pass raw audio to ChatService — P41 integration in
            # ChatService.generate_response() will generate the
            # semantic caption via AudioCaptionService.
            audio_context = {
                "filename": file.filename,
                "audio_data": audio_data,
            }
        except Exception as e:
            logger.warning(f"Audio read failed, continuing with text only: {e}")

    history = []
    context = {"user_name": user_name}
    if audio_context:
        context["audio_analysis"] = audio_context

    return await _handle_chat_request(
        user_message=message or "我上傳了一段音頻",
        user_name=user_name,
        history=history,
        session_id=session_id,
        origin="Human",
        extra_context=context if audio_context else None,
    )
