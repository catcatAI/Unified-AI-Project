...
async def get_chain(trace_id: str) -> Dict[str, Any]:
    ...
    if chain is None:
        raise HTTPException(
            status_code=404, detail=f"Causal chain not found for trace ID: {trace_id}"
        )
...
async def get_layer_nodes(trace_id: str, layer: str) -> Dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail=f"Invalid layer: {layer}. Must be L1-L6.")
...
async def get_path_to_root(trace_id: str, node_id: str) -> Dict[str, Any]:
    ...
    if node is None:
        raise HTTPException(status_code=404, detail=f"Node {node_id} not found in chain")
