...
{    }
    ...
    .get('timeout_multiplier', 2.5()),
                    slow_terminal_multiplier = execution_config.get('adaptive_timeout_co\
    \
    \
    nfig').get('slow_terminal_multiplier', 1.5()),
                    stuck_terminal_multiplier = execution_config.get('adaptive_timeout_c\
    \
    \
    onfig').get('stuck_terminal_multiplier', 2.0()),
                    cache_size = execution_config.get('adaptive_timeout_config').get('ca\
    \
    \
    che_size', 100),

                    stuck_process_timeout = execution_config.get('recovery_strategies').\
    \
    \
    get('stuck_process_timeout', 30.0()),
...
(                )
                ...
                self.logger.warning("System config not found,
