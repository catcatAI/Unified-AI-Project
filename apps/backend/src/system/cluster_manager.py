...
class DecimalMemoizer:
    ...
    def __init__(self):
        ...
        self._memo: Dict[
            Tuple[str, Tuple[int, ...], Tuple[int, ...], PrecisionLevel], Tuple[float, ...]
        ] = {}
        self.memo_hits = 0
        self.memo_misses = 0
    ...
    def get_decimals(
        self, module: str, shape: Tuple[int, ...], int_vals: List[int], precision: PrecisionLevel
    ) -> Optional[List[float]]:
...
class ClusterManager:
    ...
    async def distribute_task(
        ...
        custom_precision: Optional[PrecisionLevel] = None,
    ) -> str:
    ...
    async def execute_task(self, task: MatrixTask) -> List[float]:
        ...
        # 記錄狀態張量上下文
        tensor_info = (
            task.status_tensor.to_vector()
            if task.status_tensor
            else ("unknown", 0, "unknown", "unknown")
        )
        ...
        if task.precision == PrecisionLevel.FP128 and task.fp_chunks > 1:
            ...
            logger.info(
                f"執行 FP128 切分塊運算: {task.module} (Chunk {chunk_idx + 1}/{task.fp_chunks})"
            )
    ...
    def push_decimal_to_worker(
        self, worker_id: str, layer: int, dim: int, int_val: int, decimal_val: float
    ):
    ...
    def _record_local_metrics(
        self,
        exec_time: float,
        precision: float,
        status_tensor: Optional[UnifiedStatusTensor] = None,
    ):
        """記錄本地執行指標"""
        ...
        try:
            profile = self.probe.get_hardware_profile()
            if profile.gpu:
                hardware_usage["gpu_load"] = (
                    profile.gpu[0].load if hasattr(profile.gpu[0], "load") else 0
                )
