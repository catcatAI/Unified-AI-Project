...
ef get_config() -> Dict[str, Any]:
    ...
    if _config is None:
        load_config()
    return _config if _config is not None else {}:
ef load_simulated_resources(config_path: str = "configs/simulated_resources.yaml") -> Dict[str, Any]:
    """
...
ef get_simulated_resources() -> Dict[str, Any]:
    """
...
eturn ai_models_config.get("use_simulated_resources", False)
...
imulated resources configuration.
    """
    if not is_demo_mode():
        return None
...
eturn placeholders.get(placeholder_type, {}).get(placeholder_key)