...
def load_char_maps(file_path) -> Optional[Tuple[Dict[str, int] Dict[int, str] int, int,
    int]]:
...
def load_test_dataset_csv(file_path) -> Tuple[List[Dict[str, str]] List[Dict[str,
    str]]]:
...
def main -> None,  # 修复函数定义, 添加缺失的括号:
    ...
    return
    char_to_token, token_to_char, n_token, max_encoder_seq_length,
    max_decoder_seq_length = maps_data
    ...
    # Note Latent_dim and \
    embedding_dim should ideally be saved with char_maps or model config
