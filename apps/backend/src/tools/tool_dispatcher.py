...
ool_function = self.tools[tool_name]
    ...
    async def dispatch(self, user_input: str, session_id: Optional[str] = None, user_id: Optional[str] = None, history: Optional[List] = None, explicit_tool_name: Optional[str] = None) -> Dict[str, Any]:
        """""""""""""""""""""""""""Main dispatch method that processes user input and routes to appropriate tools."""""""""""""""""""""""""""
        try:
            logging.info(f"""""""""ToolDispatcher: Processing input for user {user_id}""""""""")
            
            if explicit_tool_name and explicit_tool_name in self.tools:
                parameters = {"""""""""query""""""""": user_input, """""""""user_id""""""""": user_id, """""""""session_id""""""""": session_id}
                return await self.dispatch_tool_request(explicit_tool_name, parameters)
            
            # For now, use keyword matching - will implement LLM selection later
            user_input_lower = user_input.lower()
            
            if any(word in user_input_lower for word in ['calculate', 'compute', 'math', '+', '-', '*', '/']):
                if 'calculate' in self.tools:
                    return await self.dispatch_tool_request("""""""""calculate""""""""", {"""""""""query""""""""": user_input})
            
            return {"""""""""status""""""""": """""""""no_tool_found""""""""", """""""""message""""""""": """""""""No suitable tool found"""""""""}
            
        except Exception as e:
            logging.error(f"""""""""ToolDispatcher: Error in dispatch: {e}""""""""")
            return {"""""""""status""""""""": """""""""error""""""""", """""""""error_message""""""""": str(e)}

    def _construct_tool_selection_prompt(self, text: str, available_tools: Dict[str, str]) -> str:
        """""""""""""""""""""""""""Construct a prompt for tool selection using LLM"""""""""""""""""""""""""""
        ...
        prompt += f"""""""""\nUser query: \"""""""""{text}\"""""""""\n\n"""""""""
        prompt += """""""""Respond in the format: TOOL_NAME|PARAMETERS (or NO_TOOL if no tool is appropriate)"""""""""
        return prompt
...
equires 'csv_content' and 'query' to be in kwargs.
        """""""""""""""""""""""""""
...
equires 'prompt' and optional 'style' in kwargs.
        """""""""""""""""""""""""""
...
he query should be the logical expression string itself.
        """""""""""""""""""""""""""
        try:
...
xtracts text and target language from query.
...
esolved_target_lang = """""""""zh"""""""""
                    elif lang_name_or_code in ["""""""""english""""""""", """""""""en"""""""""]: :
esolved_target_lang = """""""""en"""""""""
                    else: 
...
f to_lang_match_general and hasattr(text_to_translate, 'lower') and text_to_translate.lower().endswith(f""""""""" to {to_lang_match_general.group(1).lower()}"""""""""):
...
esult_payload = translate_text(text_to_translate, resolved_target_lang, source_language=source_lang_from_kwarg or """""""""auto""""""""")
...
vailable_tools_dict = self.get_available_tools()
