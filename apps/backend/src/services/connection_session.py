...
class SessionManager:
    ...
    def __init__(
        self, heartbeat_interval: int = 30, heartbeat_timeout: int = 120, max_buffer_size: int = 10
    ):
        ...
        logger.info(
            "[SessionManager] Initialized with heartbeat_interval=%ds, timeout=%ds",
            heartbeat_interval,
            heartbeat_timeout,
        )
        ...
        async with self._lock:
            ...
            if single_device_mode:
                ...
                if existing_sessions:
                    logger.info(
                        f"[SessionManager] Session {session_id} exists (single-device mode), replacing old connection"
                    )
                    for old_session in existing_sessions:
                        try:
                            ...
                            logger.warning(
                                f"Failed to close old websocket {old_session.client_id}: {e}",
                                exc_info=True,
                            )
            ...
            logger.info(
                f"[SessionManager] Registered session: client_id={client_id}, session_id={session_id}"
            )
    ...
    async def _unregister_internal(self, client_id: str, reason: str = "Normal close") -> None:
        ...
        if session.session_id in self._sessions_by_id:
            self._sessions_by_id[session.session_id] = [
                s for s in self._sessions_by_id[session.session_id] if s.client_id != client_id
            ]
            ...
            try:
                ...
                if time_since_heartbeat > self.heartbeat_timeout:
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Failed to close timed-out websocket {session.client_id}: {e}",
                            exc_info=True,
                        )
        ...
        for session in sessions:
            if session.state == SessionState.OPEN:
                try:
                    ...
                    logger.warning(
                        f"[SessionManager] Send failed for {session.client_id}: {e}", exc_info=True
                    )
    ...
    async def broadcast(
        self, message: dict, exclude_session_ids: Optional[List[str]] = None
    ) -> int:
        """
        ...
        for session in list(self._sessions.values()):
            ...
            if session.state == SessionState.OPEN:
                try:
                    ...
                    logger.warning(
                        f"[SessionManager] Broadcast to {session.client_id} failed: {err_msg}"
                    )
                    self._stats.failed_messages += 1
