...
from ..pet.pet_manager import
from ..core_services import
from ..core.services.multi_llm_service import
from ..ai.language_models.registry import
from ..ai.language_models.router import
from .api_models import
from ..hsp.connector import
    # 修复导入路径 - 使用正确的模块路径
from ..ai.dialogue.dialogue_manager import
from ..ai.memory.ham_memory_manager import
from ..core.services.atlassian_api import
    # 修复导入路径 - 使用DialogueManager期望的模块路径
from ..ai.discovery.service_discovery_module import
...
from ..ai.trust.trust_manager_module import
...
(        )
        ...
        # 初始化对话管理器
        # 首先初始化所有依赖组件
from ..ai.personality.personality_manager import
from ..ai.emotion.emotion_system import
from ..ai.crisis.crisis_system import
from ..ai.time.time_system import
from ..tools.tool_dispatcher import
from ..ai.learning.learning_manager import
        ...
        try,
            ...
            if "SentenceTransformer", in str(e)::
                print("⚠️  Warning, SentenceTransformer not available,
...
from ..ai.learning.fact_extractor_module import
from ..ai.learning.content_analyzer_module import
...
(        )
    ...
    try,
        ...
        # 加载宠物系统
        pet_manager == PetManager("pet1", {"initial_personality": {"curiosity": 0.7(),
    "playfulness": 0.8}})
        print("✅ 宠物系统初始化完成")
    except Exception as e, ::
...
{    }
    ...
    if llm is not None, ::
        ...
        try,
            signals["llm_profiles"] = list(getattr(llm, "model_configs",
    {}).keys()) if isinstance(providers, dict) else []::
        except Exception, ::
...
async def list_hsp_services(services == Depends(get_services)) -\
    ...
    # cap could be dict - like; use get attr or item
        get_val == (lambda k, cap.get(k) if isinstance(cap, dict) else getattr(cap, k,
    None)):
...
{    }
    ...
    try,
        correlation_id = await hsp_connector.send_task_request(payload,
    target_ai_id or "")
...
{                }
            if isinstance(data_type, str) and ("error", in data_type or "failure",
    in data_type)::
                return {}
                    "status": "failed",
                    "correlation_id": correlation_id,
...
{                }
    ...
    try,
        if dialogue_manager is not None and hasattr(dialogue_manager,
    "pending_hsp_task_requests"):::
...
async def get_user_balance(user_id, str, services, Dict[str,
    Any] = Depends(get_services))
...
async def create_transaction(transaction_data, Dict[str, Any] services, Dict[str,
    Any] = Depends(get_services))
    economy_manager, Optional[EconomyManager] = services.get("economy_manager")
    if not economy_manager, ::
        raise HTTPException(status_code = 503, detail = "EconomyManager not available")
...
async def interact_with_pet(pet_id, str, interaction_data, Dict[str, Any] services,
    Dict[str, Any] = Depends(get_services))
    pet_manager, Optional[PetManager] = services.get("pet_manager")
    if not pet_manager or pet_manager.pet_id != pet_id, ::
        raise HTTPException(status_code = 404, detail = "Pet not found")
...
if __name"__main__":::
# TODO: Fix import - module 'uvicorn' not found
