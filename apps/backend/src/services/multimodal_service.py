...
class MultimodalService:
    ...
    def __init__(self):
        ...
        # P39: Vision caption (LLM semantic understanding)
        self._caption_service = None
        # P40: Audio caption (LLM semantic audio understanding)
    ...
    def _get_caption_service(self):
        """Get or create the VisionCaptionService instance (P39)."""
        if self._caption_service is None:
            from services.vision_caption_service import VisionCaptionService
            self._caption_service = VisionCaptionService()
        return self._caption_service

    async def caption(self, image_data: bytes, prompt: Optional[str] = None,
                      language: str = "zh", preferred_backend: Optional[str] = None) -> Dict[str, Any]:
        """Generate a semantic caption for an image using LLM Vision API.
        ...
        Returns:
            dict with caption text, backend, language, time_ms
        """
        svc = self._get_caption_service()
        if not svc.is_available:
            await svc.initialize()
        if not svc.is_available:
            return {"caption": "", "backend": "none", "language": language,
                    "error": "No LLM Vision API keys configured (set GEMINI_API_KEY or OPENAI_API_KEY)"}
        return await svc.caption(image_data, prompt, language, preferred_backend)

    async def caption_status(self) -> Dict[str, Any]:
        """Check VisionCaptionService availability and backends."""
        svc = self._get_caption_service()
        if not svc.is_available:
            await svc.initialize()
        return {
    ...
    async def caption_pipeline(self, image_data: bytes, prompt: Optional[str] = None,
                                language: str = "zh") -> Dict[str, Any]:
        """Run full vision pipeline with semantic caption.

        Combines VisionPipeline.process() (pixel-level features)
        with VisionCaptionService.caption() (semantic description).

        Returns:
            dict with feature_vector, latent, ssim, psnr, caption, backend
        """
        # Step 1: Run vision pipeline for pixel-level features
        pipe = self._get_vision_pipeline()
        pipe_result = pipe.process(image_data)

        # Step 2: Generate semantic caption via LLM Vision
        caption_result = await self.caption(image_data, prompt, language)

        # Step 3: Merge results
        merged = dict(pipe_result)
        merged["caption"] = caption_result.get("caption", "")
        merged["caption_backend"] = caption_result.get("backend", "none")
        merged["caption_error"] = caption_result.get("error")

        return merged

    # --- P40: Audio caption ---

    def _get_audio_caption_service(self):
        """Get or create the AudioCaptionService instance (P40)."""
        if self._audio_caption_service is None:
            from services.audio_caption_service import AudioCaptionService
            self._audio_caption_service = AudioCaptionService()
        return self._audio_caption_service

    async def audio_caption(self, audio_data: bytes, prompt: Optional[str] = None,
                            language: str = "zh", preferred_backend: Optional[str] = None,
                            mode: str = "auto") -> Dict[str, Any]:
        """Generate a semantic caption for audio using LLM Audio API.
        ...
        Args:
            ...
            language: "zh" for Chinese, "en" for English
            preferred_backend: "gemini", "openai", or None for auto
            mode: "auto" (describe), "transcribe" (speech-to-text), "describe" (full)

        Returns:
            dict with caption text, backend, language, mode, duration, time_ms
        """
        svc = self._get_audio_caption_service()
        if not svc.is_available:
            await svc.initialize()
        if not svc.is_available:
            return {"caption": "", "backend": "none", "language": language,
                    "mode": mode, "duration": 0.0,
                    "error": "No LLM Audio API keys configured (set GEMINI_API_KEY or OPENAI_API_KEY)"}
        return await svc.caption(audio_data, prompt, language, preferred_backend, mode)

    async def audio_caption_status(self) -> Dict[str, Any]:
        """Check AudioCaptionService availability and backends."""
        svc = self._get_audio_caption_service()
        if not svc.is_available:
            await svc.initialize()
        return {
    ...
    async def audio_caption_pipeline(self, audio_data: bytes, prompt: Optional[str] = None,
                                      language: str = "zh", mode: str = "auto") -> Dict[str, Any]:
        """Run full audio pipeline with semantic caption.

        Combines AudioPipeline.process() (spectral features)
        with AudioCaptionService.caption() (semantic description).

        Returns:
            dict with feature_vector, latent, snr, duration, caption, backend
        """
        # Step 1: Run audio pipeline for spectral features
        pipe = self._get_audio_pipeline()
        pipe_result = pipe.process(audio_data)

        # Step 2: Generate semantic caption via LLM Audio
        caption_result = await self.audio_caption(audio_data, prompt, language, mode=mode)

        # Step 3: Merge results
        merged = dict(pipe_result)
        merged["caption"] = caption_result.get("caption", "")
        merged["caption_backend"] = caption_result.get("backend", "none")
        merged["caption_mode"] = caption_result.get("mode", mode)
        merged["caption_error"] = caption_result.get("error")
