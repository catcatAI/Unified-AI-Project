...
class AtlassianCLIBridge:
    ...
    def set_config(self, domain: str, email: str, token: str) -> None:
        """Set config."""
        self.config = {"domain": domain, "email": email, "token": token}
        ...
        try:
            ...
            if process.returncode == 0:
                ...
                try:
                    ...
                    logger.warning(
                        "Atlassian CLI JSON decode failed, returning raw output", exc_info=True
                    )
...
async def create_issue(issue: JiraIssueCreate) -> dict:
    ...
    result = await atlassian_bridge.create_jira_issue(
        issue.project_key, issue.summary, issue.description or "", issue.issue_type
    )
    if not result["success"]:
        raise HTTPException(status_code=500, detail=result.get("error"))
    return result
