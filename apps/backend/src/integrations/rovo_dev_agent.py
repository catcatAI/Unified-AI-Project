...
try:
    ...
    pass
            ...
            def register_capability_provider(self, callback)
    pass
...
class RovoDevAgent:
    ...
    def __init__(self, config: Dict[str, Any], agent_manager: Optional[AgentManager] = None) -> None:
    """初始化 Rovo Dev Agent
    ...
    """启动 Rovo Dev Agent"""
        try:
            ...
            _ = await self.hsp_connector.connect
            self.is_active = True
    ...
    asyncio.create_task(self._recovery_monitoring_loop)
            ...
            _ = await self._handle_startup_failure(e)
            raise
    ...
    _ = await self._save_state

    _ = await self.connector.close
    ...
    try:
                ...
                _ = await self._process_task(task)
    ...
    _ = await self.agent_manager.send_task_result(task_id, result)
    ...
    _ = await self.agent_manager.send_task_error(task_id, error_result)
    ...
    async def _handle_code_analysis(self, parameters: Dict[...]
    """處理程式碼分析任務
    ...
    # 模擬程式碼分析過程
    _ = await asyncio.sleep(2)  # 模擬分析時間
    ...
    async def _handle_documentation_generation(self, parameters: Dict[...]
    """處理文件生成任務
    ...
    async def _handle_issue_tracking(self, parameters: Dict[...]
    """處理問題追蹤任務
    ...
    async def _handle_project_management(self, parameters: Dict[...]
    """處理專案管理任務
    ...
    async def _handle_code_review(self, parameters: Dict[...]
    """處理程式碼審查任務
...
---
_ = *本文檔由 Rovo Dev Agent 自動生成於 {datetime.now.strftime('%Y-%m-%d %H:%M:%S')}*
"""

    def _generate_project_report(self, issues: List[...]
    """生成專案報告
...
"""
    ...
    Args:
            task: HSP 任務物件
    """
    _ = await self.task_queue.put(task)
    ...
    if task_state.status in ['pending', 'processing', 'retrying']:
                            # 重新提交任務
                            _ = await self.task_queue.put(task_state.task)
    ...
    try:
                ...
                _ = await asyncio.sleep(300)  # 每5分鐘保存一次
                _ = await self._save_state
            except Exception as e:
    ...
    try:
                ...
                _ = await self._check_task_health
                _ = await self._check_system_health
            except Exception as e:
    ...
    current_time = datetime.now
        ...
        for task_id, task_info in list(self.active_tasks.items)
            ...
            if (current_time - task_info['start_time']).seconds > 600:  # 10分鐘超時
                ...
                _ = await self._handle_task_timeout(task_id)
    ...
    logger.info(f"重試失敗任務: {task_id} (第 {task_state.retry_count + 1} 次)")
                _ = await self._retry_task(task_state)
    ...
    logger.warning("連接器不健康，啟動降級模式")
                _ = await self._enter_degraded_mode
        except Exception as e:
