...
class EnhancedAtlassianBridge(AtlassianBridge):
    ...
    def __init__(
        self,
        connector: RovoDevConnector,
        demo_learning_manager: Optional[DemoLearningManager] = None,
    ) -> None:
    ...
    async def create_jira_issue(
        ...
        """創建 Jira 問題(增強版)"""
        # In a real implementation, this would call super and then record the interaction.
        return await super().create_jira_issue(
            project_key, summary, description, issue_type, priority, assignee
        )
