...
class AtlassianCLIBridge:
    ...
    def __init__(self, acli_path: str = "acli"):
        self.acli_path = acli_path
        self.logger = logging.getLogger(__name__)
        if not self._check_acli_available():
            self.logger.warning(f"Atlassian CLI not available at: {acli_path}")
    ...
    def _run_acli_command(self, command: List[str]) -> Dict[str, Any]:
        """Runs a generic ACLI command and returns the result."""
        try:
