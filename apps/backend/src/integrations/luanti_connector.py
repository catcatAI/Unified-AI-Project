...
class LuantiConnector:
    ...
    async def _send(self, msg: Dict):
        """發送訊息"""
        if self._ws and self.state in (ConnectionState.CONNECTED, ConnectionState.AUTHENTICATING, ConnectionState.READY):
    ...
    async def send_action_wait(self, action: Dict[str, Any], timeout: float = 5.0) -> Optional[Dict]:
        """發送動作並等待回應"""
    ...
    async def move(self, forward: float = 0.0, strafe: float = 0.0, jump: bool = False, 
                   sprint: bool = False, sneak: bool = False):
        """移動指令"""
    ...
    async def look(self, yaw_delta: float = 0.0, pitch_delta: float = 0.0,
                   absolute_yaw: Optional[float] = None, absolute_pitch: Optional[float] = None):
        """視線控制"""
        await self.send_action({
            "type": "look",
            "yaw_delta": yaw_delta,
    ...
    async def inventory_action(self, action: str, **kwargs):
        ...
        await self.send_action({
            "type": "inventory",
            "action": action,  # open, close, move, drop, craft
            **kwargs
        })
