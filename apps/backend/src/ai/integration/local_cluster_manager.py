...
class LocalClusterManager:
    ...
    @staticmethod
    def _execute_task(
        worker_id: int, task: ClusterTask, task_executor: Optional[Callable] = None
    ) -> Dict[str, Any]:
        """
    ...
    def _execute_training_task(
        worker_id: int, task: ClusterTask, start_time: float
    ) -> Dict[str, Any]:
        """執行訓練任務"""
        payload = task.payload or {}
    ...
    def get_cluster_status(self) -> Dict[str, Any]:
        """獲取集群狀態"""
        alive_workers = sum(1 for w in self.workers.values() if w.process and w.process.is_alive())
...
if __name__ == "__main__":
    ...
    with LocalClusterManager(max_workers=2) as cluster:
        ...
        for i in range(3):
            task = ClusterTask(
                task_id=f"test_task_{i}", task_type="test", payload={"data": f"test_{i}"}
            )
    ...
    with LocalClusterManager(max_workers=2, task_executor=test_task_executor) as cluster:
        ...
        tasks = [
            ...
            ClusterTask(
                task_id="train_1", task_type="training", payload={"model_id": "model1", "epochs": 5}
            ),
            ClusterTask(
                task_id="infer_1",
                task_type="inference",
