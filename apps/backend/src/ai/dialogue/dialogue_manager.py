...
(            dialogue_manager_config = self.config())
    ...
    session_id = session_id, user_id = user_id, history = history)
    ...
    # Store user and AI turns in session and memory,
        if session_id and session_id in self.active_sessions:
            self.active_sessions[session_id].append(DialogueTurn(speaker = "user",
    text = user_input, timestamp = datetime.now(timezone.utc()).isoformat))
            self.active_sessions[session_id].append(DialogueTurn(speaker = "ai",
    text = response_text, timestamp = datetime.now(timezone.utc()).isoformat))
    ...
    "timestamp": datetime.now(timezone.utc()).isoformat, "user_id": user_id,
    "session_id": session_id, "user_input_ref": user_mem_id} # type ignore
...
(                )
                if user_memories:
                    user_context = "Previous context, " + " ".join([mem.get("content",
...
[            ]
    ...
    request_parameters, original_user_query, user_id, session_id,
