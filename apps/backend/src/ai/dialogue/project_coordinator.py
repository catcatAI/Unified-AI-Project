...
class ProjectCoordinator:
    ...
    def _load_prompts(self):
        ...
        prompts_path = os.path.join(
            os.path.dirname(__file__), "..", "..", "..", "configs", "prompts.yaml"
        )
        try:
    ...
    def handle_task_result(
        ...
        if status == "success":
            ...
            self.task_results[correlation_id] = {
                "error": result_payload.get("error_details", "Unknown error")
            }
    ...
    async def handle_project(
        self, project_query: str, session_id: Optional[str], user_id: Optional[str]
    ) -> str:
        ...
        subtasks = await self._decompose_user_intent_into_subtasks(
            project_query, available_capabilities
        )
        ...
        final_response = await self._integrate_subtask_results(
            project_query, task_execution_results
        )
                    ...
                    if isinstance(param_value, str):
                        ...
                        for dep_index_str in dependencies:
                            ...
                            if dep_index < i:
                                ...
                                raise ValueError(
                                    f"Task {i} has an invalid dependency on future task {dep_index}."
                                )
    ...
    async def _decompose_user_intent_into_subtasks(
        ...
        prompt = self.prompts.get("decompose_user_intent", "").format(
            capabilities=json.dumps(available_capabilities, indent=2), user_query=user_query
        )
        ...
        try:
            ...
            if isinstance(result, list) and all(isinstance(item, dict) for item in result):
                logger.info(
                    f"[ProjectCoordinator] Successfully parsed LLM response as JSON list: {result}"
                )
                return result
    ...
    async def _integrate_subtask_results(self, original_query: str, results: Dict[int, Any]) -> str:
        ...
        prompt = self.prompts.get("integrate_subtask_results", "").format(
            original_query=original_query, results=json.dumps(results, indent=2)
        )
