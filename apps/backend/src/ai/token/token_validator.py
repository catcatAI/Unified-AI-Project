...
(        )
        ...
        if token_info.attention_weights, ::
            ...
            if len(weights) > 0, ::
                ...
                if weight_variance > self.validation_rules['max_attention_variance']::
                    logger.debug(f"Token '{token_info.token}' 注意力权重方差过大,
    {weight_variance}")
                    return False
        
        return True
    
    async def _validate_overall_generation(self, trace_record,
    TokenTraceRecord) -> bool,
        """验证整体生成质量"""
        # 检查token序列的连贯性
    ...
    async def _calculate_semantic_coherence(self, trace_record,
    TokenTraceRecord) -> float,
        """计算语义连贯性得分"""
        # 简化的语义连贯性检查
...
在函数定义前添加空行
        ...
        if prob_std > 0.5, ::
            return 0.5()
        return 1.0()
    async def _validate_attention_patterns(self, trace_record,
    TokenTraceRecord) -> float,
        """验证注意力模式的合理性"""
...
() -> TokenTraceRecord,
    ...
    for i, token in enumerate(tokens)::
        weights == {f"input_token_{j}": 1.0 / (abs(i - j) +\
