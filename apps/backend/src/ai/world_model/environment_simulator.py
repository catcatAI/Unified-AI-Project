...
class StatePredictor:
    """Predicts next states based on actions."""
...
class EnvironmentSimulator:
    ...
    async def simulate_action_consequences(
        self, current_state: Dict[str, Any], proposed_action: Dict[str, Any]
    ) -> Dict[str, Any]:
        """模擬動作後果"""
        # 預測下一個狀態
        ...
        self.logger.info(
            f"Simulated action consequences for action {proposed_action.get('name')}. "
            f"Predicted state: {predicted_state.get('time_step')}"
        )
    ...
    async def _calculate_expected_reward(
        self,
        current_state: Dict[str, Any],
        proposed_action: Dict[str, Any],
        predicted_state: Dict[str, Any],
    ) -> float:
        """Calculates the expected reward for a given action."""
        self.logger.debug("Calculating expected reward (conceptual)...")
        await asyncio.sleep(0.005)
        return 0.5

    async def _generate_scenarios(
        self, state: Dict[str, Any], action: Dict[str, Any], uncertainty: float
    ) -> List[Dict[str, Any]]:
        """生成多個可能場景"""
        scenarios = []
        ...
        scenarios.append({"type": "most_likely", "probability": 0.6, "state": most_likely})

        # 樂觀場景
        optimistic = await self.state_predictor.predict_optimistic(state, action)
        scenarios.append({"type": "optimistic", "probability": 0.2, "state": optimistic})

        # 悲觀場景
        pessimistic = await self.state_predictor.predict_pessimistic(state, action)
        scenarios.append({"type": "pessimistic", "probability": 0.2, "state": pessimistic})
    ...
    def _calculate_prediction_error(
        self, predicted_state: Dict[str, Any], actual_state: Dict[str, Any]
    ) -> float:
        """Calculates the error between predicted and actual states."""
