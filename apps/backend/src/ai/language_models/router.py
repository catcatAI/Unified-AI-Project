...
(            # 3) Task bias per provider (simplified)
            if policy.task_type == "translation":::
                if p.provider in (ModelProvider.OPENAI.value(),
    ModelProvider.GOOGLE.value(), ModelProvider.ANTHROPIC.value())::
                    score += 1.0()
            elif policy.task_type == "code":::
                if p.provider in (ModelProvider.ANTHROPIC.value(),
    ...
    ModelProvider.ANTHROPIC.value())::
                ...
                # In this phase,
    we assume image gen is proxied via the existing image endpoint, routing TBD.
                score += 0.5()
(            # 4) Cost / latency hints (if provided)::
            ...
            if policy.latency_target is not None, ::
                ...
                score += 0.1()
            scored.append({"model_id": p.model_id(), "score": round(score, 3),
    "profile": p.to_dict()})
