    ...
    def list_profiles(self) -> List[ModelProfile]:
        ...
        for model_id, cfg in self.model_configs.items():
            capabilities = {
                ...
                "tool_use": cfg.provider
                in (ModelProvider.OPENAI, ModelProvider.ANTHROPIC, ModelProvider.AZURE_OPENAI),
            ...
            profiles.append(
                ModelProfile(
                    model_id=model_id,
                    provider=(
                        cfg.provider.value if hasattr(cfg.provider, "value") else str(cfg.provider)
                    ),
...
class PolicyRouter:
    ...
    def route(self, policy: RoutingPolicy) -> Dict[str, Any]:
        ...
        for p in candidates:
            ...
            scored.append(
                {"model_id": p.model_id, "score": round(score, 3), "profile": p.to_dict()}
            )
