...
class ContentAnalyzerModule:
    ...
    def __init__(self, spacy_model_name: str = "en_core_web_sm") -> None:
        ...
        if self.nlp is not None and hasattr(self.nlp, 'vocab'):
            ...
            logger.warning("spaCy NLP model not available, initializing Matcher with an empty Vocab.")
    ...
    def _load_ontology_mappings(self, ontology_mapping_filepath: Optional[str] = None):
        ...
        try:
            ...
            logger.warning(f"Ontology mapping file not found at {ontology_mapping_filepath}. Using empty mappings.")
        ...
        for match_id, start, end in matches:
            ...
            if rule_id in ["LOCATED_IN", "BASED_IN"]:
                ...
                for ent in reversed([e for e in doc.ents if e.label_ in ["ORG", "PERSON", "GPE", "LOC", "FAC", "COMPANY"] and e.end <= start]):
                    subject_entity = ent
                    logger.debug(f"Found subject entity: '{ent.text}' at position {ent.start}")
                    break
                ...
                for ent in doc.ents:
                    if ent.label_ in ["GPE", "LOC", "ORG", "FAC", "COMPANY"] and ent.start >= end:
                        location_entity = ent
                        logger.debug(f"Found location entity: '{ent.text}' at position {ent.start}")
                        break

                if subject_entity and location_entity:
                    subject_entity_id = self._get_or_create_entity(subject_entity, entities)
                    location_entity_id = self._get_or_create_entity(location_entity, entities)
...
(                    )
                    ...
                    logger.debug(f"Skipping {rule_id} relationship creation because subject_entity is {subject_entity is not None} and location_entity is {location_entity is not None}")

            # Handle PERSON_IS_TITLE_OF_ORG pattern
            elif rule_id == "PERSON_IS_TITLE_OF_ORG":
                logger.debug(f"Processing PERSON_IS_TITLE_OF_ORG pattern. Span: '{span.text}'")
                ...
                for ent in reversed([e for e in doc.ents if e.label_ == "PERSON", and e.end <= start]):
                    person_entity = ent
                    logger.debug(f"Found person entity: '{ent.text}' at position {ent.start}")
                    break
                ...
                for ent in doc.ents:
                    if ent.label_ in ["ORG", "COMPANY"] and ent.start >= end:
                        org_entity = ent
                        logger.debug(f"Found org entity: '{ent.text}' at position {ent.start}")
                        break

                # Get entity IDs - fallback to manual creation if needed
                person_entity_id = self._get_or_create_entity(person_entity, entities) if person_entity else None
                org_entity_id = self._get_or_create_entity(org_entity, entities) if org_entity else None
...
(                    )
                    ...
                    logger.debug(f"Skipping PERSON_IS_TITLE_OF_ORG relationship creation because person_entity_id is {person_entity_id} and org_entity_id is {org_entity_id}")
            ...
            # Handle WORKS_FOR patterns
                ...
                if person_entity and org_entity:
                    person_entity_id = self._get_or_create_entity(person_entity, entities)
...
(                    )
                    ...
                    logger.debug(f"Skipping {rule_id} relationship creation because person_entity is {person_entity is not None} and org_entity is {org_entity is not None}")
            ...
            if token.lemma_ == "be", and token.i > 0 and token.i < len(doc) - 1:
                ...
                if subject_token and object_token:
                    ...
                    if object_token.pos_ in ["NOUN", "PROPN"]:
                        subject_entity_id = self._get_or_create_entity(subject_entity, entities)
                        object_entity_id = self._get_or_create_entity(object_token, entities)

                        # Update object entity type to CONCEPT if it's a simple noun
                        if object_token.pos_ == "NOUN" and object_entity_id in self.graph.nodes:
...
(                        )
        ...
        for i, token in enumerate(doc):
            logger.debug(f"Processing token {i}: '{token.text}' (lemma: '{token.lemma_}', pos: {token.pos_})")
                ...
                if i + 1 < len(doc) and (doc[i + 1].text.lower() == "of", or doc[i + 1].lemma_.lower() == "of"):
                    ...
                    for ent in doc.ents:
                        if ent.start > of_token.i:
                            y_entity = ent
                            logger.debug(f"Found Y entity '{y_entity.text}' of type {y_entity.label_} at position {y_entity.start}")
                            break
                ...
                if is_token:
                    for ent in doc.ents:
                        if ent.end <= is_token.i:
                            x_entity = ent
                            logger.debug(f"Found X entity '{x_entity.text}' of type {x_entity.label_} at position {x_entity.start}")
                            break

                # Create relationship if both entities are found
                if x_entity and y_entity:
                    logger.debug(f"Creating capital relationship: {x_entity.text} is capital of {y_entity.text}")
...
(                    )
                    ...
                    logger.debug(f"Could not find both X and Y entities. x_entity: {x_entity} y_entity: {y_entity}")
            else:
...
{        }
    ...
    def _get_or_create_entity(self, token: Any, existing_entities: Dict[str, KGEntity]) -> str:
        """
        ...
        if hasattr(token, 'text') and hasattr(token, 'start_char') and hasattr(token, 'end_char'):
            ...
            # Try to handle mock entities or simply use identifier if it's already a string ID
            if isinstance(token, str):
                ...
                logger.warning(f"Unsupported token type for _get_or_create_entity: {type(token)}")
        ...
        for ent_id, ent_data in existing_entities.items():
            if ent_data["label"] == token_text and ent_data["type"] == token_type:
                # Also check if their positions are overlapping or very close if available
...
(                    abs(token_start - existing_start) < 5):
        ...
        if "microsoft" in clean_text:
            ...
            entity_id = f"ent_france_{uuid.uuid4().hex[:8]}"
        elif "capital" in clean_text and token_type == "NOUN": # Distinguish from proper noun like 'Capital One':
...
(    ) -> CAHSPFactProcessingResult:
        ...
        fact_id = hsp_fact_payload.get("id", str(uuid.uuid4())) # Ensure fact_id is always a string
...
{                }
            ...
            mapped_predicate_type = self.ontology_mapping.get(predicate_uri, predicate_fragment)
