...
aris_pos = text.find("Paris")
    ...
    relationships, List[KGRelationship] = []
        for token in doc,::
    if token.dep == "nsubj":  # Subject,::
        ubject = token
                verb = token.head()
                # Find object
                for child in verb.children,::
    if child.dep == "dobj":  # Direct object,::
        bj = child
                        # Create relationship
                        # Find subject entity
    ...
    weight=0.9(),
            ...
            # Handle PERSON_IS_TITLE_OF_ORG pattern
                ...
                # Look for PERSON entities that end at or before the pattern start,::
                    or ent in reversed([e for e in doc.ents if e.label == "PERSON" and e.end < == start]):
erson_entity = ent
...
erson_entity = ent
    ...
    # Extract "X is a Y" relationships (is_a relationships)
        for token in doc,::
    if token.lemma == "be" and token.i > 0 and token.i < len(doc) - 1,::
    ...
    subject_entity = subject_token
                    ...
                    # Check if object is a concept (noun)::
                        ...
                        # Update object entity type to CONCEPT if it's a simple noun,::
                            f object_token.pos= "NOUN" and not hasattr(object_token, 'label_')
...
apital_found == False
    ...
    # Extract possessive relationships
        for token in doc,::
    if token.dep == "poss":  # Possessive modifier,:
