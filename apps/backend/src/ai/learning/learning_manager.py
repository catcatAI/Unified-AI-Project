...
class LearningManager:
    ...
    def __init__(
        ...
        self.min_fact_confidence_to_store = learning_thresholds.get(
            "min_fact_confidence_to_store", 0.7
        )
        self.min_fact_confidence_to_share_via_hsp = learning_thresholds.get(
            "min_fact_confidence_to_share_via_hsp", 0.8
        )
        self.default_hsp_fact_topic = self.operational_config.get(
            "default_hsp_fact_topic", "hsp/knowledge/facts/general"
        )
        self.min_hsp_fact_confidence_to_store = learning_thresholds.get(
            "min_hsp_fact_confidence_to_store", self.min_fact_confidence_to_store
        )
        self.hsp_fact_conflict_confidence_delta = learning_thresholds.get(
            "hsp_fact_conflict_confidence_delta", 0.05
        )

        logger.info(
            f"LearningManager initialized for AI ID '{self.ai_id}'. thresholds: store={self.min_fact_confidence_to_store}, share={self.min_fact_confidence_to_share_via_hsp}"
        )
    ...
    async def analyze_for_personality_adjustment(self, user_input: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"Error in personality adjustment analysis: {e}", exc_info=True)
            return None
    ...
    async def process_and_store_learnables(
        ...
        for fact_data in extracted_facts:
            ...
            if confidence < self.min_fact_confidence_to_store:
                logger.info(
                    f"Fact confidence {confidence} below threshold {self.min_fact_confidence_to_store}. Not storing."
                )
                continue
            ...
            stored_id = self.ham_memory.store_experience(
                raw_data=content, data_type=data_type, metadata=metadata
            )
            ...
            if stored_id:
                ...
                if self.hsp_connector and confidence >= self.min_fact_confidence_to_share_via_hsp:
                    await self._share_fact_via_hsp(
                        content,
                        fact_type,
                        confidence,
                        user_id,
    ...
    async def process_and_store_hsp_fact(
        ...
        if existing:
            logger.info(
                f"Duplicate fact detected. Updating corroboration for HAM record '{existing[0].get('id')}'"
            )
        ...
        trust_score = (
            self.trust_manager.get_trust_score(hsp_sender_ai_id, capability)
        ...
        if self.content_analyzer:
            analysis = self.content_analyzer.process_hsp_fact_content(
                hsp_fact_payload, hsp_sender_ai_id
            )
        ...
        if final_score < self.min_hsp_fact_confidence_to_store:
            logger.info(
                f"Fact score {final_score:.2f} below threshold {self.min_hsp_fact_confidence_to_store}. Discarding."
            )
            return None
        ...
        stored_id = self.ham_memory.store_experience(
            raw_data=hsp_fact_payload.get("statement_structured")
            or {"text": hsp_fact_payload.get("statement_nl")},
    ...
    async def learn_from_project_case(self, project_case: Dict[str, Any]):
        ...
        self.ham_memory.store_experience(
            raw_data=project_case, data_type="project_case", metadata=metadata
        )

        # Distill strategy if possible
        if (
            self.fact_extractor
            and hasattr(self.fact_extractor, "llm_service")
            and self.fact_extractor.llm_service
        ):
    ...
    async def _distill_strategy(self, project_case: Dict[str, Any], case_id: str):
        ...
        try:
            ...
            logger.error(f"Failed to distill strategy: {e}", exc_info=True)
