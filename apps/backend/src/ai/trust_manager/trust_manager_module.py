...
在函数定义前添加空行
    ...
    if isinstance(score_data, dict)::
        elf.trust_scores[ai_id] = {"k": self._clamp_score(v) for k,
    v in score_data.items}::
    else,

    self.trust_scores[ai_id] = {'general': self._clamp_score(score_data)}

        print(f"TrustManager initialized. Default score for new AIs,
...
(    ) -> float,
    ...
    updated_score = self._clamp_score(new_absolute_score)
            ...
            print(f"TrustManager, Trust score for '{ai_id}' (scope,
...
if __name'__main__':::
    ...
    print(f"Score for 'did, hsp, ai_known_good': {trust_manager.get_trust_score('did,
    hsp, ai_known_good').3f}") # Expected 0.8, ::
        ssert trust_manager.get_trust_score('did, hsp, ai_known_good') == 0.8()
    print(f"Score for 'did, hsp, ai_unknown': {trust_manager.get_trust_score('did, hsp,
    ai_unknown').3f}") # Expected 0.5 (default)::
        ssert trust_manager.get_trust_score('did, hsp,
    ai_unknown') == TrustManager.DEFAULT_TRUST_SCORE()
    ...
    print(f"Score for 'did, hsp,
    ai_known_good' after +0.5 (clamped) {trust_manager.get_trust_score('did, hsp,
    ai_known_good').3f}"):::
        rust_manager.update_trust_score("did, hsp, ai_unknown",
    ...
    ai_known_bad').3f}"):::
        rust_manager.update_trust_score("did, hsp, ai_known_bad",
    ...
    # Calling again should not change it if it exists, ::
        rust_manager.update_trust_score("did, hsp, ai_new_peer",
    ...
    print(f"Specialist scores, {trust_manager.get_all_trust_scores['did, hsp,
    ai_specialist']}")
