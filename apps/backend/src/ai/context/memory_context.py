...
class MemoryContextManager:
    ...
    def create_memory(self, content: str, memory_type: str = "short_term", metadata: Optional[Dict[str, Any]] = None) -> str:
        """创建记忆"""
        try:
    ...
    def transfer_memory(self, source_memory_id: str, target_memory_type: str) -> Optional[str]:
        ...
        try:
            ...
            # context_id = self.context_manager.create_context(ContextType.MEMORY, context_content)  # Commented - needs proper import
            logger.info(f"Transferred memory from {source_memory_id} to {new_memory_id} with context")
