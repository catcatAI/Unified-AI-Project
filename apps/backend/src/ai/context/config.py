...
class ContextConfig:
    ...
    def __init__(self) -> None:
        ...
        self.disk_storage_enabled = (
            os.environ.get("CONTEXT_DISK_STORAGE_ENABLED", "true").lower() == "true"
        )
        self.database_storage_enabled = (
            os.environ.get("CONTEXT_DATABASE_STORAGE_ENABLED", "false").lower() == "true"
        )
        ...
        self.compression_enabled = (
            os.environ.get("CONTEXT_COMPRESSION_ENABLED", "false").lower() == "true"
        )
        self.async_processing_enabled = (
            os.environ.get("CONTEXT_ASYNC_PROCESSING_ENABLED", "true").lower() == "true"
        )

        # 安全配置
        self.encryption_enabled = (
            os.environ.get("CONTEXT_ENCRYPTION_ENABLED", "false").lower() == "true"
        )
        self.access_control_enabled = (
            os.environ.get("CONTEXT_ACCESS_CONTROL_ENABLED", "false").lower() == "true"
        )

        # 日志配置
        self.log_level = os.environ.get("CONTEXT_LOG_LEVEL", "INFO")
        self.audit_logging_enabled = (
            os.environ.get("CONTEXT_AUDIT_LOGGING_ENABLED", "true").lower() == "true"
        )

        # 集成配置
        self.ham_integration_enabled = (
            os.environ.get("CONTEXT_HAM_INTEGRATION_ENABLED", "true").lower() == "true"
        )
        self.mcp_integration_enabled = (
            os.environ.get("CONTEXT_MCP_INTEGRATION_ENABLED", "true").lower() == "true"
        )
    ...
    def get_cache_config(self) -> Dict[str, Any]:
        """获取缓存配置"""
        return {"cache_enabled": self.cache_enabled, "cache_max_size": self.cache_max_size}
    ...
    def get_logging_config(self) -> Dict[str, Any]:
        """获取日志配置"""
        return {"log_level": self.log_level, "audit_logging_enabled": self.audit_logging_enabled}
