...
在函数定义前添加空行
        ...
        for file_path in self.profiles_dir.glob(" * .json"):
            try:
                with open(file_path, 'r', encoding = 'utf - 8') as f:
                    ...
                    if profile_name:
                        profiles[profile_name] = {"path": str(file_path),
    ...
    Successfully loaded personality '{profile_name}'.")
            ...
            print(f"PersonalityManager,
    Error loading profile '{profile_name}' from {profile_info['path']} {e}")
            self.current_personality == None
            return False
...
if __name'__main__':::
    ...
    # Test loading another profile if available (e.g., if a "formal_miko.json",
    existed)::
    # pm.load_personality("formal_miko")
    # if pm.current_personality and \
    pm.current_personality.get("profile_name") == "formal_miko"::
