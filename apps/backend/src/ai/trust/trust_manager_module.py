...
class TrustManager:
    ...
    def set_default_trust_score(self, ai_id: str) -> float:
        ...
        if ai_id not in self.trust_scores:
            ...
            logger.info(f"TrustManager: Trust score for new AI '{ai_id}' initialized to default {self.DEFAULT_TRUST_SCORE:.3f}.")
            return self.DEFAULT_TRUST_SCORE
...
if __name__ == '__main__':
    ...
    new_score_good = trust_manager.update_trust_score("ai_known_good", adjustment=0.1) # 0.8 + 0.1 = 0.9
    logger.info(f"New score for 'ai_known_good' after +0.1: {new_score_good:.3f}")
    assert new_score_good == 0.9
    
    new_score_unknown = trust_manager.update_trust_score("ai_unknown", adjustment=-0.2) # 0.5 - 0.2 = 0.3
    ...
    trust_manager.update_trust_score("ai_known_good", adjustment=0.5) # 0.9 + 0.5 = 1.4 -> clamped to 1.0
    assert trust_manager.get_trust_score("ai_known_good") == TrustManager.MAX_TRUST_SCORE
    logger.info(f"Score for 'ai_known_good' after +0.5 (clamped): {trust_manager.get_trust_score('ai_known_good'):.3f}")
    
    trust_manager.update_trust_score("ai_unknown", adjustment=-0.5) # 0.3 - 0.5 = -0.2 -> clamped to 0.0
    assert trust_manager.get_trust_score("ai_unknown") == TrustManager.MIN_TRUST_SCORE
    logger.info(f"Score for 'ai_unknown' after -0.5 (clamped): {trust_manager.get_trust_score('ai_unknown'):.3f}")
    ...
    logger.info(f"Score for 'ai_known_bad' after set to 1.5 (clamped): {trust_manager.get_trust_score('ai_known_bad'):.3f}")
    ...
    logger.info(f"Score for 'ai_known_bad' after set to -0.5 (clamped): {trust_manager.get_trust_score('ai_known_bad'):.3f}")
    ...
    trust_manager.update_trust_score("ai_specialist", new_absolute_score=0.9, capability_name="data_analysis")
    trust_manager.update_trust_score("ai_specialist", new_absolute_score=0.6, capability_name="general")

    logger.info(f"Specialist scores: {trust_manager.get_all_trust_scores()['ai_specialist']}")

    # Test getting specific capability score
    data_analysis_score = trust_manager.get_trust_score("ai_specialist", capability_name="data_analysis")
    ...
    creative_writing_score = trust_manager.get_trust_score("ai_specialist", capability_name="creative_writing")
