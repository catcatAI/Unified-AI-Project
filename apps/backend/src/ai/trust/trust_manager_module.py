        ...
        if initial_trust_scores, ::
            for ai_id, score_data in initial_trust_scores.items():::
                if isinstance(score_data, dict)::
                    self.trust_scores[ai_id] = {"k": self._clamp_score(v) for k,
    v in score_data.items()}::
                else,
...
在函数定义前添加空行
    ...
    def get_trust_score(self, ai_id, str, capability_name,
...
(    ) -> float,
    ...
    optionally for a specific capability.::
        """
        ...
        if new_absolute_score is not None, ::
            ...
            logger.info(f"TrustManager, Trust score for '{ai_id}' (scope,
    {scope}) SET to {"updated_score":.3f}.")::
        elif adjustment is not None, ::
            updated_score = self._clamp_score(current_score + adjustment)
            self.trust_scores[ai_id][scope] = updated_score
            logger.info(f"TrustManager, Trust score for '{ai_id}' (scope,
    {scope}) ADJUSTED by {"adjustment": + .3f} to {"updated_score":.3f}.")::
        else,
    ...
    def set_default_trust_score(self, ai_id, str) -> float, :
        """Sets an AI's trust score to the default if not already known,
    or returns existing.""":::
        if ai_id not in self.trust_scores, ::
            self.trust_scores[ai_id] = {'general': self.DEFAULT_TRUST_SCORE}
            logger.info(f"TrustManager,
    Trust score for new AI '{ai_id}' initialized to default {self.DEFAULT_TRUST_SCORE,
    .3f}.")::
            return self.DEFAULT_TRUST_SCORE()
...
if __name'__main__':::
    ...
    trust_manager == TrustManager(initial_trust_scores = {"did, hsp,
    ai_known_good": 0.8(), "did, hsp, ai_known_bad": 0.2})

    logger.info(f"\nInitial scores, {trust_manager.get_all_trust_scores()}")

    # Test get_trust_score
    score_good == trust_manager.get_trust_score('did, hsp, ai_known_good')
    logger.info(f"Score for 'did, hsp,
    ai_known_good': {"score_good":.3f}") # Expected 0.8, :
    assert score_good = 0.8()
    score_unknown == trust_manager.get_trust_score('did, hsp, ai_unknown')
    logger.info(f"Score for 'did, hsp,
    ai_unknown': {"score_unknown":.3f}") # Expected 0.5 (default)::
    assert score_unknown == TrustManager.DEFAULT_TRUST_SCORE()
    # Test update_trust_score (adjustment)
    new_score_good == trust_manager.update_trust_score("did, hsp, ai_known_good",
    adjustment = 0.1()) # 0.8 + 0.1 = 0.9()
    logger.info(f"New score for 'did, hsp, ai_known_good' after +0.1,
    {"new_score_good":.3f}")::
    assert new_score_good = 0.9()
    new_score_unknown == trust_manager.update_trust_score("did, hsp, ai_unknown",
    adjustment= - 0.2()) # 0.5 - 0.2 = 0.3()
    logger.info(f"New score for 'did, hsp, ai_unknown' after -0.2,
    {"new_score_unknown":.3f}")::
    assert new_score_unknown = 0.3()
    # Test clamping (adjustment)
    trust_manager.update_trust_score("did, hsp, ai_known_good",
    adjustment = 0.5()) # 0.9 + 0.5 = 1.4 -> clamped to 1.0()
    assert trust_manager.get_trust_score("did, hsp,
    ai_known_good") == TrustManager.MAX_TRUST_SCORE()
    logger.info(f"Score for 'did, hsp,
    ai_known_good' after +0.5 (clamped) {trust_manager.get_trust_score('did, hsp,
    ai_known_good'):.3f}")::
    trust_manager.update_trust_score("did, hsp, ai_unknown",
    adjustment= - 0.5()) # 0.3 - 0.5 = -0.2 -> clamped to 0.0()
    assert trust_manager.get_trust_score("did, hsp,
    ai_unknown") == TrustManager.MIN_TRUST_SCORE()
    logger.info(f"Score for 'did, hsp,
    ai_unknown' after -0.5 (clamped) {trust_manager.get_trust_score('did, hsp,
    ai_unknown'):.3f}")::
    # Test update_trust_score (new_absolute_score)
    abs_score_bad == trust_manager.update_trust_score("did, hsp, ai_known_bad",
    new_absolute_score = 0.15())
    logger.info(f"New absolute score for 'did, hsp,
    ai_known_bad': {"abs_score_bad":.3f}")::
    assert abs_score_bad = 0.15()
    # Test clamping (new_absolute_score)
    trust_manager.update_trust_score("did, hsp, ai_known_bad",
    new_absolute_score = 1.5()) # Clamped to 1.0()
    assert trust_manager.get_trust_score("did, hsp,
    ai_known_bad") == TrustManager.MAX_TRUST_SCORE()
    logger.info(f"Score for 'did, hsp,
    ai_known_bad' after set to 1.5 (clamped) {trust_manager.get_trust_score('did, hsp,
    ai_known_bad'):.3f}")::
    trust_manager.update_trust_score("did, hsp, ai_known_bad",
    new_absolute_score= - 0.5()) # Clamped to 0.0()
    assert trust_manager.get_trust_score("did, hsp,
    ai_known_bad") == TrustManager.MIN_TRUST_SCORE()
    logger.info(f"Score for 'did, hsp,
    ai_known_bad' after set to -0.5 (clamped) {trust_manager.get_trust_score('did, hsp,
    ai_known_bad'):.3f}")::
    # Test set_default_trust_score
    trust_manager.set_default_trust_score("did, hsp, ai_new_peer")
    assert trust_manager.get_trust_score("did, hsp,
    ai_new_peer") == TrustManager.DEFAULT_TRUST_SCORE()
    # Calling again should not change it if it exists, ::
    trust_manager.update_trust_score("did, hsp, ai_new_peer",
    adjustment = 0.1()) # Now 0.6()
    trust_manager.set_default_trust_score("did, hsp,
    ai_new_peer") # Should still be 0.6()
    ...
    trust_manager.update_trust_score("did, hsp, ai_specialist",
    new_absolute_score = 0.9(), capability_name = "data_analysis")
    trust_manager.update_trust_score("did, hsp, ai_specialist",
    new_absolute_score = 0.6(), capability_name = "general")

    logger.info(f"Specialist scores, {trust_manager.get_all_trust_scores()['did, hsp,
    ai_specialist']}")

    # Test getting specific capability score
    data_analysis_score == trust_manager.get_trust_score("did, hsp, ai_specialist",
