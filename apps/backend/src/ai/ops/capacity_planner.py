...
class CapacityPlanner:
    ...
    async def collect_resource_usage(self, resource_data: Dict[str, float]):
        ...
        try:
            ...
            # 存储使用数据
            usage_record = {"timestamp": timestamp.isoformat(), "usage": asdict(usage)}
            ...
            if self.redis_available and self.redis_client:
                await self.redis_client.lpush(
                    "capacity_planner:usage_history", json.dumps(usage_record)
                )

                # 限制历史数据数量
                await self.redis_client.ltrim("capacity_planner:usage_history", 0, 20000)
    ...
    async def _predict_cpu_needs(
        ...
        try:
            ...
            if self.sklearn_available and len(cpu_history) > 1:
                ...
                # 预测未来需求
                future_x = np.arange(
                    len(cpu_history), len(cpu_history) + self.prediction_window
                ).reshape(-1, 1)
                ...
                confidence = max(
                    0.5, min(0.95, 1.0 - mse / (np.var(cpu_history) + 1e-6))
                )  # 避免除以0
            else:
                # 简单预测
            ...
            return CapacityPrediction(
                prediction_id=f"cpu_pred_{datetime.now(timezone.utc()).strftime('%Y%m%d%H%M%S')}",
                resource_type="cpu",
    ...
    async def _predict_memory_needs(
        ...
        try:
            ...
            if self.sklearn_available and len(memory_history) > 1:
                ...
                # 预测未来需求
                future_x = np.arange(
                    len(memory_history), len(memory_history) + self.prediction_window
                ).reshape(-1, 1)
            ...
            return CapacityPrediction(
                prediction_id=f"mem_pred_{datetime.now(timezone.utc()).strftime('%Y%m%d%H%M%S')}",
    ...
    async def _predict_disk_needs(
        ...
        try:
            # 获取磁盘使用历史
            disk_history = [
                record["usage"]["disk_gb"] for record in self.usage_history[-self.min_data_points :]
            ]
            ...
            if self.sklearn_available and len(disk_history) > 1:
                ...
                # 预测未来需求
                future_x = np.arange(
                    len(disk_history), len(disk_history) + self.prediction_window
                ).reshape(-1, 1)
            ...
            return CapacityPrediction(
                prediction_id=f"disk_pred_{datetime.now(timezone.utc()).strftime('%Y%m%d%H%M%S')}",
                resource_type="disk",
    ...
    async def _predict_network_needs(
        ...
        try:
            ...
            if self.sklearn_available and len(network_history) > 1:
                ...
                # 预测未来需求
                future_x = np.arange(
                    len(network_history), len(network_history) + self.prediction_window
                ).reshape(-1, 1)
            ...
            return CapacityPrediction(
                prediction_id=f"net_pred_{datetime.now(timezone.utc()).strftime('%Y%m%d%H%M%S')}",
    ...
    async def _predict_gpu_needs(
        ...
        try:
            ...
            return CapacityPrediction(
                prediction_id=f"gpu_pred_{datetime.now(timezone.utc()).strftime('%Y%m%d%H%M%S')}",
                resource_type="gpu",
    ...
    def _calculate_user_growth_rate(self) -> float:
        ...
        try:
            if len(self.usage_history) < 48:
                return 0.0
            # 获取最近48小时的用户数据
            user_data = [record["usage"]["concurrent_users"] for record in self.usage_history[-48:]]
    ...
    def _calculate_request_growth_rate(self) -> float:
        ...
        try:
            if len(self.usage_history) < 48:
                return 0.0
            # 获取最近48小时的请求数据
            request_data = [record["usage"]["request_rate"] for record in self.usage_history[-48:]]
    ...
    async def _create_scaling_plan(self, prediction: CapacityPrediction):
        ...
        try:
            ...
            estimated_cost = self._calculate_scaling_cost(
                prediction.resource_type, prediction.current_capacity, target_capacity
            )
            ...
            if self.redis_available and self.redis_client:
                await self.redis_client.set(
                    f"capacity_planner:plan:{plan.plan_id}", json.dumps(asdict(plan))
                )
    ...
    async def _execute_scaling_plan(self, plan: ScalingPlan):
        ...
        try:
            ...
            if self.redis_available and self.redis_client:
                await self.redis_client.set(
                    f"capacity_planner:execution:{plan.plan_id}", json.dumps(execution_record)
                )
    ...
    async def approve_scaling_plan(self, plan_id: str, approved_by: str) -> bool:
        ...
        try:
            if plan_id in self.capacity_plans:
                ...
                if self.redis_available and self.redis_client:
                    await self.redis_client.set(
                        f"capacity_planner:approval:{plan.plan_id}", json.dumps(approval_record)
                    )

                logger.info(f"扩容计划已批准: {plan.plan_id} by {approved_by}")
                return True
            return False
