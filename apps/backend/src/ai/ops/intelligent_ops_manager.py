...
try:
    ...
    logging.warning("Redis.asyncio not found. IntelligentOpsManager will run in in-memory mode for Redis-dependent features.")
...
try:
    ...
    logging.warning("Numpy not found. IntelligentOpsManager will use simpler calculations for numpy-dependent features.")
...
class PredictiveMaintenanceEngine: # Placeholder for actual class
    ...
    async def get_component_health(self, component_id: str):
        return ComponentHealth(component_id=component_id, health_score=70, maintenance_recommendation="None")
    ...
    async def initialize(self):
        ...
        try:
            ...
            if self.redis_available:
                try:
                    self.redis_client = redis.Redis(host=self.config.get('redis_host', 'localhost'), port=self.config.get('redis_port', 6379), db=self.config.get('redis_db', 0), decode_responses=True)
                    # 测试连接
    ...
    async def _execute_capacity_scaling(self, insight: OpsInsight) -> bool:
        ...
        try:
            ...
            plans = await self.capacity_planner.get_scaling_plans(
                resource_type=insight.affected_components[0] if insight.affected_components else None
            )
