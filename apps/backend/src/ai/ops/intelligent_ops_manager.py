...
@dataclass
class ComponentHealth:  # Placeholder for actual class
    component_id: str
    health_score: float
...
class IntelligentOpsManager:
    ...
    async def _initialize_components(self):
        ...
        try:
            ...
            logger.error(f"AI运维组件初始化失败: {e}", exc_info=True)
            raise
    ...
    async def _generate_comprehensive_insights(
        ...
        try:
            ...
            # 分析维护需求
            if self.predictive_maintenance:
                maintenance_insights = await self._analyze_maintenance_needs(
                    component_id, component_type, metrics
                )
    ...
    async def _analyze_anomalies(
        ...
        try:
            ...
            logger.error(f"分析异常失败: {e}", exc_info=True)
    ...
    async def _analyze_performance(
        ...
        try:
            if self.performance_optimizer:
                ...
                # 筛选高优先级建议
                high_priority_recs = [
                    rec for rec in recommendations if rec.priority in ["high", "critical"]
                ]
            ...
            logger.error(f"分析性能失败: {e}", exc_info=True)
    ...
    async def _analyze_capacity(
        ...
        try:
            if self.capacity_planner:
                ...
                )  # Pass component_type as resource_type

                # 筛选高紧急度预测
                urgent_predictions = [
                    pred for pred in predictions if pred.urgency in ["high", "critical"]
                ]
            ...
            logger.error(f"分析容量失败: {e}", exc_info=True)
    ...
    async def _analyze_maintenance_needs(
        ...
        try:
            ...
            logger.error(f"分析维护需求失败: {e}", exc_info=True)
        ...
        try:
            ...
            if self.redis_available and self.redis_client:
                await self.redis_client.set(
                    f"intelligent_ops:insight:{insight.insight_id}", json.dumps(asdict(insight))
                )

                # 添加到洞察列表
                await self.redis_client.lpush(
                    "intelligent_ops:insights", json.dumps(asdict(insight))
                )
        ...
        try:
            ...
            if self.redis_available and self.redis_client:
                await self.redis_client.publish(
                    "notifications:ops_insights", json.dumps(notification)
                )
    ...
    async def _execute_auto_action(self, insight: OpsInsight):
        ...
        try:
            ...
            if self.redis_available and self.redis_client:
                await self.redis_client.set(
                    f"intelligent_ops:action:{insight.insight_id}", json.dumps(action_record)
                )
            ...
            logger.error(f"执行自动操作失败: {e}", exc_info=True)
    ...
    async def _execute_anomaly_recovery(self, insight: OpsInsight) -> bool:
        ...
        try:
            ...
            logger.error(f"异常恢复失败: {e}", exc_info=True)
            return False
    ...
    async def _execute_capacity_scaling(self, insight: OpsInsight) -> bool:
        ...
        try:
            ...
            plans = await self.capacity_planner.get_scaling_plans(
                resource_type=(
                    insight.affected_components[0] if insight.affected_components else None
                )
            )
            ...
            for schedule in schedules:
                if schedule.component_id in insight.affected_components and schedule.auto_approve:
                    await self.predictive_maintenance.approve_maintenance(
                        schedule.schedule_id, "intelligent_ops_manager"
                    )
        ...
        try:
            ...
            if system_health["overall_score"] < 70:
                insight = OpsInsight(
                    ...
                    recommendations=["检查系统资源使用情况", "考虑增加系统容量", "优化系统配置"],
                    confidence=0.85,
    ...
    async def _analyze_system_health(self) -> dict[str, Any]:
        ...
        try:
            ...
            overall_score = (
                np.mean(health_scores) if self.numpy_available and health_scores else 75.0
            )
        ...
        try:
            ...
            if "utilization_trends" in capacity_report:
                ...
                increasing_resources = [
                    resource for resource, trend in trends.items() if trend == "increasing"
                ]
            ...
            logger.error(f"识别模式失败: {e}", exc_info=True)
        ...
        while True:
            try:
                ...
                cutoff_time = datetime.now(timezone.utc) - timedelta(
                    days=self.insight_retention_days
                )

                # 清理内存中的洞察
                self.ops_insights = [
                    insight for insight in self.ops_insights if insight.timestamp > cutoff_time
                ]
                ...
                logger.error(f"洞察清理失败: {e}", exc_info=True)
    ...
    async def get_ops_dashboard_data(self) -> dict[str, Any]:
        ...
        try:
            ...
            # 活跃告警
            active_alerts = len(
                [insight for insight in recent_insights if insight.severity in ["high", "critical"]]
            )
            ...
            auto_actions = len(
                [
                    action
                    for action in self.action_history[-24:]  # 最近24小时
            ...
            logger.error(f"获取运维仪表板数据失败: {e}", exc_info=True)
            return {}
    ...
    async def execute_manual_action(
        ...
        try:
            ...
            if self.redis_available and self.redis_client:
                await self.redis_client.set(
                    f"intelligent_ops:manual_action:{insight_id}", json.dumps(action_record)
                )

            return success

        except Exception as e:  # broad exception acceptable: manual action failures return False to signal failure
            logger.error(f"执行手动操作失败: {e}", exc_info=True)
            return False
