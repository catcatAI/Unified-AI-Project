...
MATRIX_LAYERS = {
    "L1": {
        "name": "Biology Layer",
        "keywords": ["bio", "tactile", "endocrine", "autonomic", "physiological"],
    },
    "L2": {
        "name": "Memory Layer",
        "keywords": ["memory", "ham", "vector", "store", "episodic", "semantic", "procedural"],
    },
    "L3": {"name": "Identity Layer", "keywords": ["identity", "self", "cyber", "persona", "soul"]},
    "L4": {
        "name": "Creation Layer",
        "keywords": ["creative", "art", "generate", "draw", "compose"],
    },
    "L5": {
        "name": "Presence Layer",
        "keywords": ["presence", "live2d", "avatar", "render", "display"],
    },
...
MATRIX_DIMENSIONS = {
    "α": {
        "name": "Physiological",
        "keywords": ["energy", "comfort", "arousal", "rest", "vitality"],
    },
    "β": {
        "name": "Cognitive",
        "keywords": ["curiosity", "focus", "confusion", "learning", "clarity"],
    },
    "γ": {
        "name": "Emotional",
        "keywords": ["happiness", "sadness", "anger", "fear", "trust", "anticipation"],
    },
...
class DesignReviewer:
    ...
    def _check_module_layer_alignment(self) -> List[ReviewFinding]:
        ...
        for py_file in self._src.rglob("*.py"):
            ...
            annotation_match = re.search(r"ANGELA-MATRIX:\s*\[(L\d+(?:-L\d+)?)\]", text)
...
class MDReviewer:
    ...
    def _is_version_context(self, text: str, version: str) -> bool:
        ...
        return any(
            kw in window.lower() for kw in ["version", "v", "目前", "current", "latest", "release"]
        )
