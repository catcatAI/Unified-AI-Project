...
(        )
    ...
    dna_chain_id: Optional[str] = None) -> Optional[CodeAnalysisResult]:
        ...
        - It first looks for an exact match (e.g.,
    "my_tool.py") in `self.tools_directory`.
        - If not found, it searches for common patterns like "tool_my_tool.py" or
            "my_tool_tool.py" in `self.tools_directory`.
        - Resolution fails if the name is ambiguous (multiple pattern matches) or \
    no match is found.
        """
    ...
    se_name}'). Found multiple pattern matches in {self.tools_directory} {found_pattern_matches}. Please provide a more specific name or direct path.")
                    return None
    ...
    n file in '{self.tools_directory}' using supported conventions, nor as a direct path.")
            return None
