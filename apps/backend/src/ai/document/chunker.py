...
from ai.data_eng.chunk import split_paragraphs, split_sections, split_sentences
...
class DocumentChunker:
    def chunk(self, text: str, source: str = "") -> ChunkTree:
        ...
        for sec_header, sec_text, sec_level in sections:
            ...
            for para_text in split_paragraphs(sec_text):
                para = Paragraph(text=para_text)
                para.sentences = split_sentences(para_text)
    ...
    def _split_sections(self, text: str) -> List[tuple[str, str, int]]:
        """Delegate to the single shared implementation (data_eng.chunk)."""
