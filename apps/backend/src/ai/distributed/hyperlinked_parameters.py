...
class HyperlinkedParameterCluster:
    ...
    async def register_parameter(
        ...
        try:
            ...
            if parameter_id in self.parameter_metadata:
                logger.warning(
                    f"[{self.cluster_id}] Parameter {parameter_id} already exists, will be overwritten"
                )
    ...
    async def create_link(
        ...
        try:
            ...
            logger.info(
                f"[{self.cluster_id}] Created link: {source_id} -> {target_id} ({link_type})"
            )
            return True
    ...
    async def update_parameter(
        self, parameter_id: str, new_data: Any, new_version: str = None
    ) -> bool:
        """Update a parameter"""
        try:
            if parameter_id not in self.parameter_metadata:
    ...
    async def _prepare_parameter(
        self, parameter_id: str, parameter_data: Any, strategy: LoadingStrategy
    ):
    ...
    async def _ensure_memory_available(self):
        ...
        while self.current_memory_usage > self.memory_limit_bytes * 0.8:
            ...
            least_used_id = min(
                self.parameters.keys(), key=lambda pid: self.parameter_metadata[pid].access_count
            )
            ...
            logger.info(
                f"[{self.cluster_id}] Memory insufficient, unloading parameter: {least_used_id}"
            )
    ...
    async def _predict_related_parameters(self, parameter_id: str) -> List[str]:
        ...
        if parameter_id in self.parameter_links:
            ...
            links = sorted(
                self.parameter_links[parameter_id], key=lambda l: l.strength, reverse=True
            )
