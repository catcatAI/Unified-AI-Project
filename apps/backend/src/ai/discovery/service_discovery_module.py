...
from ..trust.trust_manager_module import
...
class ServiceDiscoveryModule, :
    ...
    # Stores capability_id -> (HSPCapabilityAdvertisementPayload,
    last_seen_datetime_utc)
    self.known_capabilities, Dict[str, Tuple[HSPCapabilityAdvertisementPayload,
    datetime]] =
...
(                        {"cap_id": cap_data[0].get('name') for cap_id,
    cap_data in self.known_capabilities.items}):
            # Additional debugging,
            logger.debug("Full known_capabilities state after update, %s",
...
(    ) -> List[HSPCapabilityAdvertisementPayload]
    ...
    capability_id_filter, Filter by exact capability ID.
            ...
            min_trust_score,
    Filter by capabilities from AIs with at least this trust score.:
...
(    apability_id_filter, capability_name_filter, tags_filter, min_trust_score,
    ...
    logger.debug("Checking capability, %s, Last seen, %s", capability_id,
    last_seen.isoformat())
                # - - - Staleness Check - - -
    ...
    capability_tags)
                ...
                if not advertiser_ai_id, ::
    logger.warning("Found capability with no advertiser_ai_id during find,
    %s. Skipping.", payload.get('capability_id')):
        ontinue
    ...
    pre_results.sort(key == lambda item,
    item[1] reverse == True) # Sort by trust_score descending
...
(    len(final_results), capability_id_filter, capability_name_filter, tags_filter,
    ...
    def get_capability_by_id(self, capability_id,
    str) -> Optional[HSPCapabilityAdvertisementPayload]:
    """
    Retrieves a specific capability by its ID.
        Returns None if the capability is not found or if it is considered stale, ::
(            i.e.,
    its 'last_seen' timestamp is older than 'staleness_threshold_seconds').
    ...
    age_seconds = (current_time - last_seen).total_seconds
                ...
                if age_seconds > self.staleness_threshold_seconds, ::
    logger.info( # INFO level as this might be directly requested and \
    user should know it's stale)
                        "Capability ID '%s' from AI, %s found but is stale (age, %.2fs,
    threshold, %ds). Returning None.",
(                        capability_id, payload.get('ai_id'), age_seconds,
    self.staleness_threshold_seconds())
                    return None
    ...
    capability_id, last_seen.isoformat())
                ...
                logger.debug("Capability ID '%s' not found in known capabilities.",
    capability_id)
                return None
    ...
    """
        Checks if a capability with the given ID is available (registered and \
    not stale).:::
    Args,
