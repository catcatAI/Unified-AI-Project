...
class HAMCoreStorage:
    ...
    def _simulate_disk_lag_and_check_limit(self) -> bool:
        ...
        if current_usage_gb >= total_simulated_disk_gb:
            logger.critical(
                f"HAM: Simulated disk full! Usage: {current_usage_gb:.2f}GB, Limit: {total_simulated_disk_gb:.2f}GB. Save operation aborted."
            )
        ...
        if current_usage_gb >= critical_thresh_gb:
            ...
            logger.warning(
                f"HAM: Simulated disk usage ({current_usage_gb:.2f}GB) is at CRITICAL level ( > {critical_thresh_gb:.2f}GB). Simulating {lag_to_apply_seconds:.2f}s lag."
            )
        elif current_usage_gb >= warning_thresh_gb:
            lag_factor = disk_config.get("lag_factor_warning", 1.0)
            lag_to_apply_seconds = base_delay * lag_factor
            logger.info(
                f"HAM: Simulated disk usage ({current_usage_gb:.2f}GB) is at WARNING level ( > {warning_thresh_gb:.2f}GB). Simulating {lag_to_apply_seconds:.2f}s lag."
            )
    ...
    def _save_core_memory_to_file(
        self, core_memory_store: Dict[str, Any], next_memory_id: int, fernet: Optional[Any]
    ) -> bool:
            ...
            with open(self.core_storage_filepath, "w", encoding="utf-8") as f:
                ...
                for mem_id, data_pkg in core_memory_store.items():
                    serializable_store[mem_id] = {
                        ...
                        "encrypted_package_b64": data_pkg["encrypted_package"].decode(
                            "latin-1"
                        ),  # latin-1 for bytes
                        "metadata": data_pkg.get("metadata"),
                    }
                json.dump(
                    {"next_memory_id": next_memory_id, "store": serializable_store}, f, indent=2
                )
    ...
    def _load_core_memory_from_file(
        self, core_memory_store: Dict[str, Any], next_memory_id: int, fernet: Optional[Any]
        ...
        try:
            ...
            logger.info(
                f"Core memory loaded from {self.core_storage_filepath}. Next ID: {next_memory_id}"
            )
