...
class MultimodalRAGEngine:
    ...
    def index_from_entry(self, latent: List[float], entry: Dict[str, Any]) -> None:
    ...
    def query_by_image(self, image_data: bytes, top_k: int = 5) -> List[Dict[str, Any]]:
    ...
    def query_by_audio(self, audio_data: bytes, top_k: int = 5) -> List[Dict[str, Any]]:
    ...
    def query_by_latent(self, latent: List[float], top_k: int = 5) -> List[Dict[str, Any]]:
        """Query by raw latent vector."""
        return self._retriever.search_by_list(latent, top_k=top_k)

    # --- ED3N integration ---

    def to_ed3n_entries(self, results: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
