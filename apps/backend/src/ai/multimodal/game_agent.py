...
from .game_structs import GameState, VisualObservation, Proprioception, TaskProgress, StrategyDirective
from .foveated_sampler import FoveatedSampler, SamplingConfig, create_sampler_from_config
from .game_policy import GamePolicy, PolicyConfig, PolicyOutput
from .skill_selector import SkillSelector, SelectorConfig
from .game_task_executor import GameTaskExecutor, ExecutorConfig, SkillContext
from .game_planner import GamePlanner, PlannerConfig, PlanningContext, GoalManager, GoalType
...
class GameAgentConfig:
    ...
    vision_fps: int = 20
    vision_input_size: List[int] = field(default_factory=lambda: [64, 64])
    sampling_strategy: str = "log_polar"
...
class GameAgent:
    ...
    def __init__(self, config: Optional[GameAgentConfig] = None):
        ...
        # 核心組件
        self._sampler: Optional[FoveatedSampler] = None
        self._policy: Optional[GamePolicy] = None
        self._selector: Optional[SkillSelector] = None
        self._executor: Optional[GameTaskExecutor] = None
        self._planner: Optional[GamePlanner] = None
        ...
        self._luanti: Optional[LuantiConnector] = None
    ...
    def _init_components(self):
        ...
        sampler_config = SamplingConfig(
            ...
            output_size=tuple(self.config.vision_input_size),
    ...
    def _get_focus_point(self) -> Tuple[int, int]:
        ...
        if self._executor._current and self._executor._current.subgoal.params.get("target_xy"):
            return self._executor._current.subgoal.params["target_xy"]
    ...
    async def run(self):
        ...
        try:
            while self._running:
                ...
                skill_ctx = self._executor.tick(self._current_skill_result, self._state, self._state.visual.features if self._state.visual else np.zeros(128))
                ...
                # 7. 合成動作
                actions = self._compose_actions(policy_out, skill_ctx, skill_params if 'skill_params' in locals() else None)
    ...
    def _compose_actions(self, policy_out: PolicyOutput, skill_ctx: SkillContext, 
                         skill_params: Optional[Any] = None) -> Dict[str, Any]:
        """合成最終動作"""
        actions = {}
        ...
        if skill_params:
            skill_actions = self._policy.get_action_dict(
                type('obj', (object,), {
                    ...
                    'discrete_logits': np.array([1.0 if k in skill_params.discrete_triggers else -1.0 
                                                  for k in ['attack','use','jump','sprint','sneak','inventory','craft_grid','craft_output']])
                })()
            )
    ...
    async def _send_actions(self, actions: Dict[str, Any]):
        ...
        if "click_xy" in actions:
            # 點擊特定座標 (需 CSM 支援)
            await self._luanti.send_action({"type": "click", "x": actions["click_xy"][0], "y": actions["click_xy"][1]})
    
    def _create_skill_result(self, actions: Dict, policy_out: PolicyOutput) -> Any:
        """建立技能結果 (下一幀給 L2 用)"""
        # 簡化：實際應從 Luanti 回應解析
        from .game_structs import SkillResult, SkillID
        return SkillResult(
    ...
    async def _store_experience(self):
        ...
        exp = GameExperience(
            ...
            timestamp=time.time(),
            position=self._state.proprioception.position if self._state.proprioception else (0,0,0),
            action="tick",
            context={"tick": self._tick, "goals": [g for g,_ in self._goal_manager.active_goals]},
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "state": self._executor._state.value if self._executor else "unknown",
            "current_subgoal": self._executor.get_current_subgoal().subgoal_id if self._executor and self._executor.get_current_subgoal() else None,
            "queue_length": len(self._executor._queue) if self._executor else 0,
