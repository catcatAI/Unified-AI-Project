...
class GameStrategy:
    ...
    def _record_anomaly(self, reason: str, data: Dict):
        ...
        if self._anomaly_patterns[key] >= 3:
            logger.warning(f"Repeated anomaly pattern detected: {key} (count: {self._anomaly_patterns[key]})")
    ...
    def _adjust_weights(self):
        """根據統計調整權重"""
        # 計算關鍵指標
        death_rate = self.stats.death_count / max(self.stats.duration_min / 30.0, 1.0)  # 每30分鐘死亡率
        ...
        goal_success_rate = self.stats.goals_completed / max(self.stats.goals_completed + self.stats.goals_failed, 1)
        ...
        if death_rate > 0.5:  # 超過 0.5 次/30分鐘
            self.weights.risk_tolerance = max(self.config.min_risk, self.weights.risk_tolerance - self.config.death_penalty)
            self.weights.survival_priority = min(1.0, self.weights.survival_priority + self.config.death_penalty)
            self.weights.exploration_weight = max(self.config.min_exploration, self.weights.exploration_weight - self.config.death_penalty * 0.5)
            adjustments.append(f"death_rate={death_rate:.2f} -> lower risk")
        
        # 2. 異常/卡住頻繁 -> 降低探索、增加保守
        if anomaly_rate > 1.0 or stuck_rate > 1.0:
            self.weights.exploration_weight = max(self.config.min_exploration, self.weights.exploration_weight - self.config.anomaly_penalty)
            self.weights.risk_tolerance = max(self.config.min_risk, self.weights.risk_tolerance - self.config.anomaly_penalty)
            adjustments.append(f"anomaly_rate={anomaly_rate:.2f}, stuck_rate={stuck_rate:.2f} -> more conservative")
        
        # 3. 目標成功率低 -> 降低風險
        if goal_success_rate < 0.5 and (self.stats.goals_completed + self.stats.goals_failed) > 5:
            self.weights.risk_tolerance = max(self.config.min_risk, self.weights.risk_tolerance - 0.1)
        ...
        if resource_rate < 5.0 and self.stats.duration_min > 5:  # 少於 5 個/分鐘
            self.weights.exploration_weight = min(self.config.max_exploration, self.weights.exploration_weight + self.config.success_bonus)
        ...
        if adjustments:
            logger.info(f"Strategy adjusted: {'; '.join(adjustments)} | weights: {self.weights.to_dict()}")
    ...
    def _compute_forbidden_actions(self) -> List[str]:
        ...
        if self.weights.risk_tolerance < 0.3:
            forbidden.extend(["deep_mining", "cave_exploration", "night_surface_travel", "boss_fight"])
