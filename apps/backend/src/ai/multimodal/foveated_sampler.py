...
class FoveatedSampler:
    ...
    def sample(self, frame: np.ndarray, focus_xy: Optional[Tuple[int, int]] = None) -> SamplingResult:
        """
        執行非均勻採樣
    ...
    def _log_polar_mapping(self, h: int, w: int, focus_xy: Tuple[int, int]) -> np.ndarray:
        ...
        r_transformed = np.where(
            r < fovea_r,
            r * scale,  # 焦點內線性放大
            fovea_r * scale + (np.log(r - fovea_r + 1) / np.log(2 - fovea_r + 1)) * (1 - fovea_r * scale)
        )
    ...
    def map_to_original(self, output_xy: Tuple[int, int], inverse_map: np.ndarray) -> Tuple[int, int]:
        """
        將輸出張量座標映射回原圖座標
