...
from .game_structs import (
    SkillID, SkillSpec, SkillParams, SkillContext, SkillTrigger,
    SkillResult, GAME_SKILLS, check_preconditions, get_available_skills,
    GameState, Proprioception
)
...
class SkillSelector:
    ...
    def _init_embeddings(self):
        ...
        for skill_id in SkillID:
            self._skill_embeddings[skill_id] = np.random.randn(self.config.skill_embedding_dim).astype(np.float32)
            # 基礎優先級
            base_priority = {
                SkillID.EAT: 1.0,
                SkillID.COMBAT: 0.9,
    ...
    def select(self, 
               latent: np.ndarray,
               state: GameState,
               context: Optional[SkillContext] = None) -> SkillParams:
        """
        選擇技能並輸出參數
    ...
    def _score_skill(self, skill_id: SkillID, latent: np.ndarray, state: GameState) -> SkillScore:
        ...
        sim = np.dot(latent[:self.config.skill_embedding_dim], skill_emb) / (
            np.linalg.norm(latent[:self.config.skill_embedding_dim]) * np.linalg.norm(skill_emb) + 1e-8
        )
        ...
        # 綜合分數
        raw_score = sim * 0.4 + priority * 0.3 + state_bonus * 0.2 + continuity_bonus * 0.1 - cooldown_penalty
    ...
    def _state_relevance(self, skill_id: SkillID, state: GameState) -> float:
        ...
        if skill_id == SkillID.EAT:
            ...
            # 手持工具且面前有可挖方塊
            if "pickaxe" in prop.wielded_item or "axe" in prop.wielded_item or "shovel" in prop.wielded_item:
                bonus += 0.2
    ...
    def _estimate_params(self, skill_id: SkillID, state: GameState) -> Dict[str, Any]:
        ...
        if skill_id == SkillID.MOVE:
            ...
            params = {"target_entity": "nearest_hostile", "weapon": prop.wielded_item, "tactic": "melee"}
        
        elif skill_id == SkillID.NAVIGATE:
            params = {"target_pos": None, "path": None}
        
        elif skill_id == SkillID.EAT:
            food = next((k for k in prop.inventory if any(f in k for f in ["apple", "bread", "meat", "carrot"])), "apple")
    ...
    def _parameterize_skill(self, skill_id: SkillID, base_params: Dict, state: GameState, latent: np.ndarray) -> SkillParams:
        """將技能參數化為 L0 可用格式"""
...
# 使用相對索引 (每個技能自己的 continuous_dim 範圍)
        ...
        if skill_id == SkillID.MOVE:
            ...
            if params.get("jump"): discrete_triggers["jump"] = 1.0
            if params.get("sprint"): discrete_triggers["sprint"] = 1.0
            if params.get("sneak"): discrete_triggers["sneak"] = 1.0
        ...
        for skill_id in available:
            ...
            if score.confidence > 0.2:
                triggers.append(SkillTrigger(
                    ...
                    params=score.params,
                    urgency=score.confidence * (1.0 if skill_id in [SkillID.EAT, SkillID.COMBAT] else 0.5)
                ))
