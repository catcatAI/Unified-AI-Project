...
class GameMemoryBridge:
    ...
    def _init_default_recipes(self):
        ...
        default_recipes = {
            ...
            "stone_pickaxe": RecipeMemory("stone_pickaxe", {"cobblestone": 3, "stick": 2}, "stone_pickaxe", 1),
            "stone_axe": RecipeMemory("stone_axe", {"cobblestone": 3, "stick": 2}, "stone_axe", 1),
            "stone_shovel": RecipeMemory("stone_shovel", {"cobblestone": 1, "stick": 2}, "stone_shovel", 1),
            "stone_sword": RecipeMemory("stone_sword", {"cobblestone": 2, "stick": 1}, "stone_sword", 1),
            ...
            "iron_ingot": RecipeMemory("iron_ingot", {"iron_ore": 1}, "iron_ingot", 1, station="furnace"),
            "iron_pickaxe": RecipeMemory("iron_pickaxe", {"iron_ingot": 3, "stick": 2}, "iron_pickaxe", 1),
        }
    ...
    def find_nearest_resource(self, position: Tuple[float, float, float], 
                              resource: str, max_dist: float = 100) -> Optional[SpatialMemory]:
        """尋找最近的資源點"""
        best = None
        best_dist = float('inf')
    ...
    def find_safe_location(self, position: Tuple[float, float, float], 
                           radius: float = 50) -> Optional[SpatialMemory]:
        """尋找安全位置 (低危險、高訪問)"""
        best = None
        best_score = -1
    ...
    def mark_base(self, position: Tuple[float, float, float]):
        ...
        if loc_id not in self._spatial_index:
            self._spatial_index[loc_id] = SpatialMemory(
                location_id=loc_id, position=position, node_type="base",
                resources={}, last_visited=time.time()
            )
