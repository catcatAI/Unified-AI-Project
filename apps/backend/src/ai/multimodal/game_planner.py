...
from .game_structs import (
    SkillID, Subgoal, PlanDAG, TaskProgress, GameState, GAME_SKILLS,
    StrategyDirective
)
...
class GamePlanner:
    ...
    def _build_recipe_graph(self) -> Dict[str, Dict]:
        ...
        return {
            ...
            "stone_pickaxe": {"ingredients": {"cobblestone": 3, "stick": 2}, "unlocks": ["stone_axe", "stone_shovel", "stone_sword"]},
    ...
    def _build_goal_templates(self) -> Dict[GoalType, List[Dict]]:
        ...
        return {
            GoalType.SURVIVAL: [
                {"skill": SkillID.EAT, "params": {}, "preconditions": ["hunger_low", "has_food"], "criteria": "hunger_restored"},
                {"skill": SkillID.CRAFT, "params": {"recipe_id": "wooden_pickaxe"}, "preconditions": ["has_wood"], "criteria": "inventory_changed:wooden_pickaxe"},
                {"skill": SkillID.DIG, "params": {"target_node": "tree"}, "preconditions": ["has_tool:pickaxe"], "criteria": "inventory_changed:wood"},
            ],
            GoalType.TOOL: [
                {"skill": SkillID.CRAFT, "params": {"recipe_id": "wooden_pickaxe"}, "preconditions": ["has_wood>=3"], "criteria": "inventory_changed:wooden_pickaxe"},
                {"skill": SkillID.DIG, "params": {"target_node": "stone"}, "preconditions": ["has_tool:pickaxe"], "criteria": "inventory_changed:cobblestone"},
                {"skill": SkillID.CRAFT, "params": {"recipe_id": "stone_pickaxe"}, "preconditions": ["has_cobblestone>=3", "has_stick>=2"], "criteria": "inventory_changed:stone_pickaxe"},
            ],
            GoalType.RESOURCE: [
                {"skill": SkillID.NAVIGATE, "params": {"target_pos": "resource_area"}, "preconditions": [], "criteria": "reached_target"},
                {"skill": SkillID.DIG, "params": {"target_node": "auto"}, "preconditions": ["has_tool:pickaxe"], "criteria": "inventory_changed:*"},
            ],
            GoalType.BUILD: [
                {"skill": SkillID.NAVIGATE, "params": {"target_pos": "build_site"}, "preconditions": [], "criteria": "reached_target"},
                {"skill": SkillID.BUILD, "params": {"blueprint": "shelter_5x5x3"}, "preconditions": ["has_materials"], "criteria": "blueprint_complete"},
            ],
            GoalType.EXPLORE: [
                {"skill": SkillID.NAVIGATE, "params": {"target_pos": "unexplored"}, "preconditions": [], "criteria": "reached_target"},
                {"skill": SkillID.LOOK, "params": {"target": "scan"}, "preconditions": [], "criteria": "scanned"},
            ],
        }
    ...
    async def propose_plan(self, context: PlanningContext) -> PlanDAG:
        ...
        if estimated_steps <= self.config.complexity_threshold or not self.config.use_llm_for_complex:
    ...
    def _add_strategy_subgoals(self, nodes: Dict, edges: List, context: PlanningContext):
        ...
        if strat.exploration_weight > 0.7:
            ...
            # 連接到最後一個資源收集節點
            last_resource = max([n for n in nodes if nodes[n].skill_id == SkillID.DIG], default=None)
            if last_resource:
    ...
    async def _llm_assisted_plan(self, context: PlanningContext, plan_id: str) -> PlanDAG:
        ...
        llm_ctx = PlanContext(
            ...
            inventory=context.state.proprioception.inventory if context.state.proprioception else {},
            position=dict(x=context.state.proprioception.position[0], y=context.state.proprioception.position[1], z=context.state.proprioception.position[2]) if context.state.proprioception else {},
            nearby_entities=[],
        ...
        try:
            proposal = await asyncio.wait_for(
                self._llm.apropose_plan(llm_ctx),
...
class GoalManager:
    ...
    async def plan_for_current(self, state: GameState, strategy: StrategyDirective) -> Optional[PlanDAG]:
        """為當前最高優先級目標規劃"""
