...
class PrimitiveLibrary:
    ...
    def add_primitive(self, name: str, params: DrawingInstructions, 
        ...
        if len(self._primitives) >= self._max_primitives:
            logger.warning("PrimitiveLibrary full (%d/%d)", 
                          len(self._primitives), self._max_primitives)
            return False
        ...
        if embedding.shape != (self._embedding_dim,):
            logger.warning("Embedding shape mismatch: expected (%d,), got %s",
                          self._embedding_dim, embedding.shape)
            return False
    ...
    def find_similar(self, embedding: np.ndarray, top_k: int = 5) -> List[Tuple[str, float]]:
        ...
        lib_norm = self._embeddings / (np.linalg.norm(self._embeddings, axis=1, keepdims=True) + 1e-8)
    ...
    def auto_expand(self, embedding: np.ndarray, params: DrawingInstructions,
        ...
        for name, prim in self._primitives.items():
            ...
            data["primitives"][name] = {
                "params": {
                    ...
                    "lines": [(l.start.x, l.start.y, l.end.x, l.end.y, l.width, l.color) 
                             for l in params.lines],
                    "planes": [([p.x for p in pl.points], [p.y for p in pl.points],
                               pl.fill_color, pl.outline_color, pl.outline_width)
