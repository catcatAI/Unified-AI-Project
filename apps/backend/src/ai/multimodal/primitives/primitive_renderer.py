...
from .primitive_types import DrawingInstructions, Point, Line, Plane
...
class PrimitiveRenderer:
    ...
    def render(self, instructions: DrawingInstructions) -> Image.Image:
        ...
        Args:
            instructions: DrawingInstructions to render
    ...
    def _draw_line(self, draw: ImageDraw, line: Line):
        """Draw a line."""
        start = (
            int(line.start.x * self._canvas_size[0]),
            int(line.start.y * self._canvas_size[1])
        )
        end = (
            int(line.end.x * self._canvas_size[0]),
    ...
    def _draw_plane(self, draw: ImageDraw, plane: Plane):
        ...
        if not plane.points:
            return
        points = [
            (int(p.x * self._canvas_size[0]), int(p.y * self._canvas_size[1]))
