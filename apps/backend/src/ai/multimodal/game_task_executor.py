...
from .game_structs import (
    ...
    Subgoal, TaskProgress, PlanDAG, GameState, GAME_SKILLS,
    check_preconditions, get_available_skills
)
...
class GameTaskExecutor:
    ...
    def __init__(self, config: Optional[ExecutorConfig] = None):
        ...
        self._replan_callback: Optional[Callable[[str, str], None]] = None  # (task_id, blocker) -> None
    ...
    def load_plan(self, plan: PlanDAG):
        ...
        self._task_progress = TaskProgress(
            ...
            remaining=list(plan.nodes.keys()),
            completed=[]
        )
    ...
    def load_subgoals(self, subgoals: List[Subgoal], task_id: str = ""):
        ...
        for sg in subgoals:
            self._queue.append(ActiveSubgoal(subgoal=sg))
        self._task_progress = TaskProgress(task_id=task_id, remaining=[sg.subgoal_id for sg in subgoals])
        self._state = ExecutorState.EXECUTING
    
    def tick(self, skill_result: Optional[SkillResult], state: GameState, latent: np.ndarray) -> Optional[SkillContext]:
        """
        主迴圈：每幀調用一次
    ...
    def _start_subgoal(self, active: ActiveSubgoal):
        ...
        logger.info(f"Started subgoal: {active.subgoal.subgoal_id} ({active.subgoal.skill_id.value})")
    
    def _generate_skill_params(self, active: ActiveSubgoal, state: GameState, latent: np.ndarray) -> SkillParams:
    ...
    def _handle_failure(self, state: GameState, latent: np.ndarray) -> Optional[SkillContext]:
        ...
        if self._current.retry_count < self.config.max_subgoal_retries and self.config.enable_fallback:
            self._current.retry_count += 1
            logger.warning(f"Retrying subgoal {self._current.subgoal.subgoal_id} (attempt {self._current.retry_count})")
    ...
    def _handle_stuck(self, state: GameState, latent: np.ndarray) -> Optional[SkillContext]:
        """處理卡住"""
        logger.warning(f"Stuck detected at subgoal {self._current.subgoal.subgoal_id if self._current else 'unknown'}")
        
        # 嘗試簡單恢復：隨機移動、跳躍、轉向
        recovery_sg = Subgoal(
            subgoal_id=f"recovery_{self._tick}",
            skill_id=SkillID.MOVE,
            params={"forward": 0.5, "strafe": np.random.uniform(-1, 1), "yaw": np.random.uniform(-1, 1)},
            preconditions=[],
    ...
    def _check_progress(self, state: GameState):
        ...
        # 更新任務進度
        self._task_progress.eta_ticks = int(
            (len(self._queue) + 1) * 300  # 粗略估算
        )
