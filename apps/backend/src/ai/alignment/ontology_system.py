...
class OntologySystem:
    ...
    def register_entity(self, entity: Entity) -> bool:
        ...
        self.knowledge_graph[entity.entity_id] = {
            ...
            "name": entity.name,
            "existence_levels": [level.value for level in entity.existence_levels],
        }
    ...
    def add_relationship(self, relationship: Relationship) -> bool:
        ...
        # 更新知识图谱
        if relationship.source_entity not in self.knowledge_graph:
            self.knowledge_graph[relationship.source_entity] = {}
        if relationship.target_entity not in self.knowledge_graph:
            self.knowledge_graph[relationship.target_entity] = {}

        self.knowledge_graph[relationship.source_entity][relationship.relationship_id] = {
    ...
    def query_existence(self, entity_id: str, aspect: str = "all") -> Dict[str, Any]:
        ...
        if aspect == "all" or aspect == "existence_levels":
            result["existence_levels"] = [level.value for level in entity.existence_levels]
    ...
    def update_entity_properties(self, entity_id: str, properties: Dict[str, Any]) -> bool:
        ...
        if entity_id in self.knowledge_graph:
            self.knowledge_graph[entity_id].update(properties)
    ...
    def _validate_relationship_consistency(self, relationship: Relationship) -> bool:
        ...
        if relationship.relationship_type == RelationshipType.CONTRADICTS:
            source = self.entities.get(relationship.source_entity)
            target = self.entities.get(relationship.target_entity)
    ...
    def _calculate_coherence_score(self) -> float:
            ...
            for node in all_nodes:
                ...
                for rel in self.relationships.values():
                    if rel.source_entity == node:
                        connected.add(rel.target_entity)
                    if rel.target_entity == node:
                        connected.add(rel.source_entity)

            connectivity_score = len(connected) / len(all_nodes)
        ...
        if self.relationships:
            ...
            if strengths:
                ...
                strength_consistency = 1.0 - min(1.0, math.sqrt(variance))
            else:
    ...
    def _calculate_completeness_score(self) -> float:
        ...
        type_completeness = (
            len(actual_types & ideal_types) / len(ideal_types) if ideal_types else 1.0
        )
        ...
        relationship_completeness = (
            len(actual_relationship_types & ideal_relationship_types)
            / len(ideal_relationship_types)
            if ideal_relationship_types
            else 1.0
        )
    ...
    def _calculate_stability_score(self) -> float:
        ...
        if self.entities:
            ...
            if entity_ages:
                ...
                mean_age = sum(entity_ages) / len(entity_ages)
                variance = sum((age - mean_age) ** 2 for age in entity_ages) / len(entity_ages)
                age_stability = 1.0 - min(1.0, math.sqrt(variance) / (30 * 24 * 3600))
            else:
                age_stability = 1.0
        else:
            age_stability = 1.0
        ...
        if self.relationships:
            ...
            if strengths:
                ...
                mean_strength = sum(strengths) / len(strengths)
                variance = sum((s - mean_strength) ** 2 for s in strengths) / len(strengths)
                strength_stability = 1.0 - min(1.0, math.sqrt(variance))
            else:
        ...
        for rel in self.relationships.values():
            if rel.relationship_type == RelationshipType.REQUIRES:
                if self._would_create_cycle(rel.source_entity, rel.target_entity):
                    conflicts.append(f"循环依赖: {rel.source_entity} -> {rel.target_entity}")
    ...
    def _would_create_cycle(self, source: str, target: str) -> bool:
        ...
        while stack:
            ...
            for rel in self.relationships.values():
                if rel.relationship_type == RelationshipType.REQUIRES:
                    if rel.source_entity == current and rel.target_entity not in visited:
                        stack.append(rel.target_entity)
