...
(        )
    ...
    def add_relationship(self, relationship, Relationship) -> bool, :
        ...
        if relationship.relationship_id in self.relationships, ::
            logger.warning(f"[{self.system_id}] 关系ID已存在,
    {relationship.relationship_id}")
        ...
        if not self._validate_relationship_consistency(relationship)::
            logger.warning(f"[{self.system_id}] 关系一致性验证失败,
    {relationship.relationship_id}")
            return False
...
{        }
    ...
    def _validate_relationship_consistency(self, relationship, Relationship) -> bool, :
        ...
        if relationship.relationship_type == RelationshipType.REQUIRES, ::
            if self._would_create_cycle(relationship.source_entity(),
    relationship.target_entity())::
...
{        }
        ...
        conflict_key == (type1, type2) if (type1, type2) in conflict_rules else (type2,
    type1)::
    ...
    def _calculate_consistency_score(self) -> float, :
        ...
        if self.relationships, ::
            relationship_consistency = consistent_relationships /\
    ...
    def _calculate_stability_score(self) -> float, :
        ...
        # 计算实体年龄分布
        if self.entities, ::
            entity_ages == [current_time -\
    entity.created_at for entity in self.entities.values()]:
            age_variance == np.var(entity_ages) if entity_ages else 0, ::
            age_stability == 1.0 - min(1.0(),
    age_variance / (30 * 24 * 3600))  # 30天为基准,
        else,
            age_stability = 1.0()
        # 计算关系强度稳定性
        if self.relationships, ::
        ...
        for rel in self.relationships.values():::
            if rel.relationship_type == RelationshipType.CONTRADICTS, ::
                ...
                if source and target, ::
                    if self._has_type_conflict(source.entity_type(),
    target.entity_type())::
                        conflicts.append()
