...
class EmotionSystem:
    ...
    def _calculate_overall_value(self, value_scores: Dict[ValueDimension, float]) -> float:
        ...
        weighted_sum = sum(
            score * self.value_weights.get(dim, 1.0) for dim, score in value_scores.items()
        )
    ...
    def _calculate_value_confidence(
        self, action: Dict[str, Any], context: Dict[str, Any], emotional_state: EmotionalState
    ) -> float:
        """计算价值评估置信度"""
    ...
    def _calculate_empathy_score(self, predicted_emotion: EmotionalState) -> float:
        ...
        base_empathy = 0.5
        valence_boost = (
            abs(predicted_emotion.valence) * 0.4
            if predicted_emotion.valence < 0
            else predicted_emotion.valence * 0.2
        )
        arousal_boost = predicted_emotion.arousal * 0.3
        return max(0.0, min(1.0, base_empathy + valence_boost + arousal_boost))

    def _calculate_compassion_level(
        self, predicted_emotion: EmotionalState, context: Dict[str, Any]
    ) -> float:
        """计算同情心水平"""
        factors = {
    ...
    def _generate_empathetic_response(
        self, predicted_emotion: EmotionalState, compassion_level: float
    ) -> str:
        """生成共情回应建议"""
        templates = {
