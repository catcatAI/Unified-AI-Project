...
# Assuming these are in the same directory or properly accessible
try:
...
class AlignmentResult:
    ...
    reasoning: str  # 推理过程
    emotional_state: Any  # 情感状态 (EmotionalState)
...
class AlignmentManager:
    ...
    async def make_decision(
        ...
        """
        logger.info(
            f"[{self.system_id}] Making alignment decision for context: {context.get('type', 'unknown')}"
        )
        # 1. 获取三大系统的评估
        ...
        if self.adversarial_mode:
            ethical_assessments, emotional_assessments, existential_assessments = (
                await self._generate_adversarial_tests(
                    ...
                    existential_assessments,
                )
            )

        # 3. 应用决策论系统
        decision_data = await self._apply_decision_theory(
            context, options, ethical_assessments, emotional_assessments, existential_assessments
        )

        # 4. 验证对齐质量
        alignment_result = await self._validate_alignment(decision_data, context)

        # 5. 记录决策历史
        self.decision_history.append(alignment_result)

        logger.info(
            f"[{self.system_id}] Decision made with confidence {alignment_result.confidence:.2f}"
        )
        return alignment_result

    async def _get_ethical_assessments(
        self, context: Dict[str, Any], options: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        """获取伦理评估"""
        assessments = []
        ...
        for option in options:
            ...
            assessments.append({"option": option, "assessment": assessment})

        return assessments

    async def _get_emotional_assessments(
        self, context: Dict[str, Any], options: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        """获取情感评估"""
        assessments = []
    ...
    async def _get_existential_assessments(
        self, context: Dict[str, Any], options: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        """获取存在评估"""
        assessments = []
            ...
            assessments.append(
                {
                    ...
                    "worldview_consistency": {"consistency_score": worldview_consistency},
                }
            )
    ...
    async def _generate_adversarial_tests(
        ...
        existential_assessments: List[Dict[str, Any]],
    ) -> Tuple[List, List, List]:
        """生成对抗性测试"""
        logger.info(
            f"[{self.system_id}] Generating adversarial tests with intensity {self.adversarial_intensity}"
        )
    ...
    async def _generate_ethical_adversaries(
        self,
        context: Dict[str, Any],
        options: List[Dict[str, Any]],
        ethical_assessments: List[Dict[str, Any]],
        ...
        for i, assessment in enumerate(ethical_assessments):
            ...
            adversarial_assessments.append(
                {"option": adversarial_option, "assessment": adversarial_assessment}
            )

        return adversarial_assessments

    async def _generate_emotional_adversaries(
        self,
        context: Dict[str, Any],
        options: List[Dict[str, Any]],
        emotional_assessments: List[Dict[str, Any]],
    ...
    async def _generate_existential_adversaries(
        self,
        context: Dict[str, Any],
        options: List[Dict[str, Any]],
        existential_assessments: List[Dict[str, Any]],
        ...
        for i, assessment in enumerate(existential_assessments):
            # 创建存在威胁：短期利益但长期存在风险
            adversarial_option = options[i].copy()
            adversarial_option["adversarial_threat"] = (
                "short_term_benefit_long_term_existential_risk"
            )

            # 降低存在一致性
            adversarial_assessment = assessment.copy()
            if "worldview_consistency" in adversarial_assessment:
                adversarial_assessment["worldview_consistency"]["consistency_score"] *= (
                    1.0 - self.adversarial_intensity * 0.3
                )
            adversarial_assessment["is_adversarial"] = True
    ...
    async def _apply_decision_theory(
        self,
        context: Dict[str, Any],
        options: List[Dict[str, Any]],
        ethical_assessments: List[Dict[str, Any]],
        ...
        for i, option in enumerate(options):
            ...
            existential_consistency = (
                existential_assessments[i]
                .get("worldview_consistency", {})
                .get("consistency_score", 0.5)
            )
            ...
            # 检查是否满足最低阈值
            meets_thresholds = (
                ethical_score >= self.balance_thresholds["ethical_min"]
                and emotional_score >= self.balance_thresholds["emotional_min"]
                and existential_consistency >= self.balance_thresholds["existential_min"]
            )
    ...
    async def _validate_alignment(
        ...
        """验证对齐质量"""
        best_option_data = decision_data["best_option"]

        # 计算置信度
        confidence = self._calculate_confidence(decision_data)

        # 生成推理过程
        reasoning = await self._generate_reasoning(decision_data, context)
        ...
        result = AlignmentResult(
            decision=best_option_data["option"],
            reasoning=reasoning,
            emotional_state=emotional_state,
            ethical_score=best_option_data["ethical_score"],
            existential_score=best_option_data["existential_score"],
            confidence=confidence,
            priority=decision_data["priority_used"],
        )
    ...
    async def _generate_reasoning(
        self, decision_result: Dict[str, Any], context: Dict[str, Any]
    ) -> str:
        """生成推理过程描述"""
    ...
    async def analyze_alignment_trends(self) -> Dict[str, Any]:
        ...
        # 计算平均分数
        avg_ethical = sum(r.ethical_score for r in self.decision_history) / len(
            self.decision_history
        )
        avg_emotional = sum(
            r.emotional_state.emotional_arousal for r in self.decision_history
        ) / len(self.decision_history)
        avg_existential = sum(r.existential_score for r in self.decision_history) / len(
            self.decision_history
        )
        avg_confidence = sum(r.confidence for r in self.decision_history) / len(
            self.decision_history
        )
        # 统计优先级使用情况
        priority_counts = {}
    ...
    async def self_improve(self):
        ...
        # 根据趋势调整参数
        if "average_scores" in trends and trends["average_scores"]["confidence"] < 0.7:
            # 如果置信度低, 调整阈值
        ...
        if "priority_usage" in trends:
            max_usage = 0
            total = 0
            if trends["priority_usage"]:  # Check if dictionary is not empty
                max_usage = max(trends["priority_usage"].values())
                total = sum(trends["priority_usage"].values())

            if total > 0 and max_usage / total > 0.7:  # 如果某种优先级使用超过70%
