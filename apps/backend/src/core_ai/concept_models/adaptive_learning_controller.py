...
class StrategySelector:
    ...
    async def select(self, task_context: TaskContext, 
        ...
        task_complexity = task_context.complexity_level
        
        # 定义决策阈值
        IMPROVING_THRESHOLD = 0.5  # 显著改善的阈值
        DEGRADING_THRESHOLD = 0.5  # 显著退化的阈值
        COMPLEX_TASK_THRESHOLD = 0.7  # 复杂任务阈值
        LOW_CONFIDENCE_THRESHOLD = 0.6  # 低置信度阈值
        
        if trend_direction == "improving":
            if trend_magnitude > IMPROVING_THRESHOLD and self.confidence_score < 0.9:
                # 显著改善且置信度不高，继续当前策略但考虑微调
                return "current_strategy"
            else:
                return "current_strategy"  # 继续当前策略
