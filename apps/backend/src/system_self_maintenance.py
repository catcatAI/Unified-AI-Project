...
class SystemSelfMaintenanceManager: # Added missing class definition
        ...
        try:
            ...
            if issues_to_repair:
                ...
                if HAS_INTEGRATION_MANAGER:
                    ...
                    self.logger.info(f"📊 统一修复統計, 嘗試 {total_attempts} 個, 成功 {completed} 個, 失敗 {failed} 個")
                ...
                # 如果集成管理器不可用, 使用增强版完整修复系统
                    ...
                    self.logger.info(f"📊 增强版修复统计, 尝试 {total_attempts} 個, 成功 {completed} 個, 失敗 {failed} 個")
    ...
    def _perform_intelligent_validation(self, repair_result: Dict, completed: int, failed: int):
        ...
        try:
            ...
            # 分析驗證結果
            valid_repairs = sum(1 for vr in validation_results if vr['validation_result'].get('overall_success', False))
    ...
    if start_self_maintenance("full"):
        ...
        try:
            ...
            while True:
                ...
                print(f"狀態: 運行中 = {status['is_running']} | 任務週期 = {status['stats']['total_maintenance_cycles']} | 修復成功 = {status['stats']['successful_repairs']}")
