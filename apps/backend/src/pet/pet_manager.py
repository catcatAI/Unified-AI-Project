...
class PetManager:
    ...
    def __init__(
        ...
        pet_id: str,
        config: Dict[str, Any],
        biological_integrator: Any = None,
        ...
        self.personality: Dict[str, Any] = self.config.get(
            "initial_personality", {"curiosity": 0.7, "playfulness": 0.8}
        )
        self.behavior_rules: Dict[str, Any] = self.config.get(
        ...
        logger.info(
            f"PetManager for pet '{self.pet_id}' initialized with improved state management."
        )
    ...
    def sync_with_biological_state(self):
        ...
        logger.debug(
            f"Pet '{self.pet_id}' synced with biological state. Happiness: {self.state['happiness']}"
        )
