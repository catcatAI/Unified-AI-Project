...
class LifeIntensityFormula:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.constraint_states: Dict[str, ConstraintState] = (
            {}
        )  # key: domain_name + constraint_type
    ...
    def calculate_m_f(self) -> float:
        ...
        sorted_observers = sorted(
            self.observers.values(), key=lambda o: o.calculate_mf_value(), reverse=True
        )
    ...
    def calculate_life_intensity(self) -> float:
        ...
        if self.knowledge_states:
            dominant_domain = max(
                self.knowledge_states.items(), key=lambda x: x[1].calculate_inf_value()
            )[0]
