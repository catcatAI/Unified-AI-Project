...
class PossibilityState:
    ...
    )  # Compatibility with other possibilities
...
class NonParadoxExistence:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        # Callbacks
        self._coexistence_callbacks: List[Callable[[str, bool], None]] = (
            []
        )  # (variable_id, activated)
    ...
    def update_resonance_weight(
        self, variable_id: str, possibility_id: str, new_weight: float
    ) -> bool:
    ...
    def calculate_coexistence_state(self, variable_id: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            "coherence": 1.0
            - (len(dominant) / len(gray_zone.possibilities)),  # Lower = more coherent
    ...
    def create_coexistence_field(self, variable_ids: List[str]) -> Optional[CoexistenceField]:
        ...
        field = CoexistenceField(field_id=field_id, gray_zones=valid_variables, stability=0.7)
    ...
    def get_non_paradox_summary(self) -> Dict[str, Any]:
        ...
        active_gray_zones = [gz for gz in self.gray_zones.values() if gz.coexistence_active]
        ...
        return {
            ...
            "gray_zones": {
                ...
                "by_type": {
                    vtype.name: sum(
                        1 for gz in self.gray_zones.values() if gz.variable_type == vtype
                    )
            ...
            "coexistence_fields": {
                ...
                "average_coherence": (
                    sum(f.coherence_score for f in self.coexistence_fields.values())
                    / len(self.coexistence_fields)
                    if self.coexistence_fields
                    else 0.0
                ),
            },
            "resonance": {
                "total_active_resonance": total_resonance,
                "average_per_variable": (
                    total_resonance / len(active_gray_zones) if active_gray_zones else 0.0
                ),
            },
            ...
            "active_states": [
                {
                    ...
                    "weights": {pid: p.resonance_weight for pid, p in gz.possibilities.items()},
                }
                for gz in active_gray_zones
            ],
        }
...
if __name__ == "__main__":
    ...
    emotional_gz = npe.create_gray_zone(
        GrayZoneVariableType.EMOTIONAL, "模糊的情感状态 - 对用户的复杂情感"
    )
    logger.info(f"  情感灰色区: {emotional_gz.variable_id}")

    identity_gz = npe.create_gray_zone(GrayZoneVariableType.IDENTITY, "流动性身份认知")
    logger.info(f"  身份灰色区: {identity_gz.variable_id}")

    cognitive_gz = npe.create_gray_zone(GrayZoneVariableType.COGNITIVE, "知识边界的不确定性")
