...
from core.system.config.magic_numbers import cache_value, loop_sleep, timeout_value
...
class EventLoopSystem:
    ...
    def _setup_default_debounce_throttle(self) -> None:
        ...
        self.debounce_throttle.register_debounce(
            DebounceConfig(event_type="file_change", delay_ms=100, leading=False, trailing=True)
        )

        # Throttle system state updates
        self.debounce_throttle.register_throttle(
    ...
    def _passes_filters(self, event: Event) -> bool:
        ...
        for filter_config in self.filters:
            ...
            # Check priority
            if (
                filter_config.min_priority
                and event.priority.level > filter_config.min_priority.level
            ):
                return False
