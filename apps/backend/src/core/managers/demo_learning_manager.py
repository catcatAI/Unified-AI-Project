...
class DemoLearningManager:
    ...
    def _load_config(self) -> Dict[str, Any]:
        ...
        try:
            if self.config_path.exists():
                ...
                return {}
        except Exception as e:  # broad exception acceptable: config load resilience
    ...
    async def activate_demo_mode(self, credentials: Dict[str, Any]):
        ...
        for action in actions:
            ...
            except Exception as e:  # broad exception acceptable: action execution resilience
    ...
    async def _learning_monitor_loop(self):
        ...
        while self.demo_mode:
            ...
            except Exception as e:  # broad exception acceptable: learning loop resilience
    ...
    async def _cleanup_monitor_loop(self):
        ...
        while self.demo_mode:
            ...
            except Exception as e:  # broad exception acceptable: cleanup loop resilience
    ...
    async def _collect_learning_data(self):
        ...
        try:
            ...
            await self._save_learning_data()
        except Exception as e:  # broad exception acceptable: learning data collection resilience
    ...
    async def _perform_cleanup(self, cleanup_config: Dict[str, Any]):
        ...
        except Exception as e:  # broad exception acceptable: cleanup operation resilience
...
# import psutil
        ...
        except Exception as e:  # broad exception acceptable: memory usage fallback
    ...
    def _get_storage_usage(self) -> Dict[str, Any]:
        ...
        except Exception as e:  # broad exception acceptable: storage usage fallback
            logger.error(f"Error in {__name__}: {e}", exc_info=True)
            return {"error": str(e)}
    ...
    async def _save_learning_data(self):
        ...
        except Exception as e:  # broad exception acceptable: save data resilience
    ...
    async def record_user_interaction(
        ...
        if isinstance(self.learning_data["user_interactions"], list):
            if len(self.learning_data["user_interactions"]) > 1000:
                self.learning_data["user_interactions"] = self.learning_data["user_interactions"][
                    -500:
                ]
    ...
    async def record_error_pattern(
        ...
        if error_key in self.learning_data["error_patterns"]:
            if isinstance(self.learning_data["error_patterns"][error_key], dict):
                current_frequency = self.learning_data["error_patterns"][error_key].get(
                    "frequency", 0
                )
    ...
    async def get_learning_insights(self) -> Dict[str, Any]:
        ...
        except Exception as e:  # broad exception acceptable: insights resilience
    ...
    def _analyze_interactions(self) -> Dict[str, Any]:
        ...
        # 計算成功率
        success_count = sum(
            1 for i in interactions if isinstance(i, dict) and i.get("result") == "success"
        )
    ...
    def _analyze_errors(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_occurrences": sum(
                e[1].get("frequency", 0) if isinstance(e[1], dict) else 0 for e in errors.items()
            ),
        }
    ...
    def _analyze_performance(self) -> Dict[str, Any]:
        ...
        if metrics:
            avg_memory = sum(
                (
                    m.get("memory_usage", {}).get("percent", 0)
                    if isinstance(m, dict) and isinstance(m.get("memory_usage"), dict)
                    else 0
                )
                for m in metrics
            ) / len(metrics)
            avg_storage = sum(
                (
                    m.get("storage_usage", {}).get("total_mb", 0)
                    if isinstance(m, dict) and isinstance(m.get("storage_usage"), dict)
                    else 0
                )
                for m in metrics
            ) / len(metrics)
    ...
    def _get_collection_period(self) -> Dict[str, str]:
        ...
        timestamps = [
            i.get("timestamp") for i in interactions if isinstance(i, dict) and i.get("timestamp")
        ]
        # 過濾掉 None 值和非字符串值
        filtered_timestamps: List[str] = [t for t in timestamps if isinstance(t, str)]

        if len(filtered_timestamps) >= 2:  # 需要至少兩個時間戳才能計算範圍
            return {"start": min(filtered_timestamps), "end": max(filtered_timestamps)}
        elif len(filtered_timestamps) == 1:  # 只有一個時間戳
            return {"start": filtered_timestamps[0], "end": filtered_timestamps[0]}
