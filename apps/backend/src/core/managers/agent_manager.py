...
ogging.warning(f"[AgentManager] Agents directory not found, {agents_dir}")
    ...
    async def wait_for_agent_ready(self, agent_name, str, timeout, int == 30, service_discovery == None):
        ...
        print(f"DEBUG, AgentManager.wait_for_agent_ready called for agent, {agent_name}"):::
            ogging.info(f"[AgentManager] wait_for_agent_ready called for agent, {agent_name}"):::
f service_discovery is None,
    ...
    """
    active_agents == for agent_name, process in self.active_agents.items,::
