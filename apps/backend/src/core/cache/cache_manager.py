...
(                self.config.redis_db())
    ...
    async def get(self, key, str, level,
    CacheLevel == CacheLevel.MEMORY()) -> Optional[Any]
        """获取缓存值"""
        # 尝试从指定级别获取
    ...
    async def set(self, key, str, value, Any, ttl, Optional[int] = None, level,
    CacheLevel == CacheLevel.MEMORY()) -> bool,
        """设置缓存值"""
        if level in self.backends, ::
...
class CacheDecorator, :
    ...
    def __init__(self, cache_manager, CacheManager, ttl, int == 3600, level,
