...
async def feedback_engine():
    ...
    engine = FeedbackLoopEngineFactory.create_basic_engine(
        {"latency_threshold_ms": 16.0, "max_completed_history": 100}
    )
...
async def feedback_processor():
    ...
    processor = FeedbackProcessor(
        config={"success_threshold": 0.7, "learning_trigger_threshold": 0.5}
    )
...
class TestFeedbackLoopIntegrity:
    ...
    async def test_perception_to_action_cycle(self, feedback_engine):
        ...
        event = PerceptionEvent.create(
            PerceptionType.MOUSE, "test", {"position": {"x": 100, "y": 200}}, priority=5
        )
    ...
    async def test_multi_layer_feedback_generation(self, feedback_engine):
        ...
        event = PerceptionEvent.create(
            PerceptionType.USER_ACTIVITY, "test", {"state": "active"}, priority=3
        )
    ...
    async def test_learning_update_generation(self, feedback_engine):
        ...
        # Process perception event
        event = PerceptionEvent.create(
            PerceptionType.TIME, "test", {"hour": 14, "minute": 30}, priority=4
        )
    ...
    async def test_feedback_loop_connections(self):
        ...
        engine = FeedbackLoopEngine(
            hsm=mock_hsm, cdm=mock_cdm, config={"latency_threshold_ms": 16.0}
        )
        ...
        # Process event
        event = PerceptionEvent.create(
            PerceptionType.SYSTEM_STATE, "test", {"cpu": 50.0}, priority=5
        )
...
class TestRealTimeMonitoring:
    ...
    async def test_mouse_monitoring_16ms_latency(self, real_time_monitor):
        ...
        if len(update_times) >= 2:
            intervals_ms = [
                (update_times[i] - update_times[i - 1]) * 1000 for i in range(1, len(update_times))
            ]
            avg_interval = sum(intervals_ms) / len(intervals_ms)
            max_interval = max(intervals_ms)

            logger.info(
                f"Mouse monitoring - Avg interval: {avg_interval:.2f}ms, Max: {max_interval:.2f}ms"
            )
...
class TestEventProcessing:
    ...
    async def test_priority_queue_ordering(self, event_loop_system):
        ...
        if len(processed_order) >= 2:
            ...
            critical_positions = [
                i for i, (_, level) in enumerate(processed_order) if level == 0  # CRITICAL level
            ]
    ...
    async def test_event_debouncing(self, event_loop_system):
        ...
        debounce_config = DebounceConfig(
            event_type="file_change", delay_ms=50, leading=False, trailing=True
        )
...
class TestPerformance:
    ...
    async def test_feedback_loop_latency_under_16ms(self, feedback_engine):
        ...
        if latencies:
            ...
            assert (
                max_latency < 100
            ), f"Max latency {max_latency:.2f}ms exceeds acceptable threshold"
            assert (
                avg_latency < 50
            ), f"Average latency {avg_latency:.2f}ms exceeds acceptable threshold"
...
class TestIntegration:
    ...
    async def test_complete_feedback_system(self):
        ...
        for i, ptype in enumerate(perception_types):
            event = PerceptionEvent.create(ptype, "integration_test", {"test_index": i}, priority=5)
            await engine.process_perception_event(event)
...
if __name__ == "__main__":
    ...
    try:
        ...
        async def run_basic_tests():
            ...
            event = PerceptionEvent.create(
                PerceptionType.MOUSE, "test", {"x": 100, "y": 200}, priority=5
            )
