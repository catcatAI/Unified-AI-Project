    ...
    async def _check_auto_switch(self):
        ...
        try:
            ...
            if not suggested_mode:
                if (
                    memory.percent < self.thresholds["upgrade"]["memory_percent"]
                    and cpu < self.thresholds["upgrade"]["cpu_percent"]
                ):
            ...
            if suggested_mode and suggested_mode != self.current_mode:
                logger.info(
                    f"Auto-switch suggested: {self.current_mode} → {suggested_mode} ({reason})"
                )
...
async def quick_mode_switch(
    angela_core, target_mode: str, on_progress=None, on_complete=None
) -> bool:
