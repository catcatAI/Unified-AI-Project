...
class MetabolicHeartbeat:
    ...
    async def _integration_loop(self) -> None:
        ...
        while self._running:
            try:
                ...
                intent_pose = (
                    "walking"
                    if dist_to_target > self._beh_cfg.get("movement", {}).get("walk_threshold", 5.0)
                    else "standing"
                )
                ...
                cerebellum_res = self.cerebellum.execute_command(
                    pose_name=intent_pose, bio_state=bio_state
                )
                ...
                self.posture = {
                    ...
                    "finger_matrix": cerebellum_res.get(
                ...
                if dist_to_target > move_th:
                    ...
                    stress_speed_factor = self._beh_cfg.get("movement", {}).get(
                        "stress_speed_factor", 0.5
                    )
    ...
    def _subscribe_cns_events(self) -> None:
        ...
        state_store.subscribe_event(
            "routing.response_generated", self._handle_cns_event, priority=5
        )
        state_store.subscribe_event(
            "lifecycle.decision_executed", self._handle_cns_event, priority=5
        )
    ...
    async def start(self) -> None:
        ...
        self._task.add_done_callback(
            lambda t: (
                logger.critical("Heartbeat main loop failed: %s", t.exception())
        ...
        self._integration_task.add_done_callback(
            lambda t: (
                logger.critical("Heartbeat integration loop failed: %s", t.exception())
                if not t.cancelled() and t.exception()
                else None
            )
        )
        ...
        while self._running:
            try:
                ...
                dynamic_interval = max(
                    min_int, min(max_int, base_rate / (stress * mul + (arousal / div)))
                )
                state_store.emit_event(
                    "heartbeat.pulse",
                    {
                ...
                Exception
            ) as e:  # broad exception acceptable: heartbeat loop must be resilient to errors
    ...
    async def _update_spatial_state(self, arousal, stress) -> None:
        ...
        if arousal > arousal_thresh and random.random() < jump_prob:
            ...
            logger.info(
                f"🚶 [Spatial] Intent Jump: Target set to x={self.target_x} (Arousal: {arousal:.2f})"
            )
        ...
        if dx != 0:
            ...
            if not self._check_collision(next_x):
                ...
                task.add_done_callback(
                    lambda t: (
                        logger.warning("Task trigger_physical_trauma failed: %s", t.exception())
                        if not t.cancelled() and t.exception()
                        else None
                    )
                )
    ...
    async def _process_metabolism(self) -> None:
        ...
        try:
            ...
            await self.bio_integrator.process_stress_event(
                intensity=fatigue_impact, duration=self.update_interval
            )
            ...
            logger.debug(
                f"[Metabolism] Hardware Pulse - CPU: {cpu_usage}%, Battery: {energy_level}%"
            )
        except (
            Exception
        ) as e:  # broad exception acceptable: metabolism processing should be resilient to hardware errors
    ...
    async def _observe_environment(self) -> None:
        ...
        logger.debug(
            f"🌍 [Environment] Activity: {category}, BPM: {metrics['input_density_bpm']:.1f}"
        )
