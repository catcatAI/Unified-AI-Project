    ...
    async def _monitor_loop(self):
        ...
        while self._running:
            ...
            mouse_data = MouseData(
                x=x, y=y, velocity_x=velocity_x, velocity_y=velocity_y, timestamp=datetime.now()
            )
        ...
        try:
            ...
            for item in path.iterdir():
                if item.is_file() and not self._should_ignore(item):
                    ...
                    try:
                        stat = item.stat()
                        current_state = {"mtime": stat.st_mtime, "size": stat.st_size}
...
class UserActivityMonitor:
    ...
    async def _analyze_activity(self) -> UserActivityData:
        ...
        recent_events = [e for e in self._input_events if (current_time - e).total_seconds() < 60]
...
class RealTimeMonitor:
    ...
    def __init__(
        ...
        self.time_monitor = TimeMonitor(check_interval=self.config.get("time_check_interval", 1.0))
    ...
    def _setup_callback_bridges(self):
        ...
        self.file_monitor.register_callback(
            lambda event: self._dispatch(
                "file_change",
                {"path": str(event.path), "type": event.event_type, "size": event.file_size},
            )
        )
        ...
        self.time_monitor.register_callback(
            lambda event: self._dispatch(
                "time_event", {"type": event.event_type, "description": event.description}
            )
        )
