...
class ErrorHandler:
    ...
    def register_handler(
        self, error_type: Type[AngelaError], handler: Callable[[AngelaError], Any]
    ) -> None:
        """注册错误处理器"""
    ...
    def capture_exception(
        ...
        **kwargs,
        ...
        if isinstance(error, AngelaError):
            ...
            # 转换为 AngelaError
            return CoreError(
                message=str(error), cause=error, user_id=user_id, request_id=request_id, **kwargs
            )
...
def capture_exception(
    error: Exception, user_id: Optional[str] = None, request_id: Optional[str] = None, **kwargs
) -> AngelaError:
    """捕获异常"""
...
if __name__ == "__main__":
    ...
    logger.error(f"Dict: {error.to_dict()}", exc_info=True)
    logger.error(f"JSON: {error.to_json()}", exc_info=True)
