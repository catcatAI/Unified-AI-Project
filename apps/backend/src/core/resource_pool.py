        ...
        with self._lock:
            ...
            for resource in self._resources:
                if not resource.in_use:
                    # 检查过期和空闲超时
                    if resource.is_expired(self.config.max_lifetime) or resource.is_idle(
...
class ThreadPool:
    ...
    def get_stats(self) -> PoolStats:
        """获取统计信息"""
        return PoolStats(active_size=self._task_queue.qsize(), current_size=len(self._workers))
...
if __name__ == "__main__":
    # 测试资源池
    pool = ResourcePool(
        factory=lambda: {"id": id, "data": "test"}, config=PoolConfig(min_size=2, max_size=5)
    )
