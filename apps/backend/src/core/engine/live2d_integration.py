...
class Live2DIntegration:
    ...
    def set_expression(self, expression: ExpressionType, blend_duration: float = 0.3):
        ...
        self._target_expression = Live2DExpression(
            expression_type=expression, parameters=expr_params, blend_duration=blend_duration
        )

        self._current_expression_obj = Live2DExpression(
            expression_type=self.current_expression,
            parameters={k: self.parameters[k].value for k in self.parameters.keys()},
        )
    ...
    async def play_motion(self, motion: MotionType, loop: bool = False) -> bool:
        ...
        """
        action = Live2DAction(motion_type=motion, duration=2.0, loop=loop)  # Default duration
    ...
    def apply_body_touch(
        self, body_part: str, touch_type: str = "pat", intensity: float = 0.5, duration: float = 1.0
    ) -> Dict[str, float]:
        """
    ...
    def create_touch_animation(
        ...
        keyframes.append({"time": 0.0, "params": touch_params, "easing": "ease_out"})

        # Frame 2: Sustain
        sustain_params = {k: v * 0.7 for k, v in touch_params.items()}
        keyframes.append({"time": 0.2, "params": sustain_params, "easing": "linear"})

        # Frame 3: Release (return to default)
        release_params = {k: 0.0 for k in touch_params.keys()}
        keyframes.append({"time": 0.5, "params": release_params, "easing": "ease_in"})
