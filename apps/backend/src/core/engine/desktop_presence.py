...
class DesktopPresence:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None, intent_manager: Optional[Any] = None):
        ...
        self.body_size: Size = Size(
            self.config.get("body_width", 200), self.config.get("body_height", 300)
        )

        # Screen boundaries
        self.screen_bounds: Size = Size(
            self.config.get("screen_width", 1920), self.config.get("screen_height", 1080)
        )
    ...
    async def _apply_intent_gravity(self) -> None:
        ...
        # 尋找與生存/社交相關的活躍意圖
        gravity_force = 0.0
        from core.life.intent_model import IntentCategory
    ...
    def _calculate_collision(
        self, my_bounds: BoundingBox, other_bounds: BoundingBox, obj_name: str
    ) -> CollisionInfo:
        """Calculate collision details between two bounding boxes"""
        # Calculate overlap
        left = max(my_bounds.x, other_bounds.x)
        right = min(my_bounds.x + my_bounds.width, other_bounds.x + other_bounds.width)
        top = max(my_bounds.y, other_bounds.y)
        bottom = min(my_bounds.y + my_bounds.height, other_bounds.y + other_bounds.height)
    ...
    def snap_to_edge(self, edge: str) -> None:
        ...
        if edge == "left":
            ...
            self.set_position(
                Position(self.screen_bounds.width - self.body_size.width, self.current_position.y)
            )
        elif edge == "top":
            self.set_position(Position(self.current_position.x, 0))
        elif edge == "bottom":
            self.set_position(
                Position(self.current_position.x, self.screen_bounds.height - self.body_size.height)
            )
