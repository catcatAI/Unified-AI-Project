...
class MemoryNeuroplasticityBridge:
        ...
        if trace:
            ...
            if memory_id in self.reinforcement_map:
                ...
                if reinforcement.access_frequency >= self._ltp_threshold_accesses:
                    self.neuroplasticity.apply_ltp(neuro_id, frequency=15.0, duration=5.0)
                    reinforcement.reinforcement_strength = min(
                        1.0, reinforcement.reinforcement_strength + 0.1
                    )
    ...
    def associate_memories(self, memory_id_1: str, memory_id_2: str) -> bool:
        ...
        """
        if memory_id_1 not in self._external_to_neuro or memory_id_2 not in self._external_to_neuro:
            return False
    ...
    def get_memory_stats(self) -> Dict[str, Any]:
        ...
        if total_memories > 0:
            avg_retention = (
                sum(self.get_memory_retention(mid) for mid in self._external_to_neuro.keys())
                / total_memories
            )
        else:
            avg_retention = 0.0
    ...
    def reinforce_memory(self, memory_id: str, strength: float = 0.1):
        ...
        # Apply LTP
        self.neuroplasticity.apply_ltp(neuro_id, frequency=10.0 + strength * 10, duration=5.0)

        # Update reinforcement map
        if memory_id in self.reinforcement_map:
            self.reinforcement_map[memory_id].reinforcement_strength = min(
                1.0, self.reinforcement_map[memory_id].reinforcement_strength + strength
            )

    def consolidate_memory(
        self, memory_id: str, emotional_intensity: float = 0.5, priority: str = "normal"
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            priority_multipliers = {"high": 1.5, "normal": 1.0, "low": 0.7}
            ...
            self.neuroplasticity.apply_ltp(neuro_id, frequency=ltp_frequency, duration=ltp_duration)
            ...
            if emotional_intensity > 0.6:
                ...
                trace.consolidation_strength = min(
                    1.0, trace.consolidation_strength + emotional_boost
                )
            ...
            trace.consolidation_strength = min(
                1.0, trace.consolidation_strength + base_consolidation_increase
            )
    ...
    def reinforce_memory(
        ...
        try:
            ...
            reinforcement.reinforcement_strength = min(
                1.0, reinforcement.reinforcement_strength + adjusted_strength
            )
            ...
            if reinforcement.access_frequency >= self._ltp_threshold_accesses:
                ...
                bonus_strength = 0.05
                reinforcement.reinforcement_strength = min(
                    1.0, reinforcement.reinforcement_strength + bonus_strength
                )
                results["ltp_threshold_bonus"] = bonus_strength

            # 8. 更新元数据 / Update metadata
            if memory_id in self._memory_metadata:
                self._memory_metadata[memory_id]["access_count"] = int(
                    reinforcement.access_frequency
                )
                ...
                if emotional_context:
                    ...
                    if (
                        emotional_context
                        not in self._memory_metadata[memory_id]["emotional_contexts"]
                    ):
                        self._memory_metadata[memory_id]["emotional_contexts"].append(
                            emotional_context
                        )
    ...
    def get_bridge_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_reinforcement": (
                sum(r.reinforcement_strength for r in self.reinforcement_map.values())
                / len(self.reinforcement_map)
                if self.reinforcement_map
                else 0
            ),
