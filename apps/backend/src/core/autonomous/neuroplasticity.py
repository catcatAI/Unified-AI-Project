...
class HebbianRule:
    ...
    def apply(self, pre_activation: float, post_activation: float, current_weight: float) -> float:
        """
...
class EbbinghausForgettingCurve:
    ...
    def calculate_retention(
        self, hours_since_learning: float, memory_strength: float = 1.0
    ) -> float:
        """
    ...
    def estimate_strength_from_reviews(
        self, review_count: int, average_performance: float
    ) -> float:
...
class NeuroplasticitySystem:
    ...
    async def _decay_synaptic_weights(self):
        ...
        for key, synapse in self.synaptic_weights.items():
            hours_since_update = (current_time - synapse.last_update).total_seconds() / 3600.0
    ...
    def apply_ltp(self, memory_id: str, frequency: float, duration: float = 5.0):
        ...
        if frequency >= self.ltp_params.threshold_frequency:
            ...
            # Apply potentiation
            trace.current_weight = min(1.0, trace.current_weight + potentiation)
    ...
    def apply_ltd(self, memory_id: str, frequency: float = 0.5, duration: float = 10.0):
        ...
        if frequency <= self.ltd_params.threshold_frequency:
            depression = self.ltd_params.depression_rate * (duration / 10.0)
            ...
            if assoc_id in self.memory_traces:
                ...
                if synapse_key not in self.synaptic_weights:
                    self.synaptic_weights[synapse_key] = SynapticWeight(
                        pre_neuron=sorted_ids[0], post_neuron=sorted_ids[1], weight=0.1
                    )
    ...
    def access_memory(self, memory_id: str) -> Optional[MemoryTrace]:
        ...
        trace.current_weight = min(
            1.0, trace.current_weight + self.hebbian_rule.learning_rate * 0.1
        )
    ...
    def get_memory_retention(self, memory_id: str) -> float:
        ...
        hours_since = (datetime.now() - trace.last_accessed).total_seconds() / 3600.0

        # Calculate memory strength
        # We use current weight as the base, and consolidation strength as a bonus
        memory_strength = (trace.current_weight + trace.consolidation_strength * 0.5) * (
            1 + trace.access_count * 0.1
        )

        # Apply forgetting curve
        retention = self.forgetting_curve.calculate_retention(hours_since, memory_strength)
    ...
    def get_system_stats(self) -> Dict[str, Any]:
        ...
        if self.synaptic_weights:
            avg_weight = sum(s.weight for s in self.synaptic_weights.values()) / len(
    ...
    async def demo():
        ...
        memories = [
            np_system.create_memory_trace(
                f"mem_{i:03d}", f"Information chunk {i}", initial_weight=0.5 + i * 0.05
            )
            for i in range(5)
        ]
        ...
        trauma_system = TraumaMemorySystem()
        trauma = trauma_system.encode_trauma(
            "trauma_001", "traumatic_event_description", intensity=0.85
        )
        ...
        for i in range(3):
            ...
            logger.info(
                f"   学习事实 {i} 后 / after fact {i}: 平均巩固度={stats['avg_explicit_consolidation']:.2%}"
            )
        ...
        for i in range(3):
            ...
            stats = learning.get_consolidation_stats()
            logger.info(
                f"   学习技能 {i} 后 / after skill {i}: 平均巩固度={stats['avg_implicit_consolidation']:.2%}"
            )
