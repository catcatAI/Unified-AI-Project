...
class ArtLearningWorkflow:
    ...
    def __init__(
        ...
        self.progress = WorkflowProgress(
            stage=WorkflowStage.IDLE, overall_progress=0, current_task=""
        )
    ...
    def register_stage_change_callback(
        self, callback: Callable[[WorkflowStage, WorkflowStage], None]
    ):
    ...
    async def run_complete_workflow(
        ...
        try:
            ...
            result = GenerationResult(
                ...
                quality_score=(
                    self.generation_result.quality_score if self.generation_result else 0.0
                ),
        ...
        for objective in objectives:
            if objective == LearningObjective.ANIME_BASICS:
                ...
                tutorials = await self.art_learning.search_domain_tutorials(
                    ArtDomain.ANIME_ART, max_results_per_keyword=5
                )
                ...
                tutorials = await self.art_learning.search_domain_tutorials(
                    ArtDomain.LIVE2D, max_results_per_keyword=5
                )
        ...
        if hasattr(self.art_learning, "analyze_image"):
            ...
            if analyses:
                implicit_knowledge = await self.art_learning.learn_from_image_batch(
                    analyses, learning_type=None  # Uses IMPLICIT from art_learning_system
                )
    ...
    async def _stage_practice(self, target_mastery: float):
        ...
        while iterations < self.config.max_learning_iterations:
            all_mastered = all(
                a.current_level >= target_mastery for a in self.skill_assessments.values()
            )
    ...
    async def _stage_test(self):
        ...
        test_results = {"body_parts_tested": [], "touch_types_tested": [], "failed_tests": []}

        # Test key body parts
        test_body_parts = ["top_of_head", "face", "hands", "chest", "shoulders"]
                ...
                try:
                    ...
                    if body_part_enum:
                        ...
                        response = await self.tactile.process_stimulus_with_live2d(
                            stimulus, touch_type=touch_type
                        )
        ...
        self.progress.completed_tasks.append(
            f"Tested {len(set(test_results['body_parts_tested']))} body parts"
        )
    ...
    async def _stage_deploy(self):
        ...
        if self.generation_result and hasattr(self.avatar_generator, "export_for_desktop_pet"):
            export_path = await self.avatar_generator.export_for_desktop_pet(
                self.generation_result, self.config.output_directory + "/desktop_pet"
            )
    ...
    def _record_workflow_completion(self, result: GenerationResult):
        ...
        record = {
            ...
            "learning_results": {
                ...
                "final_mastery": (
                    sum(a.current_level for a in self.skill_assessments.values())
                    / len(self.skill_assessments)
    ...
    def get_learning_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "latest_result": (
                {
                    "avatar_id": (
                        self.generation_result.avatar_id if self.generation_result else None
                    ),
                    "quality": (
                        self.generation_result.quality_score if self.generation_result else 0
                    ),
                    "validated": (
                        self.generation_result.validation_passed
                        if self.generation_result
                        else False
                    ),
                }
    ...
    async def quick_generate(
        self, skip_learning: bool = False, attributes: Optional[Dict[str, Any]] = None
    ) -> GenerationResult:
        """
        ...
        if not skip_learning:
            ...
            tutorials = await self.art_learning.search_domain_tutorials(
                ArtDomain.LIVE2D, max_results_per_keyword=2
            )
        ...
        # Generate
        avatar = await self.avatar_generator.generate_avatar(
            model_name="angela_quick", attributes=attributes
        )
    ...
    async def demo():
        ...
        class MockArtLearning:
            ...
            async def analyze_image(self, image_path=None, image_url=None):
                return type("Analysis", (), {"image_id": "img_1", "style_features": []})()
        ...
        class MockTactile:
            ...
            async def process_stimulus_with_live2d(self, stimulus, touch_type):
                return type("Response", (), {"live2d_parameters": {"ParamAngleX": 10.0}})()
        ...
        result = await workflow.run_complete_workflow(
            learning_objectives=[LearningObjective.LIVE2D_TECHNIQUES], target_mastery=0.7
        )
