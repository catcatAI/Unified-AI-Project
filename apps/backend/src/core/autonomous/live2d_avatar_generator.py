...
class Live2DAvatarGenerator:
    ...
    STANDARD_LAYERS = [
        ...
        LayerDefinition(
            ...
            BodyLayer.HAIR_BACK,
            "Back hair",
            ["ParamHairBack", "ParamAngleX", "ParamAngleY"],
        ),
        LayerDefinition(
            "body",
            BodyLayer.BODY,
        ...
        LayerDefinition(
            ...
            BodyLayer.FACE,
            "Face base",
            ["ParamAngleX", "ParamAngleY", "ParamCheek", "ParamFaceColor"],
        ),
        LayerDefinition(
            "eyes_back", BodyLayer.EYES_BACK, "Eyes back layer", ["ParamEyeBallX", "ParamEyeBallY"]
        ),
        LayerDefinition(
            "eye_l",
            BodyLayer.EYES,
            "Left eye",
            ["ParamEyeLOpen", "ParamEyeLSmile", "ParamEyeBallX", "ParamEyeBallY"],
        ),
        LayerDefinition(
            "eye_r",
            BodyLayer.EYES,
            "Right eye",
            ["ParamEyeROpen", "ParamEyeRSmile", "ParamEyeBallX", "ParamEyeBallY"],
        ),
        LayerDefinition(
            "eyes_front", BodyLayer.EYES_FRONT, "Eyes highlight", ["ParamEyeBallX", "ParamEyeBallY"]
        ),
        ...
        LayerDefinition(
            ...
            BodyLayer.EYEBROWS,
            "Left eyebrow",
            ["ParamBrowLY", "ParamBrowLAngle", "ParamBrowLForm"],
        ),
        LayerDefinition(
            "eyebrow_r",
            BodyLayer.EYEBROWS,
            "Right eyebrow",
            ["ParamBrowRY", "ParamBrowRAngle", "ParamBrowRForm"],
        ),
        LayerDefinition(
            "hair_front",
            BodyLayer.HAIR_FRONT,
            "Front hair",
            ["ParamHairFront", "ParamHairSwing", "ParamAngleX", "ParamAngleY"],
        ),
        LayerDefinition(
            "hair_side",
            BodyLayer.HAIR_FRONT,
            "Side hair",
            ["ParamHairSide", "ParamAngleX", "ParamHairSwing"],
        ),
        LayerDefinition(
            "accessories",
            BodyLayer.FRONT_ACCESSORIES,
            "Accessories",
            ["ParamBodyAngleX", "ParamBodyAngleY"],
        ),
    ]
    ...
    async def generate_avatar(
        ...
        try:
            ...
            self._notify_progress(
                GenerationStage.CONFIGURING_MODEL, 80, "Configuring model3.json..."
            )
    ...
    async def _create_layers(
        ...
        for layer_def in self.STANDARD_LAYERS:
            ...
            layer = GeneratedLayer(
                ...
                image_path=str(
                    Path(avatar.output_directory) / "layers" / f"{layer_def.layer_name}.png"
                ),
            )
    ...
    async def _configure_model(self, avatar: GeneratedAvatar):
        ...
        model_config = {
            ...
            "FileReferences": {
                ...
                "Textures": [
                    f"textures/texture_{i:02d}.png" for i in range(len(avatar.texture_paths))
                ],
        ...
        model_config["Parts"] = []
        for layer in avatar.layers:
            model_config["Parts"].append({"Id": layer.layer_id, "Name": layer.layer_name})
    ...
    async def _generate_display_info(self, avatar: GeneratedAvatar):
        ...
        for param_id, param in avatar.parameters.items():
            display_info["Parameters"].append(
                {"Id": param_id, "GroupId": "", "Name": param_id.replace("Param", "")}
            )
