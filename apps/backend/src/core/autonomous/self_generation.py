...
class SelfGeneration:
    ...
    async def generate_avatar(
        ...
        base_attrs = attributes or (
            self.current_avatar.attributes if self.current_avatar else VisualAttributes()
        )
    ...
    async def generate_with_learning(
        ...
        if enable_learning and self.learning_workflow:
            ...
            return await self.generate_avatar(
                mode=GenerationMode.FULL_GENERATION, attributes=attributes
            )
    ...
    async def evolve_avatar(
        self, growth_stage: str, mood: str, maturity_level: float = 0.5
    ) -> GeneratedAvatar:
        """
        ...
        avatar = await self.generate_avatar(
            mode=GenerationMode.EVOLUTION, attributes=attrs, mood=mood, identity_stage=growth_stage
        )
    ...
    async def adapt_to_mood(self, mood: str, intensity: float = 0.5) -> Optional[GeneratedAvatar]:
        """
        ...
        return await self.create_variation(self.current_avatar.avatar_id, expression=expression)
    ...
    def get_generation_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "generation_modes_used": list(set(a.generation_mode.name for a in self.avatar_history)),
...
if __name__ == "__main__":
    ...
    async def demo():
        ...
        if gen.current_avatar:
            variation = await gen.create_variation(
                gen.current_avatar.avatar_id, expression="surprised"
            )
        if variation:
