...
class AutonomicNervousSystem:
    ...
    async def _decay_stimuli(self):
        ...
        for stimulus in self.active_stimuli:
            ...
            if elapsed < stimulus.duration:
                ...
                remaining_intensity = stimulus.intensity * (1 - (elapsed / stimulus.duration))
    ...
    async def apply_stimulus(
        ...
        stimulus = StimulusFactor(
            name=name, nerve_type=nerve_type, intensity=intensity, duration=duration
        )
    ...
    def get_current_effects(
        self,
    ) -> Tuple[PhysiologicalEffects, EmotionalEffects, CognitiveEffects]:
        """
