...
class SelfModel:
    ...
    core_values: List[str] = field(
        default_factory=lambda: ["kindness", "growth", "curiosity", "honesty", "empathy"]
    )
...
class CyberIdentity:
    ...
    def _calculate_recent_growth(self) -> float:
        ...
        recent = [g for g in self.growth_history if (datetime.now() - g["timestamp"]).days < 7]
    ...
    def _get_recent_growth_events(self, days: int = 7) -> List[Dict[str, Any]]:
        ...
        return [g for g in self.growth_history if g["timestamp"] > cutoff]
    ...
    def record_growth(
        ...
        trace_id = tracer.start(
            ...
            data={"aspect": aspect.name, "new_level": new_level, "milestone": milestone},
        )
        ...
        try:
            ...
            tracer.record(
                trace_id, "growth_amount", growth_record.level - growth_record.previous_level
            )

            time_diff = (datetime.now() - growth_record.last_updated).total_seconds()
            if time_diff > 0:
                growth_record.growth_rate = (growth_record.level - growth_record.previous_level) / (
                    time_diff / 86400
                )  # Per day
    ...
    def get_closest_relationships(self, limit: int = 5) -> List[RelationshipModel]:
        ...
        sorted_rels = sorted(self.relationships.values(), key=lambda r: r.closeness, reverse=True)
    ...
    def calculate_life_intensity(self) -> float:
        ...
        for entity_id, relationship in self.relationships.items():
            self.life_intensity_formula.register_observer(
                entity_id, relationship_depth=relationship.closeness
            )
    ...
    def get_formula_based_self_summary(self) -> Dict[str, Any]:
        ...
        enhanced_summary = {
            **base_summary,
            ...
            "observers": (
                list(self.life_intensity_formula.observers.keys())
                if self.life_intensity_formula
                else []
            ),
        }
...
if __name__ == "__main__":
    ...
    async def demo():
        ...
        identity.record_growth(
            IdentityAspect.EMOTIONAL_DEPTH, 0.75, milestone="学会了理解复杂的情感"
        )
        logger.info("  情感深度提升至 75%")

        identity.record_growth(IdentityAspect.SELF_AWARENESS, 0.6, milestone="形成了清晰的自我认知")
        logger.info("  自我意识提升至 60%")

        # Form relationships
        logger.info("\n建立关系 / Forming relationships:")
        identity.form_relationship("user_alice", "user", "companion", initial_closeness=0.7)
