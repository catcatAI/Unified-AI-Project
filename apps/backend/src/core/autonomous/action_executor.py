...
class ActionExecutor:
    ...
    async def shutdown(self):
        ...
        if self.active_actions:
            await asyncio.gather(
                *[self._wait_for_action(action) for action in self.active_actions.values()],
        ...
        for action in self.queue._failed:
            if action.action_id == action_id:
                if action.retry_count < action.max_retries:
                    ...
                    return ActionResult(
                        success=False, action_id=action_id, error="Max retries exceeded"
                    )
    ...
    def add_safety_check(
        ...
        self.register_safety_check(
            SafetyCheck(
                check_name=check_name, check_function=check_function, is_critical=is_critical
            )
        )
