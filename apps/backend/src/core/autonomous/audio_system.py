...
class AudioSystem:
    ...
    async def speak_with_emotion(self, text: str, emotion: str, intensity: float = 0.5) -> bool:
        """
    ...
    async def play_music(self, track: MusicTrack, start_time: float = 0.0) -> bool:
        """
        Play music track
        ...
        self._playback_task = asyncio.create_task(self._music_playback_loop(track.duration))
    ...
    async def _lyrics_sync_loop(self):
        ...
        while self._running and self.current_state == AudioState.SINGING:
            if self.current_lyrics:
                current_line = self.current_lyrics.get_current_line(self.current_playback_time)
...
if __name__ == "__main__":
    ...
    async def demo():
        ...
        await audio.speak_with_emotion(
            "I'm so happy to be here with you!", emotion="happy", intensity=0.8
        )

        # Music playback
        logger.info("\n音乐播放 / Music playback:")
        track = MusicTrack(
            track_id="song_001", title="Test Song", artist="Test Artist", duration=30.0
        )
        await audio.play_music(track)
