...
class TriggerCondition:
    ...
    def evaluate(self, value: DimensionValue) -> float:
        ...
        if self.operator == "eq":
            ...
            match_score = (
                1.0 if value.value > self.threshold else max(0, value.value / self.threshold)
            )
        elif self.operator == "lt":
            match_score = (
                1.0
                if value.value < self.threshold
                else max(0, (1 - value.value) / (1 - self.threshold))
            )
            ...
            match_score = (
                1.0
                if value.value <= self.threshold
                else max(0, 1 - (value.value - self.threshold) / 0.3)
            )
...
class MultidimensionalTriggerSystem:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self._dimension_change_callbacks: Dict[
            TriggerDimension, List[Callable[[DimensionValue], None]]
        ] = {}
    ...
    def get_system_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "dimension_values": {
                dim.value[0]: val.value for dim, val in self.dimension_values.items()
            },
...
if __name__ == "__main__":
    ...
    async def demo():
        ...
        for dim in [
            TriggerDimension.TIME,
            TriggerDimension.EMOTION,
            TriggerDimension.PHYSIOLOGY,
            TriggerDimension.SOCIAL,
        ]:
