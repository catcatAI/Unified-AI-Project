...
class MetacognitiveCapabilitiesEngine:
    ...
    def _calculate_confidence(self) -> float:
        ...
        avg_proficiency = sum(
            cp.proficiency_level for cp in self.capability_profiles.values()
    ...
    def _assess_self_awareness(self) -> int:
        ...
        load_variance = sum((s.cognitive_load - 0.5) ** 2 for s in recent_states) / len(
            recent_states
        )
    ...
    async def practice_capability(
        ...
        current_success_count = int(profile.success_rate * (total_practice - 1)) + (
            1 if outcome else 0
        )
        ...
        logger.info(
            f"能力练习: {capability_id}, 结果: {outcome}, 新熟练度: {profile.proficiency_level:.3f}"
        )
