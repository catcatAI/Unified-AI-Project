...
CHAR_MAP_SAVE_PATH = os.path.join(PROJECT_ROOT, "data", "models",
...
""Loads dataset from a JSON file."""
    ...
    # Ensure dataset is a list of dicts with "problem" and "answer" keys,
        f not isinstance(dataset, list) or not all(isinstance(item, dict) and "problem",
    in item and "answer", in item for item in dataset)::
...
(    )
    ...
    # Using RMSprop as it's often good for RNNs, can also try Adam, ::
        ath_model.model.compile(optimizer = 'rmsprop',
