...
class ChainValidator:
    ...
    def _collect_layer_sequence(
        self, chain: CausalChain, node: CausalNode, sequence: List[LayerType], visited: set
    ) -> None:
    ...
    def _validate_parent_child(self, chain: CausalChain, errors: List[str]) -> None:
        ...
        for node in chain.nodes:
            if node.id != chain.root_id and node.parent_id is None:
                errors.append(f"Non-root node {node.id} has no parent (orphaned node)")

    def validate_layer_coverage(
        self, chain: CausalChain, required_layers: List[LayerType]
    ) -> ValidationResult:
        """
    ...
    def get_chain_statistics(self, chain: CausalChain) -> Dict[str, any]:
        ...
        return {
            ...
            "layers_present": [layer.code for layer in LayerType if chain.has_layer(layer)],
        }
