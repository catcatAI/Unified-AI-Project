...
class KeyManagerGUI:
        ...
        for config_path in config_paths:
            if config_path.exists():
                try:
                    ...
                    logger.debug("config key scan failed", exc_info=True)
    ...
    def _create_widgets(self):
        ...
        tk.Label(title_frame, text="🔑 API Key Manager", font=("Arial", 20, "bold")).pack(
            anchor="w"
        )
        ...
        self.provider_frame = tk.LabelFrame(
            content_frame, text="API Providers", font=("Arial", 12, "bold"), padx=10, pady=10
        )
    ...
    def _create_provider_row(self, index: int, provider_id: str, config: Dict):
        ...
        # Icon and name
        icon_label = tk.Label(row, text=config["icon"], font=("Arial", 20), bg=row["bg"])
        ...
        tk.Label(info_frame, text=name_text, font=("Arial", 12, "bold"), bg=row["bg"]).pack(
            anchor="w"
        )

        # Description
        tk.Label(
            info_frame, text=config["description"], font=("Arial", 9), fg="gray", bg=row["bg"]
        ).pack(anchor="w")
        ...
        if status["configured"]:
            ...
            tk.Label(
                status_frame, text="✅ Configured", font=("Arial", 10), fg="green", bg=row["bg"]
            ).pack(anchor="e")
            ...
            # Not configured
            tk.Label(
                status_frame, text="❌ Not Configured", font=("Arial", 10), fg="red", bg=row["bg"]
            ).pack(anchor="e")
    ...
    def _add_key_dialog(self):
        ...
        tk.Label(dialog, text="Select Provider:", font=("Arial", 12, "bold")).pack(pady=10)
        ...
        # Key entry
        tk.Label(dialog, text="API Key:", font=("Arial", 12, "bold")).pack(pady=(20, 5))
        ...
        # Storage method
        tk.Label(dialog, text="Storage Method:", font=("Arial", 12, "bold")).pack(pady=(20, 5))
        ...
        tk.Radiobutton(dialog, text=".env File", variable=storage_var, value="dotenv").pack(
            anchor="w", padx=20
        )
    ...
    def _edit_key_dialog(self, provider_id: str):
        ...
        # New key entry
        tk.Label(
            dialog, text="New API Key (leave empty to keep current):", font=("Arial", 11)
        ).pack(pady=(20, 5))
