...
class PrecisionProjectionMatrix:
    ...
    def __init__(self, auto_detect: bool = True, force_mode: Optional[PrecisionMode] = None):
        ...
        self._stats = {"total_conversions": 0, "mode_switches": 0, "last_mode": None}
    ...
    def project_to_int8(self, value: float, min_val: float = 0.0, max_val: float = 1.0) -> int:
        """投射浮點數到 INT8
    ...
    def project_from_int8(
        self, int8_value: int, min_val: float = 0.0, max_val: float = 1.0
    ) -> float:
        """從 INT8 恢復為浮點數
    ...
    def create_sparse_matrix(self, dense_data: Dict[str, float]) -> Dict[str, Any]:
        ...
        logger.debug(
            f"Sparse matrix created: {len(sparse_matrix['non_zero_entries'])}/{len(dense_data)} entries, "
    ...
    def get_memory_estimate(self, entry_count: int) -> Dict[str, int]:
        ...
        Returns:
            各精度模式的內存估算 (字節)
        """
        return {"INT8": entry_count * 1, "DEC4": entry_count * 8, "DEC8": entry_count * 16}
