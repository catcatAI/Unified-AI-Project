...
# =============================================================================
# Dynamic Model Sizing (memory-aware)
# =============================================================================

_MODEL_SIZING_CACHE: Optional[Dict[str, Any]] = None


def _get_available_ram_gb() -> float:
    """Get available RAM in GB. Falls back to 4GB if psutil unavailable."""
    try:
        import psutil
...
def _get_total_ram_gb() -> float:
    """Get total RAM in GB. Falls back to 8GB if psutil unavailable."""
    try:
        import psutil
...
def model_sizing_config() -> Dict[str, Any]:
    """Return memory-aware model sizing configuration.

    Dynamically adjusts vocab size, connection budget, and layer dimensions
    based on available RAM. Two modes:
      - Conservative (default): stays under 2GB total model size
      - Extended: uses up to 8GB when充足 RAM is available

    Returns dict with keys:
        max_vocab: int         -- GARDEN SNN vocabulary limit
        connection_budget: int -- ED3N max connections
        latent_dim: int        -- Shared latent space dimension
        hidden_dim: int        -- Hidden layer dimension
        batch_size: int        -- Training batch size
        mode: str              -- 'conservative' or 'extended'
    """
    global _MODEL_SIZING_CACHE
    if _MODEL_SIZING_CACHE is not None:
        return _MODEL_SIZING_CACHE

    total_ram = _get_total_ram_gb()
    avail_ram = _get_available_ram_gb()

    # Check if extended mode is requested via config or env
    import os
    extended = os.environ.get("ANGELA_EXTENDED_MODEL", "").lower() in ("1", "true", "yes")

    if extended and total_ram >= 8.0:
        # Extended mode: up to 8GB model, needs good RAM + fast disk
        config = {
            "max_vocab": min(20000, max(5000, int(total_ram * 2500))),
            "connection_budget": min(500000, max(50000, int(total_ram * 60000))),
            "latent_dim": 128,
            "hidden_dim": 256,
            "batch_size": 64,
            "mode": "extended",
        ...
        # Conservative mode: under 2GB total
        config = {
            "max_vocab": min(10000, max(2000, int(total_ram * 1300))),
            "connection_budget": min(200000, max(20000, int(total_ram * 25000))),
            "latent_dim": 64,
            "hidden_dim": 128,
            "batch_size": 32,
            "mode": "conservative",
    ...
    cfg_max_vocab = compute_int("garden_snn", "max_vocab", 0)
    if cfg_max_vocab > 0:
        config["max_vocab"] = min(config["max_vocab"], cfg_max_vocab)

    cfg_budget = compute_int("garden_snn", "connection_budget", 0)
    if cfg_budget > 0:
        config["connection_budget"] = min(config["connection_budget"], cfg_budget)

    # Log sizing decision
    logger = logging.getLogger(__name__)
    weight_matrix_mb = (config["max_vocab"] ** 2 * 4) / (1024 ** 2)
    logger.info(
        "Model sizing: mode=%s, total_ram=%.1fGB, avail_ram=%.1fGB, "
        "max_vocab=%d, weight_matrix=%.0fMB, latent_dim=%d",
        config["mode"], total_ram, avail_ram,
        config["max_vocab"], weight_matrix_mb, config["latent_dim"],
    )

    _MODEL_SIZING_CACHE = config
    return config
