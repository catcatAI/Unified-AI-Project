...
class EnterpriseLogger:
    ...
    def _add_file_handler(self):
        ...
        main_handler = RotatingFileHandler(
            self.log_dir / f"{self.name}.log", maxBytes=10 * 1024 * 1024, backupCount=5  # 10MB
        )
        main_handler.setLevel(logging.DEBUG)

        # 錯誤日誌文件
        error_handler = RotatingFileHandler(
            self.log_dir / f"{self.name}_error.log", maxBytes=10 * 1024 * 1024, backupCount=5
        )
        error_handler.setLevel(logging.ERROR)

        # 審計日誌文件
        audit_handler = RotatingFileHandler(
            self.log_dir / f"{self.name}_audit.log", maxBytes=10 * 1024 * 1024, backupCount=10
        )
    ...
    def audit(self, action: str, user_id: Optional[str] = None, **details):
        ...
        self._log(LogLevel.INFO, LogCategory.AUDIT, f"Audit: {action}", extra=details)
