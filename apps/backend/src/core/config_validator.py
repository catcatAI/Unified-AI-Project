...
class ConfigValidator:
    ...
    def _define_specs(self) -> List[EnvVarSpec]:
        ...
        return [
            ...
            # Live2D 配置
            EnvVarSpec(name="LIVE2D_MODEL_PATH", description="Live2D 模型路径", required=False),
            # 性能设置
            EnvVarSpec(
    ...
    def _validate_value(self, spec: EnvVarSpec, value: Optional[str]) -> None:
        ...
        if value is None or value == "":
            if spec.required:
                ...
                self.result.add_info(f"使用默认值: {spec.name} = {spec.default}")
            return
        ...
        if spec.validator and not spec.validator(value):
            self.result.add_error(f"环境变量值验证失败: {spec.name} = {value}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="验证 Angela AI 环境配置")
    parser.add_argument("--env-file", type=Path, default=None, help=".env 文件路径")
    parser.add_argument("--silent", action="store_true", help="静默模式，只输出错误")
