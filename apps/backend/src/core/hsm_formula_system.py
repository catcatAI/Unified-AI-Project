...
class HSMFormulaSystem:
    ...
    async def _simulate_discovery(self, exploration: ExplorationEvent):
        ...
        for i in range(num_discoveries):
            ...
            if (
                discovery["type"] == ExplorationResult.RULE_CANDIDATE
                and discovery["confidence"] > 0.6
            ):
    ...
    async def _create_governance_blueprint(
        self, exploration: ExplorationEvent, discovery: Dict[str, Any]
    ) -> GovernanceBlueprint:
        """
        ...
        blueprint = GovernanceBlueprint(
            ...
            action={"type": "system_rule", "description": discovery["description"]},
    ...
    def get_governance_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_confidence": (
                sum(b.confidence for b in self.governance_blueprints.values()) / total
                if total > 0
                else 0
            ),
        }
    ...
    def get_hsm_status(self) -> Dict[str, Any]:
        ...
        unresolved_count = sum(
            1 for g in self.cognitive_gaps.values() if g.resolution_status == "unresolved"
        )
        ...
        return {
            ...
            "cognitive_gaps": {
                ...
                "exploring": sum(
                    1 for g in self.cognitive_gaps.values() if g.resolution_status == "exploring"
                ),
                "resolved": sum(
                    1 for g in self.cognitive_gaps.values() if g.resolution_status == "resolved"
                ),
            },
            "explorations": {
