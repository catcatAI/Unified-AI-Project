...
class EthicsManager:
    ...
    async def review_content(self, content: str, content_id: str, context: Dict[str, Any] = None) -> EthicsReviewResult:
        """对内容进行全面的伦理审查"""
    ...
    context: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if 'contains_bias' in condition:
                bias_type = condition['contains_bias']
                if self.ai_models.get('bias_detector'):
                    has_bias, confidence, _ = self.ai_models['bias_detector'].detect_bias(content, bias_type)
    ...
    async def get_ethics_statistics(self) -> Dict[str, Any]:
        ...
        # 偏见检测统计
        bias_detections = sum(1 for review in self.review_history if review.bias_analysis.get('bias_detected', False))
        
        # 隐私违规统计
        privacy_violations = sum(1 for review in self.review_history if review.privacy_check.get('has_personal_data', False))
...
# 导出主要类
__all__ = ['EthicsManager', 'EthicsSystem', 'EthicsLevel', 'BiasType', 'EthicsRuleType',
    'EthicsReviewResult']