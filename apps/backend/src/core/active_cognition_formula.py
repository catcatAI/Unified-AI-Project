...
class ActiveCognitionFormula:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self._threshold_callbacks: List[Callable[[float, bool], None]] = (
            []
        )  # (a_c, was_below_now_above)
    ...
    def update_order_baseline(
        self, order_id: str, stability: Optional[float] = None, flexibility: Optional[float] = None
    ...
    def _record_construction(
        self, a_c: float, s_stress: float, o_order: float
    ) -> ActiveConstruction:
    ...
    def get_construction_statistics(self) -> Dict[str, Any]:
        ...
        # Recent trend
        recent = (
            self.construction_history[-20:]
            if len(self.construction_history) >= 20
            else self.construction_history
        )
        ...
        return {
            ...
            "a_c_trend": (
                "rising"
                if recent_avg_a_c > avg_a_c
                else "falling" if recent_avg_a_c < avg_a_c else "stable"
            ),
        }
...
if __name__ == "__main__":
    ...
    for scenario, a_c in [("舒适", a_c_1), ("主动建构", a_c_2), ("挣扎", a_c_3)]:
        ...
        logger.info(
            f"  {scenario} (A_c={a_c:.3f}): {interpretation['state_cn']} - {interpretation['description_cn']}"
        )
