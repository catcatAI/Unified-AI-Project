...
class VisualConfiguration:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "VisualConfiguration":
        ...
        if "model" in data:
            ...
            config.model.texture_resolution = model_data.get(
                "texture_resolution", config.model.texture_resolution
            )

        if "performance" in data:
            perf_data = data["performance"]
            config.performance.target_fps = perf_data.get(
                "target_fps", config.performance.target_fps
            )
            config.performance.adaptive_quality = perf_data.get(
