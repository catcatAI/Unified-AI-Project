...
class PerceptionEvent:
    ...
    @classmethod
    def create(
        cls, perception_type: PerceptionType, source: str, data: Dict[str, Any], priority: int = 5
    ) -> PerceptionEvent:
...
class FeedbackLoopEngine:
    ...
    def _register_monitor_callbacks(self):
        ...
        if self.real_time_monitor:
            self.real_time_monitor.register_callback(
                "mouse_position", lambda data: self._handle_perception(PerceptionType.MOUSE, data)
            )
            self.real_time_monitor.register_callback(
                "file_change",
            ...
            self.real_time_monitor.register_callback(
                "time_event", lambda data: self._handle_perception(PerceptionType.TIME, data)
            )
            self.real_time_monitor.register_callback(
                "system_state",
    ...
    async def _cognitive_processing(
        self, perception_event: PerceptionEvent
        ...
        if self.cdm and hasattr(self.cdm, "generate_decision"):
            try:
                decision_data = await self.cdm.generate_decision(perception_event.data, context)
    ...
    async def _generate_cycle_feedback(self, cycle: PerceptionActionCycle, result: Any):
        ...
        physiological_signal = FeedbackSignal(
            ...
            data={
                "reaction": (
                    "satisfaction" if getattr(result, "success", False) else "disappointment"
                )
            },
            timestamp=datetime.now(),
        )
        ...
        if cycle.perception_event.perception_type in [
            ...
            social_signal = FeedbackSignal(
                ...
                data={
                    "relationship_impact": (
                        "positive" if getattr(result, "success", False) else "neutral"
                    )
                },
                timestamp=datetime.now(),
            )
    ...
    def _update_performance_metrics(self, cycle: PerceptionActionCycle):
        ...
        self.performance_metrics["average_latency_ms"] = (current_avg * (n - 1) + latency) / n
    ...
    def _handle_perception(self, perception_type: PerceptionType, data: Dict[str, Any]):
        ...
        event = PerceptionEvent.create(
            perception_type=perception_type, source="monitor", data=data, priority=5
        )
    ...
    async def wait_for_cycle(
        self, cycle_id: str, timeout: float = 10.0
    ) -> Optional[PerceptionActionCycle]:
        """
...
if __name__ == "__main__":
    ...
    async def demo():
        ...
        def on_cycle_end(cycle):
            ...
            logger.info(
                f"  Success: {cycle.execution_result.get('success', False) if cycle.execution_result else 'N/A'}"
            )
