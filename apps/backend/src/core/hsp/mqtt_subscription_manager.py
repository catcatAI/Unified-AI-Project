...
class MQTTSubscriptionManager:
    ...
    async def subscribe(
        self, topic: str, callback: Optional[Callable] = None, qos: int = 0, retry: int = 3
    ) -> bool:
        """
        订阅主题
        ...
        async with self._lock:
            ...
            subscription = Subscription(
                topic=topic, qos=qos, callback=callback, status=SubscriptionStatus.PENDING
            )
    ...
    async def batch_subscribe(
        self, topics: List[str], callback: Optional[Callable] = None, qos: int = 0
    ) -> Dict[str, bool]:
        """
        批量订阅
...
# 导出
__all__ = ["MQTTSubscriptionManager", "Subscription", "SubscriptionStatus"]
