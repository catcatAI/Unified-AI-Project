...
class LocalIPCTransport(HSPTransport):
    ...
    def __init__(self, send_queue: Optional[Queue] = None, recv_queue: Optional[Queue] = None):
        """
        初始化本地 IPC 傳輸
...
class MQTTTransport(HSPTransport):
    ...
    def __init__(self, ai_id: str, broker_address: str = "localhost", broker_port: int = 1883):
        """
        初始化 MQTT 傳輸
    ...
    def create_transport(mode: HSPTransportMode, **kwargs) -> HSPTransport:
        ...
        if mode == HSPTransportMode.LOCAL_IPC:
            return LocalIPCTransport(
                send_queue=kwargs.get("send_queue"), recv_queue=kwargs.get("recv_queue")
            )
