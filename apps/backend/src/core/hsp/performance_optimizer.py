...
class HSPPerformanceOptimizer:
    ...
    def clean_expired_cache(self) -> None:
        ...
        expired_keys = [
            key for key, value in self.message_cache.items() if current_time >= value["expires_at"]
        ]
    ...
    async def batch_send_messages(
        self, send_callback: Callable[[List[Dict[str, Any]]], Any]
    ) -> None:
        """批量发送消息"""
        ...
        if len(self.message_queue) >= self.batch_size or (
            current_time - self.last_batch_send >= 1.0
        ):  # 每秒至少发送一次
            # 发送批量消息
            ...
            try:
                ...
                Exception
            ) as e:  # broad exception acceptable: batch send callback may fail with various errors
    ...
    def compress_message(self, message: Dict[str, Any]) -> bytes:
        ...
        try:
            ...
            Exception
        ) as e:  # broad exception acceptable: compression may fail with various errors
    ...
    def decompress_message(self, compressed_data: bytes) -> Dict[str, Any]:
        ...
        try:
            ...
            Exception
        ) as e:  # broad exception acceptable: decompression may fail with various errors
    ...
    async def optimize_message_routing(self, message: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self.batch_send_enabled:
            self.add_message_to_batch(
                {"original_message": message, "compressed_data": compressed_message}
            )
    ...
    def get_network_efficiency_report(self) -> Dict[str, Any]:
        ...
        report = {
            ...
            "efficiency_metrics": {
                "messages_per_second": (
                    total_messages / 60 if total_messages > 0 else 0
                ),  # 假设统计周期为60秒
                "average_message_size_bytes": (
                    network_stats.get("bytes_sent", 0) / total_messages if total_messages > 0 else 0
                ),
    ...
    def enhance_publish(self, original_publish_func: Callable[..., Any]) -> Callable[..., Any]:
        ...
        async def enhanced_publish(*args, **kwargs) -> Any:
            ...
            try:
                ...
                Exception
            ) as e:  # broad exception acceptable: publish function execution may raise various errors
    ...
    def enhance_receive(self, original_receive_func: Callable[..., Any]) -> Callable[..., Any]:
        ...
        async def enhanced_receive(*args, **kwargs) -> Any:
            ...
            try:
                ...
                Exception
            ) as e:  # broad exception acceptable: receive function execution may raise various errors
                logger.error(f"Error in {__name__}: {e}", exc_info=True)
                result = None
