...
class HSPProtocolMiddleware(ABC):
    ...
    @abstractmethod
    async def process_response(self, response: Dict[str, Any], next_middleware: Callable) -> Dict[str, Any]:
        """
        处理响应消息
...
class HSPExtensionManager:
    ...
    async def process_with_middlewares(self, message: Dict[str, Any], is_request: bool = True) -> Dict[str, Any]:
        """通过中间件链处理消息"""
        if not self.middlewares:
            return message
        
        # 创建中间件调用链
        async def call_next(index: int, msg: Dict[str, Any]) -> Dict[str, Any]:
