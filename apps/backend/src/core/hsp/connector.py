...
class HSPConnector:
    def __init__(self, ai_id: str, mock_mode: bool = False, broker_address: str = "localhost",
                 broker_port: int = 1883, mock_mqtt_client: Optional[MagicMock] = None,
                 internal_bus: Optional[InternalBus] = None, message_bridge: Optional[MessageBridge] = None,
        ...
        # 先应用高级性能增强, 再应用基础性能增强
        enhanced_handler = self.advanced_performance_enhancer.enhance_receive(self.message_bridge.handle_external_message)
    ...
    async def close(self):
        ...
        if self.fallback_manager:
            try:
                ...
                if hasattr(self.fallback_manager, 'shutdown') and callable(self.fallback_manager.shutdown):
    ...
    async def _initialize_protocols_with_config(self, config: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if self.fallback_manager:
                ...
                if (
                    hasattr(self.fallback_manager, 'initialize') and
    ...
    async def publish_message(self, topic: str, envelope: HSPMessageEnvelope, qos: int = 1) -> bool:
        self.logger.info(f"HSPConnector: publish_message called. self.external_connector.publish is {type(self.external_connector.publish)}")
