        ...
        for priority, protocol in self.protocols, ::
            if await protocol.health_check():::
                if self.active_protocol != protocol, ::
                    ...
                    logger.info(f"切换到协议, {protocol.protocol_name} (切换次数,
    {self.protocol_switch_count})")
                return
...
(        )
    ...
    Optional[float] = None) -> Optional[FallbackMessage]
        """获取消息"""
        try,
            if timeout, ::
...
async def initialize_fallback_protocols() -> bool,
    ...
    http_port == 0 if os.environ.get('TESTING') == 'true' else 8765, ::
        rint(f"DEBUG, initialize_fallback_protocols - TESTING env var,
    {os.environ.get('TESTING')} HTTP Port, {http_port}")
