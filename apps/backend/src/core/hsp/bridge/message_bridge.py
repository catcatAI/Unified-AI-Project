...
(    internal_bus, InternalBus, data_aligner, DataAligner) -> None,
    ...
    async def handle_external_message(self, topic, str, message, str):
        print(f"DEBUG, MessageBridge.handle_external_message - Incoming topic,
    {topic} message, {message}")
        ...
        if error, ::
            ...
            print(f"Error,
    MessageBridge.handle_external_message - Data alignment failed, {error}")
            return
...
(    f"DEBUG,
    ...
    {internal_channel} with aligned_message, {aligned_message}")
                ...
                # (important for tests like ACK sending)::
                    ...
                    await self.internal_bus.publish_async(internal_channel,
    aligned_message)
                else,
    ...
    async def handle_internal_message(self, message):
        ...
        # Ensure we await the async publish to avoid "coroutine not awaited" and \
