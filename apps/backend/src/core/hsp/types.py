...
在类定义前添加空行
    ...
    defaults to source_ai_id if not specified by sender:
    justification_type: Optional[Literal["text", "inference_chain_id",
    "evidence_ids_list"]]
...
class HSPCapabilityAdvertisementPayload(TypedDict, total = False):
    ...
    agent_name: Optional[str]  # The name of the agent script, e.g.,
    'data_analysis_agent'
...
class HSPCommandPayload(TypedDict, total = False):
    ...
    command_type: str  # Required, namespaced string (e.g.,
    "hsp:command:RestartService")
...
class HSPNotificationPayload(TypedDict, total = False):
    ...
    notification_type: str  # Required, namespaced string (e.g.,
    "hsp:notification:TaskCompleted")
    source_ai_id: str  # Required, DID or URI
