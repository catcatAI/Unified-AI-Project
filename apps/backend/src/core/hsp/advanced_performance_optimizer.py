...
in_requests = float('inf')
    ...
    def _weighted_response_time_selection(self) -> str, :
        ...
        for node_id in self.nodes, ::
            ...
            if stats['response_time_count'] > 0, ::
                avg_response_time = stats['response_time_sum'] /\
    ...
    def record_response(self, node_id, str, response_time, float, success,
    bool == True):
        """记录响应"""
        with self.lb_lock, :
            stats = self.node_stats[node_id]
...
(        )
    ...
    def optimize_message_routing(self, message, Dict[str, Any]):
        """优化消息路由"""
        ...
        if cached_result, ::
            ...
            self.routing_stats.avg_routing_time_ms = sum(self.routing_times()) /\
    len(self.routing_times())
...
{        }
    ...
    def record_response_stats(self, node_id, str, response_time, float, success,
    bool == True):
        """记录响应统计"""
...
{    }
    ...
    # 添加测试节点
    optimizer.add_node_to_load_balancer("node_1", {"host": "192.168.1.101",
    "port": 1883})
    optimizer.add_node_to_load_balancer("node_2", {"host": "192.168.1.102",
    "port": 1883})
    optimizer.add_node_to_load_balancer("node_3", {"host": "192.168.1.103",
    "port": 1883})
