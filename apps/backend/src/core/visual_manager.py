...
from core.visual_config import VisualConfiguration, ModelConfiguration, RenderQuality
from core.visual_effect_generator import VisualEffectGenerator, EffectType
...
try:
    ...
    from src.core.autonomous.self_generation import SelfGeneration, GenerationMode, VisualAttributes
...
class AssetCache:
    ...
    async def put(self, asset_id: str, asset: CachedAsset):
        ...
        async with self._lock:
            ...
            while (
                self.current_size_bytes + asset.memory_usage > self.max_size_bytes and self.assets
            ):
...
class VisualManager:
    ...
    def __init__(self, config: Optional[VisualConfiguration] = None):
        ...
        # Asset management
        self.asset_cache: AssetCache = AssetCache(max_size_mb=self.config.cache.max_cache_size_mb)
    ...
    def connect_emotional_system(self, emotional_system: Any):
        ...
        if hasattr(emotional_system, "register_emotion_callback"):
            emotional_system.register_emotion_callback(self._on_emotion_changed)
    ...
    def connect_tactile_system(self, tactile_system: Any):
        ...
        if hasattr(tactile_system, "register_stimulus_callback"):
            tactile_system.register_stimulus_callback(self._on_tactile_stimulus)
    ...
    async def _preload_worker(self):
        ...
        while self._running:
            try:
                asset_id = await asyncio.wait_for(self.loading_queue.get(), timeout=1.0)
    ...
    def register_asset(
        ...
        """
        metadata = AssetMetadata(
            asset_id=asset_id, asset_type=asset_type, name=name, path=path, tags=tags or []
        )
    ...
    async def play_motion(self, motion: MotionType, loop: bool = False, priority: int = 0):
        """
    ...
    def _on_emotion_changed(self, emotion_data: Any):
        ...
        if intensity > 0.7:
            self.effect_generator.create_emotional_effect(emotion_name, intensity, x=100, y=100)
    ...
    async def update_from_biological_state(
        ...
        if emotional_state:
            ...
            if intensity > 0.8:
                self.effect_generator.create_emotional_effect(dominant_emotion, intensity)
    ...
    def create_particle_effect(
        ...
        return self.effect_generator.start_particle_effect(effect_type, x, y, duration)
    ...
    def get_visual_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "motion": (
                self.current_presentation.current_motion.name
                if self.current_presentation.current_motion
                else None
            ),
