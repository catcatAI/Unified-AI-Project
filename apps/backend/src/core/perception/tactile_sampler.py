...
class TactileSampler:
    ...
    def _classify_material(self, visual_features: Dict[str, Any]) -> MaterialType:
        ...
        if "wood" in name or "table" in name:
            return MaterialType.WOOD
        if "cloth" in name or "shirt" in name:
            return MaterialType.FABRIC
        if "glass" in name:
            return MaterialType.GLASS
        return MaterialType.UNKNOWN

    def generate_contact_feedback(
        self, props: TactileProperties, contact: TactileContactPoint
    ) -> Dict[str, Any]:
        """產生細緻到部位的觸覺反饋數據"""
        # 結合壓力、硬度與粗糙度計算即時感覺
