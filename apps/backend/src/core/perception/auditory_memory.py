...
class AuditoryMemory:
    ...
    def identify_or_register(
        self, embedding: np.ndarray, metadata: Optional[Dict] = None
    ) -> VoiceprintProfile:
        """識別現有聲紋或註冊新聲紋"""
        ...
        if best_match_id:
            ...
            if norm > 0:
                profile.embedding = profile.embedding / norm  # 保持單位長度
            return profile

        # 註冊新聲音來源
        label = "speaker" if metadata and metadata.get("is_speech") else "sound_source"
        norm = np.linalg.norm(embedding)
        unit_embedding = embedding / norm if norm > 0 else embedding
    ...
    def _find_best_match(self, embedding: np.ndarray) -> Optional[str]:
        ...
        if norm == 0:
            return None
        target = embedding / norm
        best_score = -1.0
        best_id = None

        for pid, profile in self.profiles.items():
            score = float(np.dot(profile.embedding, target))
