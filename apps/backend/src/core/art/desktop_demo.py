...
def _draw_self_portrait(desktop: Path, timestamp: str) -> Path:
    """Draw self-portrait artwork and save to desktop."""
    logger.info("   🖼️  绘制自画像...")
...
def _draw_happy_face(desktop: Path, timestamp: str) -> Path:
    """Draw happy face artwork and save to desktop."""
    logger.info("   😊 绘制快乐表情...")
...
def _draw_background(desktop: Path, timestamp: str) -> Path:
    """Draw background scene artwork and save to desktop."""
    logger.info("   🌅 绘制背景图...")
...
def _draw_surprised_face(desktop: Path, timestamp: str) -> Path:
    """Draw surprised face artwork and save to desktop."""
    logger.info("   😲 绘制惊讶表情...")
...
def _create_showcase_file(desktop: Path, timestamp: str, artworks: list) -> Path:
    """Create showcase markdown file on desktop."""
    logger.info("\n2️⃣  创建展示说明...")
...
def _report_completion(desktop: Path, artworks: list) -> None:
...
async def generate_and_save_to_desktop() -> str:
    """生成 Angela 作品并保存到桌面"""
    desktop, timestamp = _setup_generation_environment()
    artworks = []

    logger.info("1️⃣  创作美术作品...")
    artworks.append(_draw_self_portrait(desktop, timestamp))
    artworks.append(_draw_happy_face(desktop, timestamp))
    artworks.append(_draw_background(desktop, timestamp))
    artworks.append(_draw_surprised_face(desktop, timestamp))

    _create_showcase_file(desktop, timestamp, artworks)
    _report_completion(desktop, artworks)
