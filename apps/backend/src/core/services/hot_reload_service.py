...
f old_hsp is None:
    ...
    return {"reloaded": False, "error": "New HSP connector failed to connect."}
                ...
                _ = await new_hsp.subscribe(f"hsp/capabilities/advertisements/general/#", 0)
                _ = await new_hsp.subscribe(f"hsp/results/{old_hsp.ai_id}/#", 0)
                _ = await new_hsp.subscribe(f"hsp/knowledge/facts/general/#", 0)
