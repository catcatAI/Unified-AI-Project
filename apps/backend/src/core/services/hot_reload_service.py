...
class HotReloadService, :
    ...
    metrics["mcp"]["is_connected"] = getattr(mcp, "is_connected", None)
                metrics["mcp"]["fallback_initialized"] = getattr(mcp,
    "fallback_initialized", None)
        except Exception, ::
            pass
...
{                }
    ...
    data_type_filter = lis_incident_data_type_prefix, limit = 50)
                # type ignore
                ab = ham.query_core_memory(metadata_filters = ,
    data_type_filter = lis_antibody_data_type_prefix, limit = 50)
...
{    }
    ...
    async def reload_llm(self, llm_config, Optional[Dict[str, Any]] = None) -> Dict[str,
    Any]
    """
...
{            }
    ...
    async def reload_personality(self, profile_name, Optional[str] = None) -> Dict[str,
    Any]
    """Reload personality profile and propagate to consumers (EmotionSystem,
    DialogueManager)."""
    async with self._lock,
    try,
    ...
    return {"reloaded": False, "error": "PersonalityManager not initialized"}
                ...
                if es is not None and hasattr(es, "personality_profile") and hasattr(pm,
    "current_personality"):::
                    ry,
