...
class ModelRepository:
    ...
    @staticmethod
    def get_best_model_for_resource(
        resource: ComputeResource, available_models: List[str]
    ) -> Optional[str]:
    ...
    async def generate(self, prompt: str, model: str = None, **kwargs) -> Dict[str, Any]:
        ...
        if model is None:
            ...
            if not available:
                ...
                for r in self.resources:
                    if r.available:
                        best = ModelRepository.get_best_model_for_resource(
                            r, list(ModelRepository.MODEL_REQUIREMENTS.keys())
                        )
                        if best:
                            model = best
