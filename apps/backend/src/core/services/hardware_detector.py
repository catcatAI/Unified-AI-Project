...
if __name__ == "__main__":
    ...
    hardware = detect_hardware()
    print(f"\n檢測結果:")
    print(f"  加速器類型: {hardware.accelerator_type.value}")
    print(f"  加速器名稱: {hardware.accelerator_name}")
    print(f"  VRAM: {hardware.vram_mb}MB")
    print(f"  RAM: {hardware.ram_total_gb:.1f}GB total, {hardware.ram_available_gb:.1f}GB available")
    print(f"  CPU 核心: {hardware.cpu_cores}")
    print(f"  CPU Flags: {', '.join(hardware.cpu_flags[:10])}...")
    print(f"  虛擬環境: {'是' if hardware.is_virtual else '否'}")
    
    settings = get_ollama_settings()
    print(f"\n推薦配置:")
    print(f"  Ollama 配置: {json.dumps(settings['ollama_config'], indent=2)}")
    print(f"  模型推薦:")
