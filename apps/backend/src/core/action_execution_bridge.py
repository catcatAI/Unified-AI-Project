...
class ActionExecutionBridge:
    ...
    async def execute_action(
        ...
        if dependencies:
            self._dependencies[action_id] = ActionDependency(
                action_id=action_id, depends_on=dependencies
            )
        ...
        queue_item = (
            priority,
            action_id,
            {"context": context, "parameters": parameters, "created_at": datetime.now()},
        )
    ...
    def _get_next_executable_action(self) -> Optional[tuple]:
        ...
        for item in self._priority_queue:
            ...
            if action_id in self._dependencies:
                ...
                all_completed = all(dep_id in self._completed_actions for dep_id in dep.depends_on)
    ...
    def _update_stats(self, result: ExecutionResult):
        ...
        self._stats["average_execution_time"] = (
            current_avg * (n - 1) + result.execution_time_ms
        ) / n
            ...
            try:
                if hasattr(self.orchestrator, "generate_proactive_message"):
                    response = await self.orchestrator.generate_proactive_message(
                        message=message, emotion=emotion, topic=topic
                    )
    ...
    async def _handle_explore_topic(
        ...
        result = {"topic": topic, "depth": depth, "source": source, "exploration_data": {}}
    ...
    async def _handle_satisfy_need(
        ...
        if need_type == "social":
            ...
            if self.desktop_pet:
                messages = ["我想你了，在吗？", "有点无聊呢，陪我聊聊天吧~", "今天过得怎么样？"]
                import random
    ...
    async def _handle_download_resource(
        ...
        resource_type = parameters.get(
            "resource_type", "general"
        )  # live2d, knowledge, image, audio, general
        auto_ingest = parameters.get("auto_ingest", True)

        result = {"url": url, "resource_type": resource_type, "downloaded": False}
        ...
        try:
            ...
            if hasattr(self.download_manager, "download_file"):
                download_result = await self.download_manager.download_file(
                    url=url, destination=destination, resource_type=resource_type
                )
        ...
        try:
            if operation == "read":
                ...
                if hasattr(self.file_manager, "write_file"):
                    success = await self.file_manager.write_file(path, content, encoding=encoding)
    ...
    async def _handle_system_query(
        ...
        result = {"query_type": query_type, "timestamp": datetime.now().isoformat()}
...
if __name__ == "__main__":
    ...
    async def demo():
        ...
        result = await bridge.execute_action(
            action_type=ActionType.SYSTEM_QUERY, parameters={"query_type": "health"}, priority=3
        )
        logger.info(f"   Result: {result.to_dict()}")
