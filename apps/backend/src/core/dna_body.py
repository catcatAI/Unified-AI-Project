...
class AngelaDNA:
    ...
    def _get_spine_offsets(self, theta_matrix: List[float], segment_height: float = 25.0) -> List[float]:
        """
        計算 9 段脊椎的累積位移 (累積正弦偏移).
        theta_matrix: 9 個弧度值，代表各段彎曲。
        """
        offsets = [0.0] * 10 # 包含基座與 9 個節點
        current_x = 0.0
        for i, angle in enumerate(theta_matrix):
            # 累積位移 = 前一段位移 + sin(角度) * 節點高度
            current_x += np.sin(angle) * segment_height
