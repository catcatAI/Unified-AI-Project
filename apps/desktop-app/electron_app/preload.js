  ...
  "electronAPI", // This will be window.electronAPI in the renderer
  {
    ...
    sendMessage: (channel, data) => {
      ...
      if (validChannels.includes(channel)) {
        console.log(
          `Preload: Sending message on channel '${channel}' with data:`,
          data,
        );
    ...
    invoke: async (channel, ...args) => {
      ...
      if (validChannels.includes(channel)) {
        console.log(
          `Preload: Invoking main process handler on channel '${channel}' with args:`,
          args,
        );
        return await ipcRenderer.invoke(channel, ...args);
      }
      console.warn(
        `Preload: Attempted to invoke on invalid channel '${channel}'`,
      );
      return null;
    },
