...
class BackendWebSocketClient {
  ...
  _handleStateUpdate(data) {
    ...
    if (data.sequence !== undefined) {
      ...
      if (data.sequence > this.expectedSequence) {
        console.log(
          `[BackendWebSocket] 收到乱序消息: ${data.sequence}, 期望: ${this.expectedSequence}`
        )
  ...
  _handleReconnect() {
    ...
    if (this.reconnectAttempts < this.maxReconnectAttempts) {
      ...
      console.log(
        `Reconnecting in ${reconnectDelay.toFixed(0)}ms (${(reconnectDelay / 1000).toFixed(1)}s)...`
      )
  ...
  send(message) {
    if (!this.connected) {
      ...
      console.warn('Backend not connected, message queued:', message)
      return false
    }
  ...
  async sendMessage(text) {
    if (!this.connected) {
      console.warn('Backend not connected, message not sent:', text)
  ...
  _addToOfflineQueue(message) {
    ...
    const existingIndex = this._offlineQueue.findIndex(
      (msg) => this._getMessageKey(msg) === messageKey
    )
    ...
    if (existingIndex !== -1) {
      ...
      this._offlineQueue[existingIndex]._retryCount =
        (this._offlineQueue[existingIndex]._retryCount || 0) + 1
    ...
    console.log(
      `[BackendWebSocket] 消息已加入離線隊列 (${this._offlineQueue.length}/${this._maxQueueSize}), 優先級: ${queuedMessage._priority}`
    )
  }
  ...
  async _flushOfflineQueue() {
    ...
    console.log(
      `[BackendWebSocket] 離線消息發送完成: ${successCount} 成功, ${failCount} 失敗, ${this._offlineQueue.length} 剩餘`
    )
