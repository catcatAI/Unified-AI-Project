...
class UnifiedDisplayMatrix {
    ...
    handleTouch(screenX, screenY, touchType = 'pat') {
        ...
        const resourceCoords = this.canvasToResource(canvasCoords.x, canvasCoords.y);
        ...
        // 2. 识别人体区域
        const bodyPart = this.identifyBodyPart(resourceCoords.x, resourceCoords.y);
        ...
        // 3. 计算触觉强度
        const hapticIntensity = this.calculateHapticIntensity(0.8, canvasCoords, { width: 20, height: 20 });
        ...
        // 5. 触发触觉反馈 (如果已连接)
        if (this.angelaSystem.hapticHandler && hapticIntensity > 0.1) {
            this.angelaSystem.hapticHandler.triggerHaptic(hapticIntensity, touchType);
        }

        // 6. 触发 Live2D 表情 (如果已连接)
        if (this.angelaSystem.live2DManager && bodyPart.expression) {
    ...
    handleClick(screenX, screenY) {
        ...
        if (this.angelaSystem.stateMatrix && result.bodyPart) {
