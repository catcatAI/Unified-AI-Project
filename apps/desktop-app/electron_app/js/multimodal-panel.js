...
class MultimodalPanel {
  ...
  constructor() {
    this.client = new MultimodalAPIClient(
      localStorage.getItem('backend_ip') || 'http://localhost:8000'
    )
  ...
  async _startRecording() {
    try {
      ...
      this.mediaRecorder.onstop = async () => {
        ...
        this._showResult(
          'audio-result',
          { status: 'Recorded', size: this.audioData.byteLength },
          true
        )
      }
  ...
  _bindEncodeDecode() {
    document
      .getElementById('btn-vision-encode')
      .addEventListener('click', () => this._doVisionEncode())
    document
      .getElementById('btn-vision-decode')
      .addEventListener('click', () => this._doVisionDecode())
    document
      .getElementById('btn-audio-encode')
      .addEventListener('click', () => this._doAudioEncode())
    document
      .getElementById('btn-audio-decode')
      .addEventListener('click', () => this._doAudioDecode())
    document
      .getElementById('btn-refresh-items')
      .addEventListener('click', () => this._refreshItems())
    document.getElementById('btn-clear-items').addEventListener('click', () => this._clearItems())
  }

  async _doVisionEncode() {
    if (!this.visionData)
      return this._showResult('vision-result', { error: 'No image uploaded' }, false)
  ...
  async _doVisionDecode() {
    ...
    if (visionItems.length === 0)
      return this._showResult('vision-result', { error: 'No vision items to decode' }, false)
  ...
  async _doAudioEncode() {
    if (!this.audioData)
      return this._showResult('audio-result', { error: 'No audio recorded/uploaded' }, false)
  ...
  async _doAudioDecode() {
    ...
    if (audioItems.length === 0)
      return this._showResult('audio-result', { error: 'No audio items to decode' }, false)
  ...
  async _refreshQuality() {
    try {
      ...
      this._setMetric(
        'q-ssim',
        vision.avg_ssim?.toFixed(4) ?? 'N/A',
        vision.avg_ssim >= 0.7 ? 'good' : vision.avg_ssim >= 0.3 ? 'warn' : 'bad'
      )
      this._setMetric(
        'q-psnr',
        vision.avg_psnr?.toFixed(1) ?? 'N/A',
        vision.avg_psnr >= 25 ? 'good' : vision.avg_psnr >= 15 ? 'warn' : 'bad'
      )
      this._setMetric('q-vision-calls', vision.total_calls ?? 0, '')
      this._setMetric(
        'q-vision-time',
        vision.avg_time_ms ? `${vision.avg_time_ms.toFixed(0)}ms` : 'N/A',
        ''
      )

      this._setMetric(
        'q-snr',
        audio.avg_snr?.toFixed(1) ?? 'N/A',
        audio.avg_snr >= 15 ? 'good' : audio.avg_snr >= 5 ? 'warn' : 'bad'
      )
      this._setMetric('q-audio-calls', audio.total_calls ?? 0, '')
      this._setMetric(
        'q-audio-time',
        audio.avg_time_ms ? `${audio.avg_time_ms.toFixed(0)}ms` : 'N/A',
        ''
      )
