  ...
  recipes: [
    ...
    ...(CARDS.rpgRecipes || []).map((r) => {
      ...
      return {
        ...
        ingredientMap,
        output: 'rpg_item_' + (r.resultItem || r.name),
        count: r.resultQty || 1,
        name: r.name,
      }
    }),
...
function tryStack(draggedCard, targetCard) {
  ...
  if (
    dragTemplate.type === 'resource' &&
    targetTemplate.type === 'resource' &&
    ...
    if (recipe.ingredientMap) {
      ...
      if (canCraft) {
        ...
        if (recipe.output.startsWith('item_') || recipe.output.startsWith('res_')) {
          ...
          return {
            type: 'craft',
            message: `✅ ${recipe.name}！獲得 ${getCardTemplate(recipe.output)?.name || recipe.output}`,
          }
        } else {
  ...
  if (dragTemplate.type === 'character' && targetTemplate.type === 'enemy') {
    return {
      type: 'combat',
      attacker: dragTemplate,
      defender: targetTemplate,
      attackerCard: draggedCard,
  ...
  if (dragTemplate.type === 'item' && targetTemplate.type === 'character') {
    if (dragTemplate.category === 'consumable' || dragTemplate.category === 'food') {
      ...
      return {
        type: 'use',
        message: `${targetTemplate.name} 使用了 ${dragTemplate.name}，恢復 ${heal} HP`,
      }
    }
  }
...
function executeCombat(attackerTemplate, defenderTemplate, attackerCard, defenderCard) {
  ...
  const finalAtk = {
    ...atkStats,
    atk: (atkStats.atk || 0) + eqBonus.atk,
    def: (atkStats.def || 0) + eqBonus.def,
    spd: (atkStats.spd || 0) + eqBonus.spd,
  }
  ...
  const results = []
  results.push({
    text: `${attackerTemplate?.name || '你'} 攻擊 ${defenderTemplate.name}，造成 ${atkDmg} 傷害！`,
    type: 'damage',
  })
  ...
  for (const card of GameState.boardCards) {
    ...
    if (template && template.type === 'location' && template.resourceRate > 0) {
      if (GameState.tickCount % 3 === 0) {
        // Generate resources
        const resourcePool = CARDS.resources.filter(
          (r) => r.category === 'material' || r.category === 'herb' || r.category === 'food'
        )
    ...
    if (template.type === 'location') {
      if (template.hpMod)
        GameState.hp = Math.max(0, Math.min(GameState.maxHp, GameState.hp + template.hpMod * 0.1))
      if (template.sanMod)
        GameState.sanity = Math.max(0, Math.min(100, GameState.sanity + template.sanMod * 0.1))
    }
...
function saveGame() {
  try {
    const data = JSON.stringify({
      ...
      sidebarCards: GameState.sidebarCards,
      inventory: GameState.inventory,
      unlockedLocations: GameState.unlockedLocations,
      flags: GameState.flags,
      discoveredDialogues: GameState.discoveredDialogues,
