...
;(function () {
  ...
  function onDragMove(e) {
    ...
    const nearEdge =
      card.x < edgeThreshold ||
      card.y < edgeThreshold ||
  ...
  function onCardContextMenu(e) {
    ...
    if (
      template.type === 'item' &&
      ((template.category || template.itemType) === 'weapon' ||
        (template.stats && (template.stats.atk || template.stats.def)))
    ) {
      const slot = (template.stats?.atk || 0) > (template.stats?.def || 0) ? 'weapon' : 'armor'
      actions.push({
        text: `🗡️ 裝備到${slot === 'weapon' ? '武器' : '防具'}欄`,
        fn: () => {
    ...
    if (template.type === 'recipe') {
      actions.push({
        text: '📖 查看配方',
        fn: () => showNotification(template.desc || template.name),
      })
    }
    if (
      template.type === 'nation' ||
    ...
    if (template.type === 'story') {
      ...
      if (template.dialogue && triggerMet) {
        ...
        actions.push({
          text: `🔒 條件: ${template.trigger}`,
          fn: () => showNotification(`條件未滿足：${template.trigger}`),
        })
      }
      actions.push({
        text: '📋 查看詳情',
    ...
    if (actions.length === 0) {
      actions.push({
        text: template.desc ? template.desc.slice(0, 40) : '無可用動作',
        fn: () => {},
      })
    }
  ...
  function onCardDblClick(e) {
    ...
    // Item with value → show buy/sell
      ...
      const prices = E.getShopPrices(card.templateId)
      showNotification(
        `${template.icon} ${template.name} | 賣出: ${prices.sell}💰 | 價值: ${template.value}g`
      )
    }
    // RPG item → show info
    ...
    // Enemy → show enemy info
      ...
      showNotification(
        `${template.icon} ${template.name} | HP:${stats?.hp || '?'} ATK:${stats?.atk || '?'} DEF:${stats?.def || '?'}`
      )
    }
  ...
  function checkStoryTrigger(trigger) {
    ...
    try {
      ...
      if (hasItemMatch) {
        ...
        return (
          (state.sidebarCards || []).some((c) => c.templateId === itemId) ||
          (state.boardCards || []).some((c) => c.templateId === itemId)
        )
      }
  ...
  function showDialogDetail(template) {
    ...
    panel.style.cssText =
      'background:linear-gradient(135deg,#1a1a2e,#16213e);border:1px solid rgba(255,255,255,0.15);border-radius:12px;padding:24px;max-width:500px;max-height:70vh;overflow-y:auto;color:#e0e0e0;font-size:14px;'
  ...
  const npcSchedules =
    window.CARDS_DATA?.NPC_SCHEDULES || (typeof NPC_SCHEDULES !== 'undefined' ? NPC_SCHEDULES : {})
