...
const cardsClean =
  cardsRaw.replace(/if \(typeof module.*[\s\S]*$/, '') + // Remove module.exports block at end
...
function decideAction(state) {
  ...
  for (const locId of state.unlockedLocations) {
    if (!exploredLocs.includes(locId)) {
      if (totalPlaceDecisions < 30)
        decisions.push({
          action: 'place_card',
          templateId: locId,
          priority: 9,
          reason: `Place new location ${E.getCardTemplate(locId)?.name}`,
        })
    }
  }
  ...
  for (const ch of charsInSidebar) {
    if (totalPlaceDecisions < 30)
      decisions.push({
        ...
        priority: 9,
        reason: `Place character ${E.getCardTemplate(ch.templateId)?.name}`,
      })
  }
  ...
  for (const recipe of CARDS.recipes) {
    ...
    const hasInSidebar = recipe.inputs.every((i) =>
      state.sidebarCards.some((s) => s.templateId === i)
    )
        ...
        if (inSidebar && onBoard < needed) {
          if (totalPlaceDecisions < 30)
            decisions.push({
              ...
              templateId: resId,
              priority: 6,
              reason: `Place ${E.getCardTemplate(resId)?.name} for crafting`,
            })
          break
        }
      }
    ...
    if (charTemplate?.dialogue && !completedDialogues.has(charTemplate.dialogue)) {
      ...
      if (matchLoc) {
        decisions.push({
          ...
          cardB: matchLoc.id,
          priority: 8,
          reason: `Talk to ${charTemplate.name}`,
        })
      }
    }
  }
  ...
  if (boardChars.length > 0 && boardEnemies.length > 0) {
    ...
    decisions.push({
      action: 'stack',
      cardA: boardChars[0].id,
      cardB: weakest.id,
      priority: 5,
      reason: `Fight ${E.getCardTemplate(weakest.templateId)?.name}`,
    })
  }
  ...
  if (state._placeCount < 20 && state.boardCards.length < 12) {
    ...
    if (itemsInSidebar.length > 0) {
      if (totalPlaceDecisions < 30)
        decisions.push({
          action: 'place_card',
          templateId: itemsInSidebar[0].templateId,
          priority: 3,
          reason: 'Place item on board',
        })
    }
  }
  ...
  switch (decision.action) {
    ...
    case 'craft': {
      ...
      for (const input of recipe.inputs) {
        ...
        const boardRes = state.boardCards.find(
          (c) => c.templateId === input && E.getCardType(c.templateId) === 'resource'
        )
        if (boardRes) {
      ...
      if (canCraft) {
        ...
        logEvent(
          ...
          state.timeOfDay,
          'craft',
          `Crafted ${recipe.name} → ${t?.name || recipe.output}`
        )
...
function handleStackResult(result, state) {
  switch (result.type) {
    ...
    case 'combat':
      ...
      const combatResult = E.executeCombat(
        result.attacker,
        result.defender,
        result.attackerCard,
        result.defenderCard
...
function autoAdvanceDialogue(dialogueId, state) {
  ...
  for (const choice of dialogue.choices) {
    let available = true
    if (choice.requires) {
      if (choice.requires.knowledge && state.knowledge < choice.requires.knowledge)
        available = false
  ...
  if (bestChoice) {
      ...
      if (e.unlock) {
        if (e.unlock.startsWith('loc_') && !state.unlockedLocations.includes(e.unlock)) {
          ...
          logEvent(
            ...
            state.timeOfDay,
            'unlock',
            `Unlocked ${E.getCardTemplate(e.unlock)?.name || e.unlock}`
          )
        } else {
        ...
        for (const uid of e.unlocks) {
          if (uid.startsWith('loc_') && !state.unlockedLocations.includes(uid)) {
            ...
            logEvent(
              ...
              state.timeOfDay,
              'unlock',
              `Unlocked ${E.getCardTemplate(uid)?.name || uid}`
            )
          } else {
      ...
      if (e.items) {
        for (const itemId of e.items) {
          ...
          logEvent(
            ...
            state.timeOfDay,
            'item',
            `Got ${E.getCardTemplate(itemId)?.name || itemId}`
          )
        }
      }
      if (e.bond) {
    ...
    if (bestChoice.next) {
      logEvent(
        ...
        state.timeOfDay,
        'dialogue_choice',
        `${dialogue.speaker}: "${bestChoice.text}" → ${bestChoice.next}`
      )
      ...
      logEvent(
        ...
        state.timeOfDay,
        'dialogue_end',
        `${dialogue.speaker}: "${bestChoice.text}"`
      )
    }
  }
  dialogueDepth = 0
...
function detectBugs(state) {
  ...
  if (
    !balanceIssueLogged &&
    state.day === 15 &&
    !state.flags['unlocked_corridor'] &&
    !state.unlockedLocations.includes('loc_corridor')
  ) {
...
async function runGame() {
  ...
  try {
    ...
    console.log(
      `🤖 Angela AI: ${report.angelaConnected ? '✅ Connected' : '❌ Offline (using fallbacks)'}`
    )
  } catch {
  ...
  for (let day = 1; day <= MAX_DAYS; day++) {
    ...
    if (day % 5 === 0 || day === 1) {
      console.log(
        `📅 Day ${day}: HP=${state.hp} SAN=${state.sanity} Gold=${state.gold} Knowledge=${state.knowledge}`
      )
      console.log(
        `   Board: ${JSON.stringify(boardTypes)} | Sidebar: ${state.sidebarCards.length} | Inventory: ${state.inventory.length}`
      )
    }
  }
