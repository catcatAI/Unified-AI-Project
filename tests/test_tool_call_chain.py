...
class TestToolCallChainTracker(unittest.TestCase):
    ...
    def test_start_tool_chain(self) -> None:
        ...
        # 验证返回的chain_id不为空
        _ = self.assertIsNotNone(chain_id)
        _ = self.assertTrue(chain_id.startswith("chain-"))
        
        # 验证当前链已创建
        _ = self.assertIsNotNone(self.tracker.chain_context.current_chain)
        _ = self.assertEqual(self.tracker.chain_context.current_chain.root_tool_id, root_tool_id)
    ...
    def test_end_tool_chain(self) -> None:
        ...
        # 验证返回的chain_id正确
        _ = self.assertEqual(ended_chain_id, chain_id)
        
        # 验证当前链已清空
        _ = self.assertIsNone(self.tracker.chain_context.current_chain)
        
        # 验证链已存储
        _ = self.assertIn(chain_id, self.tracker.stored_chains)
    ...
    def test_start_tool_call(self) -> None:
        ...
        # 验证返回的call_id不为空
        _ = self.assertIsNotNone(call_id)
        _ = self.assertTrue(call_id.startswith(f"{chain_id}-"))
        
        # 验证调用节点已创建
        _ = self.assertIn(call_id, self.tracker.chain_context.current_chain.calls)
        call_node = self.tracker.chain_context.current_chain.calls[call_id]
        _ = self.assertEqual(call_node.tool_id, tool_id)
        _ = self.assertEqual(call_node.parameters, parameters)
        
    def test_end_tool_call(self) -> None:
        """测试结束工具调用"""
        root_tool_id = "test_tool_1"
        _ = self.tracker.start_tool_chain(root_tool_id)
        ...
        # 验证返回的call_id正确
        _ = self.assertEqual(ended_call_id, call_id)
        
        # 验证调用节点已更新
        call_node = self.tracker.chain_context.current_chain.calls[call_id]
        _ = self.assertEqual(call_node.result, result)
        _ = self.assertTrue(call_node.success)
        _ = self.assertIsNotNone(call_node.completed_at)
        _ = self.assertGreaterEqual(call_node.duration, 0)
    ...
    def test_tool_call_chain_structure(self) -> None:
        ...
        # 结束调用链
        _ = self.tracker.end_tool_chain()
        
        # 验证调用链结构
        chain = self.tracker.stored_chains[chain_id]
        _ = self.assertEqual(chain.root_tool_id, root_tool_id)
        _ = self.assertEqual(chain.root_call_id, root_call_id)
        _ = self.assertEqual(len(chain.calls), 3)
        
        # 验证根调用
        root_call = chain.calls[root_call_id]
        _ = self.assertEqual(root_call.tool_id, root_tool_id)
        _ = self.assertEqual(root_call.parameters, root_parameters)
        _ = self.assertEqual(len(root_call.child_calls), 2)
        _ = self.assertIn(child1_call_id, root_call.child_calls)
        _ = self.assertIn(child2_call_id, root_call.child_calls)
        _ = self.assertIsNone(root_call.parent_id)
        
        # 验证子调用1
        child1_call = chain.calls[child1_call_id]
        _ = self.assertEqual(child1_call.tool_id, child1_tool_id)
        _ = self.assertEqual(child1_call.parameters, child1_parameters)
        _ = self.assertEqual(child1_call.parent_id, root_call_id)
        _ = self.assertEqual(len(child1_call.child_calls), 0)
        
        # 验证子调用2
        child2_call = chain.calls[child2_call_id]
        _ = self.assertEqual(child2_call.tool_id, child2_tool_id)
        _ = self.assertEqual(child2_call.parameters, child2_parameters)
        _ = self.assertEqual(child2_call.parent_id, root_call_id)
        _ = self.assertEqual(len(child2_call.child_calls), 0)
    ...
    def test_get_call_chain_by_id(self) -> None:
        ...
        _ = self.assertEqual(chain.chain_id, chain_id)
        _ = self.assertEqual(chain.root_tool_id, root_tool_id)
        
        # 获取不存在的调用链
        non_existent_chain = self.tracker.get_call_chain_by_id("non_existent")
        _ = self.assertIsNone(non_existent_chain)
    ...
    def test_get_chains_by_tool_id(self) -> None:
        ...
        _ = self.assertEqual(len(chains), 1)
        _ = self.assertEqual(chains[0].chain_id, chain_id)
        
        # 获取不存在工具的调用链
        non_existent_chains = self.tracker.get_chains_by_tool_id("non_existent_tool")
        _ = self.assertEqual(len(non_existent_chains), 0)
    ...
    def test_error_handling_in_chain(self) -> None:
        ...
        # 验证错误信息被正确记录
        chain = self.tracker.stored_chains[chain_id]
        _ = self.assertFalse(chain.success)
        _ = self.assertEqual(chain.error_message, error_message)
