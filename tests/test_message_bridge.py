...
def mock_data_aligner():
    ...
    mock.align_message.return_value = (None, None)  # Assuming (aligned_data, error)
    return mock
...
@pytest.mark.asyncio
async def test_message_bridge_initialization(
    mock_external_connector, mock_internal_bus, mock_data_aligner
) -> None:
...
@pytest.mark.asyncio()
async def test_handle_external_message_valid_fact(
    message_bridge, mock_data_aligner, mock_internal_bus
) -> None:
    topic = "hsp/fact/some_id"
    message_payload = {
        "message_type": "HSP.Fact_v0.1",
        "payload": {"id": "fact123", "content": "test fact"},
    }
    ...
    mock_internal_bus.publish_async.assert_called_once_with("hsp.external.fact", message_payload)
...
@pytest.mark.asyncio()
async def test_handle_external_message_data_alignment_error(
    message_bridge, mock_data_aligner, mock_internal_bus, capsys
) -> None:
    topic = "hsp/fact/some_id"
    message_payload = {
        "message_type": "HSP.Fact_v0.1",
    ...
    out, err = capsys.readouterr()
    assert (
        "Error, MessageBridge.handle_external_message - Data alignment failed, alignment error"
        in out
    )


@pytest.mark.asyncio()
async def test_handle_external_message_unknown_message_type(
    message_bridge, mock_data_aligner, mock_internal_bus, capsys
) -> None:
    topic = "hsp/fact/some_id"
    message_payload = {
    ...
    out, err = capsys.readouterr()
    assert (
        "Warning, MessageBridge.handle_external_message - Unknown message_type 'UNKNOWN_TYPE'. Not publishing to internal bus."
        in out
    )


@pytest.mark.asyncio()
async def test_handle_internal_message_publish_to_external(
    message_bridge, mock_external_connector
) -> None:
    internal_message = {
...
@pytest.mark.asyncio()
async def test_handle_internal_message_payload_types(
    message_bridge, mock_external_connector
) -> None:
    # Test with string payload
    await message_bridge.handle_internal_message(
        {"topic": "test/str", "payload": "hello", "qos": 0}
    )
    ...
    await message_bridge.handle_internal_message(
        {"topic": "test/bytes", "payload": b"raw_bytes", "qos": 1}
    )
...
@pytest.mark.asyncio()
async def test_handle_external_message_no_publish_async_on_bus(
    message_bridge, mock_data_aligner, mock_internal_bus
) -> None:
    ...
    mock_internal_bus.publish.assert_called_once_with("hsp.external.fact", message_payload)
    assert not hasattr(
        mock_internal_bus, "publish_async"
    )  # Ensure it was indeed deleted for this test
