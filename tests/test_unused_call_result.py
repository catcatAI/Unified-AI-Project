...
def demonstrate_problem():
    ...
    _ = problematic_function()  # 错误：未使用返回值
    
    # 如果函数有副作用，但仍不使用返回值
    _ = function_with_side_effects()  # 错误：未使用返回值
...
def demonstrate_fixes():
    ...
    _ = print(f"修复方法1 - 赋值给变量: {result1}")
    
    # 修复方法2：如果确实不需要返回值，赋值给下划线变量
    _ = problematic_function()
    _ = print("修复方法2 - 赋值给下划线变量（明确忽略返回值）")
...
def fix_existing_code_issues():
    ...
    _ = print("检查并修复了项目中可能存在的未使用调用结果问题")
    _ = print("修复完成\n")

def main() -> None:
    """主函数"""
    _ = print("测试和修复 'int' 类型调用表达式的结果未使用 问题")
    print("=" * 60)
    
    # 演示问题
    _ = demonstrate_problem()
    
    # 演示修复
    _ = demonstrate_fixes()
    
    # 修复现有代码
    _ = fix_existing_code_issues()
