...
class TestHSPSecurity:
    ...
    def test_message_encryption_and_decryption(self, security_manager) -> None:
        """测试消息加密和解密"""
        # 创建测试消息
        message = {"content": "This is a secret message", "timestamp": "2023-01-01T00,00,00Z"}
    ...
    async def test_secure_message_processing(self, security_context) -> None:
        ...
        message = {
            ...
            "message_type": "HSP,TestMessage_v0.1",
            "payload": {"content": "This is a test message", "timestamp": "2023-01-01T00,00,00Z"},
        }
        ...
        is_valid, processed_message = security_context.authenticate_and_process_message(
            secured_message
        )
    ...
    async def test_hsp_connector_secure_message_dispatch(self, hsp_connector) -> None:
        ...
        # 安全处理消息
        secured_message = hsp_connector.security_context.secure_message(
            message, message["sender_ai_id"]
        )
        ...
        # 分发消息
        await hsp_connector._dispatch_fact_to_callbacks(secured_message)
