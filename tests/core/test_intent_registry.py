...
class TestIntentRegistry:
    ...
    def test_single_keyword_match(self, registry):
        registry.register(IntentPattern("greeting", ["hello", "hi"], priority=5))
        intent, conf = registry.detect("hello world")
        assert intent == "greeting"
        assert conf > 0.0

    def test_priority_based_matching(self, registry):
        registry.register(IntentPattern("low", ["test"], priority=1))
        registry.register(IntentPattern("high", ["test"], priority=10))
        intent, conf = registry.detect("test keyword")
        assert intent is not None

    def test_category_filter(self, registry):
        registry.register(IntentPattern("a", ["keyword"], category="cat_a", priority=5))
        registry.register(IntentPattern("b", ["keyword"], category="cat_b", priority=5))
    ...
    def test_register_dynamic_pattern(self, registry):
        new_pattern = IntentPattern("test_intent", ["test_kw"], priority=10)
    ...
    def test_get_keywords_existing(self, registry):
        registry.register(IntentPattern("test_intent", ["kw1", "kw2"], priority=5))
    ...
    def test_detect_task_type_known(self, registry):
        registry.register(IntentPattern("character_card", ["角色", "角色卡"], priority=6))
    ...
    def test_detect_multiple_keywords_increases_confidence(self, registry):
        registry.register(IntentPattern("task", ["生成", "建立"], priority=5))
        intent, conf_many = registry.detect("生成並建立檔案")
        intent, conf_one = registry.detect("生成檔案")
        assert conf_many > conf_one

    def test_register_preserves_existing_patterns(self, registry):
        registry.register(IntentPattern("p1", ["kw1"], priority=5))
        registry.register(IntentPattern("p2", ["kw2"], priority=5))
        assert len(registry.patterns) == 2

    def test_detect_task_type_prioritizes_order(self, registry):
        registry.register(IntentPattern("character_card", ["角色"], priority=6))
