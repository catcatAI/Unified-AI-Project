...
class TestToolContextManager(unittest.TestCase):
    ...
    def test_create_category(self) -> None:
        ...
        _ = self.assertIsNotNone(category)
        _ = self.assertEqual(category.category_id, category_id)
        _ = self.assertEqual(category.name, name)
        _ = self.assertEqual(category.description, description)
        
    def test_create_category_with_parent(self) -> None:
        """测试创建带父分类的子分类"""
        # 先创建父分类
        parent_id = "parent_category"
        _ = self.tool_context_manager.create_category(parent_id, "Parent Category")
        ...
        _ = self.assertIn(child_category, parent_category.sub_categories)
        _ = self.assertEqual(child_category.parent_id, parent_id)
    ...
    def test_update_category(self) -> None:
        ...
        # 创建分类
        _ = self.tool_context_manager.create_category(category_id, original_name, original_description)
        ...
        # 验证更新
        category = self.tool_context_manager.get_category(category_id)
        _ = self.assertEqual(category.name, new_name)
        _ = self.assertEqual(category.description, new_description)
    ...
    def test_delete_category(self) -> None:
        ...
        # 创建分类
        _ = self.tool_context_manager.create_category(category_id, "Test Category")
    ...
    def test_register_tool(self) -> None:
        ...
        _ = self.assertIsNotNone(tool)
        _ = self.assertEqual(tool.tool_id, tool_id)
        _ = self.assertEqual(tool.name, name)
        _ = self.assertEqual(tool.description, description)
        _ = self.assertEqual(tool.category_id, category_id)
        
        # 验证工具已添加到分类中
        category = self.tool_context_manager.get_category(category_id)
        _ = self.assertIn(tool, category.tools)
    ...
    def test_register_duplicate_tool(self) -> None:
        ...
        # 注册工具
        _ = self.tool_context_manager.register_tool(tool_id, "Test Tool")
    ...
    def test_update_tool(self) -> None:
        ...
        # 创建分类
        _ = self.tool_context_manager.create_category(original_category_id, "Original Category")
        _ = self.tool_context_manager.create_category(new_category_id, "New Category")
        
        # 注册工具
        _ = self.tool_context_manager.register_tool(tool_id, original_name, original_description, original_category_id)
        ...
        # 验证更新
        tool = self.tool_context_manager.get_tool(tool_id)
        _ = self.assertEqual(tool.name, new_name)
        _ = self.assertEqual(tool.description, new_description)
        _ = self.assertEqual(tool.category_id, new_category_id)
        ...
        _ = self.assertNotIn(tool, original_category.tools)
        _ = self.assertIn(tool, new_category.tools)
    ...
    def test_record_tool_usage(self) -> None:
        ...
        _ = self.assertTrue(record_result)
        
        # 验证使用记录
        usage_history = self.tool_context_manager.get_tool_usage_history(tool_id)
        _ = self.assertEqual(len(usage_history), 1)
        
        usage_record = usage_history[0]
        _ = self.assertEqual(usage_record.parameters, parameters)
        _ = self.assertEqual(usage_record.result, result)
        _ = self.assertEqual(usage_record.duration, duration)
        _ = self.assertEqual(usage_record.success, success)
        
        # 验证性能指标更新
        tool = self.tool_context_manager.get_tool(tool_id)
        metrics = tool.performance_metrics
        _ = self.assertEqual(metrics.total_calls, 1)
        _ = self.assertEqual(metrics.success_rate, 1.0)
        _ = self.assertEqual(metrics.average_duration, duration)
        _ = self.assertEqual(metrics.min_duration, duration)
        _ = self.assertEqual(metrics.max_duration, duration)
        _ = self.assertEqual(metrics.error_count, 0)
    ...
    def test_record_tool_usage_failure(self) -> None:
        ...
        metrics = tool.performance_metrics
        _ = self.assertEqual(metrics.total_calls, 1)
        _ = self.assertEqual(metrics.success_rate, 0.0)
        _ = self.assertEqual(metrics.average_duration, duration)
        _ = self.assertEqual(metrics.error_count, 1)
    ...
    def test_get_top_performing_tools(self) -> None:
        ...
        for i, tool_id in enumerate(tool_ids):
            _ = self.tool_context_manager.register_tool(tool_id, f"Tool {i+1}")
        ...
        # 验证排序（按成功率优先，然后按耗时）
        _ = self.assertEqual(len(top_tools), 3)
        _ = self.assertEqual(top_tools[0].tool_id, "tool1")  # 最高成功率
        _ = self.assertEqual(top_tools[1].tool_id, "tool2")  # 中等成功率
        _ = self.assertEqual(top_tools[2].tool_id, "tool3")  # 最低成功率
    ...
    def test_get_most_used_tools(self) -> None:
        ...
        # 验证排序（按使用次数）
        _ = self.assertEqual(len(most_used_tools), 3)
        _ = self.assertEqual(most_used_tools[0].tool_id, "tool1")  # 使用最多
        _ = self.assertEqual(most_used_tools[1].tool_id, "tool2")  # 使用中等
        _ = self.assertEqual(most_used_tools[2].tool_id, "tool3")  # 使用最少
...
or tool_id in tool_ids:
    ...
    def test_search_tools_by_name(self) -> None:
        ...
        for tool_id, name in zip(tool_ids, tool_names):
            _ = self.tool_context_manager.register_tool(tool_id, name)
...
or tool_id in tool_ids:
        ...
        _ = self.assertEqual(len(math_tools), 1)
        _ = self.assertEqual(math_tools[0].tool_id, "math_tool")
    ...
    def test_move_tool_to_category(self) -> None:
        ...
        # 创建分类
        _ = self.tool_context_manager.create_category(source_category_id, "Source Category")
        _ = self.tool_context_manager.create_category(target_category_id, "Target Category")
        ...
        # 验证移动结果
        tool = self.tool_context_manager.get_tool(tool_id)
        _ = self.assertEqual(tool.category_id, target_category_id)
        ...
        _ = self.assertNotIn(tool, source_category.tools)
        _ = self.assertIn(tool, target_category.tools)
