...
class TestRealCausalEngineSimplified:
    ...
    def real_causal_graph(self):
        """真实因果图实例（无AI模型）"""
        return RealCausalGraph(tokenizer=None, model=None)
    ...
    async def test_causal_strength_calculation_simplified(self):
        ...
        causal_strength_weak = await engine._calculate_real_causal_causal_strength('temperature', 'sales', data_weak)
    ...
    def test_error_handling_robustness(self):
        ...
        # 测试空数据
        empty_data = []
        correlation = engine._calculate_real_correlation(empty_data, [1, 2, 3])
        print(f"空数据相关性: {correlation}")
