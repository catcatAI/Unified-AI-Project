...
class TestCrisisSystemAssessInput:
    ...
    def test_input_with_context(self):
        ...
        system = CrisisSystem(config=config)
        level = system.assess_input_for_crisis(
            {"text": "help"}, context={"user_history": "previous"}
        )
        assert level == 2
...
class TestCrisisSystemProtocol:
    ...
    def test_trigger_protocol_notify_human(self, mock_logging):
        ...
        assert "CRISIS" in str(mock_logging.critical.call_args) or "help" in str(
            mock_logging.critical.call_args
        )
