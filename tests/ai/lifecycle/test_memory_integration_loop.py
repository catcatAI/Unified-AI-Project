...
def mock_memory_manager():
    manager = MagicMock()
    manager.get_recent_memories = AsyncMock(
        return_value=[
            "用户喜欢听音乐",
            "用户问了关于AI的问题",
            "用户谈到了天气",
...
class TestCollectNewInfo:
    ...
    async def test_collect_new_info_from_memory_manager(
        self, integration_loop, mock_memory_manager
    ):
...
class TestAnalyzePatterns:
    ...
    async def test_analyze_patterns_finds_keywords(self, integration_loop):
        ...
        for content in ["用户话题 音乐话题 喜欢", "用户话题 编程话题 喜欢", "用户话题 AI 喜欢"]:
            integration_loop.memory_infos.append(
                MemoryInfo(
                    content=content,
                    type="conversation",
                    timestamp=now,
        ...
        assert any(
            "keyword_用户话题" in k or "keyword_喜欢" in k
            for k in integration_loop.knowledge_patterns
        )
...
class TestStructureMemory:
    ...
    async def test_structure_memory_skips_already_structured(
        self, integration_loop, mock_memory_manager
    ):
        from ai.lifecycle.memory_integration_loop import MemoryInfo
    ...
    async def test_structure_memory_limits_batch(self, integration_loop, mock_memory_manager):
        ...
        now = datetime.now()
        for i in range(15):
            integration_loop.integration_queue.append(
                MemoryInfo(
                    content=f"Test {i}",
                    type="conversation",
                    timestamp=now,
...
class TestUpdateKnowledgeBase:
    ...
    async def test_update_knowledge_base_integrates_structured(
        self, integration_loop, mock_memory_manager
    ):
        from ai.lifecycle.memory_integration_loop import MemoryInfo
    ...
    async def test_update_knowledge_base_skips_unstructured(
        self, integration_loop, mock_memory_manager
    ):
        from ai.lifecycle.memory_integration_loop import MemoryInfo
