...
class TestProactiveInteractionSystemInit:
    ...
    def test_init_with_custom_values(
        self, mock_llm_service, mock_state_manager, mock_memory_manager, user_monitor
    ):
        from ai.lifecycle.proactive_interaction_system import ProactiveInteractionSystem
...
class TestMessageGeneration:
    ...
    async def test_generate_emotional_message_frustrated(self, proactive_system):
        msg = await proactive_system._generate_emotional_message(
            {"data": {"emotion": "frustrated"}}
        )
        assert "煩惱" in msg

    async def test_generate_time_based_morning(self, proactive_system):
        msg = await proactive_system._generate_time_based_message(
            {"data": {"time_type": "morning_greeting"}}
        )
        assert "早上" in msg

    async def test_generate_time_based_evening(self, proactive_system):
        msg = await proactive_system._generate_time_based_message(
            {"data": {"time_type": "evening_greeting"}}
        )
        assert "晚上" in msg

    async def test_generate_memory_message(self, proactive_system):
        msg = await proactive_system._generate_memory_message(
            {"data": {"events": ["We talked about AI yesterday"]}}
        )
        assert "記得" in msg
