...
class TestEvaluateAction:
    ...
    def test_evaluate_adds_to_history(self, mock_uss):
        ...
        rs = ReasoningSystem()
        action = {"action_id": "a1", "action_type": "help", "description": "test", "entities": []}
        context = {}
    ...
    def test_evaluate_with_symbolic_risk(self, mock_uss):
        ...
        mock_space.get_relationships.return_value = [{"source": "entity_x", "target": "Harm"}]
...
class TestEvaluateEthicalPrinciples:
    ...
    def test_no_risk_penalty(self, mock_uss):
        ...
        scores = rs._evaluate_ethical_principles({"action_id": "test"}, context, [])
    ...
    def test_risk_penalty_applied(self, mock_uss):
        ...
        scores = rs._evaluate_ethical_principles({"action_id": "test"}, context, ["some risk"])
...
class TestOverallScore:
    ...
    def test_score_clamped(self, mock_uss):
        ...
        score = rs._calculate_overall_score(["v1", "v2", "v3", "v4", "v5"], ethical_scores)
        assert score == 0.0
...
class TestGetHistory:
    ...
    def test_with_history(self, mock_uss):
        ...
        rs = ReasoningSystem()
        action = {"action_id": "a1", "action_type": "test", "description": "test", "entities": []}
...
class TestCalculateConfidence:
    ...
    def test_base_confidence(self, mock_uss):
        rs = ReasoningSystem()
        conf = rs._calculate_confidence({"action_id": "test"}, {}, [])
        assert conf == 0.7

    @patch("ai.alignment.reasoning_system._SimpleSymbolicSpace")
    def test_confidence_with_graph_risks(self, mock_uss):
        rs = ReasoningSystem()
        conf = rs._calculate_confidence({"action_id": "test"}, {}, ["some risk"])
        assert conf == pytest.approx(0.9)

    @patch("ai.alignment.reasoning_system._SimpleSymbolicSpace")
    def test_confidence_capped(self, mock_uss):
        rs = ReasoningSystem()
        conf = rs._calculate_confidence({"action_id": "test"}, {}, ["r1", "r2", "r3"])
        assert conf <= 1.0
