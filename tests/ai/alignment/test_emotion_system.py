...
class TestEmotionSummary:
    ...
    def test_summary_after_context_analysis(self):
        with patch("textblob.TextBlob") as mock_tb, patch(
            "core.system.config.tiered_loader.get_config"
        ) as mock_gc:
...
class TestAnalyzeEmotionalContext:
    _base_config = {"biological_thresholds": {"emotion_classification_stress": 0.7}}
...
class TestEmpathyAnalysis:
    ...
    def test_analyze_empathy_returns_analysis(self, emotion_system):
        analysis = emotion_system.analyze_empathy(
            "human_user",
            {
                "text": "I feel sad",
                "stress_level": 0.6,
