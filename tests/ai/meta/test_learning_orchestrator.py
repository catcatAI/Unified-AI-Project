...
def mock_evaluator():
    evaluator = MagicMock()
    evaluator.evaluate_task_execution = AsyncMock(
        return_value={
            "task_id": "task_1",
...
class TestLearningOrchestratorStatus:
    ...
    async def test_get_learning_status_uses_latest_rating(self, orchestrator, mock_evaluator):
        mock_evaluator.evaluate_task_execution = AsyncMock(
            return_value={
                "task_id": "t2",
                "timestamp": "2026-01-01T00:00:00",
                "metrics": {
                    ...
                    "coherence_score": 0.5,
                    "quality_score": 0.5,
                    "cognitive_dividend": 0.5,
                    "life_intensity_impact": 0.5,
        ...
        await orchestrator.process_learning_cycle(
            {"id": "t1"}, {"output": "first", "success": True}
        )
        await orchestrator.process_learning_cycle(
            {"id": "t2"}, {"output": "second", "success": False}
        )
        status = orchestrator.get_learning_status()
