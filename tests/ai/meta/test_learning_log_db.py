...
class TestLearningLogDBAddEntry:
    ...
    def test_add_log_entry_multiple_entries(self, db):
        id1 = db.add_log_entry(
            {"strategy_id": "s1", "current_effectiveness": 0.5, "message": "first"}
        )
        id2 = db.add_log_entry(
            {"strategy_id": "s2", "current_effectiveness": 0.6, "message": "second"}
        )
        assert id2 == id1 + 1
...
class TestLearningLogDBGetEntries:
    ...
    def test_get_all_log_entries_returns_dict_with_all_keys(self, db):
        db.add_log_entry(
            {"strategy_id": "s1", "current_effectiveness": 0.75, "message": "test msg"}
        )
        entries = db.get_all_log_entries()
        entry = entries[0]
