...
class TestBudgetScheduler:
    ...
    def _fixed_load(self):
        ...
        ``neuro_auto_selector``).
        """
        with patch("services.resource_awareness_service.ResourceAwarenessService") as MockResource:
            mock_svc = MagicMock()
    ...
    def test_schedule_clamp_min_max(self):
        ...
        scheduler = BudgetScheduler(
            config={"min_time_budget_ms": 5000, "max_time_budget_ms": 60000}
        )
...
class TestNeuroAutoSelector:
    async def test_decide_neuroblender_low_budget(self):
        ...
        with patch.object(selector.hardware, "analyze") as mock_hw, patch.object(
            selector.budget_scheduler, "schedule", return_value=3000
        ), patch.object(selector.state_interpreter, "get_energy", return_value=0.1), patch.object(
            selector.state_interpreter, "get_state_dict"
        ) as mock_state:

            mock_state.return_value = {
                "alpha_energy": 0.1,
                "epsilon_precision": 0.3,
                "delta_happiness": 0.3,
                "theta_novelty": 0.2,
                "theta_negativity": 0.2,
                "eta_success_rate": 0.5,
            }
    ...
    async def test_decide_force_backend(self):
        ...
        with patch.object(selector.hardware, "analyze") as mock_hw, patch.object(
            selector.budget_scheduler, "schedule", return_value=30000
        ), patch.object(selector.state_interpreter, "get_energy", return_value=0.5), patch.object(
            selector.state_interpreter, "get_state_dict"
        ) as mock_state, patch.object(
            selector, "_get_available_backends", return_value=[]
        ):
    ...
    def test_analyze_task(self):
        ...
        task = selector._analyze_task(
            {"intent": "general", "complexity": 0.2, "user_message": "hello"}
        )
        ...
        task = selector._analyze_task(
            {"intent": "math", "complexity": 0.8, "user_message": "Solve this integral: ∫x²dx"}
        )
        assert task.demand_score > 0.5
    ...
    def test_is_local_capable(self):
        selector = NeuroAutoSelector(config={})
        assert (
            selector._is_local_capable(
                {"ram_total_gb": 16, "vram_mb": 4096, "accelerator_type": "nvidia"}
            )
            is True
        )
        assert (
            selector._is_local_capable(
                {"ram_total_gb": 2, "vram_mb": 0, "accelerator_type": "none"}
            )
            is False
        )
        assert (
            selector._is_local_capable(
                {"ram_total_gb": 8, "vram_mb": 0, "accelerator_type": "cpu_avx2"}
            )
            is True
        )
    ...
    def test_recommend_ollama_model(self):
        ...
        model = selector._recommend_ollama_model(
            {"ram_available_gb": 24, "ram_total_gb": 32, "vram_mb": 12288}
        )
        assert "deepseek" in model

        # Medium RAM, medium VRAM → qwen
        model = selector._recommend_ollama_model(
            {"ram_available_gb": 10, "ram_total_gb": 16, "vram_mb": 6144}
        )
        assert "qwen" in model

        # Low RAM → phi
        model = selector._recommend_ollama_model(
            {"ram_available_gb": 3, "ram_total_gb": 4, "vram_mb": 0}
        )
        assert "phi" in model
