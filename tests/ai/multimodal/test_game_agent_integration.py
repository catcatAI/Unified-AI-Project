...
from ai.multimodal.game_structs import (
    SkillID, SkillSpec, SkillParams, SkillContext, SkillResult,
    Subgoal, TaskProgress, PlanDAG, GameState, VisualObservation,
    Proprioception, StrategyDirective, GAME_SKILLS, check_preconditions,
...
from ai.multimodal.game_planner import GamePlanner, PlannerConfig, PlanningContext, GoalManager, GoalType
from ai.multimodal.game_memory_bridge import GameMemoryBridge, MockHAMManager, MemoryType, GameExperience
...
from ai.multimodal.llm_game_interface import (
    LLMGameInterface, LLMConfig, RuleBasedFallback,
    PlanProposal, AnomalyContext, RecoveryStrategy,
    StrategyContext, StrategyAdjustment
)
    ...
    def test_check_preconditions(self):
        ...
        state = GameState(
            proprioception=Proprioception(
                ...
                health=20, max_health=20, hunger=15, max_hunger=20
            )
        )
...
class TestGamePolicy:
    ...
    def test_get_action_dict(self, policy):
        ...
        output = PolicyOutput(
            ...
            discrete_logits=np.array([2.0, -1.0, 0.5, -2.0, 0.0, -1.0, -1.0, -1.0], dtype=np.float32),
...
class TestGameTaskExecutor:
    ...
    def test_fallback_on_failure(self, executor, state):
        ...
        fallback_triggered = any("unblock" in sg.subgoal_id for sg in executor._queue) \
        ...
        assert fallback_triggered, f"Fallback should have been triggered. Queue: {[sg.subgoal_id for sg in executor._queue]}, Current: {executor._current.subgoal.subgoal_id if executor._current else None}"
...
class TestGameMemoryBridge:
    ...
    def test_spatial_memory(self, memory):
        ...
        memory._spatial_index["200_64_200"] = type('Node', (), {
            'position': (200, 64, 200),
            'node_type': 'resource',
            'resources': {'diamond': 5},
            'danger_level': 0.0
    ...
    async def test_single_tick(self, agent):
        ...
        with patch.object(agent._luanti, 'connect', new_callable=AsyncMock) as mock_connect:
            ...
            with patch.object(agent._luanti, 'state', Mock(value="ready")):
                ...
                actions = agent._compose_actions(policy_out, SkillContext(active_skill=SkillID.MOVE, params={}, priority=1.0))
                
                # 發送動作
                await agent._send_actions(actions)
                
                # 驗證動作被發送
                assert agent._luanti.move.called or agent._luanti.look.called or agent._luanti.dig.called
