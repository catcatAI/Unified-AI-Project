...
class TestFullTrainingPipeline:
    ...
    def test_save_and_load_weights_roundtrip(self, pipeline, tmp_path):
        ...
        pipeline.run(
            contrastive_epochs=2, contrastive_pairs=5, recon_epochs=1, recon_samples=3, lr=0.01
        )
    ...
    def test_load_weights_restores_texture_weights(self, pipeline, tmp_path):
        ...
        from ai.multimodal.training_pipeline import DEFAULT_WEIGHTS_PATH, FullTrainingPipeline
        ...
        assert np.allclose(fresh._visual_decoder._W_hidden, data_before["texture_W_hidden"])
        assert np.allclose(fresh._visual_decoder._b_hidden, data_before["texture_b_hidden"])
        assert np.allclose(fresh._visual_decoder._W_featmap, data_before["texture_W_featmap"])
        assert np.allclose(fresh._visual_decoder._b_featmap, data_before["texture_b_featmap"])
        assert np.allclose(fresh._visual_decoder._tex_kernels, data_before["texture_tex_kernels"])
...
class TestTextureTrainer:
    ...
    def test_train_texture_weights_change(self, pipeline):
        ...
        assert not np.allclose(pipeline._visual_decoder._W_hidden, snap_before["W_hidden"])
        assert not np.allclose(pipeline._visual_decoder._W_featmap, snap_before["W_featmap"])
...
class TestWavetableTrainer:
    ...
    def test_train_wavetable_weights_change(self, pipeline):
        ...
        assert not np.allclose(pipeline._audio_decoder._W_hidden, snap_before["W_hidden"])
        assert not np.allclose(pipeline._audio_decoder._W_wavetable, snap_before["W_wavetable"])
    ...
    def test_load_weights_restores_audio_wavetable_weights(self, pipeline, tmp_path):
        ...
        assert np.allclose(fresh._audio_decoder._W_hidden, data_before["audio_W_hidden"])
        assert np.allclose(fresh._audio_decoder._b_hidden, data_before["audio_b_hidden"])
        assert np.allclose(fresh._audio_decoder._W_wavetable, data_before["audio_W_wavetable"])
        assert np.allclose(fresh._audio_decoder._b_wavetable, data_before["audio_b_wavetable"])
        assert np.allclose(fresh._audio_decoder._W_noise, data_before["audio_W_noise"])
        assert np.allclose(fresh._audio_decoder._b_noise, data_before["audio_b_noise"])
...
class TestSequenceTrainer:
    ...
    def test_train_sequence_weights_change(self, pipeline):
        ...
        assert not np.allclose(pipeline._sequence_generator._W_ih, snap_before["W_ih"])
        assert not np.allclose(pipeline._sequence_generator._W_hh, snap_before["W_hh"])
        assert not np.allclose(pipeline._sequence_generator._W_ho, snap_before["W_ho"])
    ...
    def test_load_weights_restores_sequence_weights(self, pipeline, tmp_path):
        ...
        assert np.allclose(fresh._sequence_generator._W_ih, data_before["seq_W_ih"])
        assert np.allclose(fresh._sequence_generator._b_ih, data_before["seq_b_ih"])
        assert np.allclose(fresh._sequence_generator._W_hh, data_before["seq_W_hh"])
        assert np.allclose(fresh._sequence_generator._W_ho, data_before["seq_W_ho"])
        assert np.allclose(fresh._sequence_generator._W_stop, data_before["seq_W_stop"])
