...
def fake_cifar10_dir(tmp_path):
    ...
    with open(cifar_dir / "index.json", "w") as f:
        json.dump(
            {"total": 3, "classes": class_names, "class_counts": {cn: 1 for cn in class_names}}, f
        )
    return cifar_dir
...
def fake_esc50_dir(tmp_path):
    ...
    for i in range(3):
        ...
        n_samples = 160
        wav_bytes = (
            b"RIFF\x00\x00\x00\x00WAVEfmt \x10\x00\x00\x00\x01\x00\x01\x00\x80\x3e\x00\x00\x00\x7d\x00\x00\x02\x00\x10\x00data"
            + (n_samples * 2).to_bytes(4, "little")
            + b"\x00" * (n_samples * 2)
        )
