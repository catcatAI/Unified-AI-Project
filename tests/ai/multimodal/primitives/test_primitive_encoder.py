...
def sample_instructions():
    return DrawingInstructions(
        ...
        lines=[
            Line(
                Point(0.1, 0.5, (0, 0, 0), 0.0), Point(0.9, 0.5, (0, 0, 0), 0.0), 0.05, (0, 0, 255)
            )
        ],
        planes=[
            Plane(
...
class TestPrimitiveEncoder:
    ...
    def test_train_step_reduces_loss(self, encoder, sample_instructions):
        ...
        initial_loss = np.mean((sample_instructions.to_vector() - initial_decoded.to_vector()) ** 2)
        ...
        new_loss = np.mean((sample_instructions.to_vector() - new_decoded.to_vector()) ** 2)
    ...
    def test_different_instructions_different_embeddings(self, encoder):
        instr1 = DrawingInstructions(points=[Point(0.2, 0.2, (255, 0, 0), 0.1)])
        instr2 = DrawingInstructions(points=[Point(0.8, 0.8, (0, 255, 0), 0.1)])
