...
async def test_cli_no_args():
    ...
    with patch("sys.stderr", new_callable=StringIO) as mock_stderr, patch("sys.argv", ["main.py"]):
...
async def test_cli_query_with_emotion(mock_shutdown, mock_get, mock_init):
    ...
    test_cases = [
        {
            ...
            "dm_response": f"{ai_name}: Hello! How can I help you?",
            "expected_substring": "How can I help you?",
        },
        {
            "input": "I am so sad today.",
            "dm_response": f"{ai_name}: (gently) I hear that.",
            "expected_substring": "(gently)",
        },
        {
            "input": "This is great and I am happy!",
            "dm_response": f"{ai_name}: (playfully) That's awesome! ✨",
            "expected_substring": "(playfully) ✨",
        },
    ]
    ...
    for case in test_cases:
        ...
        with patch("sys.argv", ["main.py", "query", case["input"]]), patch(
...
async def test_cli_query_crisis_response(mock_shutdown, mock_get, mock_init):
    ...
    expected_crisis_output = (
        f"AI, {ai_name}: I sense this is a sensitive situation. Please reach out to support."
    )
    expected_substring = "appropriate support channels"

    mock_dm.get_simple_response = AsyncMock(return_value=expected_crisis_output)

    with patch("sys.argv", ["main.py", "query", test_query_crisis]), patch(
        "sys.stdout", new_callable=StringIO
    ) as captured_output:
