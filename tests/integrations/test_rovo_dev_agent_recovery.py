...
class TestRovoDevAgentRecovery:
    ...
    def agent(self, mock_config):
        """創建 RovoDevAgent 實例"""
        with patch('src.integrations.rovo_dev_agent.EnhancedRovoDevConnector'), \
             patch('src.integrations.rovo_dev_agent.AtlassianBridge'):
    ...
    def test_retryable_error_detection(self, agent):
        """測試可重試錯誤檢測"""
        # 測試可重試錯誤
        retryable_errors = [
    ...
    def test_task_error_handling(self, agent, mock_task):
        """測試任務錯誤處理"""
        task_id = 'error-task'
    ...
    def test_recovery_status(self, agent):
        """測試恢復狀態獲取"""
        # 設置一些狀態
