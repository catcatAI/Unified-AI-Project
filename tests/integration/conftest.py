...
def mock_agent_manager():
    ...
    with patch('apps.backend.src.managers.agent_manager.AgentManager') as mock:
        ...
        mock_instance.get_available_agents = Mock(return_value=["test_agent"])
        mock_instance.get_active_agents = Mock(return_value={"test_agent": 12345})
...
def mock_hsp_connector():
    ...
    with patch('apps.backend.src.hsp.connector.HSPConnector') as mock:
        ...
        mock_instance.publish = Mock(return_value=True)
        mock_instance.subscribe = Mock(return_value=Mock())
        mock.return_value = mock_instance
        yield mock
...
def mock_memory_manager():
    ...
    with patch('apps.backend.src.ai.memory.ham_memory_manager.HAMMemoryManager') as mock:
        ...
        mock_instance.search_memory = Mock(return_value=[{"content": "test content"}])
        mock.return_value = mock_instance
        yield mock
...
def mock_learning_manager():
    ...
    with patch('apps.backend.src.core_services.DemoLearningManager') as mock:
        ...
        mock_instance.process_feedback = Mock(return_value=True)
        mock.return_value = mock_instance
        yield mock
