...
class BaseIntegrationTest:
    ...
    def create_test_hsp_message(
        self, message_type: str = "fact", content: str = "Test message"
    ) -> Dict[str, Any]:
        """
        创建测试HSP消息
    ...
    async def assert_event_occurred(
        self, event_checker, timeout: int = 5, message: str = "Event did not occur"
    ):
        """
        断言事件发生
...
class SystemIntegrationTest(BaseIntegrationTest):
    ...
    async def simulate_agent_interaction(
        self, agent1_config: Dict, agent2_config: Dict, message: Dict
    ):
        """
        模拟代理间交互
        ...
        logger.info(
            f"Simulated interaction between {agent1_config['agent_id']} and {agent2_config['agent_id']}"
        )
