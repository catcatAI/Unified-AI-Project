...
def test_math_model_loading() -> None:
    ...
    try:
        ...
        if not model_path.exists():
            _ = print(f"❌ 数学模型文件不存在: {model_path}")
            return False
            
        if not char_map_path.exists():
            _ = print(f"❌ 数学模型字符映射文件不存在: {char_map_path}")
            return False
        ...
        _ = print("✅ 字符映射加载成功")
        _ = print(f"  - 唯一标记数: {char_maps.get('n_token', '未知')}")
        _ = print(f"  - 最大编码器序列长度: {char_maps.get('max_encoder_seq_length', '未知')}")
        _ = print(f"  - 最大解码器序列长度: {char_maps.get('max_decoder_seq_length', '未知')}")
...
def test_logic_model_loading() -> None:
    ...
    try:
        ...
        if not model_path.exists():
            _ = print(f"❌ 逻辑模型文件不存在: {model_path}")
            return False
            
        if not char_map_path.exists():
            _ = print(f"❌ 逻辑模型字符映射文件不存在: {char_map_path}")
            return False
        ...
        _ = print("✅ 字符映射加载成功")
        _ = print(f"  - 词汇表大小: {char_maps.get('vocab_size', '未知')}")
        _ = print(f"  - 最大序列长度: {char_maps.get('max_seq_len', '未知')}")
