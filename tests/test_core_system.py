...
class TestCognitiveOrchestrator:
    ...
    def orchestrator(self):
        ...
        with patch("requests.post") as mock_post:
            ...
            mock_post.return_value.json.return_value = {"response": "測試回應"}
...
class TestHAMMemoryManager:
    ...
    def memory_manager(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            manager = HAMMemoryManager(storage_type="simple", storage_path=temp_dir)
            yield manager

    @pytest.mark.asyncio
    async def test_store_memory(self, memory_manager):
        """測試記憶存儲"""
        memory_data = {"content": "測試記憶", "type": "episodic", "importance": 0.8}
    ...
    async def test_retrieve_memories(self, memory_manager):
        """測試記憶檢索"""
        # 先存儲一個記憶
        memory_data = {"content": "測試記憶檢索", "type": "semantic", "importance": 0.9}
...
class TestAgentManager:
    ...
    def test_list_agents(self, agent_manager):
        ...
        # 創建幾個測試代理
        for i in range(3):
            agent_manager.create_agent({"name": f"測試代理{i}", "type": "assistant"})
...
class TestDesktopPet:
    ...
    async def test_handle_user_input(self, desktop_pet):
        """測試用戶輸入處理"""
        response = await desktop_pet.handle_user_input("message", {"text": "你好"})
...
class TestEconomyManager:
    ...
    async def test_transaction(self, economy_manager):
        ...
        success = await economy_manager.process_transaction(
            from_user=user1, to_user=user2, amount=30, description="測試交易"
        )
