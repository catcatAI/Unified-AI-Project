...
def fix_demo_learning_manager():
    ...
    # 修复函数调用缺少括号的问题
    content = content.replace("await self._enable_demo_mode", "await self._enable_demo_mode()")
    content = content.replace("await self._initialize_learning", "await self._initialize_learning()")
    content = content.replace("await self._setup_mock_services", "await self._setup_mock_services()")
    content = content.replace("await self._configure_auto_cleanup", "await self._configure_auto_cleanup()")
    content = content.replace("await self._save_learning_data", "await self._save_learning_data()")
    content = content.replace("await self._collect_learning_data", "await self._collect_learning_data()")
    content = content.replace("await self._perform_cleanup(cleanup_config)", "await self._perform_cleanup(cleanup_config)")
