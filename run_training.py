    ...
    try:
        ...
        try:
            ...
            try:
                ...
                version = "未知"
        print(f"✅ TensorFlow可用 - 版本: {version}")
        return True
...
def run_data_generation(script_path):
    """运行数据生成脚本"""
    try:
        print(f"正在运行数据生成脚本: {script_path}")
...
def main():
    ...
    if math_success and logic_success:
        ...
        print("⚠️ 数学模型训练成功，逻辑模型训练失败")
        return True
    elif logic_success:
        print("⚠️ 逻辑模型训练成功，数学模型训练失败")
        return True
    else:
        print("❌ 所有模型训练都失败了")
        return False
