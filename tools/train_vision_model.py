...
class VisionDataset(Dataset):
    ...
    def _create_labels(self) -> List[int]:
        ...
        for item in self.data:
            ...
            labels.append(scene_to_label[scene_type])
        ...
        if processed_data_file.exists():
            try:
                with open(processed_data_file, 'r', encoding='utf-8') as f:
                    data = json.load(f)
                logger.info(f"成功加载 {len(data)} 条视觉数据")
                return data
            except Exception as e:
                logger.error(f"加载视觉数据时出错: {e}")
                return []
        else:
            logger.warning(f"未找到处理后的视觉数据文件: {processed_data_file}")
            return []
    
    def train_model(self, epochs: int = 10, batch_size: int = 8, learning_rate: float = 0.001):
        """训练视觉模型"""
        logger.info("开始训练视觉模型...")
...
def main():
    ...
    if success:
        logger.info("视觉模型训练成功完成!")
    else:
        logger.error("视觉模型训练失败!")
