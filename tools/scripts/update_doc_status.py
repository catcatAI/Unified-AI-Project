...
class DocStatusManager,
    ...
    def update_status(self, doc_path, str, status, str, notes, str == "") -> None,
        """更新文檔狀態
    ...
    def list_docs(self, status_filter, Optional[str] = None, directory, Optional[str] = None) -> None,
        """列出文檔及其狀態"""
    ...
    def show_doc_details(self, doc_path, str) -> None,
        """顯示文檔詳細信息"""
        if doc_path not in self.status_data["files"]::
    ...
    def update_doc_status(self, doc_path, str, status, str, notes, Optional[str] = None) -> bool,
        """更新文檔狀態"""
        if doc_path not in self.status_data["files"]::
            print(f"錯誤：找不到文檔 '{doc_path}'")
            return False
