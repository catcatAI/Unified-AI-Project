...
def cleanup_excess_backups():
    """清理多余的备份目录，只保留最近的3个"""
    project_root: str = Path(r"D:\Projects\Unified-AI-Project")
    ...
    for item in backup_dir.iterdir():
        if item.is_dir() and item.name.startswith('auto_fix_'):
            ...
            try:
                ...
                _ = auto_fix_dirs.append((timestamp, item))
            except Exception as e:
                _ = print(f"无法解析目录名 {item.name}: {e}")
    ...
    if len(auto_fix_dirs) > 3:
        ...
        _ = print(f"找到 {len(auto_fix_dirs)} 个备份目录，将删除 {len(dirs_to_delete)} 个")
        
        for timestamp, dir_path in dirs_to_delete:
            _ = print(f"删除备份目录: {dir_path.name} (时间: {timestamp})")
            try:
                _ = shutil.rmtree(dir_path)
                _ = print(f"  已删除: {dir_path}")
            except Exception as e:
                _ = print(f"  删除失败: {e}")
    else:
        _ = print(f"只有 {len(auto_fix_dirs)} 个备份目录，无需清理")

if __name__ == "__main__":
    _ = cleanup_excess_backups()