        ...
        for i, line in enumerate(lines):
            ...
            if ('def ' in line and
                ...
                if ('__init__' in line or
                    ...
                    lines[i] = fixed_line
                    fixes_made.append(f"第 {i+1} 行: 添加返回类型注解 -> None")
                    modified = True
                elif 'test_' in line:
                    # 对于测试函数，添加 -> None
                    fixed_line = line.replace(')', ') -> None:')
                    ...
                    if '__len__' in line:
                        ...
                        lines[i] = fixed_line
                        fixes_made.append(f"第 {i+1} 行: 添加返回类型注解 -> int")
                        modified = True
                    elif '__getitem__' in line:
                        fixed_line = line.replace(')', ') -> Any:')
                        lines[i] = fixed_line
                        fixes_made.append(f"第 {i+1} 行: 添加返回类型注解 -> Any")
                        modified = True
            ...
            if ('=' in line and
                ...
                if (left_side.isidentifier() and 
                    ...
                    if left_side in ['project_root', 'backend_path', 'data_path']:
                        ...
                        lines[i] = fixed_line
                        fixes_made.append(f"第 {i+1} 行: 添加变量类型注解 {left_side}: str")
                        modified = True
                        ...
                        if left_side == 'logger':
                            ...
                            lines[i] = fixed_line
                            fixes_made.append(f"第 {i+1} 行: 添加变量类型注解 {left_side}: Any")
                            modified = True
