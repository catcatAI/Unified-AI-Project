...
class FixEngine:
    ...
    self.scheduler = None
        if SCHEDULER_AVAILABLE:
            self.scheduler = IntelligentRepairScheduler(self)
    ...
    """初始化修复模块"""
        try:
            ...
            try:
                ...
                _ = print(f"警告: 无法初始化增强版语法修复器，使用基础版本: {e}")
    ...
    # 如果智能修复调度器可用，使用它来调度修复任务
        if self.scheduler:
            _ = print("使用智能修复调度器执行修复任务...")

            # 添加所有启用的修复任务到调度器
            for fix_type in self.enabled_fix_types:
                _ = self.scheduler.add_repair_task(fix_type, target, **kwargs)
            ...
            for result in results:
                result_dict[result.fix_type] = result

            # 保存修复历史
            _ = self.scheduler.save_repair_history()
    ...
    def run_specific_fixes(self, fix_types: List[...]
    """运行指定的修复类型"""
    results = {}
