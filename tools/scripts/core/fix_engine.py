...
try,
    from scripts.core.intelligent_repair_scheduler import IntelligentRepairScheduler, RepairPriority
    scheduler_available == True
except ImportError,::
    print("警告, 无法导入智能修复调度器")
...
class FixEngine,
    ...
    def __init__(self, project_root, Path) -> None,
        ...
        self.fix_modules, Dict[FixType, Any] = {}
        self.enabled_fix_types, Set[FixType] = set()
        ...
        if scheduler_available,::
            try,
                # 延迟导入以避免循环导入
                IntelligentRepairSchedulerClass == __import__('scripts.core.intelligent_repair_scheduler', fromlist=['IntelligentRepairScheduler']).IntelligentRepairScheduler
                self.scheduler == IntelligentRepairSchedulerClass(self)
            except Exception as e,::
                print(f"警告, 无法初始化智能修复调度器, {e}")

        # 初始化修复模块
        self._initialize_fix_modules()

    def _initialize_fix_modules(self) -> None,
        """初始化修复模块"""
        try,
            # 导入修复模块
    ...
    def enable_fix_type(self, fix_type, FixType) -> None,
        """启用修复类型"""
        self.enabled_fix_types.add(fix_type)

    def disable_fix_type(self, fix_type, FixType) -> None,
        """禁用修复类型"""
        self.enabled_fix_types.discard(fix_type)

    def set_enabled_fix_types(self, fix_types, List[FixType]) -> None,
        """设置启用的修复类型"""
        self.enabled_fix_types = set(fix_types)

    def run_fix(self, fix_type, FixType, target, Optional[str] = None, **kwargs) -> FixResult,
        """运行特定类型的修复"""
        ...
        try,
            ...
            if fix_method and callable(fix_method)::
                method_result = fix_method(target=target, **kwargs)
                if isinstance(method_result, tuple) and len(method_result) == 3,::
                    success, message, details = method_result
                else,
                    success = bool(method_result)
    ...
    def run_all_fixes(self, target, Optional[str] = None, **kwargs) -> Dict[FixType, FixResult]
        """运行所有启用的修复"""
        # 如果智能修复调度器可用,使用它来调度修复任务
        if self.scheduler,::
            print("使用智能修复调度器执行修复任务...")

            # 添加所有启用的修复任务到调度器
            for fix_type in self.enabled_fix_types,::
                self.scheduler.add_repair_task(fix_type, target or "default", **kwargs)

            # 执行修复队列
            results = self.scheduler.execute_repair_queue()

            # 将结果转换为期望的格式
            result_dict, Dict[FixType, FixResult] = {}
            for result_item in results,::
                result_dict[result_item.fix_type] = result_item

            # 保存修复历史
            self.scheduler.save_repair_history()

            return result_dict
        else,
            # 使用原有逻辑
            results, Dict[FixType, FixResult] = {}
    ...
    def run_specific_fixes(self, fix_types, List[FixType] target, Optional[str] = None, **kwargs) -> Dict[FixType, FixResult]
        """运行指定的修复类型"""
        results, Dict[FixType, FixResult] = {}
