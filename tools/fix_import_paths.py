...
def fix_import_paths_in_file(file_path):
    ...
    try:
        ...
        content = re.sub(
            r'from apps\.backend\.src\.([a-zA-Z_][a-zA-Z0-9_\.]*) import',
...
def fix_training_module_imports():
    ...
    print(f"✅ 已修复 {fixed_count}/{len(files_to_fix)} 个训练模块文件")
    return fixed_count
...
def fix_concept_models_imports():
    ...
    print(f"✅ 已修复 {fixed_count}/{len(files_to_fix)} 个概念模型文件")
    return fixed_count
...
def main():
    ...
    # 总结
    print(f"\n📊 修复总结:")
    print(f"   训练模块文件修复: {training_fixed} 个")
    print(f"   概念模型文件修复: {concept_models_fixed} 个")
